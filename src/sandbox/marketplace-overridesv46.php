...
function shoshin_sync_affiliate_filtering_prices($product_id) {
    ...
    /*
     * Keep Woo scalar price meta aligned for external products.
     * Display still uses the custom affiliate renderer, so this is
     * primarily for filtering / lookup consistency.
     */
    if ($effective_min > 0) {
        update_post_meta($product_id, '_price', wc_format_decimal($effective_min));
        update_post_meta($product_id, '_regular_price', wc_format_decimal($effective_min));
    }
...
if (!empty($affiliate['is_external'])) {
    ...
    if ($has_min && $has_max) {
        ...
        $fallback_raw = $product->get_price();

        if ($fallback_raw === '' || $fallback_raw === null) {
            $fallback_raw = $product->get_sale_price();
        }

        if ($fallback_raw === '' || $fallback_raw === null) {
            $fallback_raw = $product->get_regular_price();
        }

        $fallback_price = ($fallback_raw !== '' && $fallback_raw !== null)
            ? (float) wc_format_decimal($fallback_raw)
            : 0.0;

        $display_html = ($fallback_price > 0)
            ? wc_price($fallback_price)
            : 'See Pricing';
