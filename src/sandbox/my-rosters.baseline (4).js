...
var SHOSHIN_PRINT_BASELINE_REBUILD = true;
...
/* END SHOSHIN PRINT — ATOM TYPE HELPERS */
        ...
        'function getAvailPx(templateFrag, pageIndex){' +
          ...
          'if(!fragRect){ fragRect = cssNum(templateFrag,"height"); }' +
          'var body = q(".print-frag-body", templateFrag);' +
          'var header = q(".print-header", templateFrag);' +
          'var headerH = (pageIndex===0) ? rectH(header) : 0;' +
          'var padTop = body ? cssNum(body,"padding-top") : 0;' +
          'var padBot = body ? cssNum(body,"padding-bottom") : 0;' +
...
'if(pageIndex===0) DBG.avail0 = avail;' +
...
'function run(force){' +
  'try{' +
    'var inPrint = isPrintMode();' +
    'if(!force && !inPrint){ setStage("skip_screen_mode"); return; }' +
    'if(force){' +
      'var wrap = q(".print-pages-wrap");' +
      'if(wrap) wrap.removeAttribute("data-shoshin-paginated");' +
    '}' +
    'paginate();' +
  '}catch(e){ try{ console.warn("[SHOSHIN PRINT] Pagination error:", e && e.message ? e.message : e); }catch(_){} }' +
'}' +
...
/* END SHOSHIN PRINT — DEBUG AVAIL PER PAGE */
      ...
      'for(var i=0;i<atoms.length;i++){' +
        ...
        // Divider cannot be first → drop it
        'if(isDivider(node) && countOnPage===0){ continue; }' +

        'var h = rectH(node);' +

        'if(countOnPage>0 && (used + h) > avail){' +
          // Divider cannot be last → drop trailing divider at boundary
          'var last = body && body.lastElementChild ? body.lastElementChild : null;' +
          'if(last && isDivider(last)){' +
            'body.removeChild(last);' +
          '}' +
