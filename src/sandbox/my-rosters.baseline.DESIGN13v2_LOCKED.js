...
.print-page.print-layout-2 .l2-title{
  text-align:center;
  padding: 0.08in 0 0.06in 0;
}
.print-page.print-layout-2 .l2-title .l2-title-main{
  font-family: "Times New Roman", serif;
  font-weight: 800;
  letter-spacing: 0.02em;
  font-size: 18px;
}
.print-page.print-layout-2 .l2-title .l2-title-sub{
  font-family: Arial, sans-serif;
  font-size: 11px;
  opacity: 0.9;
  margin-top: 2px;
...
/* Bucket labels: same font-size as subcolumn headers; centered lines */
.print-page.print-layout-2 table.l2-roster-table .print-row2-group-title{
  font-size: inherit;       /* matches DMG/CRT/DIS header row */
  align-items: center;
  justify-content: center;
}
...
/* QTY hiding rule: keep the negative space (do NOT collapse) */
...
.print-page.print-layout-2 table.l2-roster-table.l2-hide-qty td.l2-qty{
  visibility: hidden; /* keeps width */
}

/* Divider between tables: honors Display Images toggle (image hides but height remains) */
.print-page.print-layout-2 .l2-between-tables{
/* Divider between bucket/group blocks (inside the table). Honors Display Images toggle. */
.print-page.print-layout-2 .l2-between-buckets{
  margin: 2px 0 2px 0;
  display:flex;
  align-items:center;
  justify-content:center;
}
.print-page.print-layout-2 .l2-between-buckets img{
  height: 14px;
  width: auto;
  display:block;
}
.print-page.print-layout-2.print-images-off .l2-between-buckets img{
  opacity: 0;
}
...
.print-page.print-layout-2 .l2-between-tables img{
  height: 18px;
  width: auto;
  display:block;
}
.print-page.print-layout-2.print-images-off .l2-between-tables img{
  opacity: 0;
}
...
function renderPrintLayout2HTML(ctx) {
  ...
  function asNumOrToken(v) {
    ...
    if (t.toLowerCase() === 'highest') return 'Max';
    if (t.toLowerCase() === 'captain') return 'Cap';
    return t;
  }
  ...
  // Locked bucket order (you provided)
  var CHAR_BUCKETS = ['Daimyo','Samurai','Ashigaru','Sohei','Ninja','Onmyoji'];
  var SUP_BUCKETS  = ['Artillery','Sailing Ships'];
...
function renderMods(row, sample) {
  ...
  // 4 per line, semicolon separated
  var perLine = 4;
  var outLines = [];
  for (var i = 0; i < lines.length; i += perLine) {
    outLines.push(lines.slice(i, i + perLine).map(function(s){ return String(s).trim(); }).join('; '));
  }
 ...
 function renderGroupHeaderRows(groupLabel, totalsQty, pct, thumbUrl, isSupport) {
  ...
  var meta = '<span class="l2-hdr-meta">(' + esc(totalsQty) + ') (' + esc(pct) + '%)</span>';
...
'<tr class="l2-unit ' + esc(zebraClass || '') + '">' +
        ...
        '<td class="l2-ref">' + esc(refId) + '</td>' +
        '<td class="l2-num l2-qty">' + esc(qty) + '</td>' +
        '<td class="l2-num">' + esc(tot) + '</td>' +

        '<td class="l2-num">' + esc(mDmg) + '</td>' +
        '<td class="l2-num">' + esc(mDis) + '</td>' +
        '<td class="l2-num">' + esc(mCrt) + '</td>' +

        '<td class="l2-num">' + esc(rDmg) + '</td>' +
        '<td class="l2-num">' + esc(rCrt) + '</td>' +
        '<td class="l2-num">' + esc(rDis) + '</td>' +
  ...
  function renderTableBlock(title, buckets, isSupport) {
...
'</colgroup>';
    ...
    html += '<tbody>';
    var zebra = 0; // counts ONLY unit rows

    function nextNonEmptyBucketIndex(startIdx) {
  for (var j = startIdx; j < buckets.length; j++) {
    var nm = buckets[j];
    var rr = byBucket[nm] || [];
...
/* INSERT THIS BLOCK RIGHT HERE (immediately after the unit loop) */
...
/* END INSERT */
  ...
  // Determine if support assets exist (any rows in support buckets)
  var supportExists = false;
  for (var s = 0; s < SUP_BUCKETS.length; s++) {
    var sr = byBucket[SUP_BUCKETS[s]];
    if (sr && sr.length) { supportExists = true; break; }
  }

  var dividerSrc = normalizeAbsUrl('/wp-content/uploads/2026/01/divider.png');
  var betweenTables =
    '<div class="l2-between-tables" data-l2-between="1">' +
      '<img src="' + esc(dividerSrc) + '" alt="" aria-hidden="true" />' +
    '</div>';

  var titleHtml =
    '<div class="l2-title">' +
      '<div class="l2-title-main">' + esc(ctx.rosterName || 'ROSTER') + '</div>' +
      '<div class="l2-title-sub">Ref: ' + esc(ctx.rosterRef || '—') +
        ' • Layout #2 — Portrait Table • ' + esc(mode.charAt(0).toUpperCase() + mode.slice(1)) +
        ' • Clan Points: ' + esc(asInt(ctx.totals && ctx.totals.points, 0)) +
      '</div>' +
    '</div>';

  var charactersTable = renderTableBlock('CHARACTERS', CHAR_BUCKETS, false);
  var supportTable    = supportExists ? renderTableBlock('SUPPORT', SUP_BUCKETS, true) : '';

  return (
    '<div class="print-page print-layout-2 print-images-' + onOff + '" data-print-layout="layout2-main">' +
      '<div class="print-pages-wrap">' +
        '<div class="print-frag">' +
          '<div class="print-frag-inner">' +
