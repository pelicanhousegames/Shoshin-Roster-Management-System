...
function shoshin_format_savings_text($amount, $percent, $prefix = 'You Save') {
    ...
    return $prefix . ' ' . wc_price($amount) . ' (' . $percent . '%)';
}
...
function shoshin_format_member_teaser_text($amount, $percent = 10, $is_additional = false, $is_range = false) {
    ...
    if ($is_range) {
        return $membership_link . ' save up to ' . wc_price($amount) . ' (' . $percent . '%) on this item';
    }

    if ($is_additional) {
        return $membership_link . ' save an additional ' . wc_price($amount) . ' (' . $percent . '%) on this item';
    }

    return $membership_link . ' save ' . wc_price($amount) . ' (' . $percent . '%) on this item';
}
...
function shoshin_marketplace_price_html($price_html, $product) {
    ...
    if ($sale_active && !$member_discount_active) {
        ...
        <div class="shoshin-price-block shoshin-price-block--sale-only">
            ...
            <div class="shoshin-price-savings">' . shoshin_format_savings_text($sale_amount, $sale_percent, 'You Save') . '</div>
            ' . $teaser_html . '
        </div>';
    }
