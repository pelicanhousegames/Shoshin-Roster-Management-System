...
$sale_active                  = ($regular > 0 && $sale > 0 && $sale < $regular);
    ...
    if (($is_loop_context || !is_product()) && !wp_doing_ajax()) {
        ...
        if ($product->is_type('variable')) {
            ...
            if ($min_current > 0 && $max_current > 0) {
                $archive_final_price = ((float) $min_current === (float) $max_current)
                    ? wc_price($min_current)
                    : wc_price($min_current) . ' – ' . wc_price($max_current);
            }

            if ($min_regular > 0 && $max_regular > 0) {
                $archive_original = ((float) $min_regular === (float) $max_regular)
                    ? wc_price($min_regular)
                    : wc_price($min_regular) . ' – ' . wc_price($max_regular);
            }

            if ($max_regular > 0 && $max_current > 0 && $max_current < $max_regular) {
                $archive_percent = round((($max_regular - $max_current) / $max_regular) * 100);
            }

            if ($archive_final_price !== '') {
                $html = '<ins>' . $archive_final_price . '</ins>';

                if ($archive_percent > 0 && $archive_original !== '') {
