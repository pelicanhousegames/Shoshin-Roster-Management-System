...
if ( ! function_exists( 'shoshin_cart_meta_membership_discount_applied' ) ) {
	function shoshin_cart_meta_membership_discount_applied( $cart_item, $product ) {
		...
		if ( $check_product instanceof WC_Product && $check_product->is_type( 'variation' ) ) {
			$parent_id = $check_product->get_parent_id();
			if ( $parent_id ) {
				$parent_product = wc_get_product( $parent_id );
				if ( $parent_product instanceof WC_Product ) {
					$check_product = $parent_product;
				}
			}
		}

		return (bool) apply_filters( 'shoshin_cart_meta_membership_discount_applied', false, $cart_item, $check_product );
	}
}
