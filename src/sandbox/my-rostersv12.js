document.addEventListener('DOMContentLoaded', function () {
  ...
  // Falls back to Helmet-grey if not provided.
  var LOCK_IMAGE = (function () {
    try {
...
function openRmsLockedModal(msg) {
  ...
  if (imgEl) imgEl.setAttribute('src', String(LOCK_IMAGE || ''));
...
if (qtySel) {
  ...
  function getRosterIcon(r) {
    ...
    /* BEGIN RMS SOFT LOCK — USE LOCK_IMAGE (NO RMS_LOCK_ICON_URL) */
    // If caller sets r.locked, override iconUrl here (single place)
    // LOCK_IMAGE is defined at top-scope by the resolver near the fallback paths.
    if (r && r.locked) iconUrl = LOCK_IMAGE;
    /* END RMS SOFT LOCK — USE LOCK_IMAGE (NO RMS_LOCK_ICON_URL) */
...
var isLockedRoster = !!(rosterObjC1 && rosterObjC1.locked);
// END RMS C1


   // BEGIN RMS C1 — padlock images if roster locked OR asset locked
var img = String(u.img || u.image || u.imgUrl || '').trim();
if (isLockedRoster || (u && u.locked)) img = LOCK_IMAGE;
// END RMS C1

/* BEGIN RMS SOFT LOCK — REMOVE RMS_LOCK_ICON_URL (USE LOCK_IMAGE ABOVE) */
// (No-op) This block intentionally removed.
// Padlock override is already handled via LOCK_IMAGE earlier in this function.
/* END RMS SOFT LOCK — REMOVE RMS_LOCK_ICON_URL (USE LOCK_IMAGE ABOVE) */
...
var rowALocked = !!(rowA && (rowA.locked || rowA.isLocked || rowA.rmsLocked));
if (rowALocked) {
  imgA = String(LOCK_IMAGE || '').trim();
} else {
