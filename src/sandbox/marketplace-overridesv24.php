...
function shoshin_get_variable_price_bounds($product) {
    ...
    foreach ($child_ids as $child_id) {
        ...
        /*
         * Use the actual child variation values instead of the parent
         * variable range getters, which are what is currently collapsing
         * the range display in this regression case.
         */
        $child_regular_raw = $variation->get_regular_price();
        $child_current_raw = $variation->get_price();

        if ($child_regular_raw !== '' && $child_regular_raw !== null) {
            $regulars[] = (float) $child_regular_raw;
        }

        if ($child_current_raw !== '' && $child_current_raw !== null) {
            $currents[] = (float) $child_current_raw;
        }
    }
