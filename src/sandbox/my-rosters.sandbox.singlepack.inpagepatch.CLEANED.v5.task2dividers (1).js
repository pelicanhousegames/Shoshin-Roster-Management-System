/* BEGIN SHOSHIN PRINT — DOM READY SHIM (RUN EVEN IF DOMContentLoaded ALREADY FIRED) */
(function(shoshinInit){
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', shoshinInit);
  } else {
    shoshinInit();
  }
})(function () {
/* END SHOSHIN PRINT — DOM READY SHIM */
...
/* =====================================================================
SHOSHIN PRINT — PAGINATION CORE (Option 3)
Goals:
- One canonical paginator used by:
  (A) about:blank print tab opened via openPrintTabDocument()
  (B) same-origin "print view" pages IF this script is present on that page
- Deterministic vertical justification via spacer elements (not flex hacks)
- Measure-mode is always enabled only for the duration of measurement/pagination.

Contract assumptions:
- Root: .print-page
- Body container: .print-body (contains .print-pack elements)
- Header (optional): .print-header (moved into first fragment only)
- Each .print-pack is an atomic unit to keep together (divider + title + unit row)
===================================================================== */

var SHOSHIN_PRINT_JUSTIFY_MIN_GAP_PX = 12; // smallest meaningful gap between groups
var SHOSHIN_PRINT_JUSTIFY_MAX_GAP_PX = 0;  // 0 = no cap

function shoshinEnsureExtraPrintCSS(doc, cssText){
  try{
    if (!doc) return;
    if (doc.getElementById('shoshin-extra-print-css')) return;
    var st = doc.createElement('style');
    st.id = 'shoshin-extra-print-css';
    st.type = 'text/css';
    st.appendChild(doc.createTextNode(String(cssText || '')));
    (doc.head || doc.getElementsByTagName('head')[0] || doc.documentElement).appendChild(st);
  } catch(_){}
}

function shoshinIsVisible(win, el){
...
function shoshinRemoveDirectChildrenByClass(parent, className){
  if (!parent) return;
  try{
    // Fast path: iterate backwards over children
    for (var i = (parent.children ? parent.children.length : 0) - 1; i >= 0; i--){
      var ch = parent.children[i];
      if (ch && ch.classList && ch.classList.contains(className)){
...
function shoshinJustifyFragBody(doc, fragBody){
  if (!fragBody) return;

  // Clean idempotently (remove any prior spacers)
  shoshinRemoveDirectChildrenByClass(fragBody, 'print-justify-spacer');

  // Collect groups (direct children only)
  var groups = [];
  try{
    for (var i = 0; i < (fragBody.children ? fragBody.children.length : 0); i++){
      var ch = fragBody.children[i];
      if (ch && ch.classList && ch.classList.contains('print-group')) groups.push(ch);
    }
  } catch(_){ groups = []; }

  // Skip if not enough content to justify
  if (groups.length <= 2) return;

  // Must not overflow
  var overflow = (fragBody.scrollHeight || 0) - (fragBody.clientHeight || 0);
  if (overflow > 0) return;

  // Sum used height
  var used = 0;
  for (var g = 0; g < groups.length; g++){
    var r = groups[g].getBoundingClientRect();
    used += (r && r.height) ? r.height : 0;
  }

  var free = (fragBody.clientHeight || 0) - used;
  if (free <= 0) return;

  var gaps = groups.length - 1;
  var gap = free / gaps;
  if (gap < SHOSHIN_PRINT_JUSTIFY_MIN_GAP_PX) return;

  var gapPx = Math.floor(gap);
  if (SHOSHIN_PRINT_JUSTIFY_MAX_GAP_PX > 0) gapPx = Math.min(gapPx, SHOSHIN_PRINT_JUSTIFY_MAX_GAP_PX);

  // Insert deterministic spacers between groups
  for (var i2 = 0; i2 < groups.length - 1; i2++){
    ...
    var after = groups[i2].nextSibling;
    if (after) fragBody.insertBefore(spacer, after);
    else fragBody.appendChild(spacer);
  }
}

function shoshinPaginateAndJustify(win, doc, opts){
  opts = opts || {};
  if (!win || !doc || !doc.body) return false;

  var root = opts.root || doc.querySelector('.print-page');
  if (!root) return false;

  // Idempotent: one pass per document instance
  if (root.getAttribute('data-paginated-space-evenly') === '1') return true;

  var header = root.querySelector('.print-header');
  var body   = root.querySelector('.print-body');
  if (!body) return false;

  // Ensure fragment CSS exists for this document
  if (opts.extraCss) shoshinEnsureExtraPrintCSS(doc, opts.extraCss);

  // Collect packs in DOM order (atomic content blocks)
  var packs = [];
  try{
    packs = Array.prototype.slice.call(body.querySelectorAll('.print-pack')).filter(function(el){
      return shoshinIsVisible(win, el);
    });
  } catch(_){ packs = []; }
  if (!packs.length) return false;

  // Turn on measure-mode for accurate clientHeight/scrollHeight comparisons (screen preview)
  var measureOn = false;
  try{
    doc.documentElement.classList.add('shoshin-print-measure');
    measureOn = true;
  } catch(_){}

  try{
    // Build wrap and first fragment
    var wrap = doc.createElement('div');
    wrap.className = 'print-pages-wrap';

    // Insert wrap before body (keeps DOM stable; prevents "detached measure" edge cases)
    try{
      if (body.parentNode) body.parentNode.insertBefore(wrap, body);
      else root.appendChild(wrap);
    } catch(_){ root.appendChild(wrap); }

    function startFrag(isFirst){
      var frag = doc.createElement('div');
      frag.className = 'print-frag';

      var inner = doc.createElement('div');
      inner.className = 'print-frag-inner';
      /* BEGIN SHOSHIN PRINT — HEADER OUTSIDE FRAGS (PRINT ONCE) */
      // Header must remain outside .print-pages-wrap so it does not steal frag-body height.
      /* END SHOSHIN PRINT — HEADER OUTSIDE FRAGS (PRINT ONCE) */
    ...
    function appendPackToBody(fragBody, packEl){
      var group = doc.createElement('div');
      group.className = 'print-group';
      group.appendChild(packEl);
      fragBody.appendChild(group);
      return group;
    }

    function bodyOverflows(fragBody){
      try{ fragBody.getBoundingClientRect(); } catch(_){}
      return (fragBody.scrollHeight > fragBody.clientHeight);
    }

    var currentBody = startFrag(true);

    /* BEGIN SHOSHIN TASK 2 — DIVIDER PACK PAGINATION RULES (NEVER FIRST/LAST) */
    function isDividerPack(el){
      try{ return !!el && el.classList && el.classList.contains('print-pack-divider'); } catch(_){ return false; }
    }
/* END SHOSHIN TASK 2 — DIVIDER PACK PAGINATION RULES (NEVER FIRST/LAST) */

    for (var i = 0; i < packs.length; i++){
      var pack = packs[i];
      if (!pack) continue;

      /* BEGIN SHOSHIN TASK 2 — DIVIDER PACK MUST BE BETWEEN TWO UNIT PACKS */
      if (isDividerPack(pack)){
        // Divider packs only render BETWEEN unit packs.
        // Enforce: never first in frag (must have prior content), never last in frag (must be followed by a unit pack on same frag).
        var hasPriorInFrag = (currentBody.children && currentBody.children.length > 0);
        if (!hasPriorInFrag){
          // Divider would be first -> drop it.
          continue;
        }

        // Place divider tentatively
        var placedDiv = appendPackToBody(currentBody, pack);
        if (bodyOverflows(currentBody)){
          // Divider doesn't fit at end -> remove and drop (do not carry to next frag because it would become first)
          try{ currentBody.removeChild(placedDiv); } catch(_){}
          continue;
        }

        // Divider must not be last; force next unit pack to be placed immediately after divider on same frag.
        var nextPack = packs[i + 1] || null;
        if (!nextPack){
          // No follower -> remove divider
          try{ currentBody.removeChild(placedDiv); } catch(_){}
          continue;
        }

        var placedNext = appendPackToBody(currentBody, nextPack);
        if (bodyOverflows(currentBody)){
          // If next pack doesn't fit, remove both and move next pack to the next frag WITHOUT divider.
          try{ currentBody.removeChild(placedNext); } catch(_){}
          try{ currentBody.removeChild(placedDiv); } catch(_){}
          currentBody = startFrag(false);
          appendPackToBody(currentBody, nextPack);
        }

        // We consumed nextPack in this iteration.
        i++;
        continue;
      }
      /* END SHOSHIN TASK 2 — DIVIDER PACK MUST BE BETWEEN TWO UNIT PACKS */

      var hadPrior = (currentBody.children && currentBody.children.length > 0);
      var placed = appendPackToBody(currentBody, pack);

      if (bodyOverflows(currentBody) && hadPrior){
        // Move to next page
        try{ currentBody.removeChild(placed); } catch(_){}
        currentBody = startFrag(false);
        appendPackToBody(currentBody, pack);
      }
    }


    // Remove original body (all packs have been moved into fragments)
    try{
      if (body && body.parentNode) body.parentNode.removeChild(body);
    } catch(_){}

    // Justify per fragment (skip pages that overflow or have too few groups)
    try{
      var fragBodies = wrap.querySelectorAll('.print-frag-body');
      for (var bi = 0; bi < fragBodies.length; bi++){
        shoshinJustifyFragBody(doc, fragBodies[bi]);
      }
    } catch(_){}
    ...
    if (measureOn){
      try{ doc.documentElement.classList.remove('shoshin-print-measure'); } catch(_){}
    }
  }
}

/* =====================================================================
OPEN PRINT TAB DOCUMENT
- Writes a complete standalone HTML document into a new tab
- Runs pagination/justification after layout settles (2x rAF) and images load
===================================================================== */
function openPrintTabDocument(title, cssText, bodyHtml) {
  ...
  var faviconTag = faviconHref ? ('<link rel="icon" href="' + esc(faviconHref) + '">') : '';

  // Canonical extra CSS for fragment pagination + measure-mode
  var EXTRA_PRINT_CSS =
    "\n/* PRINT-FRAGMENTS (Option 3): print-only pagination contract; screen uses measure-mode */" +
    "\n@media print{" +
    "\n  .print-pages-wrap{ position:relative; z-index:1; }" +
    "\n  .print-frag{ height: 749px; box-sizing:border-box; }" +
    ...
    "\n  .print-frag-body{ flex:1 1 auto; min-height:0; display:flex; flex-direction:column; justify-content:flex-start; }" +
    "\n  .print-group{ display:block; break-inside:avoid; page-break-inside:avoid; }" +
    "\n}" +

    "\n@media screen{" +
    "\n  /* Screen preview: keep header aligned with the page stack (filled elsewhere if needed) */" +
    "\n}" +

    "\n.shoshin-print-measure .print-pages-wrap{ position:relative; z-index:1; }" +
    "\n.shoshin-print-measure .print-frag{ height: 749px !important; min-height:0 !important; box-sizing:border-box !important; padding:0 !important; margin:0 !important; box-shadow:none !important; background:transparent !important; }" +
    "\n.shoshin-print-measure .print-frag-inner{ height:100% !important; min-height:100% !important; display:flex !important; flex-direction:column !important; }" +
    "\n.shoshin-print-measure .print-frag-body{ flex:1 1 auto !important; min-height:0 !important; display:flex !important; flex-direction:column !important; justify-content:flex-start !important; }" +

    "\n/* COMPACT PACKS (FIT 4/PAGE): tighten vertical padding in print + measure-mode */" +
    "\n@media print{" +
    "\n  .print-unit-row{ margin-top:4px !important; padding:4px 6px !important; }" +
    "\n  .print-row2-group-title{ margin-bottom:2px !important; font-size:11px !important; line-height:1.05 !important; }" +
    "\n  .print-row2-grid .print-row2-h{ padding-top:2px !important; padding-bottom:2px !important; }" +
    "\n  .print-cm-keyline{ margin:2px 0 4px 0 !important; font-size:10px !important; line-height:1.05 !important; }" +
    "\n  .print-cm-tags-grid{ gap:2px !important; }" +
    "\n  .print-cm-tag{ padding-top:2px !important; padding-bottom:2px !important; }" +
    "\n}" +
    "\n.shoshin-print-measure .print-unit-row{ margin-top:4px !important; padding:4px 6px !important; }" +
    "\n.shoshin-print-measure .print-row2-group-title{ margin-bottom:2px !important; font-size:11px !important; line-height:1.05 !important; }" +
    "\n.shoshin-print-measure .print-row2-grid .print-row2-h{ padding-top:2px !important; padding-bottom:2px !important; }" +
    "\n.shoshin-print-measure .print-cm-keyline{ margin:2px 0 4px 0 !important; font-size:10px !important; line-height:1.05 !important; }" +
    "\n.shoshin-print-measure .print-cm-tags-grid{ gap:2px !important; }" +
    "\n.shoshin-print-measure .print-cm-tag{ padding-top:2px !important; padding-bottom:2px !important; }" +

    "\n/* BEGIN SHOSHIN — TRUE HEIGHT CUTS (TARGET 4 PACKS/FRAG) */" +
    "\n@media print{" +
    "\n  .shoshin-divider-row{ height: 20px !important; }" +
    "\n  .shoshin-divider-row .print-section-divider{ height: 12px !important; }" +
    "\n  .print-section-thumb{ width: 40px !important; height: 40px !important; }" +
    "\n  .print-section-title{ font-size: 16px !important; gap: 8px !important; }" +
    "\n  .print-row2-v{ min-height: 42px !important; padding-top: 6px !important; padding-bottom: 6px !important; }" +
    "\n}" +
    "\n@media screen{" +
    "\n  .shoshin-divider-row{ height: 20px !important; }" +
    "\n  .shoshin-divider-row .print-section-divider{ height: 12px !important; }" +
    ...
    "\n  .print-row2-v{ min-height: 42px !important; padding-top: 6px !important; padding-bottom: 6px !important; }" +
    "\n}" +
    "\n.shoshin-print-measure .shoshin-divider-row{ height: 20px !important; }" +
    "\n.shoshin-print-measure .shoshin-divider-row .print-section-divider{ height: 12px !important; }" +
    "\n.shoshin-print-measure .print-section-thumb{ width: 40px !important; height: 40px !important; }" +
    "\n.shoshin-print-measure .print-section-title{ font-size: 16px !important; gap: 8px !important; }" +
    "\n.shoshin-print-measure .print-row2-v{ min-height: 42px !important; padding-top: 6px !important; padding-bottom: 6px !important; }" +
    "\n/* END SHOSHIN — TRUE HEIGHT CUTS (TARGET 4 PACKS/FRAG) */" +
    "\n";
  ...
  function paginateThenPrint() {
    try {
      w.requestAnimationFrame(function(){
        w.requestAnimationFrame(function(){
          try {
            shoshinPaginateAndJustify(w, w.document, { extraCss: EXTRA_PRINT_CSS });
          } catch (_) {}
          setTimeout(tryPrintNow, 60);
        });
      });
    } catch (_) {
      try { shoshinPaginateAndJustify(w, w.document, { extraCss: EXTRA_PRINT_CSS }); } catch (_) {}
      setTimeout(tryPrintNow, 60);
    }
  }
...
/* =====================================================================
IN-PAGE CONSOLE HOOK (debuggable)
- If this script is present on a same-origin print view, you can call:
  window.SHOSHIN_PRINT_PAGINATE()
===================================================================== */
try{
  window.SHOSHIN_PRINT_PAGINATE = function(){
    try{
      return shoshinPaginateAndJustify(window, document, { extraCss: '' });
    } catch(_){
      return false;
    }
  };
...
function renderRow(rowObj, allowCmOverlap, prefixHtml) {
    ...
    return (
      '<div class="print-unit-row">' +
        ...
        // Bucket header occurs outside the row (section title).
...
/* BEGIN SHOSHIN PRINT — SINGLE PACK PER UNIT (FINAL: HEADER INSIDE PACK) */
var bucketThumb = pickBucketThumbFromRows(rows);
var hasThumbHeader = !!(ctx.includeImages && bucketThumb);

// Divider HTML (shared image; spacing controlled via CSS)
var dividerHtml = hasThumbHeader
  ? (
...
for (var r = 0; r < rows.length; r++) {
  ...
  // Original behavior: overlap allowed when header exists
  var allowCmOverlap = hasThumbHeader;

  /* BEGIN SHOSHIN TASK 2 — DIVIDER AS OWN PRINT PACK */
  body += '<div class="print-pack">';
/* END SHOSHIN TASK 2 — DIVIDER AS OWN PRINT PACK */


  // Thumb + bucket title row (INSIDE pack)
  body +=
  ...
  // Unit row
  body += renderRow(rows[r], allowCmOverlap);

  body += '</div>'; // .print-pack
  /* BEGIN SHOSHIN TASK 2 — DIVIDER PACK BETWEEN UNIT PACKS */
  // Divider should be a standalone pack that only appears BETWEEN unit packs.
  // It must never be first or last element in a fragment; paginator enforces that.
  if (hasThumbHeader && r < rows.length - 1){
    body += '<div class="print-pack print-pack-divider" aria-hidden="true">';
    body += dividerHtml;
    body += '</div>';
  }
  /* END SHOSHIN TASK 2 — DIVIDER PACK BETWEEN UNIT PACKS */
  renderedRows++;
}
/* END SHOSHIN PRINT — SINGLE PACK PER UNIT (FINAL: HEADER INSIDE PACK) */
...
'<div class="print-page print-mode-' + (ctx.mode === 'consolidated' ? 'consolidated' : 'comprehensive') + '">' +
  '<div class="print-bg"></div>' +
      ...
      header +
      '<div class="print-body">' + (body || '<div class="print-empty">No units assigned.</div>') + '</div>' +
...
.print-pack{
  ...
  /* Divider micro-adjust “knob” (default 0). Set per pack via style.setProperty('--divider-mb', '...px') */
  --divider-mb: -10px;
}

.print-pack .print-section-divider{
