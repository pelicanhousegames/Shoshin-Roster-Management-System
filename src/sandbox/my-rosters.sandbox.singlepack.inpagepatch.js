...
var w = window.open('', '_blank');
  ...
  //
  // Groupings:
  //  - .shoshin-divider-row => grouping (solo) [only when visible]
  //  - .print-section-title + FIRST .print-unit-row => grouping (paired)
  //  - remaining .print-unit-row => grouping (solo)
  //  
  //  NOTE - Do not make these rules apply to all media; it will reintroduce screen clipping.
  // ================================================================
var EXTRA_PRINT_CSS =
  '\n/* PRINT-FRAGMENTS (MVP): keep print-only rules for actual printing; use measure-mode for JS justification */' +
  '\n@media print{' +
  ...
  '\n/* MEASURE-MODE: temporarily impose page-height contract in SCREEN media so JS can compute free space. */' +
  '\n.shoshin-print-measure .print-pages-wrap{ position:relative; z-index:1; }' +
	  // IMPORTANT: Beat the tall-sheet SCREEN preview overrides (which use !important).
...
// BEGIN SHOSHIN PRINT — IN-PAGE (print-mode-consolidated) PAGINATION BRIDGE
// Some deployments open the print view as a same-origin /my-rosters tab (not about:blank).
// In that case, there is no opener-side JS running inside the print tab. This bridge
// exposes a safe in-page paginator so Option 3 (fragments + justification) can run.
function shoshinEnsureExtraPrintCSS(doc){
  try{
    if (!doc) return;
    ...
    st.type = 'text/css';
    st.appendChild(doc.createTextNode(String(EXTRA_PRINT_CSS || '')));
    (doc.head || doc.getElementsByTagName('head')[0] || doc.documentElement).appendChild(st);
  } catch(_){ }
}

/* BEGIN SHOSHIN PRINT — IN-PAGE PAGINATION BRIDGE (FIX MEASURE TIMING) */
function shoshinPaginateAndJustifyInPlace(){
  var doc = document;
  ...
  // Ensure fragment + measure-mode CSS exists in this tab
  shoshinEnsureExtraPrintCSS(doc);

  // Collect visible packs in DOM order
  var packs = [];
  try{
    packs = Array.prototype.slice.call(body.querySelectorAll('.print-pack')).filter(isVisible);
  } catch(_){ packs = []; }
  if (!packs.length) return false;

  // Turn on measure-mode NOW and KEEP IT ON until after run() finishes.
  var _measureOn = false;
  try{ doc.documentElement.classList.add('shoshin-print-measure'); _measureOn = true; } catch(_){}

  function run(){
    // Mark paginated now (prevents loops)
    root.setAttribute('data-paginated-space-evenly', '1');

    var wrap = doc.createElement('div');
    wrap.className = 'print-pages-wrap';

    /* BEGIN SHOSHIN PRINT — ATTACH WRAP BEFORE OVERFLOW MEASURE (IN-PAGE DETACHED FIX) */
var _wrapInserted = false;
try {
...
/* END SHOSHIN PRINT — ATTACH WRAP BEFORE OVERFLOW MEASURE (IN-PAGE DETACHED FIX) */
    ...
    function pageOverflows(){
      // Force layout
      try{ currentBody.getBoundingClientRect(); } catch(_){}
    ...
    for (var i=0; i<packs.length; i++){
      ...
      var placed = appendPack(p);

      if (pageOverflows() && hadPrior){
...
/* BEGIN SHOSHIN PRINT — REMOVE ORIGINAL BODY AFTER PAGINATION (IN-PAGE DETACHED FIX) */
try {
  if (body && body.parentNode) {
    if (!_wrapInserted) body.parentNode.insertBefore(wrap, body);
    body.parentNode.removeChild(body);
  }
} catch (_) {}
/* END SHOSHIN PRINT — REMOVE ORIGINAL BODY AFTER PAGINATION (IN-PAGE DETACHED FIX) */


    // Justification spacers (skip if <=2 groups)
    try{
      ...
      for (var bi=0; bi<fragBodies.length; bi++){
        ...
        // remove old spacers
        try{
          var olds = b.querySelectorAll(':scope > .print-justify-spacer');
          for (var oi=0; oi<olds.length; oi++){
            try{ b.removeChild(olds[oi]); } catch(_){}
          }
        } catch(_){}

        var groups = [];
        for (var ki=0; ki<(b.children ? b.children.length : 0); ki++){
          var child = b.children[ki];
          if (child && child.classList && child.classList.contains('print-group')) groups.push(child);
        }
        ...
        for (var gi=0; gi<groups.length; gi++){
          var r = groups[gi].getBoundingClientRect();
          used += (r && r.height) ? r.height : 0;
        }

        var free = (b.clientHeight||0) - used;
        if (free <= 0){
          b.style.justifyContent = 'flex-start';
          continue;
        }
        ...
        if (PRINT_JUSTIFY_MAX_GAP_PX > 0) gapPx = Math.min(gapPx, PRINT_JUSTIFY_MAX_GAP_PX);

        for (var sj=0; sj<groups.length-1; sj++){
          var spacer = doc.createElement('div');
          spacer.className = 'print-justify-spacer';
          spacer.style.display = 'block';
          spacer.style.height = String(gapPx) + 'px';
          spacer.style.flex = '0 0 auto';
          b.insertBefore(spacer, groups[sj].nextSibling);
        }
      }
    } catch(_){}
  ...
  // IMPORTANT: remove measure-mode ONLY after run() has executed.
...
/* END SHOSHIN PRINT — IN-PAGE PAGINATION BRIDGE (FIX MEASURE TIMING) */



/* BEGIN SHOSHIN PRINT — EXPOSE + AUTO-RUN IN-PAGE PAGINATION (PRINT TAB) */
try {
  window.SHOSHIN_PRINT_PAGINATE = function(){
    try { return shoshinPaginateAndJustifyInPlace(); } catch(_) { return false; }
  };
  window.paginateAndSpaceEvenly = window.SHOSHIN_PRINT_PAGINATE;
} catch(_){}

(function(){
  try {
    var root = document.querySelector('.print-page');
    if (!root) return;

    // If already fragmented, do nothing
    if (document.querySelector('.print-pages-wrap, .print-frag')) return;

    // If no packs, nothing to paginate
    if (!document.querySelector('.print-pack')) return;

    window.requestAnimationFrame(function(){
      window.requestAnimationFrame(function(){
        try { shoshinPaginateAndJustifyInPlace(); } catch(_){}
      });
    });
  } catch(_){}
})();
/* END SHOSHIN PRINT — EXPOSE + AUTO-RUN IN-PAGE PAGINATION (PRINT TAB) */
...
// END PRINT JUSTIFY — MIN GAP THRESHOLD
  ...
  function paginateAndSpaceEvenly() {
    ...
    // Collect visible packs (atomic units) in DOM order inside .print-body
    var packs = [];
    try {
      packs = Array.prototype.slice.call(
        body.querySelectorAll('.print-pack')
      ).filter(isVisible);
    } catch (_) { packs = []; }
    if (!packs.length) return;

    // ==============================================================
    // SLOT MODEL (simplified)
    // Each .print-pack contains: (optional) divider + title + unit row.
    // Pagination + justification must treat each pack as atomic.
    // ==============================================================
    var slots = [];
    for (var i = 0; i < packs.length; i++) {
      slots.push({ kind: 'PACK', nodes: [packs[i]] });
    }

    if (!slots.length) return;

// Enable temporary measure mode so JS can compute free space against a fixed page height in screen media.
/* BEGIN SHOSHIN PRINT — MEASURE MODE GUARDED */
var _measureOn = false;
try {
  doc.documentElement.classList.add('shoshin-print-measure');
  _measureOn = true;
} catch (_) {}
/* END SHOSHIN PRINT — MEASURE MODE GUARDED */
...
/* BEGIN SHOSHIN PRINT — ATTACH WRAP BEFORE OVERFLOW MEASURE (DETACHED FIX) */
var _wrapInserted = false;
try {
  if (body && body.parentNode) {
    body.parentNode.insertBefore(wrap, body);
...
/* END SHOSHIN PRINT — ATTACH WRAP BEFORE OVERFLOW MEASURE (DETACHED FIX) */
...
function pageOverflows() {
  // Force layout flush before reading scroll/client sizes.
  // Without this, Chrome can report equal values during the build loop,
  // causing pagination to never split into multiple fragments.
  try { currentBody.getBoundingClientRect(); } catch (_) {}
  return (currentBody.scrollHeight > currentBody.clientHeight);
}
...
/* BEGIN SHOSHIN PRINT — REMOVE ORIGINAL BODY AFTER PAGINATION (DETACHED FIX) */
try {
  if (body && body.parentNode) {
    // wrap was already inserted before body (preferred), but keep fallback safety
    if (!_wrapInserted) body.parentNode.insertBefore(wrap, body);
    body.parentNode.removeChild(body);
  }
} catch (_) {}
/* END SHOSHIN PRINT — REMOVE ORIGINAL BODY AFTER PAGINATION (DETACHED FIX) */
...
try {
  ...
  /* BEGIN SHOSHIN PRINT — MEASURE MODE CLEANUP */
  if (_measureOn) {
    try { doc.documentElement.classList.remove('shoshin-print-measure'); } catch (_) {}
  }
  /* END SHOSHIN PRINT — MEASURE MODE CLEANUP */
}
  }
  ...
  function tryPrintWhenReady() {
    ...
    // Paginate + space-evenly, then print
	  // IMPORTANT: Let the print tab finish layout before we measure/split into fragments.
	  // (Without this, Chrome can report no overflow and we end up with a single giant .print-frag.)
	  /* BEGIN SHOSHIN PRINT — POST-LAYOUT PAGINATION (2x rAF) */
	  try {
	    w.requestAnimationFrame(function(){
	      w.requestAnimationFrame(function(){
	        try { paginateAndSpaceEvenly(); } catch (_) {}
	      });
	    });
	  } catch (_) {
	    try { paginateAndSpaceEvenly(); } catch (_) {}
	  }
	  /* END SHOSHIN PRINT — POST-LAYOUT PAGINATION (2x rAF) */
...
// Divider HTML (included inside each pack when thumbs/dividers are ON)
...
for (var r = 0; r < rows.length; r++) {
  // Overlap is only allowed when the thumb/header exists (images ON mode)
