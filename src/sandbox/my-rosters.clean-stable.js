...
function appendSlot(slotObj) {
  ...
  /* BEGIN SHOSHIN — DIVIDER NEVER FIRST ON PAGE */
  var isFirstGroupOnPage = !(currentBody.children && currentBody.children.length > 0);

  var nodes = (slotObj && slotObj.nodes) ? slotObj.nodes : [];

  // If this slot would become the first element on the page,
  // remove any leading divider row so a divider is never the first element.
  if (isFirstGroupOnPage && nodes && nodes.length) {
    // Only strip the leading divider (do not touch title/unit).
    while (
      nodes.length &&
      nodes[0] &&
      nodes[0].classList &&
      nodes[0].classList.contains('shoshin-divider-row')
    ) {
      nodes = nodes.slice(1);
    }
  }
  /* END SHOSHIN — DIVIDER NEVER FIRST ON PAGE */
...
// ==============================================================
/* BEGIN SHOSHIN — DIVIDER NOT FIRST/LAST PER FRAG (PER-PAGE) */
try {
  var fragBodiesFix = wrap.querySelectorAll('.print-frag-body');
  for (var fbi = 0; fbi < fragBodiesFix.length; fbi++) {
    var fb = fragBodiesFix[fbi];
    if (!fb) continue;

    // First element on this frag
    var firstGroup = fb.querySelector('.print-group:first-child');
    if (firstGroup && firstGroup.firstElementChild &&
        firstGroup.firstElementChild.classList &&
        firstGroup.firstElementChild.classList.contains('shoshin-divider-row')) {
      try { firstGroup.removeChild(firstGroup.firstElementChild); } catch (_) {}
    }

    // Last element on this frag
    var lastGroup = fb.querySelector('.print-group:last-child');
    if (lastGroup && lastGroup.lastElementChild &&
        lastGroup.lastElementChild.classList &&
        lastGroup.lastElementChild.classList.contains('shoshin-divider-row')) {
      try { lastGroup.removeChild(lastGroup.lastElementChild); } catch (_) {}
    }
  }
} catch (_) {}
/* END SHOSHIN — DIVIDER NOT FIRST/LAST PER FRAG (PER-PAGE) */
// ==============================================================
