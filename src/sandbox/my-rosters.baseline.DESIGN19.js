    ...
    '<div class="shoshin-modal-body shoshin-print-body">' +
      ...
      '<div class="shoshin-print-layouts">' +
        ...
        '<button type="button" class="shoshin-print-layout-card" data-layout="layout2" aria-pressed="false">' +
          ...
          '</div>' +
          '<div class="shoshin-print-layout-note">Info Intensive • Paper Saver</div>' +
        '</button>' +
...
/* Combat Modifiers header: bottom + left aligned */
...
var meta =
  ...
  var row1 =
    '<tr class="l2-buckets">' +
      ...
      '<th class="l2-modshdr" rowspan="2"><div class="l2-mods-hdr">Combat Modifiers</div></th>' +
    '</tr>';
...
// END SHOSHIN PRINT — SLOT MODEL (SIMPLIFIED PER-ROW)
    ...
    // ==============================================================
// ==============================================================
/* BEGIN PRINT JUSTIFY — FLEX DISTRIBUTION (POST-PAGINATION, SAFE) */
// NOTE: The runtime paginator already applies vertical justification (Task 1) after pagination.
// This block MUST NOT override paginator justification (it caused “only last page justifies” behavior).
try{
  var wrap0 = w.document && w.document.querySelector
    ? w.document.querySelector('.print-pages-wrap')
    : null;

  // If the deterministic paginator ran, do nothing here.
  // (Prevents the overflow-guard from disabling justification on “tight” pages.)
  var alreadyPaginated = !!(wrap0 && String(wrap0.getAttribute('data-shoshin-paginated') || '') === '1');
  if(!alreadyPaginated){
    // Legacy fallback only (should rarely run now)
    var fragBodies = w.document.querySelectorAll ? w.document.querySelectorAll(".print-frag-body") : [];
    for (var fi = 0; fi < fragBodies.length; fi++){
      var bodyJ = fragBodies[fi];
      if (!bodyJ) continue;

      var unitGroups = bodyJ.querySelectorAll('.print-unit-group');
      var unitCount = unitGroups && unitGroups.length ? unitGroups.length : 0;
      if (!unitCount){
        var unitRows = bodyJ.querySelectorAll('.print-unit-row');
        unitCount = unitRows && unitRows.length ? unitRows.length : 0;
      }

      if (unitCount <= 2){
        bodyJ.style.justifyContent = 'flex-start';
        bodyJ.style.rowGap = '';
        continue;
      }

      // Use a more forgiving tolerance if this fallback ever runs.
      if (bodyJ.scrollHeight > bodyJ.clientHeight + 2){
        bodyJ.style.justifyContent = 'flex-start';
        bodyJ.style.rowGap = '';
        continue;
      }

      bodyJ.style.display = 'flex';
      bodyJ.style.flexDirection = 'column';
      bodyJ.style.justifyContent = 'space-between';
      bodyJ.style.rowGap = '0px';
    }
  }
}catch(e){
  try{ console.warn('[SHOSHIN PRINT] justify (flex) failed:', e); }catch(_e){}
}
/* END PRINT JUSTIFY — FLEX DISTRIBUTION (POST-PAGINATION, SAFE) */
...
'<body>' +
  ...
  '<script>(function(){' +
    ...
    'var DBG = window.__SHOSHIN_PAGINATION_DEBUG__ = { ts: Date.now(), stage: null, samples: [] };' +
    'function setStage(s){ try{ DBG.stage=s; window.__SHOSHIN_PAGINATION_STAGE__=s; }catch(e){} }' +
...
'var __geom = 0;' +
  ...
  'try{ DBG && DBG.samples && DBG.samples.push({event:"availCalc", page: (pageIndex+1), mode: __mode, avail: avail, geom: __geom, fragH: rectH(templateFrag), headerH: ((pageIndex===0 && q(".print-header, .print-frag-header", templateFrag)) ? rectH(q(".print-header, .print-frag-header", templateFrag)) : 0), padTop: padTop, padBot: padBot}); }catch(e){}' +
  'return Math.max(0, avail);' +
'}' +

'try{ w.__SHOSHIN_GETAVAILPX__ = getAvailPx; }catch(e){}' +
...
'}' +
'DBG.samples.push({event:\"atomHeights\", count: atoms.length, zeros: atomH.filter(function(x){return !x;}).length});' +
...
/* BEGIN SHOSHIN PRINT — L2 PENDING DIVIDER (DEFER UNTIL BUCKET COMPLETES) */
'var __PENDING_DIV__ = null;' +
'var __PENDING_DIV_H__ = 0;' +
/* END SHOSHIN PRINT — L2 PENDING DIVIDER (DEFER UNTIL BUCKET COMPLETES) */


/* BEGIN SHOSHIN PRINT — BONDED DIVIDER+UNIT PACKING */
'for(var i=0;i<atoms.length;i++){' +
'var node = atoms[i];' +
'if(!node){ continue; }' +

  'var next = (i+1<atoms.length) ? atoms[i+1] : null;' +
  'var nodeIsDiv = isDivider(node);' +

  // Always resolve GAP fresh (match appendAtom spacing decisions)
  'if(!GAP_PX) GAP_PX = getGapPx();' +

  // Project used height if we add a sequence of heights (gap-before-each-item-except-first)
  ...
  '}' +
    ...
    /* BEGIN SHOSHIN PRINT — L2 BUCKET SPLIT HELPERS (HEADER DUPLICATION) */
  'var __MEAS = null;' +
  'function ensureMeasurer(){' +

    // If it already exists, keep its width synced to the current page body.
    'if(__MEAS){' +
      'try{' +
        'if(body){ __MEAS.style.width = Math.round(body.getBoundingClientRect().width) + "px"; }' +
      '}catch(e){}' +
      'return __MEAS;' +
    '}' +

    'try{' +
      '__MEAS = document.createElement("div");' +
      '__MEAS.setAttribute("data-shoshin-measure","1");' +

      // IMPORTANT:
      // Absolute positioned elements with width:auto are shrink-to-fit.
      // That can be narrower than the real print body and cause extra wrapping → taller measurements.
      '__MEAS.style.cssText="position:absolute;left:-9999px;top:0;visibility:hidden;pointer-events:none;box-sizing:border-box;";' +

      // Match the live body width so wrapping matches the real page.
      'if(body){ __MEAS.style.width = Math.round(body.getBoundingClientRect().width) + "px"; }' +

      'document.body.appendChild(__MEAS);' +
    '}catch(e){ __MEAS = null; }' +
    'return __MEAS;' +
  '}' +
...
'function measureNodeHeight(node){' +
  'var m = ensureMeasurer();' +
  'if(!m || !node) return 0;' +
  'try{' +
    'm.innerHTML = "";' +

    // Measure a CLONE so we never disturb the real DOM or the atom queue.
...
'}' +
  ...
  'function bucketRowCount(bucketEl){' +
    'try{' +
      'var rows = bucketEl.querySelectorAll("tr.l2-unit");' +
      'return rows ? rows.length : 0;' +
    '}catch(e){ return 0; }' +
  '}' +

  'function cloneBucketKeepingFirstN(bucketEl, keepN){' +
    'var clone = bucketEl.cloneNode(true);' +
    'try{' +
      'var rows = clone.querySelectorAll("tr.l2-unit");' +
      'for(var i=rows.length-1; i>=0; i--){' +
        'if(i >= keepN){ rows[i].parentNode.removeChild(rows[i]); }' +
      '}' +
    '}catch(e){}' +
    'return clone;' +
  '}' +

  'function cloneBucketKeepingAfterN(bucketEl, dropN){' +
    'var clone = bucketEl.cloneNode(true);' +
    'try{' +
      'var rows = clone.querySelectorAll("tr.l2-unit");' +
      'for(var i=dropN-1; i>=0; i--){' +
        'if(rows[i]) rows[i].parentNode.removeChild(rows[i]);' +
      '}' +
    '}catch(e){}' +
    'return clone;' +
  '}' +
  /* END SHOSHIN PRINT — L2 BUCKET SPLIT HELPERS (HEADER DUPLICATION) */


  // -----------------------------------------------------------------------
  // Divider as SEPARATOR between units (never first, never last)
  // We only place divider if [divider + nextUnit] fits.
  // If not, start a new page so nextUnit can start clean; divider is skipped.
  // -----------------------------------------------------------------------
  'if(nodeIsDiv){' +
    // no next → divider can’t separate anything
    'if(!next){ continue; }' +

    'var hDiv = atomH[i] || 0;' +
    'var hNext = atomH[i+1] || 0;' +

    // divider cannot be first on a page
    'if(countOnPage===0){ continue; }' +

    // Dividers are OPTIONAL: never force a page break to keep a divider.
    // If it doesn't fit here, skip it and keep packing content.
    'if(!projWillFit([hDiv], used, countOnPage, avail)){' +
      'continue;' +
    '}' +

    // Fits → place divider alone (do NOT bond it to the next atom)
  ...
  // -----------------------------------------------------------------------
  // Unit flow (Layout2 bucket-aware splitting)
  // Policy:
  // - Never move the entire bucket just because it doesn't fit.
  // - If it would split, duplicate header + continue unit rows on next page.
  // - Never leave only a header at the bottom: only split if (header + 1 row) fits.
  // -----------------------------------------------------------------------
  'var isBucket = !!(node && node.classList && node.classList.contains("l2-bucket-atom"));' +
  'var h = atomH[i] || 0;' +

  // Non-bucket atoms keep existing behavior
  'if(!isBucket){' +

    // Never allow a divider to be the first item on a page.
    // If it would land at top-of-page, drop it.
    'if(node && node.classList && node.classList.contains("l2-divider-atom") && countOnPage===0){' +
      'continue;' +
    '}' +
    ...
    'if(countOnPage>0 && !projWillFit([h], used, countOnPage, avail)){' +
      ...
      // If we broke to a new page, and this is a divider, drop it (no divider at top)
      'if(node && node.classList && node.classList.contains("l2-divider-atom")){' +
        'continue;' +
      '}' +
    '}' +
  ...
  // Bucket atom path: split if needed
  'var totalRows = bucketRowCount(node);' +
  'if(totalRows <= 1){' +
    // Not splittable meaningfully; allow normal behavior
    'if(countOnPage>0 && !projWillFit([h], used, countOnPage, avail)){' +
      'startNewPage();' +
    '}' +
    'appendAtom(node, h);' +
  ...
  // If full bucket fits, just place it
  'if(projWillFit([h], used, countOnPage, avail)){' +
    'appendAtom(node, h);' +
    'continue;' +
  '}' +

  // Compute minimum piece height: header + 1 row
  'var minPiece = cloneBucketKeepingFirstN(node, 1);' +
  'var minH = measureNodeHeight(minPiece);' +

  // If we have content already on this page and header+1row doesn't fit, start a new page
  // (prevents header alone at page bottom)
  'if(countOnPage>0 && !projWillFit([minH], used, countOnPage, avail)){' +
    'startNewPage();' +
  '}' +

  // Now we are either on a new page or we can fit at least header+1 row.
  // Find the largest k that fits on this page, leaving at least 1 row for the tail.
  'var kBest = 1;' +

  // Hard rule: never place a split head that contains 0 unit rows.
  // (kBest is "unit rows kept" — header always comes along for the ride.)
  'if(totalRows >= 2){ kBest = 1; }' +

  'for(var k=2; k<totalRows; k++){' +
    'var test = cloneBucketKeepingFirstN(node, k);' +
    'var testH = measureNodeHeight(test);' +
    'if(projWillFit([testH], used, countOnPage, avail)){' +
      'kBest = k;' +
    '} else {' +
      'break;' +
    '}' +
  ...
  // If we somehow still cannot fit header+1row on this page (geometry drift), force new page.
  'if(!projWillFit([minH], used, countOnPage, avail) && countOnPage>0){' +
    'startNewPage();' +
  '}' +


// Build head + tail buckets
  'var head = cloneBucketKeepingFirstN(node, kBest);' +
  'var headH = measureNodeHeight(head);' +
  'appendAtom(head, headH);' +

  // Tail becomes the remaining rows; enqueue it as the NEXT atom.
  'var tail = cloneBucketKeepingAfterN(node, kBest);' +
  'var tailRows = bucketRowCount(tail);' +

  // If there is a divider right after this bucket in the atom stream,
  // defer it globally until the bucket fully completes (may span many pages).
  'try{' +
    'if(i+1 < atoms.length && atoms[i+1] && isDivider(atoms[i+1])){' +
      '__PENDING_DIV__ = atoms[i+1];' +
      '__PENDING_DIV_H__ = atomH[i+1] || 0;' +
      'atoms.splice(i+1, 1);' +
      'atomH.splice(i+1, 1);' +
    '}' +
  '}catch(e){}' +
  ...
  // Retire the original full bucket atom so it cannot be appended later.
  'atoms[i] = null;' +
  'atomH[i] = 0;' +

  'if(tailRows > 0){' +
    'var tailH = measureNodeHeight(tail);' +
    'atoms.splice(i+1, 0, tail);' +
    'atomH.splice(i+1, 0, tailH);' +
    // Force continuation on the next page (no mid-page divider issues)
    'startNewPage();' +
  '}' +

  // If this bucket COMPLETES here (no tail), and a divider is pending,
  // enqueue it NEXT so it appears only after the final tail piece.
  'if(tailRows <= 0 && __PENDING_DIV__){' +
    'atoms.splice(i+1, 0, __PENDING_DIV__);' +
    'atomH.splice(i+1, 0, __PENDING_DIV_H__);' +
    '__PENDING_DIV__ = null;' +
    '__PENDING_DIV_H__ = 0;' +
  '}' +
...
/* Final safety: no page can end with divider (should be impossible with bonded placement, but keep guard) */
'try{' +
  'var last2 = body && body.lastElementChild ? body.lastElementChild : null;' +
  'if(last2 && isDivider(last2)){' +
    'body.removeChild(last2);' +
  '}' +
'}catch(e){}' +
/* END SHOSHIN PRINT — BONDED DIVIDER+UNIT PACKING */
...
/* END SHOSHIN — DIVIDER ATOM (CENTERED; HIDE IMAGE ONLY WHEN IMAGES OFF) */
  ...
  if (renderedRows > 0 && renderedRows < 4) {
    ...
    for (var i = 0; i < need; i++) {
      ...
      // Preserve separator invariants even for placeholders
      if (!firstUnit) body += renderDividerAtom();
      body += renderUnitAtom(placeholderRow);
      firstUnit = false;
      renderedRows++;
    }
  }
...
/* BEGIN SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT1 LANDSCAPE STUB) */
function renderQuickReferenceLayout1HTML(qr) {
  qr = qr || {};
  var rosterName = String(qr.rosterName || 'Quick Reference');
  var rosterRef  = String(qr.rosterRef || '');
  ...
  // - No “display images” dependency
  // - Keep it deterministic + single page
  return (
    '<div class="print-page print-page-qr" data-print-layout="layout1-qr">' +
      '<div class="print-frag">' +
        '<div class="print-frag-inner">' +

          // Title band (logo + title only, per your spec)
          '<div class="print-header" style="display:flex;align-items:center;gap:12px;padding:0.20in 0.25in 0.10in;">' +
            '<img class="print-logo" src="/wp-content/uploads/2025/11/Header_logo_300x150_1.png" alt="" aria-hidden="true" style="height:40px;width:auto;display:block;">' +
            '<div style="display:flex;flex-direction:column;line-height:1.05;">' +
              '<div style="font-size:18px;font-weight:800;">Quick Reference</div>' +
              '<div style="font-size:12px;font-weight:600;opacity:0.85;">' + esc(rosterName) + ' ' + (rosterRef ? ('• ' + esc(rosterRef)) : '') + '</div>' +
            '</div>' +
          '</div>' +

          // Body placeholder (curated content comes later)
          '<div class="print-frag-body" style="padding:0.10in 0.25in 0.25in;">' +
            '<div style="font-size:12px;font-weight:700;margin-bottom:8px;">Coming next:</div>' +
            '<ul style="margin:0;padding-left:18px;font-size:12px;line-height:1.35;">' +
              '<li>Turn Sequence</li>' +
              '<li>Conditions</li>' +
              '<li>Saving Throws</li>' +
              '<li>Movement Modifiers</li>' +
              '<li>Combat Modifiers</li>' +
            '</ul>' +
          '</div>' +
...
/* END SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT1 LANDSCAPE STUB) */

/* BEGIN SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT2 PORTRAIT STUB) */
      ...
      '<div class="print-frag">' +
        '<div class="print-frag-inner">' +
          '<div class="print-header" style="display:flex;align-items:center;gap:12px;padding:0.20in 0.25in 0.10in;">' +
            ...
            '</div>' +
          '</div>' +
          '<div class="print-frag-body" style="padding:0.10in 0.25in 0.25in;font-size:12px;">Portrait QR stub (Phase 2)</div>' +
        '</div>' +
      '</div>' +
    '</div>'
  );
}
/* END SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT2 PORTRAIT STUB) */
