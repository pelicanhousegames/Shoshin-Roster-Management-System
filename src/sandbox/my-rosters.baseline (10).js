...
'function getGapPx(){' +
  'try{' +
    'if(!body) return 0;' +
    'var st = w.getComputedStyle(body);' +
...
'for(var i=0;i<atoms.length;i++){' +
  ...
  // Always use the SAME gap logic as appendAtom() (avoid drift / stale globals)
  'function gapPx(){ var g = getGapPx(); return (g && isFinite(g)) ? g : 0; }' +
  'function willFit(addH){' +
    'addH = (addH && isFinite(addH) && addH > 0) ? addH : 0;' +
    'var g = gapPx();' +
    'var projected = used + ((countOnPage>0 && g) ? g : 0) + addH;' +
    'return projected <= avail;' +
  '}' +

  // -----------------------------------------------------------------------
  // Divider bonded to PREVIOUS unit (divider is "below" the unit above it)
  // Rules:
  // - Divider cannot be first on a page (skip)
  // - Divider cannot be last on a page (skip if last atom)
  // - If divider doesn't fit, drop it AND start new page (next unit starts page)
  // -----------------------------------------------------------------------
  'if(nodeIsDiv){' +
    // cannot be first
    'if(countOnPage===0){ continue; }' +
    // cannot be last atom in entire stream
    'if(i===atoms.length-1){ continue; }' +

    'var hDiv = atomH[i] || 0;' +

    // If divider would overflow: drop divider and page-break so next unit starts fresh
    'if(countOnPage>0 && !willFit(hDiv)){' +
      'startNewPage();' +
      'continue;' +
    '}' +

    // Normal: append divider (it is after a unit and fits)
    'appendAtom(node, hDiv);' +
    'continue;' +
  '}' +

  // -----------------------------------------------------------------------
  // Unit flow (enforce avail strictly)
  // If it won't fit, page break BEFORE it.
  // Also ensure we do not end a page with a divider (remove if present).
  // -----------------------------------------------------------------------
  'var h = atomH[i] || 0;' +

  'if(countOnPage>0 && !willFit(h)){' +
    // If we somehow ended with divider, remove it before breaking
