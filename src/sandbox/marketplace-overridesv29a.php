...
function shoshin_pmprowoo_get_membership_price_for_variations($discount_price, $lowest_price_level, $price, $product) {
    ...
    if (!empty($resolved['final_current']) && (float) $resolved['final_current'] > 0) {
...
function shoshin_resolve_variation_pricing_for_current_user($variation) {
    ...
    return array(
        ...
        'has_member_adjustment' => ((float) $final_current > 0 && (float) $public_current > 0 && (float) $final_current < (float) $public_current),
    );
}
...
if (is_product() && !$is_loop_context && $product->is_type('variable')) {
    ...
    if ($min_regular > 0 && $max_regular > 0 && $min_public > 0 && $max_public > 0 && $min_final > 0 && $max_final > 0) {
        ...
        if ($has_member_adjustment) {
             ...
             */
            if (!$has_flat_override && $teaser_percent > 0) {
                if ($has_public_sale) {
                    $savings_html = '<div class="shoshin-price-savings">You save an additional ' . $teaser_percent . '% on this item</div>';
                } else {
                    $savings_html = '<div class="shoshin-price-savings">You save ' . $teaser_percent . '% on this item</div>';
                }
            }
