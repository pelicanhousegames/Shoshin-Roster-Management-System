...
function ensurePrintModal() {
  ...
  '<div class="shoshin-print-option">' +
    ...
    '<div class="shoshin-print-option-text">' +
      '<div class="shoshin-print-option-label">Include Images?</div>' +
...
function openPrintTabDocument(title, cssText, bodyHtml) {
  ...
  // Use canonical escaper (deduped helper)
  ...
  // Use canonical favicon resolver (centralized helper)
  var faviconHref = '';
  try {
    faviconHref = (typeof getSiteFaviconHref === 'function') ? (getSiteFaviconHref() || '') : '';
  } catch (_) {
    faviconHref = '';
  }
  ...
  // Trigger print reliably (some browsers don't fire the load event the way you expect on document.write tabs)
  var didPrint = false;
  ...
  function tryPrintWhenReady() {
    ...
    try {
      ...
      for (var i = 0; i < imgs.length; i++) {
        // If any image isn't ready yet, wait a beat
        if (!imgs[i].complete) {
  ...
  // Fallback 2: if load doesn't fire / already fired, kick shortly after we finish writing
  setTimeout(tryPrintWhenReady, 120);
...
function pickBucketThumbFromRows(rows) {
  ...
  function buildCombatModifiersBlockHTML(unitEntryId, refId, qty, cost) {
    ...
    for (var n = 0; n < lines.length; n++) {
      // Bottom row start index = (rows-1)*cols
      var rowFromBottom = Math.floor(n / TAG_COLS);
      var col = n % TAG_COLS;
      var idx = ((TAG_ROWS - 1 - rowFromBottom) * TAG_COLS) + col;
      if (idx >= 0 && idx < total) placed[idx] = lines[n];
    }

    // Block 1 is a 6x4 grid:
    // - Col 1: empty (all 4 rows)
    // - Col 2: only bottom cell has "Combat Modifiers" (Viner Hand, right aligned)
    // - Col 3-6: 4x4 tag grid (MRBPA tags)
    var out = '';
    out += '<div class="print-cm-block">';
    out += '<div class="print-cm-grid">';

    // 4 rows * 6 cols = 24 cells
    // We output cells row-major (top->bottom, left->right)
    var tagPtr = 0;
    for (var r = 0; r < 4; r++) {
      for (var c = 0; c < 6; c++) {

        // Col 1: REF/(QTY) and COST/(% of clan points)
        if (c === 0) {
...
// r=0 top ... r=3 bottom
if (r === 2) {
  // REF ID + (QTY) on same line; QTY shown only in consolidated
...
// Future cells (empty placeholders)


          // Future cells (empty placeholders)
          out += '<div class="print-cm-cell print-cm-col1 print-cm-spacer">&nbsp;</div>';
          continue;
        }



        // Col 2: Combat Modifiers (single cell, vertically centered)
        if (c === 1) {
          if (r === 1) {
            out += ''
              + '<div class="print-cm-cell print-cm-combat">'
              +   '<div class="print-cm-combat-inner">Combat Modifiers</div>'
              + '</div>';
          } else {
            out += '<div class="print-cm-cell print-cm-spacer">&nbsp;</div>';
          }
          continue;
        }
        ...
        if (c === 2) {
          if (r === 1) {
            out += ''
              + '<div class="print-cm-cell print-cm-brace">'
        ...
        // Col 3-6: tags (4 cols)
        var val = placed[tagPtr++] || '';
        out += '<div class="print-cm-cell print-cm-tag">' + (val ? esc(val) : '&nbsp;') + '</div>';
      }
    }

    out += '</div>'; // .print-cm-grid
    out += '</div>'; // .print-cm-block
    return out;
  }


  function renderRow(rowObj) {
    ...
    return (
      '<div class="print-unit-row">' +
        ...
        '<div class="print-row-block print-row-block-cm">' +
          buildCombatModifiersBlockHTML(unitEntryId, refId, qty, cost) +

        '</div>' +

        // Block 2: REF ID + stat strip (site-style-ish)
        '<div class="print-row-block print-row-block-row2">' +
          buildRow2BlockHTML(
            stat(sample, 'mDmg'), stat(sample, 'mCrt'), stat(sample, 'mDis'),
            stat(sample, 'rDmg'), stat(sample, 'rCrt'), stat(sample, 'rDis'),
            stat(sample, 'atk'), stat(sample, 'def'), stat(sample, 'bod'),
            stat(sample, 'mov'), stat(sample, 'ldr'), stat(sample, 'ini')
          ) +
        '</div>' +
  ...
  function buildRow2BlockHTML(mDmg, mCrt, mDis, rDmg, rCrt, rDis, atk, def, bod, mov, ldr, ini){
  // Values may be raw, null, or already formatted; normalize via fmtStatValue.
  const v = (x) => fmtStatValue(x);
  const cellHead = (txt, extraCls) =>
    '<div class="print-row2-head ' + (extraCls || '') + '">' + esc(txt) + '</div>';
  const cellVal = (val) =>
    '<div class="print-row2-val">' + esc(v(val)) + '</div>';

  return '' +
    '<div class="print-row2">' +

      // MELEE WEAPON (3)
      '<div class="print-row2-group print-row2-melee">' +
        '<div class="print-row2-title">MELEE WEAPON</div>' +
        '<div class="print-row2-grid row2-3">' +
          cellHead('Damage','is-melee') +
          cellHead('Critical','is-melee') +
          cellHead('Distance','is-melee') +
          cellVal(mDmg) +
          cellVal(mCrt) +
          cellVal(mDis) +
        '</div>' +
      ...
      // RANGED WEAPON (3)
      '<div class="print-row2-group print-row2-ranged">' +
        '<div class="print-row2-title">RANGED WEAPON</div>' +
        '<div class="print-row2-grid row2-3">' +
          cellHead('Damage','is-ranged') +
          cellHead('Critical','is-ranged') +
          cellHead('Distance','is-ranged') +
          cellVal(rDmg) +
          cellVal(rCrt) +
          cellVal(rDis) +
        '</div>' +
      ...
      // CHARACTER STATS (6)
      '<div class="print-row2-group print-row2-stats">' +
        '<div class="print-row2-title">CHARACTER STATS</div>' +
        '<div class="print-row2-grid row2-6">' +
          cellHead('Attack','is-stats') +
          cellHead('Defense','is-stats') +
          cellHead('Body','is-stats') +
          cellHead('Movement','is-stats') +
          cellHead('Leadership','is-stats') +
          cellHead('Initiative','is-stats') +
          cellVal(atk) +
          cellVal(def) +
          cellVal(bod) +
          cellVal(mov) +
...
function renderStatBox(label, value) {
  ...
  // Count only sections that actually render (rows.length > 0)
  var totalRenderedSections = 0;
  for (var _s = 0; _s < ctx.grouped.length; _s++) {
    var _section = ctx.grouped[_s];
    var _rows = safeArray(_section && _section.rows);
    if (_rows.length) totalRenderedSections++;
  }

  var renderedSectionsSoFar = 0;
  ...
  for (var s = 0; s < ctx.grouped.length; s++) {
    ...
    // Manual pagination by rendered section count (4 sections per printed page)
    var SECTIONS_PER_PAGE = 4;

    var sectionIndexOnPage = (renderedSectionsSoFar % SECTIONS_PER_PAGE);
    var isLastOnPage = (sectionIndexOnPage === (SECTIONS_PER_PAGE - 1));
    var isLastOverall = (renderedSectionsSoFar === (totalRenderedSections - 1));

    // Divider: default SHOW (visibility is controlled only by the Include Images toggle wrapper)
    var showDivider = true;


    // Force a page break after the last row of each page (except if it’s the final row overall)
    var sectionBreakStyle = (isLastOnPage && !isLastOverall)
      ? ' style="break-after:page;page-break-after:always;"'
      : '';

    body += '<section class="print-section"' + sectionBreakStyle + '>';


    var bucketThumb = pickBucketThumbFromRows(rows);

    // Divider row (collapses completely when images are OFF)
    body +=
      (ctx.includeImages
        ? (
          '<div class="shoshin-divider-row" style="position:relative;width:100%;height:32px;display:flex;align-items:center;justify-content:center;">' +
            (showDivider
              ? ('<img class="print-section-divider" src="' + esc(dividerSrc) + '" alt="" aria-hidden="true" style="height:24px;width:auto;display:block;">')
              : '') +
          '</div>'
        )
        : '');

    // Thumb + bucket row (unchanged behavior)
    body +=
      '<div class="print-section-title" style="position:relative;width:100%;">' +
        ((ctx.includeImages && bucketThumb)
    ...
    renderedSectionsSoFar++;




    for (var r = 0; r < rows.length; r++) {
      body += renderRow(rows[r]);
      renderedRows++;
    }
...
'<div class="print-page print-mode-' + (ctx.mode === 'consolidated' ? 'consolidated' : 'comprehensive') + '">' +
  '<div class="print-bg"></div>' +
      ...
      '<div class="print-footer">' +
        '<div class="print-footer-left">Mode: ' + (ctx.mode === 'consolidated' ? 'Consolidated' : 'Comprehensive') + '</div>' +
        '<div class="print-footer-right">' + (ctx.rosterRef ? esc(ctx.rosterRef) : '') + '</div>' +
      '</div>' +
    '</div>'
  );
}
...
function getPrintBaseCSS() {
  // NOTE: This is layout-agnostic print "normalization".
...
.print-section, .print-unit-row { break-inside: avoid; page-break-inside: avoid; }
`
  );
}
...
.print-section { margin-top: 10px; page-break-inside: avoid; }
.print-section-title {
  font-family: "Viner Hand ITC", "Viner Hand", cursive;
  font-size: 18px;
  font-weight: 500;
...
.print-unit-row { margin-top: 8px; padding: 8px 10px; page-break-inside: avoid; }
...
.print-cm-grid {
  display: grid;
  grid-template-columns: 0.55fr max-content max-content repeat(4, 1fr);
  gap: 4px 4px;                 /* tighter horizontal gap */
  align-items: stretch;
}
...
.print-row2-grid.row2-3{ grid-template-columns: repeat(3, 1fr); }
.print-row2-grid.row2-6{ grid-template-columns: repeat(6, 1fr); }

.print-row2-head,
.print-row2-val{
  border-right: 1px solid #111;
}
.print-row2-grid > :nth-child(3n):is(.print-row2-head,.print-row2-val),
.print-row2-grid.row2-6 > :nth-child(6n):is(.print-row2-head,.print-row2-val){
  border-right: 0;
}
...
.print-row2-head.is-melee{ background:#a34d2a; }
.print-row2-head.is-ranged{ background:#3f546b; }
.print-row2-head.is-stats{ background:#2e6a50; }
...
.print-footer { margin-top: 10px; display:flex; justify-content:space-between; font-size: 11px; opacity: .85; position:relative; z-index:1; }
`
  );
}
