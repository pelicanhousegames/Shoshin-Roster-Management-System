...
if ( ! function_exists( 'shoshin_render_batch_estimate_bridge' ) ) {
	function shoshin_render_batch_estimate_bridge() {
		...
		(function() {
            ...
            return row.querySelector('.woocommerce-shipping-destination, .shipping-destination, small, p');
            }
			...
			function ensureEstimateContainer(row) {
				...
				var target = row.querySelector('td, th, .shipping, li, ul.woocommerce-shipping-methods');
            ...
            function renderBatchEstimateShell(row, state) {
                ...
                if ((isCartPage() && addressKnown && !hasEstimate) || (isCheckoutPage() && addressKnown && !hasEstimate)) {
                    linkHtml = '<a href="#" class="shoshin-batch-estimate-link">Estimate Shipping</a>';
                }
                ...
                if (hasEstimate) {
                    ...
                    estimateLabel = 'Estimate: ' + state.estimate.service + ' — ' + state.estimate.formatted_amount;
                    }
                ...
                var rowTextNodes = cell.querySelectorAll('small, p, .woocommerce-shipping-destination, .shipping-destination');
                rowTextNodes.forEach(function(node) {
                    var text = (node.textContent || '').toLowerCase();

                    if (text.indexOf('shipping calculated at a later date') !== -1) {
                    node.style.display = 'none';
                    }

                    if (isCheckoutPage() && text.indexOf('shipping will be charged at a later date') !== -1 && !node.classList.contains('shoshin-batch-deferred-line')) {
                    node.style.display = 'none';
                    }
                });
                ...
                if (isCheckoutPage()) {
                    ...
                    deferredText.style.marginTop = '6px';

                    if (addressNode && addressNode.parentNode) {
                        addressNode.parentNode.insertBefore(deferredText, addressNode.nextSibling);
                    } else {
                        cell.appendChild(deferredText);
                    }
                    } else {
