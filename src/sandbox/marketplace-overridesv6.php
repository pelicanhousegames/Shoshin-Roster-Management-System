 ...
 * Applies only to the CURRENT user's membership level
 * -------------------------------------------------- */
...
if (function_exists('pmpro_getMembershipLevelForUser') && is_user_logged_in()) {
    ...
    if ($level) {
        ...
        $member_price_meta = get_post_meta($product->get_id(), '_pmpro_membership_pricing', true);

        if (isset($member_price_meta[$level->id]['price']) && (float)$member_price_meta[$level->id]['price'] === 0.0) {
...
/* --------------------------------------------------
 * MEMBERSHIP FREE ITEM DISPLAY OVERRIDE
 * -------------------------------------------------- */
...
if ($membership_free) {
    ...
    if (shoshin_has_annual_membership_discount()) {
        ...
        return '
        <div class="shoshin-price-block shoshin-price-block--member-free">
            <div class="shoshin-price-label">Member Discount</div>
            <div class="shoshin-price-final shoshin-price-final--member">FREE</div>
            <div class="shoshin-price-savings">You receive this item for FREE</div>
        </div>';
    ...
    <div class="shoshin-price-block shoshin-price-block--member-free">
        ...
        <div class="shoshin-price-savings shoshin-price-member-teaser">Paid Members receive this item for FREE</div>
    </div>';
}
    ...
    if (!$sale_active && !$member_discount_active) {
        ...
        if (!shoshin_has_annual_membership_discount()) {
            $teaser_html = '<div class="shoshin-price-savings shoshin-price-member-teaser">' . ($membership_free_for_user ? '<a href="/membership">Supporting Members</a> receive this item for FREE' : shoshin_format_member_teaser_text($guest_member_amount, $guest_member_percent, false, false)) . '</div>';
        }
    ...
    if (!$sale_active && $member_discount_active) {
        ...
        <div class="shoshin-price-block shoshin-price-block--member-only">
            ...
            <div class="shoshin-price-savings">' . ($membership_free_for_user ? 'You receive this item for FREE' : shoshin_format_savings_text($member_amount, $member_percent, 'You Save')) . '</div>
        </div>';
    }
    ...
    <div class="shoshin-price-block shoshin-price-block--member-sale">
        ...
        <div class="shoshin-price-savings">' . ($membership_free_for_user ? 'You receive this item for FREE' : shoshin_format_savings_text($total_amount, $total_percent, 'Total Savings')) . '</div>
    </div>';
}
