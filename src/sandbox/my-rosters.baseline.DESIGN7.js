...
@media screen{
  ...
  .print-pages-wrap{ display:flex; flex-direction:column; align-items:center; gap:24px; }
    .print-frag{
    background:#fff;
    background-image: url("/wp-content/uploads/2026/01/bgtest.jpg");
...
/* BEGIN SHOSHIN BASELINE — FORCE OVERFLOW SEQ (12 units) */
// Sequence: U D U D ... U (12 units, 11 dividers)
const seq = (function(){
  const out = [];
  for (let i = 0; i < 12; i++) {
    out.push(unitHTML(i));
    if (i < 11) out.push(dividerHTML());
  }
  return out.join("");
})();
/* END SHOSHIN BASELINE — FORCE OVERFLOW SEQ (12 units) */
...
function openPrintTabDocumentBaseline(cfg){
  ...
  /* BEGIN SHOSHIN PRINT BASELINE — FAVICON TAG (CLEAN) */
  // Clone ALL icon-like <link rel="..."> tags from the opener document (absolute hrefs).
  // NOTE: This keeps favicon handling isolated to a single, deterministic mechanism.
...
/* END SHOSHIN PRINT BASELINE — FAVICON TAG (CLEAN) */
...
/* END SHOSHIN PRINT — BASELINE LOG DEBUG-ONLY */
        ...
        '}' +
                'var run = measure;' +
/* BEGIN SHOSHIN PRINT — BEFORE/AFTERPRINT STABLE REPAGINATE */
'function runSoon(delay){' +
  'try{' +
    'setTimeout(function(){' +
      // two RAFs makes sure print media + layout have settled
      'requestAnimationFrame(function(){ requestAnimationFrame(run); });' +
    '}, (delay||0));' +
  '}catch(e){ try{ setTimeout(run, (delay||0)); }catch(__e){} }' +
'}' +
'window.addEventListener("load", function(){ runSoon(80); });' +
'window.addEventListener("beforeprint", function(){ runSoon(120); });' +
'window.addEventListener("afterprint", function(){ runSoon(120); });' +
/* END SHOSHIN PRINT — BEFORE/AFTERPRINT STABLE REPAGINATE */
...
/* END SHOSHIN PRINT BASELINE — RUNTIME PAGINATION (PRINT WINDOW ONLY) */
  ...
  /* BEGIN SHOSHIN PRINT BASELINE — FAVICON REAPPLY (POST-WRITE) */
  // Intentionally disabled.
  // Favicons are injected once into <head> via getSiteFaviconLinksHTML() during doc.write().
/* END SHOSHIN PRINT BASELINE — FAVICON REAPPLY (POST-WRITE) */
...
//    Pagination must treat this trio as inseparable so divider/title never orphan.
  ...
  // ================================================================
var EXTRA_PRINT_CSS =
  '\n/* PRINT CONTRACT: page-sized frags + deterministic stacking */' +

  /* APPLY TO SCREEN + PRINT (so we can see it in the print tab preview too) */
  '\n.print-frag{ width: 11in; height: 8.5in; box-sizing:border-box;' +
  '\n  background: url("/wp-content/uploads/2026/01/bgtest.jpg") no-repeat center center;' +
  '\n  background-size: 100% 100%; }' +
...
'<body>' +
  ...
  '<script>(function(){' +
    ...
    'function isDivider(node){' +
      ...
      'if(node.classList && node.classList.contains("shoshin-divider-row-inner")) return true;' +
      'return false;' +
    '}' +
...
/* END SHOSHIN PRINT — AVAIL FROM IN-DOM FRAG (V7) */
        ...
        /* BEGIN SHOSHIN PRINT — DEBUG AVAIL PER PAGE */
'DBG.samples.push({event:"newPage", page: pageIndex+1, avail: avail});' +
/* END SHOSHIN PRINT — DEBUG AVAIL PER PAGE */
...
'for(var i=0;i<atoms.length;i++){' +
  ...
  'if(nodeIsDiv){' +
    ...
    // If divider + next unit does NOT fit, force next unit to next page.
    // Divider is skipped (because it would become first on the new page).
    'if(!projWillFit([hDiv, hNext], used, countOnPage, avail)){' +
      'startNewPage();' +
      'continue;' +
    '}' +

    // Fits → place divider + next together so divider can’t end the page
    'appendAtom(node, hDiv);' +
    'appendAtom(next, hNext);' +
    'i++;' + // consume next too
    'continue;' +
  '}' +

  // -----------------------------------------------------------------------
  // Unit flow (strict avail)
  // If unit won't fit, page break BEFORE it.
  // -----------------------------------------------------------------------
  'var h = atomH[i] || 0;' +
...
/* BEGIN SHOSHIN PRINT — TASK 1: VERTICAL JUSTIFY (POST-PAGINATION, UNITS>2) */
'try{' +
  ...
  'for(var pi=0; pi<frags.length; pi++){' +
    ...
    // count units (support BOTH atom types)
    'var unitCount = 0;' +
    'try{' +
      'var ug = body.querySelectorAll(".print-unit-group, .print-unit-row");' +
      'unitCount = ug ? ug.length : 0;' +
    '}catch(e){ unitCount = 0; }' +
  ...
  '}' +
'}catch(e){}' +
/* END SHOSHIN PRINT — TASK 1: VERTICAL JUSTIFY (POST-PAGINATION, UNITS>2) */
...
/* BEGIN SHOSHIN JUSTIFY AUDIT (POST-PAGINATION) */
      'try{' +
        'function __shNum(v){ v=parseFloat(v); return isFinite(v)?v:null; }' +
        'function __shRect(el){ return el ? el.getBoundingClientRect() : null; }' +
        'function __shAudit(){' +
          'var out=[];' +
          'var frags=document.querySelectorAll(".print-frag");' +
          'for(var p=0;p<frags.length;p++){' +
            'var frag=frags[p];' +
            'var body=frag.querySelector(".print-frag-body");' +
                        'var inner=frag.querySelector(".print-frag-inner");' +
            'var header=frag.querySelector(".print-header");' +

            'if(!body){ out.push({page:(p+1),err:"no .print-frag-body"}); continue; }' +
            'var cs=getComputedStyle(body);' +
            'var kids=Array.prototype.slice.call(body.children||[]);' +
            'var gaps=[];' +
            'var heights=[];' +
            'for(var i=0;i<kids.length;i++){' +
              'var r=__shRect(kids[i]);' +
              'if(!r) continue;' +
              'heights.push(__shNum(r.height));' +
            '}' +
            'for(var i2=0;i2<kids.length-1;i2++){' +
              'var r1=__shRect(kids[i2]);' +
              'var r2=__shRect(kids[i2+1]);' +
              'if(!r1||!r2) continue;' +
              'gaps.push(__shNum(r2.top-(r1.top+r1.height)));' +
            '}' +
            'var br=__shRect(body);' +
                        'out.push({' +
              'page:(p+1),' +
              'media:((window.matchMedia&&window.matchMedia("print").matches)?"print":"screen"),' +

              'hasHeader:!!header,' +
              'headerRectH:(header?__shNum(__shRect(header).height):null),' +

              'fragRectH:__shNum(__shRect(frag).height),' +
              'innerRectH:(inner?__shNum(__shRect(inner).height):null),' +

              'bodyClientH:body.clientHeight,' +
              'bodyScrollH:body.scrollHeight,' +
              'bodyRectH:(br?__shNum(br.height):null),' +
              'bodyPadTop:__shNum(getComputedStyle(body).paddingTop),' +
              'bodyPadBot:__shNum(getComputedStyle(body).paddingBottom),' +
          ...
          'return out;' +
        '}' +
        'window.__SHOSHIN_JUSTIFY_AUDIT__=__shAudit();' +
        'try{ console.log("[SHOSHIN JUSTIFY AUDIT]", window.__SHOSHIN_JUSTIFY_AUDIT__); }catch(e){}' +
      '}catch(e){}' +
/* END SHOSHIN JUSTIFY AUDIT (POST-PAGINATION) */
...
/* BEGIN SHOSHIN PRINT — FAVICON REAPPLY AFTER REPLACESTATE */
  // Intentionally disabled.
  // Favicons are injected once into <head> via getSiteFaviconLinksHTML() during doc.write().
/* END SHOSHIN PRINT — FAVICON REAPPLY AFTER REPLACESTATE */
}
...
@media screen{
  ...
  /* SCREEN ONLY: keep Row2 composition fixed */
  .print-row2{ justify-content: flex-start; }
  ...
  .print-page{
   ...
   /* ================================
     [SHOSHIN] SCREEN PAPER HEIGHT LOCK
     - Make the "paper" have a definite height
     - Make inner/body stretch to usable page height
     ================================ */

  .print-frag{
    width: 11in;
    height: 8.5in;              /* <-- critical: definite height (not just min-height) */
    min-height: 8.5in;
  ...
  .print-frag-inner{
    flex: 1 1 auto;             /* stretch to fill the page */
    min-height: 0 !important;   /* allow children to size without forcing overflow */
  ...
  .print-frag-body{
    flex: 1 1 auto;             /* THIS becomes your usable height (≈ 729.6px) */
...
/* SCREEN+PRINT: deterministic width math */
:root{
  --row2-colw: 76px;
}
...
/* =========================================================
   ATOMIC UNITS (this is the one rule that owns pagination)
========================================================= */
.print-unit-row{
  margin-top: 0;
  padding: 2px 10px;

  /* pagination hard-guard (ONLY HERE) */
  break-inside: avoid;
...
/* IMPORTANT: restore positioning context for absolute keyline */
.print-row-block-cm{
  margin-top: 4px;
  position: relative;
  z-index: 2;
...
/* SCREEN ONLY: narrower CM grid */
@media screen{
  .print-cm-tags-grid{ grid-template-columns: repeat(4, 107px); }
  ...
  .print-cm-keyline{ font-size: 8px; }
...
/* SCREEN ONLY: rebalance Row2 typography */
@media screen{
...
/* =========================================================
   PRINT-ONLY COMPACTION (to chase 4 units + 3 dividers)
   - avoids changing screen preview
========================================================= */
@media screen, print{
  /* tighten unit padding */
.print-unit-row{ padding: 2px 8px; }

  /* tighten vertical rhythm between Row1 and Row2 blocks */
/*.print-row-block-cm{ margin-top: 2px; }*/
/*.print-row-block-row2{ margin-top: 2px; }*/

  /* tighten Row2 spacing + title band + cell height */
  .print-row2{ gap: 12px; }
