...
/* =========================================================
   ...
   Scope: single-product form.cart only
   Relies on:
   - window.shoshinCart from PHP
   - widget setting now handles side-cart auto-open
   ========================================================= */
(function ($) {
  'use strict';
  ...
  function confirmButtonState($button) {
    if (!$button || !$button.length) return;
  ...
  function refreshCartFragments(done) {
    ...
    $.ajax({
      type: 'POST',
      url: CFG.wc_ajax_url.toString().replace('%%endpoint%%', 'get_refreshed_fragments'),
  ...
  function handleSingleProductAddToCart($form, $button) {
        ...
        console.log('SHOSHIN handleSingleProductAddToCart fired');
  ...
  function bindSingleProductCartButton() {
    ...
    /* Prevent native Woo/browser submit */
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bindSingleProductCartButton);
  } else {
    bindSingleProductCartButton();
  }
