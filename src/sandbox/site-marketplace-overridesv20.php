...
if ( ! function_exists( 'shoshin_shipping_rate_is_pod_method' ) ) {
	function shoshin_shipping_rate_is_pod_method( $rate ) {
		...
		return ( stripos( $label, 'Print on Demand' ) === 0 );
	}
}
...
function shoshin_filter_pod_and_shippo_shipping_rates( $rates, $package ) {
	...
	foreach ( $rates as $rate_id => $rate ) {
		 ...
		 */
		if ( $profile['is_pod_only'] && shoshin_shipping_rate_is_shippo_method( $rate ) ) {
			unset( $rates[ $rate_id ] );
			continue;
		}
		...
		/*
		 * Non-POD-only carts:
		 * remove POD flat rates so stale POD methods do not persist.
		 */
		if ( $profile['is_non_pod_only'] && shoshin_shipping_rate_is_pod_method( $rate ) ) {
			unset( $rates[ $rate_id ] );
			continue;
		}
	}
