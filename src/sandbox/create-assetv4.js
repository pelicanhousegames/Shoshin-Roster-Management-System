...
let summary = {
  name: assetName ? displayName : 'Select a Support Asset',
    ...
    const trainingReq = !assetName ? ['None'] : (isOzutsu ? ['Kayakujutsu'] : (isVessel ? ['Suieijutsu'] : ['None']));
...
const tables = {
  ...
  munitions:{ showSection: isOzutsu, selected: selectedMunitions }
    ...
    // Ozutsu: Reset Detachment/Crew (field #19) anytime the user CHECKS an additional munition.
...
function renderHiddenFields(model) {
  ...
  // Weapon stats (Ozutsu only)
  const wpn = model.weaponStats || { damage: '--', critical: '--', distance: '--' };

  // Ryu (training requirement)
  const ryu = !assetName ? '' : (isOzutsu ? 'Kayakujutsu' : (isVessel ? 'Suieijutsu' : ''));
  ...
  // Core stats
  // Note: model.panelStats already reflects detachment effects for Ozutsu.
  ...
  // Ranged weapon stats (Ozutsu only; otherwise '--')
  setHidden(FIELD_ID_H_R_DMG, isOzutsu ? (wpn.damage || '--') : '--');
  setHidden(FIELD_ID_H_R_CRT, isOzutsu ? (wpn.critical || '--') : '--');
  setHidden(FIELD_ID_H_R_DIS, isOzutsu ? (wpn.distance || '--') : '--');
...
if (isVessel) {
  ...
  // Equip items: Ozutsu munitions list (already in your model.hidden)
...
function reassertMunitionsEmptyMessageSoon() {
  ...
  _munitionsReassertTimer = setTimeout(function () {
    ...
    // Only relevant when Ozutsu is selected
    const assetName = canonicalLabel(getSingleSelectText(FIELD_ID_ASSET_TYPE));
    const isOzutsu = assetName === 'Ozutsu';
    ...
    const shouldShow = isOzutsu && selected.length === 0;
