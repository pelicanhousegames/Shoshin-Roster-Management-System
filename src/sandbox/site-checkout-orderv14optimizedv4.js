...
/**
 * =========================================================
 * SHOSHIN CHECKOUT — STRUCTURE NORMALIZATION (PASS 1)
 * =========================================================
 * - Builds Cart-style checkout shell
 * - Moves existing Woo nodes into structured layout
 * - Preserves all mounts and behaviors
 * =========================================================
 */
...
function buildCheckoutShell() {
  ...
  /* -----------------------------------------
     Native node selectors
  ----------------------------------------- */

  const express =
    document.querySelector('.wcpay-express-checkout-wrapper') ||
    document.querySelector('#wc-stripe-express-checkout-element') ||
    document.querySelector('#wc-stripe-express-checkout-button-separator')?.previousElementSibling;
  ...
  const billing = document.querySelector(
    '.col-1 .woocommerce-billing-fields:last-of-type'
  );
  ...
  function wrapCard(el, className) {
    if (!el || !isSafeToMove(el)) return null;

    if (el.parentElement && el.parentElement.classList.contains('shoshin-card')) {
  ...
  function createCardHeading(text) {
  ...
  /* -----------------------------------------
     Extract shipping methods out of order review
     So layout matches Blocks flow better:
     shipping address -> billing -> shipping options -> payment
  ----------------------------------------- */

  let shippingOptionsCard = null;

  if (orderReview) {
    const reviewTable = orderReview.querySelector('table.shop_table');
    const shippingRow = reviewTable
      ? reviewTable.querySelector('tr.woocommerce-shipping-totals.shipping')
      : null;

    if (shippingRow) {
      shippingOptionsCard = createCard('card-shipping-methods');
      shippingOptionsCard.appendChild(createCardHeading('Shipping options'));

      const shippingInner = document.createElement('div');
      shippingInner.className = 'shoshin-checkout-shipping-methods';

      const shippingCell = shippingRow.querySelector('td');
      if (shippingCell) {
        shippingInner.appendChild(shippingCell);
      } else {
        shippingInner.appendChild(shippingRow);
      }
  ...
  /* -----------------------------------------
     Pull terms out of place-order row
     so acknowledgements can live before actions
  ----------------------------------------- */

  let termsCard = null;
  if (terms) {
    termsCard = wrapCard(terms, 'card-terms');
  }
  ...
  if (placeOrder) {
    actionsCard = wrapCard(placeOrder, 'card-actions');
  }

  /* -----------------------------------------
     Build MAIN column in Blocks-like order
  ----------------------------------------- */

  if (express && isSafeToMove(express)) {
    const expressCard = wrapCard(express, 'card-express');
    if (expressCard) {
      if (stripeExpressSeparator && isSafeToMove(stripeExpressSeparator)) {
        moveNode(stripeExpressSeparator, expressCard);
      }
  ...
  if (payment) {
    const paymentCard = wrapCard(payment, 'card-payment');
    if (paymentCard) main.appendChild(paymentCard);
  }
  ...
  if (orderReview) {
    moveNode(orderReview, sidebarCard);
  }

  sidebar.appendChild(sidebarCard);

  /* -----------------------------------------
     Clean up original two-column native wrappers
  ----------------------------------------- */
  ...
  document.addEventListener('DOMContentLoaded', buildCheckoutShell);

  // Woo AJAX refresh safety
