 ...
 */
if (!empty($affiliate['is_external']) && !empty($affiliate['has_any_price'])) {
    ...
    /*
     * Archive / loop:
     * use the normal loop price markup so existing archive CSS styles it.
     */
    if (($is_loop_context || !is_product()) && !wp_doing_ajax()) {
        return '<span class="price"><ins>' . $display_html . '</ins></span>';
    }

    /*
     * Main single product:
     * use the full affiliate price block.
     */
    if (is_product() && !$is_loop_context) {
        return '
        <div class="shoshin-price-block shoshin-price-block--affiliate">
            <div class="shoshin-price-label">Affiliate Price</div>
            <div class="shoshin-price-final shoshin-price-final--standard">' . $display_html . '</div>
...
/*
 * External / affiliate products:
 * In archive / loop view, use the manual affiliate price range
 * instead of Woo's native external price, which may be blank/zero.
 */
if (($is_loop_context || !is_product()) && !wp_doing_ajax() && !empty($affiliate['is_external']) && !empty($affiliate['has_price_range'])) {
    $min_html = wc_price($affiliate['price_min']);
    $max_html = wc_price($affiliate['price_max']);

    $archive_price_html = ((float) $affiliate['price_min'] === (float) $affiliate['price_max'])
        ? $min_html
        : $min_html . ' – ' . $max_html;

    return '<span class="price"><ins>' . $archive_price_html . '</ins></span>';
}
 ...
 */
if (is_product() && !$is_loop_context && !empty($affiliate['is_external']) && !empty($affiliate['has_price_range'])) {
    $min_html = wc_price($affiliate['price_min']);
    $max_html = wc_price($affiliate['price_max']);
