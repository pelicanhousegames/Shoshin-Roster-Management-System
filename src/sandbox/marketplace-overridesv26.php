...
function shoshin_get_variable_price_bounds($product) {
    ...
    foreach ($child_ids as $child_id) {
        ...
        /*
         * Use raw variation meta here.
         * Do NOT use get_price()/get_regular_price() because those can already
         * be filtered/resolved by PMPro or Woo runtime logic, which then causes
         * the downstream range renderer to collapse or double-discount.
         */
        $child_regular_raw = get_post_meta($child_id, '_regular_price', true);
        $child_sale_raw    = get_post_meta($child_id, '_sale_price', true);
        $child_price_raw   = get_post_meta($child_id, '_price', true);
        ...
        $child_current = ($child_price_raw !== '' && $child_price_raw !== null)
            ? (float) $child_price_raw
            : 0.0;

        /*
         * If _price is empty for any reason, reconstruct the raw public-facing
         * current price from sale/regular meta.
         */
        if ($child_current <= 0) {
    ...
    if (($is_loop_context || !is_product()) && !wp_doing_ajax()) {
        ...
        if ($product->is_type('variable')) {
            ...
            if ($product_level_override_raw !== '') {
                ...
                $min_final = round($min_current * 0.9, wc_get_price_decimals());
                $max_final = round($max_current * 0.9, wc_get_price_decimals());
