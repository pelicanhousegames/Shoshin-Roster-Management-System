...
// (Preserve whatever you already compute for "isDaimyoRow" above this line.)
// If your function already has `var isDaimyoRow = ...;` then this works as-is.
if (typeof isDaimyoRow !== 'undefined' && isDaimyoRow) {
  // Existing Daimyo restrictions (roster already has a Daimyo, or selection conflicts)
  if (rosterHasDaimyo) isDisabled = true;
  if (selectedDaimyoKey && String(selectedDaimyoKey) !== String(u.unitKey || u.key || '')) isDisabled = true;
}

// RMS: locked assets always disabled
if (isLockedAsset) isDisabled = true;

// Checked state should NEVER be true when disabled
var checked = (!isDisabled && (currentQtyMap && (currentQtyMap[u.unitKey] || currentQtyMap[u.key] || 0) > 0));
// END RMS A3
  ...
  // ============================================================
  // NEW: Default dim rows unless toggle is ON OR already assigned.
  // Also: Daimyo-disabled rows get a clear tooltip + not-allowed cursor.
  // ============================================================
  var shouldDim = ((curQty === 0 && !checked) || isDisabled);
...
try {
  ...
  if (rosterObjU && rosterObjU.locked) {
    openRmsLockedModal(null);
    return;
  }
} catch (e) {}
...
try {
  ...
  if (ownedU && isRmsLockedAsset(ownedU)) {
    // keep delete available; unassign opens locked modal instead of writing
    openRmsLockedModal(null);
    return;
  }
} catch (e2) {}
