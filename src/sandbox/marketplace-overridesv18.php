...
function shoshin_override_available_variation_price_html($variation_data, $product, $variation) {
     ...
     */
    $variation_data['price_html'] = shoshin_marketplace_price_html($variation->get_price_html(), $variation);
...
if ($regular_raw === '' && $current_raw === '') {
     ...
     */
    if (is_product() && !$is_loop_context && ($product->is_type('variable') || $product->is_type('variation'))) {
if ($product->is_type('variation')) {
        $min_regular = (float) $product->get_regular_price();
        $max_regular = (float) $product->get_regular_price();
        $min_current = (float) $product->get_price();
        $max_current = (float) $product->get_price();
    } else {
