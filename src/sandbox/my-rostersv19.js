...
if (kind === 'support') {
  ...
  // Sailing Ships: display subtype by inferring from Movement (MVP: no schema changes)
  var clsKey = String(u.cls || u.class || u.className || '').toLowerCase().replace(/\s+/g, ' ').trim();
  if (clsKey === 'sailing ships') {
    var movRaw = pick('mov', 'movement');
    var movNum = null;
    if (movRaw != null) {
      var s = String(movRaw).replace(/[^\d.]/g, '').trim();
      if (s) movNum = parseFloat(s);
    }
