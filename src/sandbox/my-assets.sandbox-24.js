...
`;
    ...
    function renderAssignModeNote() {
      ...
      if (assignState.isDaimyoAsset) {
        ...
        // Some rosters may be unavailable because a DIFFERENT Daimyo is already assigned.
        // Precision: show this only when SOME are locked but at least ONE is still available.
        var totalRostersForMsg = rosters.length;
        var availableCountForMsg = 0;
        for (var mi = 0; mi < rosters.length; mi++) {
          if (rosters[mi] && !rosters[mi].hasDaimyo) availableCountForMsg++;
        }

        var extraLockMsg = (totalRostersForMsg > 0 && availableCountForMsg > 0 && availableCountForMsg < totalRostersForMsg)
          ? '<div style="margin-top:6px; color:#c0392b; font-weight:700;">Some clans are unavailable because another Daimyo is already assigned.</div>'
          : '';
        ...
        if (wantEntryId > 0 || wantRefId) {
          hasAnyAssignments = rosters.some(function (r) {
            if (!r) return false;

            // Use the shared parser (handles string/array/object/value shapes)
            var arr = (typeof getRosterAssignedUnitsArray === 'function') ? getRosterAssignedUnitsArray(r) : [];
            ...
            return arr.some(function (u) {
              ...
              if (wantRefId) {
                return String(u.refId || '').trim() === wantRefId;
              }
        ...
        // Compute counts for correct Daimyo messaging states.
        // - availableCount: rosters with NO Daimyo yet (eligible)
        // - assignedThisCount: rosters that already contain THIS Daimyo (by entryId/refId scan)
        var availableCount = 0;
        for (var ai = 0; ai < rosters.length; ai++) {
          if (rosters[ai] && !rosters[ai].hasDaimyo) availableCount++;
        }
        ...
        function rosterHasThisAsset(r) {
          ...
          var arr = [];
          try {
            // raw can be JSON string OR already-parsed array OR {value:"..."} object
...
empty.innerHTML =
  ...
  '</div>';
      ...
      rostersToRender.forEach(function (r) {
        ...
        if (isSelected) {
          effectiveQty = isOneToMany ? (existingQty + perRosterQty) : perRosterQty;
        }

        var basePts = parseInt(r && r.points ? r.points : 0, 10) || 0;
        var aPts = parseInt(asset && asset.points ? asset.points : 0, 10) || 0;

        // Points delta for this roster (how roster total changes pre-save)
        // - one_to_many: add points for the added qty
        // - many_to_one: adjust by (new - existing)
        var deltaQty = 0;
        if (isSelected) {
          deltaQty = isOneToMany ? perRosterQty : (effectiveQty - existingQty);
        }
