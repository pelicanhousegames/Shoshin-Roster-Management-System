...
if ( ! function_exists( 'shoshin_build_roster' ) ) {
	function shoshin_build_roster( $entry ) {
		...
		// Also: if the entry no longer exists (admin hard-delete), mark as pruned and skip it.
		// ------------------------------------------------------------
		$entry_full = null;
		...
		if ( ( $owner_user_id <= 0 || $raw_fields === '' ) && $entry_id > 0 && function_exists( 'wpforms' ) && isset( wpforms()->entry ) ) {
			...
			// HARD-DELETE / missing entry: prune this row
			if ( ! $entry_full ) {
				$pruned_any = true;
				$changed    = true;
				continue;
			...
			// Normalize access (array vs object)
			if ( is_array( $entry_full ) ) {
				if ( $raw_fields === '' ) {
...
if ( ! function_exists( 'shoshin_my_rosters_shortcode' ) ) {
	...
	function shoshin_my_rosters_shortcode() {
        ...
        // Prevent any page/fragment caching for live roster data.
// This ensures auto-heal runs on every load and stale admin edits/deletes cannot linger.
