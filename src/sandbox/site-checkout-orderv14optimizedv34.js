(function () {
  ...
  window.__SHOSHIN_DEBUG_WALLETS__ = true;
function shoshinDebug() {
  if (!window.__SHOSHIN_DEBUG_WALLETS__) return;
...
function syncClassicCartExpressGate() {
  /* EXPRESS GATING DISABLED FOR TESTING
     Intentionally left inert so cart express buttons remain enabled
     and no shipping-selection helper notice is injected.
  */
  return;
}
  ...
  function applyAllCartEnhancements(root) {
    ...
    /* syncClassicCartExpressGate(); */ // disabled for testing
  }
...
/*
  Show the card only if something is in it (or when hydration later adds content).
*/
if (expressCard.children.length) {
  expressCard.style.display = '';
}
...
expressCard.style.display = '';
  ...
  function renderClassicCheckoutReviewItems() {
    ...
    rows.forEach(function (row) {
      ...
      nameClone
        .querySelectorAll('strong.product-quantity, .product-quantity')
        .forEach(function (el) {
          el.remove();
        });
  ...
  function syncClassicCheckoutExpressGate() {
    /* EXPRESS GATING DISABLED FOR TESTING
       Intentionally left inert so checkout express buttons remain enabled
       and no shipping-selection helper notice is injected.
    */
    return;
  }
...
function toggleClassicCheckoutShippingCardForNoShipping() {
  ...
  function reconcileClassicCheckoutShell() {
    ...
    /* syncClassicCheckoutExpressGate(); */ // disabled for testing
  ...
  /* =========================================
   CHECKOUT — Debounce shipping-triggered order review updates
   Stops noisy keystroke updates for Shippo/Woo fields
========================================= */
function initClassicCheckoutShippingUpdateControl() {
  if (!isClassicCheckoutPage()) return;
  if (document.body.dataset.shoshinShippingUpdateControlBound === '1') return;

  document.body.dataset.shoshinShippingUpdateControlBound = '1';
  ...
  /* Block noisy typing events before Woo's bubbling listeners see them */
  form.addEventListener(
    'input',
    function (event) {
      var target = event.target;
      if (!target || !target.matches(textSelectors)) return;
  ...
  /* Use blur as the main stable trigger for text fields */
  form.addEventListener(
    'blur',
    function (event) {
      var target = event.target;
  ...
  /* Let select/dropdown changes refresh quickly */
  form.addEventListener(
    'change',
    function (event) {
      var target = event.target;
      ...
      if (target.matches(immediateSelectors)) {
        queueUpdate(150);
        return;
      }
  ...
  /* After Shippo applies a suggested address, force one clean refresh */
  document.addEventListener(
    'click',
    function (event) {
      var btn = event.target.closest(
        '.woocommerce-NoticeGroup-updateOrderReview .button, ' +
        '.woocommerce-NoticeGroup-updateOrderReview button, ' +
        '.woocommerce-NoticeGroup-updateOrderReview input[type="button"], ' +
        '.woocommerce-NoticeGroup-updateOrderReview input[type="submit"]'
      );

      if (!btn) return;

      var txt = (btn.textContent || btn.value || '').toLowerCase();
      if (txt.indexOf('apply suggested address') === -1) return;
...
if (window.jQuery) {
  window.jQuery(document.body).on('update_checkout', function (e, args) {
    shoshinDebug('[WC] update_checkout', args || null, Date.now());
  });

  window.jQuery(document.body).on('updated_checkout', function (e, data) {
    shoshinDebug('[WC] updated_checkout', Date.now());
  });
}
...
function initClassicCheckoutShell() {
  reconcileClassicCheckoutShell();
  initClassicCheckoutShippingUpdateControl();
...
jQuery(document.body).on('updated_checkout', function () {
  /*
    Avoid full shell reconcile here (it moves lots of DOM and can collide with gateways).
    Only run the pieces we need after checkout fragments update.
  */
  window.setTimeout(function () {
    hydrateClassicCheckoutExpressCard();
    renderClassicCheckoutReviewItems();
  ...
  /* EXPRESS GATING SHIPPING RECONCILE DISABLED FOR TESTING
  jQuery(document.body).on('change', '#order_review input.shipping_method', function () {
    scheduleClassicCheckoutReconcile(0);
  });
  */
})();
