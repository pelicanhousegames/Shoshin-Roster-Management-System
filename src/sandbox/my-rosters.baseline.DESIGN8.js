...
//    Pagination must treat this trio as inseparable so divider/title never orphan.
  ...
  // ================================================================
/* =======================================================================
   [SHOSHIN] PRINT CSS TOGGLES
   - Contract CSS MUST stay on (pagination + page breaks)
   - Design CSS can be disabled to get a true "0 baseline"
======================================================================= */
const SHOSHIN_PRINT_ZERO_BASELINE = true; // <-- set TRUE for zero pass, FALSE for design pass

var PRINT_CONTRACT_CSS =
  '\n/* PRINT CONTRACT: deterministic stacking + page breaks */' +
  '\n@media print{' +
...
var PRINT_DESIGN_CSS =
  '\n/* DESIGN SAFE CANVAS (LOCKED): must survive Chrome "Minimum" margins */' +
  '\n.print-frag{ width: 10.50in; height: 8.00in; box-sizing:border-box;' +
  '\n  background: url("/wp-content/uploads/2026/01/bgtest.jpg") no-repeat center center;' +
  '\n  background-size: 100% 100%; }' +
  '\n/* DESIGN PHASE: RESET BODY INSET (header unchanged) */' +
  '\n.print-frag-body{ padding: 0 !important; }' +
  '\n@media print{' +
  '\n  /* DESIGN: top-left anchored (no centering) */' +
  '\n  /* PRINT-ONLY: no nudges (top-left anchored) */' +
  '\n}' +
  '\n';

var EXTRA_PRINT_CSS = PRINT_CONTRACT_CSS + (SHOSHIN_PRINT_ZERO_BASELINE ? '' : PRINT_DESIGN_CSS);
...
for (var i2 = 0; i2 < total; i2++) {
  ...
  var isDangerTag =
    (v === '[M] Lucky Hits on 6 only') ||
    (v === '[R] Lucky Hits on 6 only') ||
    (v === '[B] May not attack in water') ||
    (v === '[P] May not operate ship');
...
function renderRow(rowObj, allowCmOverlap, prefixHtml) {
  ...
  // =========================================================
  // BEGIN SHOSHIN — Lucky tag detection (drives Row2 red values)
  // =========================================================
  var _mods = '';
  try { _mods = String((sample && sample.modsBlock) ? sample.modsBlock : ''); } catch (e) { _mods = ''; }

  var _lines = [];
  try { _lines = parseModsBlockToLines(_mods) || []; } catch (e) { _lines = []; }

  function _hasLine(exact) {
    for (var i = 0; i < _lines.length; i++) {
      if (String(_lines[i] || '').trim() === exact) return true;
    }
    return false;
  }

  var _hasMLucky = _hasLine('[M] Lucky Hits on 6 only');
  var _hasRLucky = _hasLine('[R] Lucky Hits on 6 only');
  // =========================================================
  ...
  function row2VCell(val, extraClass) {
  ...
  // =========================================================
  var _sb = null;
  try { _sb = supportBucketFromUnit(sample || {}); } catch (e) { _sb = null; }

  var _isSupportStats = (_sb === "artillery" || _sb === "ship");
...
/* BEGIN SHOSHIN — SAFE RED CLASS (fallback) */
.shoshin-red{ color: var(--shoshin-red) !important; }
/* END SHOSHIN — SAFE RED CLASS (fallback) */
...
/* BEGIN SHOSHIN — TAG-DRIVEN RED STATES */

/* Tag text itself */
.print-row-block-cm.has-m-lucky .print-cm-cell.print-cm-tag,
.print-row-block-cm.has-r-lucky .print-cm-cell.print-cm-tag,
.print-row-block-cm.has-b-water .print-cm-cell.print-cm-tag,
.print-row-block-cm.has-p-ops   .print-cm-cell.print-cm-tag{
  color: var(--shoshin-red);
}

/* Melee Lucky → Melee stats */
.print-row-block-cm.has-m-lucky
  .print-row2-group-melee
  .print-row2-v{
  color: var(--shoshin-red);
}

/* Ranged Lucky → Ranged stats */
.print-row-block-cm.has-r-lucky
  .print-row2-group-ranged
  .print-row2-v{
  color: var(--shoshin-red);
}

/* END SHOSHIN — TAG-DRIVEN RED STATES */
