...
function syncLimitedDownloadButtonsFromCartState() {
  ...
  document.querySelectorAll('.elementor-menu-cart__product').forEach(function (row) {
    ...
    if (removeLink) {
      ...
      if (pid) {
        cartProductIds[String(pid)] = true;
      }
    }
  });
  ...
  document.querySelectorAll(
    ...
    var inCart = (!!productId && !!cartProductIds[String(productId)]) || (!!href && !!cartProductUrls[href]);
    ...
    btn.classList.remove('disabled', 'shoshin-disabled-add-to-cart', 'is-locked');
    btn.classList.add('ajax_add_to_cart', 'add_to_cart_button');
    btn.removeAttribute('aria-disabled');
    btn.removeAttribute('tabindex');
    btn.onclick = null;

    if ((btn.textContent || '').trim() === 'In Cart') {
