...
document.addEventListener('DOMContentLoaded', function () {
  function findBatchAckCheckbox() {
    return document.querySelector(
      '.shoshin-checkout input[type="checkbox"][name*="batch-production-ack"], ' +
      '.shoshin-checkout input[type="checkbox"][id*="batch-production-ack"], ' +
      '.shoshin-checkout input[type="checkbox"][name*="shoshin/batch-production-ack"]'
    );
  }
  ...
  function syncBatchAckButtonState() {
    ...
    /* no batch checkbox present = normal checkout */
    if (!checkbox) {
      button.disabled = false;
      button.removeAttribute('aria-disabled');
      button.classList.remove('shoshin-batch-ack-lock');
      return;
    }
