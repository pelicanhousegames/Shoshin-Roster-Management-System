...
if (!empty($affiliate['is_external'])) {
    ...
    if ($has_min && $has_max) {
        ...
        $sale_active = ($sale > 0 && $regular > 0 && $sale < $regular && $current > 0 && $current <= $sale);

        if ($sale_active) {
            $final_html    = wc_price($current);
            $original_html = wc_price($regular);
            $save_percent  = round((($regular - $current) / $regular) * 100);
            ...
            $single_savings_html = ($save_percent > 0)
                ? 'Save ' . $save_percent . '%'
                : 'Membership discounts are not available for this item';
    ...
    if (is_product() && !$is_loop_context) {
        ...
        $original_row = ($single_original_html !== '')
            ? '<div class="shoshin-price-original">' . $single_original_html . '</div>'
            : '';
