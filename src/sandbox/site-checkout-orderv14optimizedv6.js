...
/**
 * =========================================================
 * SHOSHIN CHECKOUT — STRUCTURE NORMALIZATION (SAFE PASS)
 * =========================================================
 * - Restores cart-style checkout shell
 * - Uses non-destructive, one-time shell build
 * - Preserves native Woo mounts
 * - Orders sections closer to Blocks template
 * =========================================================
 */
...
(function () {
  function buildCheckoutShell() {
    ...
    // Do not rebuild if shell already exists.
    if (form.querySelector(':scope > .shoshin-checkout-shell')) return;
    if (form.classList.contains('shoshin-checkout-ready')) return;

    const customerDetails = document.querySelector('#customer_details');
    const col1 = customerDetails ? customerDetails.querySelector('.col-1') : null;
    const col2 = customerDetails ? customerDetails.querySelector('.col-2') : null;

    const contact = document.querySelector('#contact_details');

    const billing =
      col1?.querySelector('.woocommerce-billing-fields:not(#contact_details)') ||
      document.querySelector('.col-1 .woocommerce-billing-fields:last-of-type');

    const shipping = document.querySelector('.woocommerce-shipping-fields');
    const notes = document.querySelector('.woocommerce-additional-fields');
    ...
    const expressPrimary = document.querySelector('#wc-stripe-express-checkout-element');
    const expressWcPay = document.querySelector('.wcpay-express-checkout-wrapper');
    const stripeExpressSeparator = document.querySelector(
    ...
    function wrapCard(el, className, headingText) {
      ...
      const card = createCard(className, headingText);
      moveNode(el, card);
      return card;
    }
    ...
    if (expressPrimary || expressWcPay || stripeExpressSeparator) {
      const expressCard = createCard('card-express');
    ...
    ----------------------------------------- */
    if (contact) {
      const contactCard = wrapCard(contact, 'card-contact');
    ...
    ----------------------------------------- */
    if (shipping) {
      const shippingCard = wrapCard(shipping, 'card-shipping');
    ...
    ----------------------------------------- */
    if (billing) {
      const billingCard = wrapCard(billing, 'card-billing');
      if (billingCard) main.appendChild(billingCard);
    }

    /* -----------------------------------------
       SHIPPING OPTIONS
       Clone shipping methods out of order summary,
       keep native row in DOM but hide it with class.
    ----------------------------------------- */
    let shippingOptionsCard = null;

    if (orderReview) {
      const shippingRow = orderReview.querySelector('tr.woocommerce-shipping-totals.shipping');
      const shippingList = shippingRow ? shippingRow.querySelector('ul#shipping_method') : null;

      if (shippingRow && shippingList) {
        shippingOptionsCard = createCard('card-shipping-methods', 'Shipping options');

        const shippingInner = document.createElement('div');
        shippingInner.className = 'shoshin-checkout-shipping-options';

        const shippingClone = shippingList.cloneNode(true);
        shippingInner.appendChild(shippingClone);
        shippingOptionsCard.appendChild(shippingInner);

        shippingRow.classList.add('shoshin-native-shipping-row');
      }
    }

    if (shippingOptionsCard) {
      main.appendChild(shippingOptionsCard);
    }

    /* -----------------------------------------
       PAYMENT / TERMS / ACTIONS
       In classic checkout, #payment contains terms + place order.
       We must extract nested pieces BEFORE wrapping payment.
    ----------------------------------------- */
    ...
    if (payment) {
      const nestedTerms = payment.querySelector(':scope > .form-row.place-order .woocommerce-terms-and-conditions-wrapper');
      const nestedPlaceOrder = payment.querySelector(':scope > .form-row.place-order');
      const paymentMethods = payment.querySelector(':scope > .wc_payment_methods');

      if (nestedTerms) {
        termsCard = createCard('card-terms');
        termsCard.appendChild(nestedTerms);
      }

      if (nestedPlaceOrder) {
        actionsCard = createCard('card-actions');
        actionsCard.appendChild(nestedPlaceOrder);
      }

      if (paymentMethods) {
        paymentCard = createCard('card-payment');
        paymentCard.appendChild(paymentMethods);
      }
    ...
    ----------------------------------------- */
    if (notes) {
      const notesCard = wrapCard(notes, 'card-notes');
      if (notesCard) main.appendChild(notesCard);
    }

    if (termsCard) {
      main.appendChild(termsCard);
    }
    ...
    /* -----------------------------------------
       SIDEBAR
    ----------------------------------------- */
    const sidebarCard = createCard('card-order-summary');

    if (orderHeading) moveNode(orderHeading, sidebarCard);
    ...
    /* -----------------------------------------
       HIDE NATIVE TWO-COLUMN WRAPPERS
       Only after shell is fully built
    ----------------------------------------- */
  ...
  function initCheckoutShell() {
    ...
    window.setTimeout(buildCheckoutShell, 80);
    window.setTimeout(buildCheckoutShell, 220);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCheckoutShell);
  } else {
    initCheckoutShell();
  }

  jQuery(document.body).on('updated_checkout', function () {
    window.setTimeout(buildCheckoutShell, 50);
  });
})();