...
function openPrintTabDocument(title, cssText, bodyHtml) {
  ...
  // ================================================================
  var EXTRA_PRINT_CSS =
    '\n/* PRINT-FRAGMENTS (MVP): apply in all media because this tab is print-only */' +
    '\n.print-pages-wrap{ position:relative; z-index:1; }' +
    '\n.print-frag{ break-after: page; page-break-after: always; height: 7.6in; box-sizing:border-box; }' +
    '\n.print-frag:last-child{ break-after:auto; page-break-after:auto; }' +
...
var PRINT_JUSTIFY_MIN_GAP_PX = 12; // if space-evenly gap would be smaller, don't justify that page
// END PRINT JUSTIFY — MIN GAP THRESHOLD
  ...
  function paginateAndSpaceEvenly() {
    ...
    var footer = root.querySelector('.print-footer');
    if (!body) return;
    ...
    // ==============================================================
    // SLOT MODEL (authoritative grammar)
    // Slot A: divider + title + first unit    (divider -> title -> unit)
    // Slot B: (0..n inner dividers) + unit    (divider -> unit) belongs with that unit
    // ==============================================================
    var slots = [];
    var innerDivs = [];
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // Divider row: decide role purely via lookahead grammar
      if (el.classList.contains('shoshin-divider-row')) {
        var n1 = (i + 1 < rows.length) ? rows[i + 1] : null;
        var n2 = (i + 2 < rows.length) ? rows[i + 2] : null;

        // Bucket-start: divider -> title -> unit  => Slot A
        if (n1 && n1.classList.contains('print-section-title')) {
          if (n2 && n2.classList.contains('print-unit-row')) {
            slots.push({ kind: 'A', nodes: [el, n1, n2] });
            i += 2;
            continue;
          }
          // Fallback: divider -> title with missing unit (should not happen, but keep stable)
          slots.push({ kind: 'A_fallback', nodes: [el, n1] });
          i += 1;
          continue;
        }

        // Inner divider: divider -> unit  => belongs to upcoming Slot B
        if (n1 && n1.classList.contains('print-unit-row')) {
          innerDivs.push(el);
          continue;
        }

        // Unknown context: treat as inner divider decoration (never page-force)
        innerDivs.push(el);
        continue;
      }

      // Title row without a preceding divider (rare). Pair with first unit if present.
      if (el.classList.contains('print-section-title')) {
        var u1 = (i + 1 < rows.length) ? rows[i + 1] : null;
        if (u1 && u1.classList.contains('print-unit-row')) {
          slots.push({ kind: 'A_titleOnly', nodes: [el, u1] });
          i += 1;
        } else {
          slots.push({ kind: 'title', nodes: [el] });
        }
        continue;
      }

      // Unit row: Slot B = (inner dividers) + unit
      if (el.classList.contains('print-unit-row')) {
        var nodes = [];
        for (var d = 0; d < innerDivs.length; d++) nodes.push(innerDivs[d]);
        innerDivs = [];
        nodes.push(el);
        slots.push({ kind: 'B', nodes: nodes });
        continue;
      }
    }

    // If we end with inner dividers and no following unit, drop them (divider forbidden as last element)
    innerDivs = [];
    ...
    // Footer stays last-page-only like baseline: place it after all fragments
    ...
    try {
      ...
      for (var bi = 0; bi < fragBodies.length; bi++) {
        ...
        var kids = b.children ? b.children.length : 0;
        if (!kids) continue;

        var sum = 0;
        for (var ki = 0; ki < kids; ki++) {
          var k = b.children[ki];
          if (!k) continue;
          var r = k.getBoundingClientRect();
          sum += (r && r.height) ? r.height : 0;
        }

        // First: if this page overflows at all, it must pack naturally
        ...
        var free = (b.clientHeight || 0) - sum;
        if (free <= 0) {
          b.style.justifyContent = 'flex-start';
          continue;
        }

        // space-evenly has (kids + 1) gaps
        var gapEvenly = free / (kids + 1);

        // MVP gate: only justify when spacing is visually meaningful
        b.style.justifyContent =
          (gapEvenly >= PRINT_JUSTIFY_MIN_GAP_PX)
            ? 'space-evenly'
            : 'flex-start';
      }
    } catch (_) {}
...
body += '<section class="print-section">';
    ...
    // Divider HTML (used either above bucket title OR inside first unit row when no thumb)
    var dividerHtml = '';
    ...
    // If thumb exists, keep divider in the bucket header area.
    // If thumb does NOT exist, do NOT render divider here (we will inject it into the first unit row so it can't orphan).
    if (ctx.includeImages && bucketThumb) {
      body += dividerHtml;
    }
...
for (var r = 0; r < rows.length; r++) {
  var allowCmOverlap = (r === 0 && ctx.includeImages && bucketThumb);

  // Divider between unit rows inside the SAME bucket (so row #2+ is visually separated)
  if (r > 0 && ctx.includeImages) {
    body +=
      '<div class="shoshin-divider-row shoshin-divider-row-inner" style="position:relative;width:100%;height:32px;display:flex;align-items:center;justify-content:center;">' +
        (showDivider
          ? ('<img class="print-section-divider" src="' + esc(dividerSrc) + '" alt="" aria-hidden="true" style="height:18px;width:auto;display:block;">')
          : '') +
      '</div>';
...
@media screen{
  ...
  .print-page{
    ...
    min-height: 8.5in;
    margin: 0 auto;
    padding: 0.45in;          /* visually matches @page margin */
    box-sizing: border-box;

    background: #fff;         /* paper */
    box-shadow: 0 8px 28px rgba(0,0,0,0.18);
    overflow: hidden;         /* hard stop: nothing visually escapes the paper */
  }
