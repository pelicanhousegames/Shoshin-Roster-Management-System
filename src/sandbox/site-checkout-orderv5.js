...
function relocateClassicCoupon() {
  ...
  function moveCouponNotices() {
    ...
    notices.forEach(function (notice) {
      ...
      if (feedback.contains(notice)) return;
    ...
    return movedAny || feedback.children.length > 0;
  }
  ...
  setOpen(hasCouponNotice ? true : false);

  window.setTimeout(function () {
    var hasDelayedCouponNotice = moveCouponNotices();
    setOpen(hasDelayedCouponNotice ? true : false);
  }, 50);

  window.setTimeout(function () {
    var hasLateCouponNotice = moveCouponNotices();
    setOpen(hasLateCouponNotice ? true : false);
  }, 250);
}
...
function structureClassicMainColumn() {
  ...
  var crossSells = woocommerce.querySelector(':scope > .cross-sells');

  if (!form || !crossSells) return;
