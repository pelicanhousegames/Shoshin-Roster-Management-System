...
function syncClassicCartExpressGate() {
  ...
  var shippingRow = totals.querySelector(
    '.shoshin-classic-cart-totals-card tr.shipping, ' +
    '.shoshin-classic-cart-totals-card tr.woocommerce-shipping-totals, ' +
    '.shoshin-classic-cart-totals-card tr.woocommerce-shipping-totals.shipping'
  );
  if (!shippingRow) return;

  var helperParent =
    shippingRow.querySelector('td[data-title="Shipment"]') ||
    shippingRow.querySelector('td') ||
    shippingRow;

  if (!helperParent) return;

  var helper = helperParent.querySelector('.shoshin-express-gate-helper');

  if (!helper) {
    helper = document.createElement('div');
    helper.className = 'shoshin-express-gate-helper';
    helper.textContent = 'A shipping method must be selected to enable Express Checkout.';
    helperParent.appendChild(helper);
  }

  var selectedShipping = totals.querySelector(
    'input.shipping_method[type="radio"]:checked'
  );

  var isEnabled = !!(selectedShipping && !selectedShipping.disabled);

  expressCard.classList.toggle('is-disabled', !isEnabled);
  expressCard.setAttribute('aria-disabled', isEnabled ? 'false' : 'true');

  helper.classList.toggle('is-visible', !isEnabled);
}
  ...
  function syncClassicCheckoutExpressGate() {
    ...
    var shippingCell = document.querySelector(
      '#order_review tr.woocommerce-shipping-totals.shipping td[data-title="Shipment"], ' +
      '#order_review tr.woocommerce-shipping-totals.shipping td'
    );

    var helper = shippingCell
      ? shippingCell.querySelector('.shoshin-express-gate-helper')
      : null;

    if (shippingCell && !helper) {
      helper = document.createElement('div');
      helper.className = 'shoshin-express-gate-helper';
      helper.textContent = 'Select a shipping option to enable Express Checkout.';
      shippingCell.appendChild(helper);
    }

    var selectedShipping = document.querySelector(
  ...
  function syncClassicCheckoutAddressSections() {
    ...
    cardHeading.textContent = 'Shipping address';
  ...
  function reconcileClassicCheckoutShell() {
    ...
    syncClassicCheckoutExpressGate();
