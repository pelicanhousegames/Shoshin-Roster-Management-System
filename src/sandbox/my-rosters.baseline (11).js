...
'for(var i=0;i<atoms.length;i++){' +
  ...
  // Always use SAME gap logic as appendAtom()
  'function gapPx(){ var g = getGapPx(); return (g && isFinite(g)) ? g : 0; }' +

  // Project "used" if we append a sequence of heights, matching appendAtom() gap behavior
  'function projWillFit(addHeights){' +
    'addHeights = addHeights || [];' +
    'var u = used;' +
    'var c = countOnPage;' +
    'var g = gapPx();' +
    'for(var ai=0; ai<addHeights.length; ai++){' +
      'var hh = addHeights[ai];' +
      'hh = (hh && isFinite(hh) && hh > 0) ? hh : 0;' +
      'if(c > 0 && g){ u += g; }' +
      'u += hh;' +
      'c++;' +
    '}' +
  ...
  // -----------------------------------------------------------------------
  // Divider as separator BEFORE the NEXT unit (never first, never last)
  // Rule enforcement:
  // - Divider cannot be first on a page -> skip
  // - Divider cannot be last on a page -> only place if [divider + next unit] fits
  // - If the pair doesn't fit: page-break BEFORE next unit (skip divider), so units never become adjacent
  // -----------------------------------------------------------------------
  'if(nodeIsDiv){' +
    // must have a next atom to be a valid separator
    'if(!next){ continue; }' +

    'var hDiv = atomH[i] || 0;' +
    'var hNext = atomH[i+1] || 0;' +

    // divider cannot be first
    'if(countOnPage===0){ continue; }' +

    // if divider+next unit won't fit here, force the next unit to start a new page.
    // (skip divider so it cannot be first on the new page)
    'if(!projWillFit([hDiv, hNext])){' +
      // Safety: never leave a divider as last element on the current page
      'var lastD = body && body.lastElementChild ? body.lastElementChild : null;' +
      'if(lastD && isDivider(lastD)){' +
        'try{ body.removeChild(lastD); }catch(e){}' +
      '}' +
      'startNewPage();' +
      'continue;' +
    '}' +

    // normal: place divider + next unit together (divider can’t end page)
  ...
  // If unit doesn't fit, page-break BEFORE it.
  // Also: ensure we don't end a page with a divider (safety).
  // -----------------------------------------------------------------------
  'var h = atomH[i] || 0;' +
  'if(countOnPage>0 && !projWillFit([h])){' +
...
'}' +
  ...
  // Final safety: last page cannot end with divider
'try{' +
...
'function run(){ try{ paginate(); }catch(e){ try{ console.warn("[SHOSHIN PRINT] Pagination error:", e && e.message ? e.message : e); }catch(_){} } finally { try{ window.__SHOSHIN_PAGINATION_RUNNING__ = false; }catch(__e){} } }' +
