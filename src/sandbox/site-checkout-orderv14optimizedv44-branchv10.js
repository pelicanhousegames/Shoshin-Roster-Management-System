...
function disableLiveSingleQuantityCtas(productId) {
  ...
  document.querySelectorAll(selector).forEach(function (btn) {
    ...
    btn.classList.remove('ajax_add_to_cart', 'added', 'shoshin-added-to-cart', 'loading');
    btn.classList.add('disabled', 'shoshin-disabled-add-to-cart', 'is-locked');
    btn.removeAttribute('aria-disabled');
    btn.removeAttribute('tabindex');
    btn.removeAttribute('aria-busy');
...
function refreshSingleProductExpressState() {
  ...
  var isDownloadLocked = form.classList.contains('shoshin-download-in-cart');
  ...
  function bindLiveSingleQuantityCtaSync() {
    ...
    function runSyncSoon() {
      window.setTimeout(syncSingleProductPurchaseStateFromCartTruth, 0);
      window.setTimeout(syncSingleProductPurchaseStateFromCartTruth, 120);
      window.setTimeout(syncSingleProductPurchaseStateFromCartTruth, 320);
    }

    if (window.jQuery) {
      window.jQuery(document.body).on(
        'added_to_cart wc_fragments_refreshed removed_from_cart shoshin_single_product_cart_synced',
        function () {
          runSyncSoon();
        }
      );
    } else {
      document.body.addEventListener('wc_fragments_refreshed', runSyncSoon);
      document.body.addEventListener('removed_from_cart', runSyncSoon);
      document.body.addEventListener('shoshin_single_product_cart_synced', runSyncSoon);
    }
