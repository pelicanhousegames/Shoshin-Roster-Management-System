...
/* BEGIN SHOSHIN PRINT BASELINE — RUNTIME PAGINATION (PRINT WINDOW ONLY) */
    ...
    'function setStage(s){ try{ DBG.stage=s; window.__SHOSHIN_PAGINATION_STAGE__=s; }catch(e){} }' +
        ...
        /* BEGIN SHOSHIN PRINT — ATOM TYPE HELPERS */
    'function isSectionTitleAtom(node){' +
      'if(!node || node.nodeType!==1) return false;' +
      'return !!(node.querySelector && node.querySelector(".print-section-title"));' +
    '}' +

/* BEGIN SHOSHIN PRINT — ROBUST UNIT ATOM DETECTION */
'function isUnitAtom(node){' +
  'if(!node || node.nodeType!==1) return false;' +
  // direct class (group wrapper)
  'if(node.classList && node.classList.contains("print-unit-group")) return true;' +
  // common internal markers
  'if(node.querySelector && node.querySelector(".print-unit-block")) return true;' +
  'if(node.querySelector && node.querySelector(".print-unit-row")) return true;' +
  'if(node.querySelector && node.querySelector(".print-unit-group")) return true;' +
  // ultra-fallback (harmless)
  'if(node.querySelector && node.querySelector(".shoshin-unit")) return true;' +
  'return false;' +
'}' +
/* END SHOSHIN PRINT — ROBUST UNIT ATOM DETECTION */

/* END SHOSHIN PRINT — ATOM TYPE HELPERS */
...
/* BEGIN SHOSHIN PRINT — GET AVAIL PX (INCH-CONTRACT, NON-ZERO) */
'function getAvailPx(templateFrag, pageIndex){' +
  // Measure px per inch reliably
  'var pxPerIn = 96;' +
  'try{' +
    ...
    'if(m && isFinite(m) && m > 0) pxPerIn = m;' +
    'document.body.removeChild(r);' +
  '}catch(e){}' +

  // Page contract: Letter landscape => 8.5in tall
  'var pageH = 8.5 * pxPerIn;' +

  // Header height (page 1 only)
  'var headerH = 0;' +
  'if(pageIndex===0){' +
    'var hdr = q(".print-header", templateFrag);' +
    'headerH = rectH(hdr) || 0;' +
  '}' +

  // Body padding (from the template frag)
  ...
  // Available vertical space for atoms inside body
  'var avail = Math.max(0, pageH - headerH - padTop - padBot);' +
  'return avail;' +
'}' +
/* END SHOSHIN PRINT — GET AVAIL PX (INCH-CONTRACT, NON-ZERO) */


        'function paginate(){' +
          'var wrap = q(".print-pages-wrap");' +
/* BEGIN SHOSHIN PRINT — PAGINATION DEBUG COLLECTOR */
'var DBG = { ts: Date.now(), avail0: null, samples: [] };' +
'window.__SHOSHIN_PAGINATION_DEBUG__ = DBG;' +
/* END SHOSHIN PRINT — PAGINATION DEBUG COLLECTOR */


          'if(!wrap) return;' +
          // Idempotent guard
          'if(wrap.getAttribute("data-shoshin-paginated")==="1") return;' +

          'var firstFrag = q(".print-frag", wrap);' +
          /* BEGIN SHOSHIN PRINT — SKIP IF ALREADY MULTI-FRAG SOURCE */
      'var existingFrags = wrap.querySelectorAll(".print-frag");' +
      'if(existingFrags && existingFrags.length > 1){' +
        'window.__SHOSHIN_PAGINATION_DONE__ = { pages: existingFrags.length, ts: Date.now(), skipped: "source_already_paginated" };' +
        'try{ console.log("[SHOSHIN PRINT] Pagination skipped (source already paginated):", window.__SHOSHIN_PAGINATION_DONE__); }catch(e){}' +
        'wrap.setAttribute("data-shoshin-paginated","1");' +
        'return;' +
      '}' +
/* END SHOSHIN PRINT — SKIP IF ALREADY MULTI-FRAG SOURCE */

          'if(!firstFrag) return;' +
          'var firstBody = q(".print-frag-body", firstFrag);' +
          /* BEGIN SHOSHIN PRINT — PAGINATION CONTRACT ASSERT */
'try{' +
  'var bad=[];' +
  'var kids=firstBody.children||[];' +
  'for(var ai=0;ai<kids.length;ai++){' +
    'var el=kids[ai];' +
    'if(!el||el.nodeType!==1) continue;' +
    'if(!el.classList||!el.classList.contains("print-group")){' +
      'bad.push(el);' +
    '}' +
  '}' +
  'if(bad.length){' +
    'console.warn("[SHOSHIN PRINT][ASSERT] Pagination expects .print-frag-body > .print-group only.",{badCount:bad.length,badNodes:bad});' +
  '}' +
'}catch(e){}' +
/* END SHOSHIN PRINT — PAGINATION CONTRACT ASSERT */

          'if(!firstBody) return;' +

          'var atoms = Array.prototype.slice.call(firstBody.children || []);' +
          ...
          /* BEGIN SHOSHIN PRINT — DEBUG AVAIL */
'if(pageIndex===0){ DBG.avail0 = avail; DBG.samples.push({event:"pageStart", page:1, avail:avail}); }' +
/* END SHOSHIN PRINT — DEBUG AVAIL */
          ...
          'for(var i=0;i<atoms.length;i++){' +
            ...
            /* BEGIN SHOSHIN PRINT — KEEP SECTION TITLE WITH FIRST UNIT */
        'var next = (i+1<atoms.length) ? atoms[i+1] : null;' +
        'if(isSectionTitleAtom(node) && next && isUnitAtom(next)){' +
          'var hTitle = rectH(node);' +
          'var hNext = rectH(next);' +
          // If title+first-unit won't fit in remaining space, start new page BEFORE title
          'if(countOnPage>0 && (used + hTitle + hNext) > avail){' +
            // divider cannot be last → drop trailing divider if present
            'var lastT = body && body.lastElementChild ? body.lastElementChild : null;' +
            'if(lastT && isDivider(lastT)){' +
              'body.removeChild(lastT);' +
            '}' +
            'startNewPage();' +
          '}' +
        '}' +
/* END SHOSHIN PRINT — KEEP SECTION TITLE WITH FIRST UNIT */


            // RULE: divider cannot be first on a page → drop it
            'if(isDivider(node) && countOnPage===0){ continue; }' +

            'var h = rectH(node);' +

            // If it doesn't fit and we already have content, break page
            'if(countOnPage>0 && (used + h) > avail){' +
              // RULE: divider cannot be last → drop trailing divider at page boundary
              'var last = body && body.lastElementChild ? body.lastElementChild : null;' +
              'if(last && isDivider(last)){' +
                'body.removeChild(last);' +
              '}' +
              'startNewPage();' +
              // divider still cannot be first → drop it
              'if(isDivider(node) && countOnPage===0){ continue; }' +
            '}' +

            // If still too tall, place it anyway (prevents infinite loop)
            'appendAtom(node);' +
          '}' +

          // Final page cleanup: divider cannot be last
          'try{' +
        ...
        /* BEGIN SHOSHIN PRINT — RUN ONLY IN PRINT MODE (FIX DOUBLE-RUN) */
        'function isPrintMode(){ try{ return !!(window.matchMedia && window.matchMedia("print").matches); }catch(e){ return false; } }' +
        'function run(force){ try{ var inPrint=isPrintMode(); if(!force && !inPrint){ setStage("skip_screen_mode"); return; } if(force){ var wrap=q(".print-pages-wrap"); if(wrap) wrap.removeAttribute("data-shoshin-paginated"); } paginate(); setStage("done"); }catch(e){ try{ console.warn("[SHOSHIN PRINT] Pagination error:", e && e.message ? e.message : e); }catch(_){} } }' +
        'window.addEventListener("load", function(){ setTimeout(function(){ run(false); }, 80); });' +
        'window.addEventListener("beforeprint", function(){ setTimeout(function(){ run(true); }, 0); });' +
        /* END SHOSHIN PRINT — RUN ONLY IN PRINT MODE (FIX DOUBLE-RUN) */
      '})();</scr' + 'ipt>' +
...
//    Pagination must treat this trio as inseparable so divider/title never orphan.
  ...
  // ================================================================
var EXTRA_PRINT_CSS =
  '\n/* PRINT CONTRACT (PRINT ONLY): page-sized frags + deterministic stacking */' +
  '\n@media print{' +
...
'<body>' +
  ...
  '<script>(function(){' +
    ...
    // Divider detection: supports divider row OR divider group OR contains divider row
    'function isDivider(node){' +
      'if(!node || node.nodeType!==1) return false;' +
      'if(node.classList && node.classList.contains("shoshin-divider-row")) return true;' +
...
'function getAvailPx(templateFrag, pageIndex){' +
  'var fragRect = rectH(templateFrag);' +
  // If the frag is auto-height in screen mode, rectH will be content-height (too small/too large).
  // In print mode (our target), CSS contract sets height:8.5in, so rectH is trustworthy.
  'if(!fragRect || fragRect < 400){ fragRect = __PAGE_H_PX; }' +
  ...
  'return Math.max(0, fragRect - headerH - padTop - padBot);' +
'}' +
/* END SHOSHIN PRINT — AVAIL HEIGHT (PAGE CONTRACT, PPI-BASED FALLBACK) */
    ...
    'function paginate(){' +
      ...
      'if(wrap.getAttribute("data-shoshin-paginated")==="1"){ setStage("already_paginated_flag"); return; }' +
...
/**
 * Divider rule:
 * - Divider belongs to the NEXT unit/group (pre-unit separator).
 * - Therefore treat [divider + next] as a bonded pair for fit decisions.
 */
'if(nodeIsDiv && !nextExists){ continue; }' + // trailing divider (shouldn’t happen) → drop safely

// Divider cannot be first on a page → if divider appears first, we must also place the next item with it.
// If divider+next won't fit, start a new page; if still first, we’ll place pair anyway (unless page is empty at doc start).
'if(nodeIsDiv){' +
  // Measure pair by temporarily appending both
  'body.appendChild(node);' +
  'body.appendChild(next);' +
  'var hDiv = rectH(node);' +
  'var hNext = rectH(next);' +
  'var pairH = hDiv + hNext;' +
  // Roll back temp append
  'try{ body.removeChild(next); }catch(e){}' +
  'try{ body.removeChild(node); }catch(e){}' +

  // If we already have content and the pair won’t fit, go new page BEFORE divider
  'if(countOnPage>0 && (used + pairH) > avail){' +
    'startNewPage();' +
  '}' +

  // If page is empty and divider would be first, we must place divider+next together.
  // Append divider
  'body.appendChild(node);' +
  'hDiv = rectH(node);' +
  'used += hDiv; countOnPage++;' +
  'DBG.samples.push({event:"append", page: pageIndex+1, kind:"divider", h:hDiv, used:used, avail:avail});' +

  // Append next immediately (bonded)
  'body.appendChild(next);' +
  'hNext = rectH(next);' +
  'used += hNext; countOnPage++;' +
  'DBG.samples.push({event:"append", page: pageIndex+1, kind:"bonded_next", h:hNext, used:used, avail:avail});' +

  // Skip the next item because we already appended it
  'i++;' +
  'continue;' +
'}' +

/**
 * Normal (non-divider) atom:
 * append then measure overflow (deterministic)
 */
'body.appendChild(node);' +
'var h2 = rectH(node);' +

'if(countOnPage>0 && (used + h2) > avail){' +
  // Roll back
  'try{ body.removeChild(node); }catch(e){}' +
  // If last is a divider, remove it (divider cannot be last) — but bonded logic should prevent this
  'var last = body && body.lastElementChild ? body.lastElementChild : null;' +
  'if(last && isDivider(last)){' +
    'try{ body.removeChild(last); }catch(e){}' +
  '}' +
  'startNewPage();' +
  // Append on new page
  'body.appendChild(node);' +
...
'used += h2; countOnPage++;' +
'DBG.samples.push({event:"append", page: pageIndex+1, kind:"atom", h:h2, used:used, avail:avail});' +
/* END SHOSHIN PRINT — BONDED DIVIDER+UNIT PACKING */


      // Final page: divider cannot be last
      'try{' +
        'var last2 = body && body.lastElementChild ? body.lastElementChild : null;' +
        'if(last2 && isDivider(last2)) body.removeChild(last2);' +
      '}catch(e){}' +
    ...
    /* BEGIN SHOSHIN PRINT — RUN ONLY IN PRINT MODE (FIX DOUBLE-RUN) */
    'function isPrintMode(){ try{ return !!(window.matchMedia && window.matchMedia("print").matches); }catch(e){ return false; } }' +
    'function run(force){ try{ var inPrint=isPrintMode(); if(!force && !inPrint){ setStage("skip_screen_mode"); return; } if(force){ var wrap=q(".print-pages-wrap"); if(wrap) wrap.removeAttribute("data-shoshin-paginated"); } paginate(); setStage("done"); DBG.samples.push({event:"done", pages:(window.__SHOSHIN_PAGINATION_DONE__?window.__SHOSHIN_PAGINATION_DONE__.pages:null)}); }catch(e){ try{ console.warn("[SHOSHIN PRINT] Pagination error:", e && e.message ? e.message : e); }catch(_){} } }' +
    'window.addEventListener("load", function(){ setTimeout(function(){ run(false); }, 80); });' +
    'window.addEventListener("beforeprint", function(){ setTimeout(function(){ run(true); }, 0); });' +
    /* END SHOSHIN PRINT — RUN ONLY IN PRINT MODE (FIX DOUBLE-RUN) */
  '})();</scr' + 'ipt>' +
...
// Divider HTML (used above each per-row bucket header)
...
for (var r = 0; r < rows.length; r++) {
  ...
  // Divider + bucket header per unit row (only when includeImages + thumb exists)
  if (hasThumbHeader) body += dividerHtml;

/* BEGIN SHOSHIN PRINT — NO-IMAGES TITLE OVERLAP (HTML) */
body +=
  '<div class="print-section-title' + ((ctx.includeImages) ? '' : ' is-noimg') + '" style="position:relative;width:100%;">' +
/* END SHOSHIN PRINT — NO-IMAGES TITLE OVERLAP (HTML) */
