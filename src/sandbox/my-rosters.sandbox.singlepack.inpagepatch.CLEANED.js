...
function shoshinPaginateAndJustify(win, doc, opts){
  ...
  try{
    ...
    function startFrag(isFirst){
      ...
      if (isFirst && header && header.parentNode){
        inner.appendChild(header); // moves header into first fragment
      }
...
/* BEGIN SHOSHIN PRINT — PACK HEADER ONLY ON FIRST ROW (RESTORE 4/PAGE) */
for (var r = 0; r < rows.length; r++) {

  // Only the first row in a bucket gets the divider + thumb/title header.
  var isFirstInBucket = (r === 0);
  var showPackHeader = !!(hasThumbHeader && isFirstInBucket);

  // Overlap is only allowed when the header is present on THIS pack.
  var allowCmOverlap = !!showPackHeader;

  // Single atomic pack per unit row (pagination target)
  body += '<div class="print-pack">';

  if (showPackHeader) body += dividerHtml;
...
/* END SHOSHIN PRINT — PACK HEADER ONLY ON FIRST ROW (RESTORE 4/PAGE) */
