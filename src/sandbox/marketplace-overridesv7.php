...
if (is_array($member_price_meta)) {
    foreach ($member_price_meta as $level_id => $pricing) {
        $level_price = null;

        if (is_array($pricing)) {
            if (isset($pricing['price'])) {
                $level_price = $pricing['price'];
            } elseif (isset($pricing['initial_payment'])) {
                $level_price = $pricing['initial_payment'];
            } elseif (isset($pricing['initial_payment_amount'])) {
                $level_price = $pricing['initial_payment_amount'];
            }
        } elseif ($pricing !== '' && $pricing !== null) {
            $level_price = $pricing;
        }

        if ($level_price !== null && (float) $level_price === 0.0) {
            ...
            if (is_user_logged_in() && function_exists('pmpro_getMembershipLevelForUser')) {
                ...
                if ($user_level && (int) $user_level->id === (int) $level_id) {
 ...
 * treat that as free for the current user even if meta shape differs.
 */
if (!$membership_free_for_user && $member_discount_active && (float) $current === 0.0) {
    ...
    if (!$sale_active && !$member_discount_active) {
        ...
        if (!shoshin_has_annual_membership_discount()) {
            $teaser_html = '<div class="shoshin-price-savings shoshin-price-member-teaser">' . ($membership_free_for_others ? '<a href="/membership">Supporting Members</a> receive this item for FREE' : shoshin_format_member_teaser_text($guest_member_amount, $guest_member_percent, false, false)) . '</div>';
        }
