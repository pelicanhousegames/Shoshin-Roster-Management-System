...
add_action('woocommerce_product_options_general_product_data', 'shoshin_pmpro_simple_product_fields');

function shoshin_pmpro_simple_product_fields() {
    global $post;
    ...
    // Only simple (includes downloadable simple)
    ...
    echo '<p><strong>Membership Pricing</strong></p>';

    foreach ( $levels as $level ) {
        $meta_key = '_level_' . $level->id . '_price';
        $value = get_post_meta( $post->ID, $meta_key, true );

        woocommerce_wp_text_input( array(
            'id'          => $meta_key,
            'label'       => esc_html( $level->name ) . ' Price',
            'desc_tip'    => true,
            'description' => 'Flat member price for this level',
            'value'       => $value,
            'type'        => 'text',
            'data_type'   => 'price',
        ));
...
add_action('save_post_product', 'shoshin_save_pmpro_simple_product_fields', 20, 1);

function shoshin_save_pmpro_simple_product_fields( $post_id ) {
    ...
    // Only simple (includes downloads)
    if ( $product->is_type( 'variable' ) ) return;
    if ( $product->is_type( 'external' ) ) return;
...
function shoshin_storefront_get_shipping_class_slug( $product ) {
    ...
    $slug = (string) $context_product->get_shipping_class();
...
function shoshin_cart_meta_build_lines( $product, $cart_item = array() ) {
    ...
    } elseif ( $shipping_class === 'download' ) {
        $lines[] = array(
