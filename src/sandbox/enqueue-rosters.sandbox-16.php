...
if ( ! function_exists( 'shoshin_rosters_autoheal_assigned_units' ) ) {
	...
	function shoshin_rosters_autoheal_assigned_units( $assigned_arr, $owner_user_id ) {
		...
		foreach ( $assigned_arr as $row ) {
				...
				if ( ! $entry_full ) {
					...
					if ( (int) $entry_user_id !== (int) $owner_user_id ) {
						...
						if ( $kind === 'character' && $entry_form_id !== 2247 ) {
							...
							// Common mapping used across your builders:
							// field 3 = refId
							// field 4 = name (character) OR supportType (support)
							$new_ref = shoshin_rosters_get_field_value_from_blob( $blob, 3, '' );
							$new_f4  = shoshin_rosters_get_field_value_from_blob( $blob, 4, '' );
			...
			// Update snapshot fields if changed (refId + name for characters)
			$before_ref = isset( $row['refId'] ) ? (string) $row['refId'] : '';
			$after_ref  = (string) $rehydrated['refId'];
			...
			if ( $kind === 'character' ) {
				...
				$after_name  = (string) $rehydrated['f4'];
