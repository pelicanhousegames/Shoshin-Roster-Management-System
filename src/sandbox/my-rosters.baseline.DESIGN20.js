...
'if(!node){ continue; }' +
  ...
  'function ensureMeasurer(){' +
    ...
    'try{' +
      ...
      // CRITICAL:
      // Many Layout2 table styles are scoped under ".print-page.print-layout-2 ...".
      // Ensure the measurer participates in that scope so heights match the real page.
      '__MEAS.className = "print-page print-layout-2 shoshin-measurer";' +

      // Keep offscreen & non-interactive; do not affect page layout.
      '__MEAS.style.cssText="position:absolute;left:-9999px;top:0;visibility:hidden;pointer-events:none;box-sizing:border-box;margin:0;padding:0;";' +
...
// Placement:
// - place divider whenever it fits BY ITSELF
// -----------------------------------------------------------------------
'if(nodeIsDiv){' +
  ...
  // If divider doesn't fit here, skip it (we will NOT force a page break for a divider)
  'if(!projWillFit([hDiv], used, countOnPage, avail)){' +
    'continue;' +
  '}' +

  // Place divider now. If the next atom later forces a page break,
  // we will strip this divider right before breaking pages.
...
'try{' +
  ...
  'for(var p=0; p<frags2.length; p++){' +
    ...
    // never start with a divider
    'while(bdy.firstElementChild && isDivider(bdy.firstElementChild)){' +
      'bdy.removeChild(bdy.firstElementChild);' +
    '}' +

    // never end with a divider
    'while(bdy.lastElementChild && isDivider(bdy.lastElementChild)){' +
      'bdy.removeChild(bdy.lastElementChild);' +
    '}' +
  '}' +
'}catch(e){}' +
