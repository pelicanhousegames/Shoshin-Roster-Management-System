 ...
 * - no refresh
 * - no top notice
 * - no injected View Cart link
 * - auto-open Elementor side cart
 * ========================================================= */
add_action('wp_enqueue_scripts', function () {
    ...
    wc_enqueue_js("
        var shoshinLoopButtonSelector = 'ul.products li.product a.ajax_add_to_cart';
        var shoshinAjaxNonce = '" . esc_js($nonce) . "';

        function shoshinOpenMenuCart() {
            var \$panel = jQuery('.elementor-menu-cart__container');
        ...
        function shoshinRefreshCartFragments(done) {
            jQuery.ajax({
                type: 'POST',
        ...
        /* -----------------------------------------
           Loop buttons: preserve custom text state
           ----------------------------------------- */
        jQuery(document.body).on('adding_to_cart', function (event, \$button) {
            if (!\$button || !\$button.length || !\$button.is(shoshinLoopButtonSelector)) {
                return;
            }

            if (!\$button.data('shoshin-original-text')) {
                \$button.data('shoshin-original-text', jQuery.trim(\$button.text()));
            }
        });
        ...
        jQuery(document.body).on('added_to_cart', function (event, fragments, cart_hash, \$button) {
            if (\$button && \$button.length && \$button.is(shoshinLoopButtonSelector)) {
                ...
                window.setTimeout(function () {
                    ...
                    \$button
                        .removeClass('shoshin-added-to-cart')
                        .text(originalText);
                }, 2400);
            }
            ...
            window.setTimeout(function () {
                ...
                jQuery('a.added_to_cart.wc-forward').remove();
                shoshinOpenMenuCart();
            }, 60);
        });
        ...
        /* -----------------------------------------
           ...
           server-backed AJAX add to cart
           ----------------------------------------- */
        jQuery(document).on('submit', 'form.cart', function (e) {
            ...
            jQuery.each(serialized, function (_, field) {
                var name = field.name;
            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    ...
                    shoshinRefreshCartFragments(function () {
                        jQuery('.woocommerce-message, .woocommerce-error, .woocommerce-info').remove();
                        ...
                        window.setTimeout(function () {
                            ...
                            \$button
                                .removeClass('shoshin-added-to-cart')
                                .text(\$button.data('shoshin-original-text') || 'Add to Cart');
                        }, 2400);
                    });
                },
                error: function () {
