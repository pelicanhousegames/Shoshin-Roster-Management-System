      ...
      updateBtn.addEventListener('click', function () {
        ...
        postAjax('shoshin_update_roster_banner', {
          ...
          .then(function (data) {
            ...
            bannerCurrentRoster.field_8 = (data && data.bannerRaw != null) ? String(data.bannerRaw) : valueToSave;
      ...
      '<div class="shoshin-unassign-preview-wrap">' +
        ...
        '<div class="shoshin-unassign-preview-columns">' +
          '<div class="shoshin-unassign-preview-col">' +
            '<div class="shoshin-unassign-preview-col-title">Current</div>' +
            '<div id="shoshinUnassignPreviewBefore" class="shoshin-unassign-preview-panel"></div>' +
          '</div>' +

          '<div class="shoshin-unassign-preview-col">' +
            '<div class="shoshin-unassign-preview-col-title">After</div>' +
...
function openUnassignModal(ctx) {
  ...
  if (titleEl) titleEl.textContent = 'Unassign: ' + label;
  if (descEl)  descEl.textContent  = 'Select the new final quantity for this unit and click Unassign.';
...
if (qtySel) {
  ...
  if (isDaimyo) {
    // Daimyo can only be 1 (kept) or 0 (removed)
    ...
    opt1.value = '1';
    opt1.textContent = '1';
    qtySel.appendChild(opt1);

    // Default to 1 to prevent accidental Daimyo deletion
    qtySel.value = '1';
  } else {
  ...
  function renderAssignedStripRow(u, rosterEntryId, clanPoints) {
    var img = String(u.img || u.image || u.imgUrl || '').trim();
  ...
  function renderRosterCard(r, idx) {
    ...
    // Fallback: some rosters don't store points server-side; compute from assignments
    if (!clanPoints || clanPoints <= 0) clanPoints = rosterTotals.points;
    ...
    var rosterTotals = computeRosterTotalsFromAssigned(assigned);
