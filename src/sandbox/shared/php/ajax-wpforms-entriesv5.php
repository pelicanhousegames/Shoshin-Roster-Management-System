...
/* ------------------------------------------------------------------------- */
/* DB-backed WPForms reads (bypass WPForms capability gating for user-owned data) */
/* ------------------------------------------------------------------------- */

if (!function_exists('shoshin_wpforms_db_get_entries_by_user')) {
  function shoshin_wpforms_db_get_entries_by_user($form_id, $user_id, $limit = 500) {
    global $wpdb;
    ...
    // WPForms table is typically {$wpdb->prefix}wpforms_entries
    $table = $wpdb->prefix . 'wpforms_entries';

    // Return shape compatible with the rest of this file: arrays with entry_id + fields
    $sql = $wpdb->prepare(
    ...
    // Normalize keys to match shoshin_get(..., 'entry_id') usage
    $out = [];
    foreach ($rows as $r) {
...
/* ------------------------------------------------------------------------- */
/* HARDENING GUARDS — Login + Rate Limit                                     */
/* ------------------------------------------------------------------------- */

if (!function_exists('shoshin_require_login')) {
  function shoshin_require_login() {
...
if (!function_exists('shoshin_rate_limit')) {
  function shoshin_rate_limit($action_key, $limit = 10, $window_seconds = 60) {
    $user_id = get_current_user_id();
    if (!$user_id) return;

    $key = 'rms_rl_' . $user_id . '_' . sanitize_key($action_key);
    $now = time();
...
if (!function_exists('shoshin_wpforms_db_delete_entry')) {
  function shoshin_wpforms_db_delete_entry($entry_id, $expected_user_id = 0) {
    global $wpdb;
    ...
    // Enforce ownership check before delete
    $row = shoshin_wpforms_db_get_entry($entry_id);
    if (!$row) return false;
    ...
    $deleted = $wpdb->delete($entries_table, ['entry_id' => $entry_id], ['%d']);

    // If no row deleted, treat as failure
...
if (!function_exists('shoshin_wpforms_get_user_entry_id_set')) {
  function shoshin_wpforms_get_user_entry_id_set($form_id, $user_id, $limit = 2000) {
    ...
    // Use DB-backed read to avoid WPForms capability gating for non-admin roles
    $rows = shoshin_wpforms_db_get_entries_by_user((int) $form_id, (int) $user_id, (int) $limit);
...
if (!function_exists('shoshin_compute_roster_totals')) {
  function shoshin_compute_roster_totals($assigned_units) {
    ...
    // Safe numeric parse (handles strings like "4", ignores "Highest", "Captain", etc.)
    $to_int = function($v) {
      if ($v === null) return 0;
      ...
      if (is_string($v)) {
        $s = trim($v);
        // allow simple numeric strings only
    ...
    foreach ($assigned_units as $u) {
      ...
      // Initiative priority:
      // 1) top-level ini / initiative if > 0
      // 2) stats.ini (common in assigned_units_json snapshots)
      $ini_top = 0;
      if (isset($u['ini'])) {
        $ini_top = $to_int($u['ini']);
      } elseif (isset($u['initiative'])) {
        $ini_top = $to_int($u['initiative']);
      }

      $ini_stats = 0;
      if (isset($u['stats']) && is_array($u['stats']) && isset($u['stats']['ini'])) {
        $ini_stats = $to_int($u['stats']['ini']);
      }

      $ini = ($ini_top > 0) ? $ini_top : $ini_stats;
...
if (!function_exists('shoshin_cascade_unassign_deleted_asset_from_rosters')) {
  function shoshin_cascade_unassign_deleted_asset_from_rosters($user_id, $deleted_entry_id, $deleted_kind = '') {
    ...
    // DB-backed read to avoid WPForms capability gating for non-admin roles
    $entries = shoshin_wpforms_db_get_entries_by_user($ROSTER_FORM_ID, (int) $user_id, 500);
    ...
    foreach ($entries as $e) {
      ...
      // Load full entry for update (DB-backed; avoids WPForms capability gating)
      $entry = shoshin_wpforms_db_get_entry($rid);
      if (!$entry) continue;
      ...
      $ok = shoshin_wpforms_db_update_entry_fields($rid, $fields_json, (int) $ROSTER_FORM_ID, (int) $user_id);
...
add_action('wp_ajax_shoshin_delete_wpforms_entry', function () {
  ...
  shoshin_rate_limit('delete_entry', 10, 60);
  ...
  // Perform the delete
  // DB-backed delete to avoid WPForms capability gating for non-admin roles
  $ok = shoshin_wpforms_db_delete_entry($entry_id, (int) $user_id);
...
add_action('wp_ajax_shoshin_get_my_rosters', function () {
  ...
  // WPForms entry API is capability-gated; non-admin roles may see zero entries.
  // Use DB-backed read for user-owned rosters.
  $entries = shoshin_wpforms_db_get_entries_by_user($ROSTER_FORM_ID, $user_id, 500);
  ...
  foreach ($entries as $e) {
    ...
    if (function_exists('shoshin_build_roster')) {
      ...
      if ($did_prune) {
        ...
        $entry_full = shoshin_wpforms_db_get_entry((int) shoshin_get($e, 'entry_id', 0));
        ...
        // DB-backed update to avoid WPForms capability gating for non-admin roles
        shoshin_wpforms_db_update_entry_fields(
          (int) shoshin_get($e, 'entry_id', 0),
          $fields_json,
          (int) $ROSTER_FORM_ID,
          (int) $user_id
        );
      ...
      $r['locked'] = ( function_exists( 'shoshin_rms_is_locked_entry' ) ? (bool) shoshin_rms_is_locked_entry( $ROSTER_FORM_ID, $entry_id, $user_id ) : false );
      $out[] = $r;
      continue;
    }
...
add_action('wp_ajax_shoshin_assign_asset_to_rosters', function () {
  shoshin_ajax_require_logged_in();
  shoshin_ajax_verify_nonce();
  shoshin_rate_limit('assign_asset', 20, 60);
  shoshin_wpforms_available_or_fail();
  ...
  if (!is_array($unit) || empty($unit['unitKey'])) {
    ...
    // -----------------------------------------------------------------------
  // RMS Soft Lock (backend-first)
  // - Block if the unit entry is locked (when entryId is present)
  // - Block if ANY target roster is locked
  // -----------------------------------------------------------------------
  if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {

    // Unit lock (if entryId provided)
    $unit_entry_id = 0;
    if ( isset( $unit['entryId'] ) ) $unit_entry_id = absint( $unit['entryId'] );
    else if ( isset( $unit['entry_id'] ) ) $unit_entry_id = absint( $unit['entry_id'] );

    if ( $unit_entry_id > 0 ) {
      $unit_kind = '';
      if ( isset( $unit['kind'] ) ) $unit_kind = strtolower( trim( (string) $unit['kind'] ) );

      // Mirror My Assets JS convention: kind === 'support' ? 2501 : 2247
      $unit_form_id = ( $unit_kind === 'support' ) ? 2501 : 2247;

      if ( shoshin_rms_is_locked_entry( $unit_form_id, $unit_entry_id, $user_id ) ) {
        wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
      }
    }

    // Roster locks (this endpoint mutates rosters)
    foreach ( $roster_ids as $rid_check ) {
      $rid_check = absint( $rid_check );
      if ( $rid_check > 0 && shoshin_rms_is_locked_entry( $ROSTER_FORM_ID, $rid_check, $user_id ) ) {
        wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
      }
    }
  }
...
foreach ($assigned as &$u) {
  ...
  $same_unitkey = (is_array($u) && isset($u['unitKey']) && (string) $u['unitKey'] === (string) $unit['unitKey']);

  // Support fallback: merge by refId if unitKey changed due to naming (Wasen ↔ subtype)
  $same_support_ref = false;
  if (!$same_unitkey && is_array($u)) {
    $u_kind = isset($u['kind']) ? (string) $u['kind'] : '';
    $n_kind = isset($unit['kind']) ? (string) $unit['kind'] : '';
    $u_ref  = isset($u['refId']) ? trim((string) $u['refId']) : '';
    $n_ref  = isset($unit['refId']) ? trim((string) $unit['refId']) : '';
    $u_cls  = isset($u['cls']) ? (string) $u['cls'] : '';
    $n_cls  = isset($unit['cls']) ? (string) $unit['cls'] : '';

    if ($u_kind === 'support' && $n_kind === 'support' && $u_ref !== '' && $u_ref === $n_ref && $u_cls === $n_cls) {
      $same_support_ref = true;
    }
  }

  if ($same_unitkey || $same_support_ref) {
...
add_action('wp_ajax_shoshin_bulk_assign_units_to_roster', function () {
  shoshin_ajax_require_logged_in();
  shoshin_ajax_verify_nonce();
  shoshin_rate_limit('bulk_assign', 10, 60);
  shoshin_wpforms_available_or_fail();
  ...
  if (!$rid) wp_send_json_error(['message' => 'Missing rosterEntryId.'], 400);
    ...
    // -----------------------------------------------------------------------
  // RMS Soft Lock (backend-first) — block all bulk assign on locked rosters
  // -----------------------------------------------------------------------
  if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {
    if ( shoshin_rms_is_locked_entry( $ROSTER_FORM_ID, $rid, $user_id ) ) {
      wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
    }
  }
  ...
  // Load roster entry (DB-backed; avoids WPForms capability gating for non-admin roles)
  $entry = shoshin_wpforms_db_get_entry($rid);
...
add_action('wp_ajax_shoshin_set_unit_qty', function () {
  shoshin_ajax_require_logged_in();
  shoshin_ajax_verify_nonce();
  shoshin_rate_limit('set_qty', 60, 60);
  shoshin_wpforms_available_or_fail();
  ...
  if ($unit_key === '') wp_send_json_error(['message' => 'Missing unitKey.'], 400);
    ...
    // -----------------------------------------------------------------------
  // RMS Soft Lock (backend-first) — block qty changes on locked rosters
  // -----------------------------------------------------------------------
  if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {
    if ( shoshin_rms_is_locked_entry( $ROSTER_FORM_ID, $roster_entry_id, $user_id ) ) {
      wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
    }
  }

  // Load entry + verify ownership + form (DB-backed; avoids WPForms capability gating for non-admin roles)
  $entry = shoshin_wpforms_db_get_entry($roster_entry_id);
  $entry_id_found = (int) shoshin_get($entry, 'entry_id', 0);
...
$new_assigned = shoshin_sanitize_assigned_payload($new_assigned);
  ...
  $ok = shoshin_wpforms_db_update_entry_fields($roster_entry_id, $fields_json, (int) $ROSTER_FORM_ID, (int) $user_id);
...
add_action('wp_ajax_shoshin_update_roster_banner', function () {
  shoshin_ajax_require_logged_in();
  shoshin_ajax_verify_nonce();
  shoshin_rate_limit('update_banner', 6, 60);
  shoshin_wpforms_available_or_fail();
  ...
  if (!$entry_id) {
    ...
    // -----------------------------------------------------------------------
  // RMS Soft Lock (backend-first) — block banner updates on locked rosters
  // -----------------------------------------------------------------------
  if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {
    $user_id = (int) get_current_user_id();
    if ( shoshin_rms_is_locked_entry( $ROSTER_FORM_ID, $entry_id, $user_id ) ) {
      wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
    }
  }
  ...
  // Load entry + enforce ownership (DB-backed; avoids WPForms capability gating for non-admin roles)
  $entry = shoshin_wpforms_db_get_entry($entry_id);
  if (empty($entry) || empty($entry['entry_id'])) {
    wp_send_json_error(['message' => 'Entry not found.'], 404);
  }
  if ((int) shoshin_get($entry, 'form_id', 0) !== (int) $ROSTER_FORM_ID) {
  ...
  if ($is_remove) {
    ...
    $fields_json = wp_json_encode($fields, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
    $ok = shoshin_wpforms_db_update_entry_fields($entry_id, $fields_json, (int) $ROSTER_FORM_ID, (int) $user_id);
