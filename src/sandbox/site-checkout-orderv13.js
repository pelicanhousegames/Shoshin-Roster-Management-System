...
if (!expressCard.querySelector('.shoshin-express-divider')) {
...
if (!expressCard.querySelector('.shoshin-express-divider')) {
  ...
  expressCard.appendChild(expressCard.querySelector('.shoshin-express-divider'));
}
...
function toggleClassicExpressCardForZeroTotal() {
  ...
  // Normalize common currency text like 0.00
...
var preservedExpressNodes = preserveClassicExpressMounts();
    ...
    fetch(href, {
      ...
      .then(function (html) {
        ...
        // Remove WooCommerce notices (e.g., "Item removed. Undo?")
        var notices = doc.querySelectorAll('.woocommerce-notices-wrapper, .woocommerce-message, .woocommerce-error');
