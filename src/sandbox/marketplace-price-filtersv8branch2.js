...
var shoshinFeCleanupTimer = null;
var shoshinFeObserver = null;

function scheduleFilterEverythingCleanup() {
  if (shoshinFeCleanupTimer) {
    clearTimeout(shoshinFeCleanupTimer);
  }

  shoshinFeCleanupTimer = setTimeout(function () {
...
function runPostRenderUiSync() {
  ...
  bindFilterEverythingObserver();
}

function bindFilterEverythingObserver() {
  if (shoshinFeObserver) {
    shoshinFeObserver.disconnect();
    shoshinFeObserver = null;
  }

  var widget = document.querySelector('.widget_wpc_filters_widget');
  if (!widget) return;

  shoshinFeObserver = new MutationObserver(function () {
    scheduleFilterEverythingCleanup();
  });

  shoshinFeObserver.observe(widget, {
    childList: true,
    subtree: true
  });
}
