...
function replaceFilterEverythingFacets(doc) {
  /*
   * Replace the full Filter Everything widget shell inside the popup.
   *
   * We intentionally do NOT hardcode the data-set value (e.g. 3967),
   * because that is a plugin/widget instance identifier and may change
   * if the filter set or popup is rebuilt.
   */
  var currentWidget = document.querySelector(
    '.dialog-lightbox-message .widget.widget_wpc_filters_widget .wpc-filters-main-wrap[data-set]'
  );

  var newWidget = doc.querySelector(
    '.widget.widget_wpc_filters_widget .wpc-filters-main-wrap[data-set]'
  );
  ...
  currentWidget.replaceWith(newWidget.cloneNode(true));
}
...
function applyPriceFilterAjax(url) {
  ...
  xhr.onload = function () {
    ...
    /* Replace Filter Everything facet markup so option availability stays in sync */
    replaceFilterEverythingFacets(doc);
