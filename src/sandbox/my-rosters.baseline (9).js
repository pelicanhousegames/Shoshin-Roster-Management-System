...
'for(var i=0;i<atoms.length;i++){' +
  ...
  // Divider belongs to NEXT unit/group → treat [divider + next] as bonded.
  'if(nodeIsDiv){' +
    'if(!next){ continue; }' +
    ...
    // If we already have content and the pair won’t fit, start a new page BEFORE divider.
'if(countOnPage>0 && (used + __GAP_PX__ + __GAP_PX__ + pairH) > avail){' +

      // Divider cannot be last → drop trailing divider at page boundary if present
    ...
    // RULE: Divider may NEVER be first on a page.
    // If we're at top-of-page, drop divider and start with the unit.
    'if(countOnPage===0){' +
    ...
    // Normal case: divider is not first → place divider WITH its unit.
  ...
  // Non-divider atom flow
  'var h = atomH[i] || 0;' +
'if(countOnPage>0 && (used + __GAP_PX__ + h) > avail){' +

    // Divider cannot be last → drop trailing divider at page boundary if present
    'var last2 = body && body.lastElementChild ? body.lastElementChild : null;' +
    'if(last2 && isDivider(last2)){' +
      'try{ body.removeChild(last2); }catch(e){}' +
    '}' +
    'startNewPage();' +
  '}' +
  'appendAtom(node, h);' +
