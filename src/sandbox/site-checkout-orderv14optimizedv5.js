...
(function () {
  function buildCheckoutShell() {
    ...
    const oldShell = form.querySelector(':scope > .shoshin-checkout-shell');
    ...
    function appendIf(card, node) {
      if (card && node) card.appendChild(node);
    }
    ...
    if (expressPrimary || expressWcPay || stripeExpressSeparator) {
      ...
      appendIf(expressCard, expressPrimary);
      appendIf(expressCard, expressWcPay);
      appendIf(expressCard, stripeExpressSeparator);
    ...
    if (contact) {
      const contactCard = createCard('card-contact');
      contactCard.appendChild(contact);
      main.appendChild(contactCard);
    }

    if (shipping) {
      const shippingCard = createCard('card-shipping');
      shippingCard.appendChild(shipping);
      main.appendChild(shippingCard);
    }

    if (billing) {
      const billingCard = createCard('card-billing');
      billingCard.appendChild(billing);
      main.appendChild(billingCard);
    }
    ...
    if (orderReview) {
      ...
      if (shippingRow) {
        const shippingMethodsCard = createCard('card-shipping-methods', 'Shipping options');
        const shippingCell = shippingRow.querySelector('td');

        if (shippingCell) {
          shippingMethodsCard.appendChild(shippingCell);
        } else {
          shippingMethodsCard.appendChild(shippingRow);
        }

        shippingRow.remove();
        main.appendChild(shippingMethodsCard);
      }
    }

    if (payment) {
      const paymentCard = createCard('card-payment');
      paymentCard.appendChild(payment);
      main.appendChild(paymentCard);
    }

    if (notes) {
      const notesCard = createCard('card-notes');
      notesCard.appendChild(notes);
      main.appendChild(notesCard);
    }

    if (terms) {
      const termsCard = createCard('card-terms');
      termsCard.appendChild(terms);
      main.appendChild(termsCard);
    }

    if (placeOrder) {
      const actionsCard = createCard('card-actions');
      actionsCard.appendChild(placeOrder);
      main.appendChild(actionsCard);
    }

    if (orderHeading || orderReview) {
      const sidebarCard = createCard('card-order-summary');
      appendIf(sidebarCard, orderHeading);
      appendIf(sidebarCard, orderReview);
  ...
  function initCheckoutShell() {
    ...
    window.setTimeout(buildCheckoutShell, 250);
  }
