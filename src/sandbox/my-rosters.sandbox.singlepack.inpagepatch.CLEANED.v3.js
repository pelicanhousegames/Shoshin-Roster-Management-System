...
function openPrintTabDocument(title, cssText, bodyHtml) {
  ...
  var EXTRA_PRINT_CSS =
    ...
    "\n}" +
    "\n@media screen{" +
    "\n  /* Screen preview: keep header aligned with the page stack */" +
    "\n  .print-header{ width: calc(11in - 0.7in); max-width: calc(100vw - 48px); margin: 0 auto 10px auto; background:#fff; box-sizing:border-box; }" +
    "\n}" +
...
/* BEGIN SHOSHIN PRINT — SINGLE PACK PER UNIT (divider + title + unit) */
...
// Divider + bucket title should render ONCE per section (not per unit pack),
// so unit packs stay compact and we can fit 4 per page.
if (hasThumbHeader) {
  body += ''
    + '<div class="shoshin-divider-row" style="position:relative;width:100%;height:18px;display:flex;align-items:center;justify-content:center;">'
    +   '<img class="print-section-divider" src="' + esc(dividerSrc) + '" alt="" aria-hidden="true" style="height:12px;width:auto;display:block;">'
    + '</div>'
    + '<div class="print-section-title" style="position:relative;width:100%;">'
...
for (var r = 0; r < rows.length; r++) {
  ...
  // Overlap is only allowed when the section header is present; after we render it once,
  // packs should not rely on overlap behavior.
  var allowCmOverlap = false;

  // Single atomic pack per unit row (pagination + justification target)
...
body += '</section>';
/* END SHOSHIN PRINT — SINGLE PACK PER UNIT (divider + title + unit) */
