...
'for(var i=0;i<atoms.length;i++){' +
  ...
  'function ensureMeasurer(){' +
    'if(__MEAS) return __MEAS;' +
    'try{' +
      '__MEAS = document.createElement("div");' +
      '__MEAS.setAttribute("data-shoshin-measure","1");' +
      '__MEAS.style.cssText="position:absolute;left:-9999px;top:0;visibility:hidden;pointer-events:none;";' +
  ...
  // Tail becomes the remaining rows; it will be processed immediately on the next page
  'var tail = cloneBucketKeepingAfterN(node, kBest);' +
  'var tailRows = bucketRowCount(tail);' +
  'if(tailRows > 0){' +
    'startNewPage();' +
    // IMPORTANT: re-run the same logic for the tail on the new page by inserting it as "node"
    'node = tail;' +
    'h = measureNodeHeight(node);' +
    'i--; /* reprocess this index as the tail bucket */' +
  '}' +
