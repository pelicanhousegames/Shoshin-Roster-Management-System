...
function buildCheckoutShell() {
  ...
  // EXPRESS — DO NOT USE parentElement
  const express = document.querySelector('.wcpay-express-checkout-wrapper');
  ...
  function isSafeToMove(el) {
    ...
    // Never move the form or anything containing it
  ...
  if (isSafeToMove(express)) {
    const card = wrapCard(express, 'card-express');
    if (card) main.appendChild(card);
  }

  if (contact) main.appendChild(wrapCard(contact, 'card-contact'));
  if (billing) main.appendChild(wrapCard(billing, 'card-billing'));
  if (shipping) main.appendChild(wrapCard(shipping, 'card-shipping'));
  if (notes) main.appendChild(wrapCard(notes, 'card-notes'));
  if (payment) main.appendChild(wrapCard(payment, 'card-payment'));
  if (terms) main.appendChild(wrapCard(terms, 'card-terms'));
  if (placeOrder) main.appendChild(wrapCard(placeOrder, 'card-actions'));

  /* -----------------------------------------
     SIDEBAR
  ----------------------------------------- */

  if (orderHeading) sidebar.appendChild(orderHeading);
  if (orderReview) sidebar.appendChild(orderReview);
