...
function shoshin_product_is_effectively_zero_for_current_user($product) {
    ...
    // If Woo already resolves the current shopper's price to zero, we are done.
    if ($current <= 0) {
        return true;
    }

    /*
     * PMPro WooCommerce per-level pricing:
     * _level_{LEVEL_ID}_price
     *
     * If the current logged-in user's active non-free level maps this product
     * to 0.00, treat the product as effectively free for that shopper.
     */
    if (is_user_logged_in() && function_exists('pmpro_getMembershipLevelForUser')) {
        ...
        if ($user_level && !empty($user_level->id)) {
            ...
            if ($level_price !== '' && $level_price !== null && (float) $level_price === 0.0 && $level_name !== 'free') {
                return true;
            }
        }
    }
...
function shoshin_render_product_trust_block() {
    ...
    foreach ($trust['rows'] as $row_text) {
