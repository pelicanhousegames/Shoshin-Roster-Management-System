...
function replaceFilterEverythingMarkup(doc) {
  ...
  var currentFilterRoot =
    document.querySelector('.wpc-filters-widget-wrapper') ||
    document.querySelector('.wpc-filters-section');

  var newFilterRoot =
    doc.querySelector('.wpc-filters-widget-wrapper') ||
    doc.querySelector('.wpc-filters-section');

  if (!currentFilterRoot || !newFilterRoot) return;

  var currentWidgetWrapper = currentFilterRoot.closest('.wpc-filters-widget-wrapper');
  var newWidgetWrapper = newFilterRoot.closest('.wpc-filters-widget-wrapper');

  if (currentWidgetWrapper && newWidgetWrapper) {
    currentWidgetWrapper.innerHTML = newWidgetWrapper.innerHTML;
    return;
  }

  currentFilterRoot.innerHTML = newFilterRoot.innerHTML;
}
...
document.addEventListener('touchend', function (e) {
  ...
  document.addEventListener('change', function (e) {
    ...
    if (target.matches('.wpc-term-item input[data-wpc-link]')) {
      var feUrl = target.getAttribute('data-wpc-link');
      if (feUrl) {
        applyPriceFilterAjax(feUrl);
        return;
      }
    }
