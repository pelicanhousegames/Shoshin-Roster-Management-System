...
/* BEGIN SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT1 LANDSCAPE ROTATION TEST) */
function renderQuickReferenceLayout1HTML(qr) {
  qr = qr || {};

  // NOTE: Rotation test.
  // - We render the SAME portrait QR content as Layout2
  // - Then rotate it -90deg inside the landscape page box
      ...
      '<div class="print-frag">' +
        '<div class="print-frag-inner">' +
          '<div class="print-frag-body qr-body">' +
            ...
            '<div class="qr-rot-frame">' +
              '<div class="qr-rot-content">' +
                renderQuickReferencePortraitCoreHTML(qr) +
              '</div>' +
            '</div>' +
...
/* END SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT1 LANDSCAPE ROTATION TEST) */


/* BEGIN SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT2 PORTRAIT ROTATION TEST CORE) */
function renderQuickReferenceLayout2HTML(qr) {
  qr = qr || {};

  // Portrait QR (canonical). Layout1 will rotate THIS for the rotation test.
  return (
...
// Shared portrait QR core (2 cols, 6 boxes, lorem, random logos)
function renderQuickReferencePortraitCoreHTML(qr){
  qr = qr || {};
  ...
  return (
    '<div class="qr-page">' +
      ...
      // a couple “random” logos (must use .print-logo to survive QR image rules)
      '<img class="print-logo qr-logo qr-logo-a" src="' + logoSrc + '" alt="" aria-hidden="true">' +
      '<img class="print-logo qr-logo qr-logo-b" src="' + logoSrc + '" alt="" aria-hidden="true">' +

      '<div class="qr-title">' + title + '</div>' +

      '<div class="qr-grid">' +
        box('Turn Sequence', '<p>' + lipsum + '</p><ul><li>Step A</li><li>Step B</li><li>Step C</li></ul>') +
        box('Conditions', '<p>' + lipsum + '</p><p><b>Example:</b> ' + lipsum + '</p>') +
        box('Saving Throws', '<p>' + lipsum + '</p><table class="qr-mini"><tr><th>Type</th><th>TN</th></tr><tr><td>Fear</td><td>6+</td></tr><tr><td>Poison</td><td>5+</td></tr></table>') +
        box('Movement Modifiers', '<table class="qr-mini"><tr><th>State</th><th>Mod</th></tr><tr><td>Rough</td><td>-1</td></tr><tr><td>Water</td><td>-2</td></tr><tr><td>Climb</td><td>-1</td></tr></table>') +
        box('Combat Modifiers', '<p>' + lipsum + '</p><p><i>' + lipsum + '</i></p>') +
        box('Extra Box', '<p>' + lipsum + '</p><p>' + lipsum + '</p>') +
      '</div>' +

    '</div>'
  );
}
/* END SHOSHIN PRINT — QUICK REFERENCE PAGE (LAYOUT2 PORTRAIT ROTATION TEST CORE) */
...
/* BEGIN SHOSHIN PRINT — QR ROTATION TEST STYLES */
.print-page[data-print-layout$="qr"] .qr-body{
  padding: 0.35in 0.35in 0.35in !important; /* safe padding for both orientations */
}

/* Portrait core layout */
...
.print-page[data-print-layout$="qr"] .qr-box-b p{ margin: 0 0 6px 0; }
.print-page[data-print-layout$="qr"] .qr-box-b ul{ margin: 0; padding-left: 16px; }
...
.print-page[data-print-layout$="qr"] .qr-logo-a{ top: 0; left: 0; }
.print-page[data-print-layout$="qr"] .qr-logo-b{ top: 0; right: 0; }

/* Layout1 rotation frame:
   - Rotates the portrait content -90deg inside a landscape page box.
   - We size the content to portrait dimensions and then rotate/translate into view.
*/
.print-page[data-print-layout="layout1-qr"] .qr-rot-frame{
  position: relative;
  width: 100%;
  height: 100%;
  overflow: hidden;
...
.print-page[data-print-layout="layout1-qr"] .qr-rot-content{
  position: absolute;
  top: 0;
  left: 0;

  /* Pretend this content is a portrait sheet */
  width: 8.5in;
  height: 11in;
...
/* Layout2: no rotation wrapper needed */
.print-page[data-print-layout="layout2-qr"] .qr-rot-frame,
.print-page[data-print-layout="layout2-qr"] .qr-rot-content{
  display: none;
}
/* END SHOSHIN PRINT — QR ROTATION TEST STYLES */
