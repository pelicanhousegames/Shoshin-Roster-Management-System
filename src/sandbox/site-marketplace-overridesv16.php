...
add_filter( 'woocommerce_widget_cart_item_quantity', function( $html, $cart_item, $cart_item_key ) {
	...
	/* Sold individually products: keep simple text only */
	if ( $product->is_sold_individually() ) {
		return '<div class="shoshin-mini-cart-price-stack">' . $html . '</div>';
	}
	...
	return '<div class="shoshin-mini-cart-price-stack">' . $html . $controls . '</div>';
}, 20, 3 );
...
function shoshin_update_mini_cart_qty() {
	...
	$line_total_html = '';
	if ( ! empty( $item ) ) {
		$line_total_html = WC()->cart->get_product_price( $item['data'] );
		$line_total_html = '<span class="quantity"><span class="product-quantity">' . esc_html( $quantity ) . ' ×</span> ' . $line_total_html . '</span>';
	}
...
add_filter( 'woocommerce_package_rates', function( $rates, $package ) {
	...
	foreach ( $package['contents'] as $item ) {
		...
		// Use variation shipping class if present, otherwise parent/product class.
		$shipping_class = $product->get_shipping_class();

		if ( $shipping_class === 'print-on-demand' ) {
			$has_pod = true;
		} elseif ( in_array( $shipping_class, array( 'immediate', 'delayed-batch' ), true ) ) {
			$has_non_pod = true;
		}
	}
	...
	foreach ( $rates as $rate_id => $rate ) {
		...
		$is_pod_method   = in_array( $label, array( 'Print on Demand Standard', 'Print on Demand Express' ), true );
		$is_usps_method  = ( $label === 'USPS Standard Shipping' );

		// POD-only cart: show only POD methods.
		if ( $has_pod && ! $has_non_pod ) {
			if ( $is_usps_method ) {
				unset( $rates[ $rate_id ] );
			}
		}

		// Non-POD-only cart: show only USPS method.
		if ( $has_non_pod && ! $has_pod ) {
			if ( $is_pod_method ) {
				unset( $rates[ $rate_id ] );
			}
		}
