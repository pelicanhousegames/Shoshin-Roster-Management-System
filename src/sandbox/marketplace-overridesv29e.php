...
if (is_product() && !$is_loop_context && $product->is_type('variable')) {
    ...
    if ($min_regular > 0 && $max_regular > 0 && $min_public > 0 && $max_public > 0 && $min_final > 0 && $max_final > 0) {
        ...
        if ($has_member_adjustment) {
            ...
            /*
             * Included-with-membership takes precedence for unresolved member range
             * when any applicable variation resolves to 0.00.
             *
             * If the aggregated range is $0.00 - $N.00 for the current user,
             * we show the softer variable-product message:
             * "A version of this item is included with your membership."
             */
            if ($has_included) {
                if ((float) $min_final === 0.0 && (float) $max_final > 0.0) {
                    $savings_html = '<div class="shoshin-price-savings">A version of this item is included with your membership</div>';
                } else {
                    $savings_html = '<div class="shoshin-price-savings">This item is included with your membership</div>';
                }
            } else {
                /*
                 * Keep your direct "You save..." wording for the unresolved member block.
                 * Only show it for the pure global-% case; do not force a generic % line
                 * when flat variation overrides are part of the unresolved range.
                 */
                ...
                if (!$has_flat_override && $current_member_percent > 0) {
                    if ($has_public_sale) {
