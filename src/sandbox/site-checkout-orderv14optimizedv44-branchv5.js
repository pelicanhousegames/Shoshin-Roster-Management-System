(function () {
  ...
  var inCartProductIds = {};

  document.querySelectorAll(
    '.elementor-menu-cart__product-name a[href]'
  ).forEach(function (link) {
    var href = link.getAttribute('href') || '';
    if (!href) return;

    document.querySelectorAll(
      'ul.products li.product a[href="' + href + '"], ' +
      '.related ul.products li.product a[href="' + href + '"], ' +
      '.upsells ul.products li.product a[href="' + href + '"], ' +
      '.elementor-widget-woocommerce-products li.product a[href="' + href + '"]'
    ).forEach(function (productLink) {
      var card = productLink.closest('li.product, .product');
      if (!card) return;

      var btn = card.querySelector('a.add_to_cart_button, a.button.product_type_simple');
      if (!btn) return;

      var productId = btn.getAttribute('data-product_id');
      if (productId) {
        inCartProductIds[String(productId)] = true;
      }
  ...
  document.querySelectorAll(
    ...
    '.elementor-widget-woocommerce-products a.add_to_cart_button[data-product_id]'
  ).forEach(function (btn) {
    var productId = btn.getAttribute('data-product_id');
    if (!productId) return;

    if (inCartProductIds[String(productId)]) return;

    if (!btn.classList.contains('shoshin-disabled-add-to-cart')) return;
