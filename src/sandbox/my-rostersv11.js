...
if (qtySel) {
  ...
  function getRosterIcon(r) {
    ...
    // BEGIN RMS SOFT LOCK — Padlock icon path (keep next to fallback paths)
    // Used to override roster + asset images when a record is soft-locked.
    // IMPORTANT: do not move; requested to live next to fallback image path.
    var RMS_LOCK_ICON_URL = '/wp-content/uploads/2026/02/padlock.jpg';
    // END RMS SOFT LOCK — Padlock icon path

    // If caller sets r.locked, override iconUrl here (single place)
    if (r && r.locked) iconUrl = RMS_LOCK_ICON_URL;
...
// If the roster is locked OR the asset itself is locked, show padlock image
if (isLockedRoster || (u && u.locked)) {
  img = RMS_LOCK_ICON_URL;
}
