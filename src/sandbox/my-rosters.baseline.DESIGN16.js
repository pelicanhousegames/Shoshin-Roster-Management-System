...
/* BEGIN SHOSHIN — Layout2 atoms renderer (enables on-screen pagination) */
function renderLayout2Atoms(bucketDefs) {
  ...
  function renderBucketAtom(bucketName, isSupport, zebraStart) {
    ...
    // IMPORTANT: bucket atom is a standalone block so paginator can move it whole
    html += '<div class="print-unit-group l2-bucket-atom">';

    html += '<table class="' + tblClass + '">';

    // Keep your existing colgroup widths (do NOT change)
    html += '<colgroup>' +
  ...
  for (var bi = 0; bi < bucketDefs.length; bi++) {
    ...
    // Divider between buckets only (not after last non-empty)
...
/* END SHOSHIN — Layout2 atoms renderer (enables on-screen pagination) */
