...
document.addEventListener('DOMContentLoaded', function () {
  var scope = document.querySelector('.shoshin-desc-accordion-item');
  if (!scope) return;

  // Prefer a direct rich-content area inside the description panel
  var content =
    scope.querySelector('.elementor-tab-content .elementor-widget-theme-post-content') ||
    scope.querySelector('.elementor-tab-content .elementor-widget-container') ||
    scope.querySelector('.elementor-tab-content') ||
    scope.querySelector('.elementor-widget-theme-post-content') ||
    null;

  if (!content) return;

  // Prevent duplicate init
  if (scope.querySelector('.shoshin-desc-readmore-btn')) return;
  ...
  // Only add button if content actually exceeds cap
  var BUTTON_ROW = document.createElement('div');
  BUTTON_ROW.className = 'shoshin-desc-readmore-wrap';
  ...
  function refreshVisibility() {
    // small timeout helps after accordion/render layout settles
