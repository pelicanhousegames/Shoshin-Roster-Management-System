(function () {
  ...
  function buildClassicCheckoutShell() {
    ...
    /* EXPRESS */
    if (expressPrimary || expressWcPay || expressPayPal || expressRecaptcha || stripeExpressSeparator) {
    ...
    expressCard.querySelectorAll('.ppc-button-wrapper > [id^="ppc-button-"]').forEach(function (gateway) {
    ...
    rows.forEach(function (row) {
      ...
      nameClone.querySelectorAll('strong.product-quantity, .product-quantity').forEach(function (el) {
        el.remove();
      });
      ...
      itemRow.innerHTML =
        '<div class="shoshin-checkout-review-row__name">' + nameHtml + '</div>' +
        '<div class="shoshin-checkout-review-row__total">' + totalHtml + '</div>';
  ...
  function relocateClassicCheckoutCoupon() {
    ...
    function syncButtonState() {
      ...
      var isLoading = panel.dataset.shoshinCouponLoading === '1';

      uiButton.disabled = !hasValue || isLoading;
      uiButton.setAttribute('aria-disabled', (!hasValue || isLoading) ? 'true' : 'false');
    }
...
function renderCouponNoticesFromDoc(doc) {
  ...
  return false;
    ...
    async function applyCheckoutCouponAjax() {
      if (!couponForm || !nativeInput || !uiInput) return;
