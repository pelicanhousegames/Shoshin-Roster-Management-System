document.addEventListener('DOMContentLoaded', function () {
  ...
  function supportBucketFromUnit(u) {
    ...
    return '';
        ...
        for (var i = 0; i < assignedArr.length; i++) {
          ...
          var clsKey = cls.toLowerCase().replace(/\s+/g, ' ').trim();
          var CLS_MAP = {
            'daimyo': 'Daimyo',
            'samurai': 'Samurai',
            'ashigaru': 'Ashigaru',
          ...
          var clsCanon = CLS_MAP[clsKey] || cls;
  ...
  function closeDeleteModal() {
    ...
    function ensureBannerModal() {
      ...
      // Phase 1 baseline: Update does nothing yet (we’ll wire it in Phase 2)
      var updateBtn = bannerModal.querySelector('.shoshin-banner-update-btn');
      if (updateBtn) {
        updateBtn.addEventListener('click', function () {
          // baseline: no server wiring yet
          // keep this as a clear placeholder so we do not “half wire” anything.
          alert('Banner update wiring is next (Phase 2). Modal layout + uploader mount baseline is complete.');
        });
      }
    ...
    function getRosterBannerUrlFromRosterObj(r) {
      // field_8 is the banner upload field for roster entry
      // In your roster renderer you also have getRosterIcon() that falls back,
      // but we want the RAW banner url if present, otherwise empty.
      var v =
    ...
    function restoreBannerField8ToHost() {
      if (!bannerField8Container) return;

      try {
        // remove from modal slot
        if (bannerField8Container.parentNode) {
          bannerField8Container.parentNode.removeChild(bannerField8Container);
        }
      } catch (_) {}

      // put it back exactly where it was
      if (bannerField8OriginalParent) {
        try {
          if (bannerField8OriginalNextSibling && bannerField8OriginalNextSibling.parentNode === bannerField8OriginalParent) {
            bannerField8OriginalParent.insertBefore(bannerField8Container, bannerField8OriginalNextSibling);
          } else {
            bannerField8OriginalParent.appendChild(bannerField8Container);
          }
        } catch (_) {}
      }

      bannerField8Container = null;
      bannerField8OriginalParent = null;
      bannerField8OriginalNextSibling = null;
    }

    function mountField8IntoBannerModal() {
      ...
      // WPForms container for field_8
      var field8 = document.querySelector('#wpforms-2799-field_8-container');
      if (!field8) {
        slot.innerHTML =
          '<div class="shoshin-muted" style="text-align:center;">' +
            'Banner uploader is not available on this page (missing WPForms Form 2799 Field #8).' +
          '</div>';
        return;
      }

      // Save restore pointers (first time we mount)
      bannerField8Container = field8;
      bannerField8OriginalParent = field8.parentNode;
      bannerField8OriginalNextSibling = field8.nextSibling;

      // Move into modal
      slot.appendChild(field8);
    }
    ...
    function syncBannerModalFromRoster(rosterObj) {
      ...
      // Phase 1 baseline preview:
      // - If banner exists, show it.
      // - If not, show fallback.
      if (imgEl) imgEl.src = bannerUrl || fallbackUrl;

      // Toggle lock state:
      // - Locked/disabled if no existing banner URL
      var hasExisting = !!bannerUrl;
      if (removeToggle) {
        removeToggle.checked = false; // default off when opening
    ...
    function openBannerModalForRosterEntryId(rosterEntryId, focusReturnEl) {
      ...
      // Find roster object from in-memory "rosters" array
      var rObj = getRosterObjByEntryId(rosterEntryId);
      bannerCurrentRoster = rObj || null;

      // 1) Sync UI from roster
      syncBannerModalFromRoster(bannerCurrentRoster);

      // 2) Mount the uploader field into row3 (if present)
      mountField8IntoBannerModal();
    ...
    function closeBannerModal() {
      ...
      // Restore field_8 back to hidden host so page remains stable
      restoreBannerField8ToHost();
