...
function ensureAssignUnitsModal() {
  ...
  assignUnitsModalBackdrop.className = 'shoshin-modal-backdrop';
  ...
  '<div class="shoshin-modal-body">' +
    '<div id="shoshin-assign-units-title" class="shoshin-modal-title">Assign Units</div>' +

    // body wrap expected by CSS
    '<div class="shoshin-modal-body-wrap">' +

      // Block 1 (Roster strip host)
      '<div class="shoshin-assign-units-roster"></div>' +

      // Daimyo restriction message (between blocks 1 and 2)
      '<div class="shoshin-assign-units-restriction"></div>' +

      // Block 2 controls (filters + list)
      '<div class="shoshin-assign-units-controls">' +
        ...
        '<div class="shoshin-assign-units-filters">' +
          ...
          '<button type="button" class="shoshin-asset-filter-btn" data-filter="Support Assets">Support Assets</button>' +
        '</div>' +
  ...
  document.body.appendChild(assignUnitsModalBackdrop);
  document.body.appendChild(assignUnitsModal);

  function close() { closeAssignUnitsModal(); }

  assignUnitsModalBackdrop.addEventListener('click', close);

  var xBtn = assignUnitsModal.querySelector('.shoshin-modal-x');
  var cancelBtn = assignUnitsModal.querySelector('.shoshin-modal-btn-cancel');
  if (xBtn) xBtn.addEventListener('click', close);
  if (cancelBtn) cancelBtn.addEventListener('click', close);

  // Filter clicks (empty-state swaps per filter)
  var filtersWrap = assignUnitsModal.querySelector('.shoshin-assign-units-filters');
  if (filtersWrap) {
    filtersWrap.addEventListener('click', function (e) {
      var b = e && e.target && e.target.closest ? e.target.closest('button[data-filter]') : null;
      if (!b) return;

      var allBtns = filtersWrap.querySelectorAll('button[data-filter]');
      Array.prototype.forEach.call(allBtns, function (x) { x.classList.remove('is-active'); });
      b.classList.add('is-active');

      if (assignUnitsCtx) assignUnitsCtx.filter = String(b.getAttribute('data-filter') || 'all');
      renderAssignUnitsList();
recomputeAssignUnitsPreview();
...
if (assignBtn && !assignUnitsModal.__assignUnitsConfirmBound) {
  assignUnitsModal.__assignUnitsConfirmBound = true;
...
function assignUnitsGetPoints(asset) {
  // prefer points then cost (support assets often use cost)
  var p = asset && (asset.points != null ? asset.points : asset.cost);
  return asInt(p, 0);
}

function assignUnitsGetInitiative(asset) {
  var v = (asset && (asset.initiative != null ? asset.initiative : (asset.stats ? asset.stats.initiative : null)));
  return asInt(v, 0);
}
...
function assignUnitsGetLeadership(asset) {
  var v = (asset && (
    ...
    asset.LDR != null ? asset.LDR :
    (asset.stats ? (asset.stats.leadership != null ? asset.stats.leadership : asset.stats.ldr) : null)
  ));
  return asInt(v, 0);
}
...
function assignUnitsGetDisplayType(asset) {
  ...
  return String(asset.cls || asset.class || '').trim() || '—';
}
...
function assignUnitsMatchesFilter(asset, filterLabel) {
  ...
  var cls = String(asset.cls || asset.class || '').trim();
...
function assignUnitsRenderRestriction(rosterObj) {
  ...
  var msg = assignUnitsModal.querySelector('.shoshin-assign-units-daimyo-msg');
  if (!msg) return;

  var blocked = rosterHasAssignedDaimyo(rosterObj);

  if (blocked) {
    msg.innerHTML = '<b>Restriction:</b> Only one Daimyo is allowed to be assigned to a clan.';
  ...
  if (assignUnitsCtx && assignUnitsCtx.selected) {
    Object.keys(assignUnitsCtx.selected).forEach(function (k) {
      ...
      if (!map[k]) {
        ...
        var clsKey = String(map[k].cls || map[k].class || '').toLowerCase().replace(/\s+/g, ' ').trim();
...
function assignUnitsRenderRosterStrip(rosterObj) {
  if (!assignUnitsModal) return;
  var host = assignUnitsModal.querySelector('.shoshin-assign-units-block1');
  if (!host) return;
  ...
  var previewAssigned = assignUnitsBuildPreviewAssignedList(rosterObj);
  var totals = computeRosterTotalsFromAssigned(previewAssigned);
  host.innerHTML = renderUnassignPreviewRow(rosterObj, totals);
}

function assignUnitsRenderList() {
  ...
  var filterLabel = String(assignUnitsCtx.filter || 'all');
  var daimyoBlocked = rosterHasAssignedDaimyo(rosterObj);
  var currentQtyMap = buildRosterAssignedQtyMapForRoster(rosterObj);

  // filter/sort
  var items = ownedAssets.slice().filter(function (a) { return assignUnitsMatchesFilter(a, filterLabel); });
  items.sort(function (a, b) {
    var ak = classOrderKey(a.kind, a.cls || a.class || a.supportType || a.name || a.title);
    var bk = classOrderKey(b.kind, b.cls || b.class || b.supportType || b.name || b.title);
  ...
  for (var i = 0; i < items.length; i++) {
    var a = items[i] || {};
    var unitKey = String(a.unitKey || '').trim();
    if (!unitKey) unitKey = makeUnitKey(a);

    var img = String(a.img || a.image || a.imgUrl || '').trim();
    if (!img) img = '/wp-content/uploads/2025/12/Helmet-grey.jpg';

    var typeText = assignUnitsGetDisplayType(a);
    var refText = String(a.refId || a.ref_id || '').trim() || '—';

    var curQty = asInt(currentQtyMap[unitKey], 0);

    var pts = assignUnitsGetPoints(a);
    var ini = assignUnitsGetInitiative(a);
    var ldr = assignUnitsGetLeadership(a);

    var clsKey = String(a.cls || a.class || '').toLowerCase().replace(/\s+/g, ' ').trim();
    var isDaimyoRow = (clsKey === 'daimyo');
    var isDisabled = !!(isDaimyoRow && daimyoBlocked);

    var addQty = (assignUnitsCtx.selected && assignUnitsCtx.selected[unitKey] != null) ? asInt(assignUnitsCtx.selected[unitKey], 0) : 0;
    var isOn = (!isDisabled && addQty > 0);

    if (isOn && addQty < 1) addQty = 1;
    if (!isOn) addQty = 1;

    var totalCost = pts * (isOn ? asInt((assignUnitsCtx.selected || {})[unitKey], 0) : 0);

    // 1–99
    var opts = '';
    for (var q = 1; q <= 99; q++) {
      var sel = (q === addQty) ? ' selected' : '';
      opts += '<option value="' + q + '"' + sel + '>' + q + '</option>';
    }

    html +=
      '<div class="shoshin-assign-units-row' + (isDisabled ? ' is-disabled' : '') + '" data-unit-key="' + esc(unitKey) + '">' +

        '<div class="shoshin-assign-units-toggle">' +
          '<label class="shoshin-ios-toggle">' +
            '<input type="checkbox" class="shoshin-assign-units-toggle-input"' + (isOn ? ' checked' : '') + (isDisabled ? ' disabled' : '') + ' />' +
            '<span class="track"><span class="thumb"></span></span>' +
          '</label>' +
        '</div>' +

        '<img class="shoshin-assign-units-img" src="' + esc(img) + '" alt="">' +

        '<div class="shoshin-assign-units-info">' +
          '<div class="shoshin-assign-units-type">' + esc(typeText) + '</div>' +
          '<div class="shoshin-assign-units-ref">' + esc(refText) + '</div>' +
        '</div>' +

        '<div class="shoshin-assign-units-divider"></div>' +

        '<div class="shoshin-assign-units-current">' +
          '<span class="label">Current</span>' +
          '<span class="value">' + esc(String(curQty)) + '</span>' +
        '</div>' +

        '<div class="shoshin-assign-units-divider"></div>' +

        '<div class="shoshin-assign-units-qty">' +
          '<span class="label">Add QTY</span>' +
          '<select class="shoshin-assign-units-qty-select"' + ((isOn && !isDisabled) ? '' : ' disabled') + '>' +
            opts +
          '</select>' +
        '</div>' +

        '<div class="shoshin-assign-units-metric">' +
          '<span class="label">Total</span>' +
          '<span class="value shoshin-assign-units-total">' + esc(String(totalCost)) + '</span>' +
        '</div>' +

        '<div class="shoshin-assign-units-metric">' +
          '<span class="label">Initiative</span>' +
          '<span class="value">' + esc(String(ini)) + '</span>' +
        '</div>' +

        '<div class="shoshin-assign-units-metric">' +
          '<span class="label">Leadership</span>' +
          '<span class="value">' + esc(String(ldr)) + '</span>' +
        '</div>' +
...
function assignUnitsUpdateRowFromUi(rowEl) {
  ...
  if (qtySel) qtySel.disabled = !isOn;
  if (totalEl) totalEl.textContent = String(assignUnitsGetPoints(a) * addQty);
...
function assignUnitsBuildRow3BodyHtmlForRoster(rosterObj, clanPoints) {
  ...
  for (var i = 0; i < grouped.length; i++) {
    html += renderAssignedStripRow(grouped[i], asInt(rosterObj.entryId || rosterObj.id || rosterObj.entry_id, 0), clanPoints);
  }
...
function assignUnitsBuildSubmitUnits(rosterObj) {
  ...
  Object.keys(assignUnitsCtx.selected).forEach(function (unitKey) {
    ...
    // Daimyo hard cap at 1
    var clsKey = String(asset.cls || asset.class || '').toLowerCase().replace(/\s+/g, ' ').trim();
    if (clsKey === 'daimyo') finalQty = 1;

    if (finalQty < 1) finalQty = 1;
    if (finalQty > 99) finalQty = 99;

    // IMPORTANT: unit must be the canonical unit object (without qty)
    var unit = Object.assign({}, asset);
    delete unit.qty;

    // Ensure unitKey present
    unit.unitKey = String(unit.unitKey || '').trim() || makeUnitKey(unit);
...
function assignUnitsSubmit() {
  ...
  if (!units.length) {
    assignUnitsSetError('Select at least one unit to assign.');
    return;
  }
...
function getAssignUnitsDisplayType(u) {
  ...
  // Support assets show name/title (Ozutsu, Mokuzo Hansen)
  if (kind === 'support') {
    return String(u.name || u.title || u.supportType || u.cls || u.class || '').trim() || '—';
  }

  // Characters show class (Daimyo/Samurai/etc.)
  return String(u.cls || u.class || u.supportType || '').trim() || '—';
}
...
function assetMatchesAssignUnitsFilter(u, filterLabel) {
  ...
  return (String(cls || '').trim() === filterLabel);
}

function buildRosterAssignedQtyMap(rosterObj) {
  // Map unitKey -> currentQty (grouped)
  var map = {};
  try {
    var grouped = groupAssigned(parseAssigned(rosterObj || {}));
    for (var i = 0; i < grouped.length; i++) {
...
function renderAssignUnitsRosterStrip() {
  if (!assignUnitsModal || !assignUnitsCtx) return;
  ...
  // Use the same renderer as the delete roster modal: renderUnassignPreviewRow(rosterObj, totals)
  var previewAssigned = buildPreviewAssignedList(rosterObj);
  var totals = computeRosterTotalsFromAssigned(previewAssigned);

  host.innerHTML = renderUnassignPreviewRow(rosterObj, totals);
}

function renderAssignUnitsRestrictionNote() {
  if (!assignUnitsModal || !assignUnitsCtx) return;
  ...
  var hasDaimyo = rosterHasDaimyoAssigned(rosterObj);

  // Always show the message
  el.innerHTML = '<b>Restriction:</b> Only one Daimyo is allowed to be assigned to a clan.';
...
function buildAssignUnitsRowHtml(u, rosterObj, currentQtyMap, rosterHasDaimyo, selectedDaimyoKey) {
  ...
  var isDaimyoRow = (String(u.cls || u.class || '').toLowerCase().replace(/\s+/g, ' ').trim() === 'daimyo');
  ...
  for (var q = 1; q <= 99; q++) {
    ...
    var sel = (q === selectedAdd) ? ' selected' : '';
    opts += '<option value="' + q + '"' + sel + '>' + q + '</option>';
  }

  return (
    '<div class="shoshin-assign-units-row' + (isDisabled ? ' is-disabled' : '') + '" data-unit-key="' + esc(unitKey) + '" data-filter-kind="' + esc(normalizeKind(u.kind)) + '" data-filter-cls="' + esc(String(u.cls || u.class || '').trim()) + '">' +
      ...
      '<div class="shoshin-assign-units-cur">' +
        '<div class="shoshin-assign-units-label">Current</div>' +
        '<div class="shoshin-assign-units-value">' + esc(String(curQty)) + '</div>' +
      '</div>' +
      ...
      '<div class="shoshin-assign-units-totalwrap">' +
        ...
        '<div class="shoshin-assign-units-total">' + esc(String(points * (checked ? selectedAdd : 0))) + '</div>' +
      '</div>' +

      '<div class="shoshin-assign-units-ini">' + esc(String(ini)) + '</div>' +
      '<div class="shoshin-assign-units-ldr">' + esc(String(ldr)) + '</div>' +
    '</div>'
  );
}
...
function renderAssignUnitsList() {
  ...
  // Filter + sort
  var items = ownedAssets.slice().filter(function (u) { return assetMatchesAssignUnitsFilter(u, filterLabel); });
...
function recomputeAssignUnitsPreview() {
  renderAssignUnitsRestrictionNote();
  renderAssignUnitsRosterStrip();
  // Row totals update is already reflected via roster strip totals renderer.
  // Per-row totals are updated on events (toggle/qty change).
}

function updateAssignUnitsRowTotals(rowEl) {
  if (!rowEl) return;

  var unitKey = String(rowEl.getAttribute('data-unit-key') || '').trim();
  if (!unitKey) return;

  // Find asset to compute points + class (for Daimyo enforcement)
  var asset = null;
  for (var i = 0; i < ownedAssets.length; i++) {
    var a = ownedAssets[i] || {};
    var k = String(a.unitKey || '').trim();
    if (!k) k = makeUnitKey(a);
    if (k === unitKey) { asset = a; break; }
  }
  if (!asset) return;

  var points = getAssetPoints(asset);
  ...
  // Daimyo: hard cap at 1
  var clsKey = String(asset.cls || asset.class || '').toLowerCase().replace(/\s+/g, ' ').trim();
  var isDaimyo = (clsKey === 'daimyo');

  var addQty = 0;
  if (isOn) {
    addQty = isDaimyo ? 1 : asInt(qtySel && qtySel.value, 1);
    if (addQty < 1) addQty = 1;
    if (addQty > 99) addQty = 99;
  }

  if (qtySel) {
    qtySel.disabled = !isOn || isDaimyo;
    if (isDaimyo) qtySel.value = '1';
  }

  // Enforce: only one Daimyo selection (when roster does NOT already have one)
  if (isOn && isDaimyo && assignUnitsCtx) {
    var rosterObj = getRosterObjByEntryId(assignUnitsCtx.rosterEntryId);
    var rosterHasDaimyo = rosterHasDaimyoAssigned(rosterObj);

    if (!rosterHasDaimyo && assignUnitsCtx.selected) {
      Object.keys(assignUnitsCtx.selected).forEach(function (k) {
        if (k === unitKey) return;
        ...
        var a2 = assignUnitsGetOwnedAssetByUnitKey(k);
        if (!a2) return;

        var ck = String(a2.cls || a2.class || '').toLowerCase().replace(/\s+/g, ' ').trim();
        if (ck === 'daimyo') {
          // clear any other selected Daimyo
          setAssignUnitsSelectedAddQty(k, 0);
        }
      });
    }
  }
  ...
  if (totalEl) totalEl.textContent = String(points * addQty);

  // Re-render list so Daimyo disabling updates immediately (no multiple Daimyo selections)
  renderAssignUnitsList();
...
function bindAssignUnitsListEvents() {
  if (!assignUnitsModal) return;
  ...
  list.addEventListener('change', function (e) {
    ...
    if (t.classList && (t.classList.contains('shoshin-assign-units-toggle-input') || t.classList.contains('shoshin-assign-units-qty-select'))) {
      updateAssignUnitsRowTotals(row);
    }
  });
}
