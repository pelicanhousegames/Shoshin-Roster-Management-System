...
/* BEGIN SHOSHIN TASK 2 — DIVIDER PACK RULES (NEVER FIRST/LAST) */
function shoshinIsDividerPack(el){
  try { return !!(el && el.classList && el.classList.contains('print-pack-divider')); }
...
for (var i = 0; i < packs.length; i++){
  ...
  // Divider packs are "between" markers: they must sit between two unit packs in the SAME fragment.
  if (shoshinIsDividerPack(pack)) {

    // Rule: divider cannot be first in a fragment
    var fragEmpty = !(currentBody.children && currentBody.children.length > 0);
    if (fragEmpty) continue;

    // If no next pack, drop divider (can't be last either)
    var next = packs[i + 1] || null;
    if (!next || shoshinIsDividerPack(next)) continue;

    // Place divider, then immediately place its paired next pack.
    var divGroup = appendPackToBody(currentBody, pack);

    // If divider itself caused overflow (unlikely), remove and drop it.
    if (bodyOverflows(currentBody)) {
      try { currentBody.removeChild(divGroup); } catch(_){}
      continue;
    }

    // Now place the next pack
    var nextGroup = appendPackToBody(currentBody, next);

    if (bodyOverflows(currentBody)) {
      // Can't keep divider as last; remove both, move next to new frag WITHOUT divider.
      try { currentBody.removeChild(nextGroup); } catch(_){}
      try { currentBody.removeChild(divGroup); } catch(_){}
      currentBody = startFrag(false);
      appendPackToBody(currentBody, next);
    }

    // Consume the next pack since we handled it here.
    i++;
    continue;
  }
...
/* END SHOSHIN TASK 2 — DIVIDER PACK RULES (NEVER FIRST/LAST) */
...
/* BEGIN SHOSHIN TASK 2 — DIVIDER AS OWN PRINT PACK (BETWEEN UNIT PACKS) */
var bucketThumb = pickBucketThumbFromRows(rows);
var hasThumbHeader = !!(ctx.includeImages && bucketThumb);

// Divider HTML (row only; becomes its own .print-pack)
var dividerRowHtml = hasThumbHeader
  ? (
...
for (var r = 0; r < rows.length; r++) {
  ...
  // Insert divider BETWEEN packs (never before the very first rendered unit)
  // Note: paginator enforces "divider cannot be first/last on a fragment";
  // renderer guarantees it is never first in the whole document.
  if (hasThumbHeader && renderedRows > 0) {
    body += '<div class="print-pack print-pack-divider" aria-hidden="true">' + dividerRowHtml + '</div>';
  }

  body += '<div class="print-pack">';

  // Thumb + bucket title row (INSIDE unit pack)
  body +=
  ...
  body += '</div>'; // .print-pack (unit)
  renderedRows++;
}
/* END SHOSHIN TASK 2 — DIVIDER AS OWN PRINT PACK (BETWEEN UNIT PACKS) */
