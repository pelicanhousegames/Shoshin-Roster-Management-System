(function () {
  function initShoshinCappedGallery() {
    ...
    // Determine actual gallery state using both slides and thumbs.
    var thumbItems = thumbs ? thumbs.querySelectorAll('li') : [];
    var isSingleImageGallery = (!thumbs || images.length <= 1 || thumbItems.length <= 1);

    if (gallery.classList) {
      gallery.classList.toggle('shoshin-single-image-gallery', isSingleImageGallery);
    }
