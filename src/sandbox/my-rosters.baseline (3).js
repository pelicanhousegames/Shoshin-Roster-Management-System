...
/* BEGIN SHOSHIN PRINT — AVAIL HEIGHT FROM BODY RECT (MORE TRUSTWORTHY) */
'function getAvailPx(templateFrag, pageIndex){' +
  'var bodyEl = q(".print-frag-body", templateFrag);' +
  'if(!bodyEl) return 0;' +

  // Prefer the actual container height we are packing into
  'var bodyRect = rectH(bodyEl);' +
  'if(!bodyRect){ bodyRect = cssNum(bodyEl,"height"); }' +

  // If header exists and is OUTSIDE the body, bodyRect already excludes it.
  // If header is INSIDE body (unlikely), subtract it only in that case.
  'var header = q(".print-header", templateFrag);' +
  'var headerH = 0;' +
  ...
  // No need to subtract padding; bodyRect already reflects the usable box height for children packing.
  'return Math.max(0, bodyRect - headerH);' +
'}' +
/* END SHOSHIN PRINT — AVAIL HEIGHT FROM BODY RECT (MORE TRUSTWORTHY) */
