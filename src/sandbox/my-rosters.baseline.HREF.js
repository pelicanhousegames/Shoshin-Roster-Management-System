...
function openPrintTabDocumentBaseline(cfg){
  ...
  var faviconHref = '';
/* BEGIN SHOSHIN PRINT — FAVICON NORMALIZE ABSOLUTE */
try {
  if (faviconHref) faviconHref = String(new URL(String(faviconHref), window.location.href));
} catch(e) {}
/* END SHOSHIN PRINT — FAVICON NORMALIZE ABSOLUTE */
  ...
  /* BEGIN SHOSHIN PRINT BASELINE — FAVICON ABSOLUTE */
  try {
    if (faviconHref) faviconHref = String(new URL(faviconHref, window.location.href));
  } catch(e) {}
  /* END SHOSHIN PRINT BASELINE — FAVICON ABSOLUTE */

/* BEGIN SHOSHIN PRINT — FAVICON TAG (ROBUST) */
var faviconTag = faviconHref
  ...
  : '';
/* END SHOSHIN PRINT — FAVICON TAG (ROBUST) */
...
try {
  if (faviconHref && w && w.document && w.document.head) {

    // Remove any existing favicon links
    try {
      var _oldIcons = w.document.head.querySelectorAll('link[rel*="icon"]');
      if (_oldIcons && _oldIcons.forEach) {
        _oldIcons.forEach(function(n){
          try { n.parentNode.removeChild(n); } catch(e) {}
        });
      }
    } catch(e) {}

    // Apply favicon deterministically
    var _icon1 = w.document.createElement('link');
    _icon1.setAttribute('rel', 'icon');
    _icon1.setAttribute('href', faviconHref);
    w.document.head.appendChild(_icon1);

    var _icon2 = w.document.createElement('link');
    _icon2.setAttribute('rel', 'shortcut icon');
    _icon2.setAttribute('href', faviconHref);
    w.document.head.appendChild(_icon2);
  }
} catch(e) {}
/* END SHOSHIN PRINT BASELINE — FAVICON REAPPLY (POST-WRITE) */
  ...
  /* BEGIN SHOSHIN PRINT — FAVICON REAPPLY (POST-WRITE) */
try {
  if (faviconHref && w && w.document && w.document.head) {

    // Remove any existing favicon links (prevents duplicates)
    try {
      var _oldIcons = w.document.head.querySelectorAll('link[rel*="icon"]');
      if (_oldIcons && _oldIcons.forEach) {
        _oldIcons.forEach(function(n){
    ...
    // Re-apply favicon deterministically
    var _icon1 = w.document.createElement('link');
    _icon1.setAttribute('rel', 'icon');
    _icon1.setAttribute('href', faviconHref);
    w.document.head.appendChild(_icon1);
...
/* END SHOSHIN PRINT — FAVICON REAPPLY (POST-WRITE) */
...
/* END SHOSHIN PRINT — FAVICON NORMALIZE ABSOLUTE */
  ...
  /* BEGIN SHOSHIN PRINT — FAVICON ABSOLUTE + REAPPLY */
  // Make faviconHref absolute (about:blank cannot resolve relative hrefs)
  try {
    if (faviconHref) faviconHref = String(new URL(faviconHref, window.location.href));
  } catch(e) {}
...
/* END SHOSHIN PRINT — FAVICON TAG (ROBUST) */
  ...
  /* END SHOSHIN PRINT — FAVICON ABSOLUTE + REAPPLY */
...
try{
  // Visual-only: do NOT insert nodes, do NOT affect pagination decisions.
  // Rule: ignore justification when a page frag has 2 or fewer units.
  // Units are counted as:
  //  - preferred: .print-unit-group (baseline test HTML)
  //  - fallback:  .print-unit-row   (live data layout)
  var fragsJ = document.querySelectorAll('.print-frag');
  for (var fj = 0; fj < fragsJ.length; fj++){
    var fragJ = fragsJ[fj];
    var bodyJ = fragJ ? fragJ.querySelector('.print-frag-body') : null;
    if (!bodyJ) continue;

    // Count units (prefer unit-group; fallback to unit-row)
    ...
    // Skip if content already overflows (never worsen overflow)
    if (bodyJ.scrollHeight > bodyJ.clientHeight + 0.5){
      bodyJ.style.justifyContent = 'flex-start';
      bodyJ.style.rowGap = '';
      continue;
    }

    // Apply visual-only justification
    // Remove explicit gap so justify-content controls spacing deterministically.
...
'try{' +
  ...
  'for(var pi=0; pi<frags.length; pi++){' +
    ...
    // count unit groups (eligibility rule)
    'var unitCount = 0;' +
    'try{' +
      'var ug = body.querySelectorAll(".print-unit-group");' +
...
try {
  if (faviconHref && w && w.document && w.document.head) {
    // remove any existing icon links first (prevents duplicates)
    try {
      var old = w.document.head.querySelectorAll('link[rel~="icon"]');
      old && old.forEach && old.forEach(function(n){ try{ n.parentNode.removeChild(n); }catch(e){} });
    } catch(e) {}
