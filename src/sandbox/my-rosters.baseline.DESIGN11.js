...
// Add Quick Reference? (default OFF)
  '<div class="shoshin-print-option">' +
    ...
    '<div class="shoshin-print-option-text">' +
      ...
      '<div class="shoshin-print-option-sub">Appends 1-page quick reference (Layout-specific)</div>' +
    '</div>' +
  '</div>' +
...
function getIncludeImages() {
  ...
  return !!(tImg && tImg.checked);
    ...
    function getIncludeQuickRef(){
      var el = document.getElementById('shoshin_print_toggle_qr');
...
/* PAGE SHELL (inches; print uses SAFE canvas to survive Chrome "Minimum" margins) */
:root{
  --paper-w: 11in;
  --paper-h: 8.5in;

  /* SAFE canvas (historically what kept 4+3 stable under "Minimum") */
  --safe-w: 10.50in;
  --safe-h: 8.00in;

  /* PRINT ONLY: tiny scale to survive Chrome "Minimum" margin reflow */
  --print-zoom: 0.96 !important;

}

.print-pages-wrap{ width: var(--paper-w); margin: 0 auto; }

/* Default (screen + general) */
.print-frag{
  width: var(--paper-w);
  height: var(--paper-h);
...
/* PRINT ONLY: lock to SAFE rectangle and center it on the paper */
@media print{
  /* IMPORTANT:
     Do NOT lock html/body to a single-page height.
     That breaks multi-page print flow and causes "Minimum" margin chaos. */
  html, body{ width: auto !important; height: auto !important; }

  /* zoom is OK (unlike transform) because it participates in layout */
  body{ zoom: var(--print-zoom); }

  /* Keep print layout deterministic */
  .print-pages-wrap{ width: auto !important; margin: 0 !important; }

  /* SAFE canvas: anchor top-left (avoid centering side-effects under margin modes) */
  .print-frag{
    width: var(--safe-w) !important;
    height: var(--safe-h) !important;
    margin: 0 !important;
...
/* BEGIN SHOSHIN PRINT — BASELINE CSS OWNER (SAFE CANVAS) */
var css =
  (typeof getPrintBaselineCSS === "function" ? getPrintBaselineCSS() : getPrintBaseCSS()) +
  "\n" +
  getPrintLayout1CSS();
/* END SHOSHIN PRINT — BASELINE CSS OWNER (SAFE CANVAS) */
...
function renderPrintLayout2HTML(rosterObj, mode, includeImages) {
  // Stub output so the function exists when we start Layout 2 work.
  return (
    '<div class="print-page">' +
      '<div class="print-frag">' +
        '<div class="print-frag-inner">' +
          '<div class="print-frag-body" style="display:flex;align-items:center;justify-content:center;height:100%;">' +
            '<div style="font-family:Arial,sans-serif;font-size:16px;">Layout #2 is not implemented yet.</div>' +
          '</div>' +
        '</div>' +
      '</div>' +
    '</div>'
...
function openPrintTabLayout2(rosterObj, mode, includeImages) {
  // Stub only — not currently callable from the modal (modal blocks layout2).
  var html = renderPrintLayout2HTML(rosterObj, mode, includeImages);
  ...
  w.document.write(
    ...
    '<style>' + getPrintBaseCSS() + '</style>' +
    '<style>' + getPrintLayout2CSS() + '</style>' +
    '<style>' + EXTRA_PRINT_CSS + '</style>' +
...
function renderPrintLayout1HTML(ctx, opts) {
...
:root{
  /* =========================================================
     SAFE PRINT CANVAS (critical for Chrome "Minimum" margins)
     (Defined here so Layout1 works even if getPrintBaselineCSS() is OFF)
  ========================================================= */
  --safe-w: 10.50in;
  --safe-h: 8.00in;
...
@media print{
  :root{
   ...
   /* BEGIN SHOSHIN — PRINT HEIGHT LOCK (ENABLE TRUE JUSTIFICATION) */
  /* IMPORTANT:
     Layout1 must NOT set .print-frag width/height/margins in print.
     Baseline (getPrintBaselineCSS) owns the page shell + SAFE canvas sizing. */
  .print-page.print-layout-1 .print-frag{
    ...
    /* ok to enforce overflow (prevents bleed), but do not size the page here */
    overflow: hidden !important;
  }

  /* ensure inner/body are a true flex column that fills the page */
  .print-frag-inner{
    height: 100%;
    min-height: 0;
    display: flex;
