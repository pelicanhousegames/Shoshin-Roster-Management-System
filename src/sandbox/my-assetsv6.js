...
'Sailing Ships': { size: 'Medium', displayName: 'Wasen', description: '<em>Wooden Sailing Ship</em>' }
  ...
  function initMyAssetsRenderer() {
    ...
    const wrapperEl = listEl.closest('.shoshin-asset-list-wrapper') || listEl.parentElement;
        ...
        /* BEGIN RMS SOFT LOCK — LOCK IMAGE RESOLVER (My Assets) */
    // Provide lock image via either:
    // 1) window.ShoshinRmsLockImgUrl (global), OR
    // 2) data-shoshin-lock-img attribute on the listEl
    // Falls back to FALLBACK_IMAGE if not provided.
    const LOCK_IMAGE = (function () {
      try {
        const g = (window && window.ShoshinRmsLockImgUrl) ? String(window.ShoshinRmsLockImgUrl) : '';
        if (g) return g;
      } catch (e) {}
      try {
        const a = listEl.getAttribute('data-shoshin-lock-img') || '';
        if (a) return String(a);
      } catch (e2) {}
      return FALLBACK_IMAGE;
    })();
    /* END RMS SOFT LOCK — LOCK IMAGE RESOLVER (My Assets) */
    ...
    function ensureRmsLockedModal() {
      ...
      const okBtn = rmsLockedModal.querySelector('.shoshin-modal-btn-confirm');
    ...
    function openRmsLockedModal(msg) {
      ...
      const msgEl = rmsLockedModal ? rmsLockedModal.querySelector('.shoshin-modal-msg') : null;
