...
/* ------------------------------------------------------------------------- */
/* DB-backed WPForms reads (bypass WPForms capability gating for non-admins)  */
/* ------------------------------------------------------------------------- */
if ( ! function_exists( 'shoshin_wpforms_db_get_entries_by_user' ) ) {
	function shoshin_wpforms_db_get_entries_by_user( $form_id, $user_id, $limit = 999 ) {
		global $wpdb;
...
if ( ! function_exists( 'shoshin_build_character_asset' ) ) {
	...
	function shoshin_build_character_asset( $entry ) {
		...
		return [
			'kind'          => 'character',
			'entryId'       => $entry_id,
			'locked'        => ( function_exists( 'shoshin_rms_is_locked_entry' ) ? (bool) shoshin_rms_is_locked_entry( 2247, $entry_id ) : false ),
			'refId'         => $ref_id,
...
if ( ! function_exists( 'shoshin_build_support_asset' ) ) {
	...
	function shoshin_build_support_asset( $entry ) {
		...
		$support_type = shoshin_get_wpforms_field_value( $fields, 4 ); // Support Asset Type (Taiho / Wasen / ship subtypes)
  ...
  default:
    ...
    break;
		...
		return [
			'kind'          => 'support',
			'entryId'       => $entry_id,
			'locked'        => ( function_exists( 'shoshin_rms_is_locked_entry' ) ? (bool) shoshin_rms_is_locked_entry( 2501, $entry_id ) : false ),
			'refId'           => $ref_id,
			'className'       => $class_name,
...
if ( ! function_exists( 'shoshin_my_assets_shortcode' ) ) {
	...
	function shoshin_my_assets_shortcode( $atts ) {
		...
		$char_entries = shoshin_wpforms_db_get_entries_by_user( $character_form_id, $user_id, 999 );
		...
		$support_entries = shoshin_wpforms_db_get_entries_by_user( $support_form_id, $user_id, 999 );
		...
		?>
<?php
// Ensure AJAX config exists BEFORE my-assets JS initializes.
$shoshin_ajax_cfg = [
	'ajaxUrl' => admin_url( 'admin-ajax.php' ),
	'nonce'   => wp_create_nonce( 'shoshin_ajax' ),
];
?>
<script>
window.ShoshinAjax = window.ShoshinAjax || <?php echo wp_json_encode( $shoshin_ajax_cfg ); ?>;
</script>
