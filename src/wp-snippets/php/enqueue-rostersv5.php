...
if ( ! function_exists( 'shoshin_support_bucket' ) ) {
	function shoshin_support_bucket( $u ) {
		...
		// Artillery: Taiho (and future artillery types likely under cls "Artillery")
		if ( $stype === 'taiho' || $name === 'taiho' || $cls === 'artillery' ) {
		return 'Artillery';
		}
...
/* ------------------------------------------------------------------------- */
/* AUTO-HEAL assigned_units_json on /my-rosters render                          */
/* - Prune stale entryIds (admin deleted entries bypass cascade)               */
/* - Rehydrate refId/name from source entries (admin edits)                    */
/* - Persist healed JSON back into roster entry field 9 + digest 10            */
/* ------------------------------------------------------------------------- */

if ( ! function_exists( 'shoshin_rosters_decode_fields_blob' ) ) {
	function shoshin_rosters_decode_fields_blob( $entry ) {
		$raw = '';

		if ( is_array( $entry ) ) {
			$raw = isset( $entry['fields'] ) ? $entry['fields'] : '';
...
/* ------------------------------------------------------------------------- */
/* DB-backed WPForms reads/writes (bypass WPForms capability gating)          */
/* ------------------------------------------------------------------------- */
if ( ! function_exists( 'shoshin_wpforms_db_get_entry' ) ) {
	function shoshin_wpforms_db_get_entry( $entry_id ) {
		global $wpdb;
		...
		$sql = $wpdb->prepare(
			"SELECT entry_id, form_id, user_id, fields
			   FROM {$table}
			  WHERE entry_id = %d
			  LIMIT 1",
			$entry_id
		...
		if ( ! is_array( $row ) || empty( $row['entry_id'] ) ) return null;
...
if ( ! function_exists( 'shoshin_wpforms_db_get_entries_by_user' ) ) {
	function shoshin_wpforms_db_get_entries_by_user( $form_id, $user_id, $limit = 999, $order = 'ASC' ) {
		global $wpdb;
		...
		$sql = $wpdb->prepare(
			"SELECT entry_id, fields, user_id
			   FROM {$table}
			  WHERE form_id = %d
			    AND user_id = %d
...
if ( ! function_exists( 'shoshin_wpforms_db_update_entry_fields' ) ) {
	function shoshin_wpforms_db_update_entry_fields( $entry_id, $fields_json, $expected_form_id = 0, $expected_user_id = 0 ) {
		global $wpdb;

		$entry_id = (int) $entry_id;
		if ( $entry_id <= 0 ) return false;

		$expected_form_id = (int) $expected_form_id;
		$expected_user_id = (int) $expected_user_id;

		$row = shoshin_wpforms_db_get_entry( $entry_id );
		if ( ! $row ) return false;

		if ( $expected_form_id > 0 && (int) $row['form_id'] !== $expected_form_id ) return false;
		if ( $expected_user_id > 0 && (int) $row['user_id'] !== $expected_user_id ) return false;

		$table = $wpdb->prefix . 'wpforms_entries';

		$updated = $wpdb->update(
			$table,
			[ 'fields' => (string) $fields_json ],
			[ 'entry_id' => $entry_id ],
			[ '%s' ],
			[ '%d' ]
		);
...
if ( ! function_exists( 'shoshin_rosters_get_field_value_from_blob' ) ) {
	function shoshin_rosters_get_field_value_from_blob( $fields_blob, $field_id, $default = '' ) {
		$field_id = (string) (int) $field_id;
		if ( ! is_array( $fields_blob ) || ! isset( $fields_blob[ $field_id ] ) ) return $default;

		$f = $fields_blob[ $field_id ];
		if ( is_array( $f ) && array_key_exists( 'value', $f ) ) return (string) $f['value'];

		return $default;
	}
}

if ( ! function_exists( 'shoshin_rosters_set_field_value_in_blob' ) ) {
	function shoshin_rosters_set_field_value_in_blob( &$fields_blob, $field_id, $value ) {
		$field_id = (string) (int) $field_id;

		if ( ! isset( $fields_blob[ $field_id ] ) || ! is_array( $fields_blob[ $field_id ] ) ) {
			$fields_blob[ $field_id ] = [
				'id'    => (int) $field_id,
				'value' => '',
			];
		}

		$fields_blob[ $field_id ]['value'] = (string) $value;
	}
}

if ( ! function_exists( 'shoshin_rosters_autoheal_assigned_units' ) ) {
	/**
	 * @return array [ 'assigned' => array, 'changed' => bool ]
	 */
	function shoshin_rosters_autoheal_assigned_units( $assigned_arr, $owner_user_id ) {

		if ( ! is_array( $assigned_arr ) ) {
			return [ 'assigned' => [], 'changed' => false ];
		}

		$changed = false;

		// Simple per-request cache: "kind:entryId" => built minimal fields
		static $cache = [];
...
if ( ! is_array( $row ) ) {
	...
	continue;
			...
			$eid  = isset( $row['entryId'] ) ? (int) $row['entryId'] : 0;
			$kind = isset( $row['kind'] ) ? (string) $row['kind'] : '';

			// If row has no entryId, we can't validate it — keep it unchanged.
			if ( $eid <= 0 ) {
				$out[] = $row;
				continue;
			}

			// Only heal known kinds.
			...
			if ( ! array_key_exists( $key, $cache ) ) {
				...
				$entry_full = ( function_exists( 'wpforms' ) && isset( wpforms()->entry ) )
					? shoshin_wpforms_db_get_entry( $eid )
					: null;

				if ( ! $entry_full ) {
					$cache[ $key ] = null; // missing => prune
				} else {
					$entry_user_id = 0;
					$entry_form_id = 0;

					if ( is_array( $entry_full ) ) {
						$entry_user_id = isset( $entry_full['user_id'] ) ? (int) $entry_full['user_id'] : 0;
						$entry_form_id = isset( $entry_full['form_id'] ) ? (int) $entry_full['form_id'] : 0;
					} elseif ( is_object( $entry_full ) ) {
						$entry_user_id = isset( $entry_full->user_id ) ? (int) $entry_full->user_id : 0;
						$entry_form_id = isset( $entry_full->form_id ) ? (int) $entry_full->form_id : 0;
					}

					// Must belong to the roster owner (fail-closed)
					if ( (int) $entry_user_id !== (int) $owner_user_id ) {
						$cache[ $key ] = null;
					} else {
						// Enforce form per kind
						if ( $kind === 'character' && $entry_form_id !== 2247 ) {
							$cache[ $key ] = null;
						} elseif ( $kind === 'support' && $entry_form_id !== 2501 ) {
							$cache[ $key ] = null;
						} else {
$blob = shoshin_rosters_decode_fields_blob( $entry_full );

// IMPORTANT: Asset field mappings differ by form.
// Character (2247): Reference ID = field 4
// Support   (2501): Reference ID = field 3
$ref_field_id = ( $kind === 'character' ) ? 4 : 3;

$new_ref  = shoshin_rosters_get_field_value_from_blob( $blob, $ref_field_id, '' );
$new_f4   = shoshin_rosters_get_field_value_from_blob( $blob, 4, '' ); // Support Type for supports (Ozutsu, Mokuzo Hansen, etc.)

// Option A: refId is canonical.
// - Characters: name mirrors refId (no separate name field)
// - Supports: name mirrors field 4 when present, else fall back to refId
$new_name = '';
if ( $kind === 'character' ) {
	$new_name = $new_ref;
} else {
	$new_name = ( $new_f4 !== '' ) ? $new_f4 : $new_ref;
}
...
$cache[ $key ] = [
	...
	'name'  => $new_name,
			...
			$rehydrated = $cache[ $key ];

// Missing or invalid => prune row
if ( $rehydrated === null ) {
	$pruned_any = true;
	$changed = true;
	continue;
...
// Update snapshot fields if changed (Option A: refId authoritative)
$before_ref = isset( $row['refId'] ) ? (string) $row['refId'] : '';
$after_ref  = isset( $rehydrated['refId'] ) ? (string) $rehydrated['refId'] : '';

if ( $after_ref !== '' && $after_ref !== $before_ref ) {
	$row['refId'] = $after_ref;
	$changed = true;
}

// name rehydration:
// - Characters: name mirrors refId
// - Supports: name mirrors support type (field 4) when present; else refId
$before_name = isset( $row['name'] ) ? (string) $row['name'] : '';
$after_name  = isset( $rehydrated['name'] ) ? (string) $rehydrated['name'] : '';
if ( $after_name !== '' && $after_name !== $before_name ) {
	$row['name'] = $after_name;
	$changed = true;
}

// Keep unitKey consistent with canonical identity.
// Existing unitKey format: kind|cls|refId|name|img
$cls = isset( $row['cls'] ) ? (string) $row['cls'] : ( isset( $row['class'] ) ? (string) $row['class'] : '' );
$img = isset( $row['img'] ) ? (string) $row['img'] : '';

$expected_unit_key =
	$kind . '|' .
	$cls . '|' .
	( isset( $row['refId'] ) ? (string) $row['refId'] : '' ) . '|' .
	( isset( $row['name'] ) ? (string) $row['name'] : '' ) . '|' .
	$img;

$before_unit_key = isset( $row['unitKey'] ) ? (string) $row['unitKey'] : '';
if ( $expected_unit_key !== $before_unit_key ) {
	$row['unitKey'] = $expected_unit_key;
	$changed = true;
}
...
// SAFETY RAIL (smarter): only block empty output if we did NOT intentionally prune anything.
// This prevents logic regressions from wiping assignments, while allowing "all units deleted" to resolve to [].
if ( is_array( $assigned_arr ) && count( $assigned_arr ) > 0 && count( $out ) === 0 && ! $pruned_any ) {
	return [ 'assigned' => $assigned_arr, 'changed' => false ];
}

		return [ 'assigned' => $out, 'changed' => $changed ];
	}
}
...
if ( ! function_exists( 'shoshin_build_roster' ) ) {
	function shoshin_build_roster( $entry ) {
		...
		// ------------------------------------------------------------
		// Extract minimal fields from incoming $entry (list row or full)
		// ------------------------------------------------------------
		if ( is_array( $entry ) ) {
			$raw_fields    = $entry['fields'] ?? '';
			$entry_id      = isset( $entry['entry_id'] ) ? (int) $entry['entry_id'] : 0;
			$owner_user_id = isset( $entry['user_id'] ) ? (int) $entry['user_id'] : 0;
		} elseif ( is_object( $entry ) ) {
			$raw_fields    = $entry->fields ?? '';
			$entry_id      = isset( $entry->entry_id ) ? (int) $entry->entry_id : 0;
			$owner_user_id = isset( $entry->user_id ) ? (int) $entry->user_id : 0;
		...
		// ------------------------------------------------------------
		// If user_id missing (common for get_entries rows), load full entry ONCE
		// ------------------------------------------------------------
		$entry_full = null;

		if ( ( $owner_user_id <= 0 || $raw_fields === '' ) && $entry_id > 0 && function_exists( 'wpforms' ) && isset( wpforms()->entry ) ) {
			$entry_full = shoshin_wpforms_db_get_entry( $entry_id );

			if ( $entry_full ) {
				if ( is_array( $entry_full ) ) {
					if ( $raw_fields === '' ) {
						$raw_fields = $entry_full['fields'] ?? '';
					}
					if ( $owner_user_id <= 0 ) {
						$owner_user_id = isset( $entry_full['user_id'] ) ? (int) $entry_full['user_id'] : 0;
					}
				} elseif ( is_object( $entry_full ) ) {
					if ( $raw_fields === '' ) {
						$raw_fields = $entry_full->fields ?? '';
		...
		// ------------------------------------------------------------
		// AUTO-HEAL assigned units snapshot (rehydrate + prune)
		// ------------------------------------------------------------
		if ( function_exists( 'shoshin_rosters_autoheal_assigned_units' ) && $owner_user_id > 0 ) {

			$heal = shoshin_rosters_autoheal_assigned_units( $assigned_arr, $owner_user_id );

			if ( is_array( $heal ) && ! empty( $heal['changed'] ) && isset( $heal['assigned' ] ) && is_array( $heal['assigned' ] ) ) {

				$assigned_arr = $heal['assigned'];

				// Persist healed snapshot back to roster entry (field 9 + field 10)
				$new_assigned = wp_json_encode( $assigned_arr, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
				$new_digest   = sha1( $new_assigned );

				// Ensure we have the full entry to update
				if ( ! $entry_full && $entry_id > 0 && function_exists( 'wpforms' ) && isset( wpforms()->entry ) ) {
					$entry_full = shoshin_wpforms_db_get_entry( $entry_id );
				}

				if ( $entry_full && $entry_id > 0 ) {
					$fields_full = shoshin_normalize_wpforms_fields( shoshin_get( $entry_full, 'fields', '' ) );

					if ( ! isset( $fields_full[9] ) || ! is_array( $fields_full[9] ) ) {
						$fields_full[9] = [ 'id' => 9, 'value' => '' ];
					}
					if ( ! isset( $fields_full[10] ) || ! is_array( $fields_full[10] ) ) {
						$fields_full[10] = [ 'id' => 10, 'value' => '' ];
					}

					$fields_full[9]['value']  = $new_assigned;
					$fields_full[10]['value'] = $new_digest;

					$fields_json = wp_json_encode( $fields_full, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );

					shoshin_wpforms_db_update_entry_fields( $entry_id, $fields_json, 2799, (int) $owner_user_id );

					// Make sure this response uses the healed values
		...
		return [
			'kind'                  => 'roster',
			'entryId'               => $entry_id,
			/* BEGIN RMS SOFT LOCK — EXPOSE LOCK FLAG (Roster) */
			'locked'                => ( function_exists( 'shoshin_rms_is_locked_entry' ) ? (bool) shoshin_rms_is_locked_entry( 2799, $entry_id ) : false ),
			/* END RMS SOFT LOCK — EXPOSE LOCK FLAG (Roster) */
			'refId'                 => $ref_id,
			'name'                  => $name,
			'icon'                  => $icon,
...
if ( ! function_exists( 'shoshin_ajax_unassign_unit' ) ) {
	function shoshin_ajax_unassign_unit() {
		...
		$entry = shoshin_wpforms_db_get_entry( $entry_id );
		...
		if ( $owner_id !== $current_user_id ) {
			wp_send_json_error( [ 'message' => 'Not authorized for this roster.' ], 403 );
				...
				/* BEGIN RMS SOFT LOCK — BACKEND GUARD (unassign unit) */
		if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {
			if ( shoshin_rms_is_locked_entry( 2799, $entry_id, $current_user_id ) ) {
				wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
			}
		}
		/* END RMS SOFT LOCK — BACKEND GUARD (unassign unit) */
		...
		// Persist update
		$updated = shoshin_wpforms_db_update_entry_fields( $entry_id, $save_fields_json, 2799, (int) $user_id );
	...
	function shoshin_ajax_set_unit_qty() {
		...
		if ( $owner_id !== $current_user_id ) {
			wp_send_json_error( [ 'message' => 'Not authorized for this roster.' ], 403 );
				...
				/* BEGIN RMS SOFT LOCK — BACKEND GUARD (set unit qty) */
		if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {
			if ( shoshin_rms_is_locked_entry( 2799, $entry_id, $current_user_id ) ) {
				wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
			}
		}
		/* END RMS SOFT LOCK — BACKEND GUARD (set unit qty) */
...
// Prevent page caching flags (safe — no headers sent)
if ( ! defined( 'DONOTCACHEPAGE' ) ) {
	define( 'DONOTCACHEPAGE', true );
}

		$entries = shoshin_wpforms_db_get_entries_by_user( 2799, $user_id, 999, 'ASC' );
		...
		if ( is_array( $entries ) ) {
			foreach ( $entries as $entry ) {
				...
				// -------------------------------------------------------------
				// AUTO-HEAL assigned_units_json BEFORE building roster payload
				// - Prune stale entryIds (admin deletes)
				// - Rehydrate refId/name snapshots (admin edits)
				// - Persist field 9 + digest 10 back to WPForms
				// -------------------------------------------------------------
				if (
					function_exists( 'shoshin_rosters_decode_fields_blob' ) &&
					function_exists( 'shoshin_rosters_get_field_value_from_blob' ) &&
					function_exists( 'shoshin_rosters_set_field_value_in_blob' ) &&
					function_exists( 'shoshin_rosters_autoheal_assigned_units' ) &&
					function_exists( 'shoshin_parse_assigned_units_json' ) &&
					...
					$fields_blob   = shoshin_rosters_decode_fields_blob( $entry );
					$assigned_json = shoshin_rosters_get_field_value_from_blob( $fields_blob, 9, '' );
					$assigned_arr  = shoshin_parse_assigned_units_json( $assigned_json );

					$healed = shoshin_rosters_autoheal_assigned_units( $assigned_arr, $user_id );

					if ( is_array( $healed ) && ! empty( $healed['changed'] ) && $entry_id > 0 ) {
						$new_assigned_arr  = isset( $healed['assigned'] ) && is_array( $healed['assigned'] ) ? $healed['assigned'] : [];
						$new_assigned_json = wp_json_encode( array_values( $new_assigned_arr ) );
						$new_digest        = sha1( $new_assigned_json );

						shoshin_rosters_set_field_value_in_blob( $fields_blob, 9, $new_assigned_json );
						shoshin_rosters_set_field_value_in_blob( $fields_blob, 10, $new_digest );

						$save_fields_json = wp_json_encode( $fields_blob );

						// Persist healed snapshot back to the roster entry
						shoshin_wpforms_db_update_entry_fields( $entry_id, $save_fields_json, 2799, (int) $user_id );

						// Also update the in-memory entry so shoshin_build_roster() reads the healed data immediately
						if ( is_array( $entry ) ) {
							$entry['fields'] = $save_fields_json;
						} elseif ( is_object( $entry ) ) {
							$entry->fields = $save_fields_json;
						}
					}
				...
				// Build roster payload (now reads healed fields if a heal occurred)
		...
		if ( function_exists( 'wpforms' ) && $user_id ) {
			...
			$char_entries = shoshin_wpforms_db_get_entries_by_user( $character_form_id, $user_id, 999, 'ASC' );
			...
			$support_entries = shoshin_wpforms_db_get_entries_by_user( $support_form_id, $user_id, 999, 'ASC' );
	...
	wp_send_json_error( [ 'message' => 'Invalid nonce.' ], 403 );
		...
		if ( $owner_id !== $current_user_id ) {
			wp_send_json_error( [ 'message' => 'Not authorized for this roster.' ], 403 );
				...
				/* BEGIN RMS SOFT LOCK — BACKEND GUARD (assign unit) */
		if ( function_exists( 'shoshin_rms_is_locked_entry' ) ) {
			if ( shoshin_rms_is_locked_entry( 2799, $entry_id, $current_user_id ) ) {
				wp_send_json( array( 'ok' => false, 'locked' => true, 'msg' => 'Over Free-tier limit...' ) );
			}
		}
		/* END RMS SOFT LOCK — BACKEND GUARD (assign unit) */
