...
/**
 * Suppress misleading WooCommerce temporary-password notice for social-login users.
 *
 * Social users can authenticate through Google/Discord and do not need to act on
 * WooCommerce's generated-password banner immediately.
 */

/**
 * Mark new Nextend-created users so we can treat them as social-login accounts.
 */
add_action('nsl_register_new_user', function($user_id, $provider) {

    // Normalize provider (fix fatal error)
...
add_action('woocommerce_before_account_navigation', function() {
    ...
    foreach ($notices as $type => $messages) {
        foreach ($messages as $key => $notice) {
            ...
            if (stripos($message, 'temporary password') !== false) {
