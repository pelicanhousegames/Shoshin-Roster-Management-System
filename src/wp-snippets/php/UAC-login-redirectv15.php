...
/**
 * Detect whether current user is a social-login user.
 */
function uac_current_user_is_social_login_user() {
	...
	// Fallback: detect Nextend-linked account metadata.
	$all_meta = get_user_meta($user_id);

	foreach ($all_meta as $key => $value) {
		$key = strtolower((string) $key);

		if (strpos($key, 'nsl') !== false || strpos($key, 'nextend') !== false) {
			return true;
		}
	}
...
/**
 * Remove only the generated/temporary password WooCommerce notice.
 */
function uac_remove_social_temp_password_notice($notices) {
	if (!function_exists('is_account_page') || !is_account_page()) {
		return $notices;
	}

	if (!uac_current_user_is_social_login_user()) {
		return $notices;
	}
	...
	foreach ($notices as $type => $messages) {
		...
		foreach ($messages as $index => $notice) {
			...
			if (
				strpos($message_lc, 'temporary password') !== false ||
				strpos($message_lc, 'change your password') !== false
			) {
...
add_filter('woocommerce_get_notices', 'uac_remove_social_temp_password_notice', 20);
...
/**
 * UAC FIX: Mark social login users as having a password (prevents Woo temp password notice)
 */
add_filter('woocommerce_customer_has_password', function($has_password, $user_id) {
