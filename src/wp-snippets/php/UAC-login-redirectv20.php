 ...
 *
 * Behaviors:
 * 1) Direct login at /login (no redirect_to) -> /resource-center/
 * 2) If redirected to /login from a protected page -> back to original page after login
 * 3) Logged-in users can access any legal URL directly (no forced /resource-center/),
...
$UAC_ACCOUNT_URL    = '/resource-center/';
$UAC_LOGIN_URL      = '/login/';
...
if (!function_exists('uac_password_policy_message')) {
    function uac_password_policy_message() {
        return 'Please create a stronger password. Use at least 12 characters, including 1 uppercase letter, 1 lowercase letter, and 1 number.';
    }
}
...
if (!function_exists('uac_validate_password_strength')) {
    function uac_validate_password_strength($password) {
        ...
        $ok = ($length_ok && $upper_ok && $lower_ok && $number_ok);
 ...
 * - Otherwise (manual login), go to /resource-center/
 *  ------------------------------- */
 ...
 *   - low-level/front-end users go to /resource=center/
 *  ------------------------------- */
...
/**
 * UAC Guard: Do not assign Free if the user has, or is scheduled for,
 * a higher legacy entitlement.
 */
function uac_user_has_or_pending_legacy_entitlement($user_id) {
    if (!$user_id) {
        return false;
    }
    ...
    if (get_user_meta((int) $user_id, '_shoshin_legacy_entitlement_pending', true)) {
        return true;
    }
...
add_action('woocommerce_created_customer', function ($customer_id) use ($UAC_FREE_LEVEL_ID) {
    ...
    if (!uac_user_has_or_pending_legacy_entitlement((int) $customer_id)) {
...
add_action('pmpro_after_change_membership_level', function ($level_id, $user_id, $cancel_level) use ($UAC_FREE_LEVEL_ID) {
    ...
    if (uac_user_has_or_pending_legacy_entitlement((int) $user_id)) {
    return;
}
...
add_action('pmpro_after_checkout', function ($user_id, $morder) use ($UAC_FREE_LEVEL_ID) {
    ...
    if (!get_user_meta((int)$user_id, 'uac_pmpro_first_checkout_done', true)) {
        ...
        update_user_meta((int)$user_id, '_uac_fire_registration_complete_once', 1);
    }
    ...
    if (!$has_level && !uac_user_has_or_pending_legacy_entitlement((int) $user_id)) {
...
if (!function_exists('uac_render_nextend_social_buttons')) {
    ...
    <div class="shoshin-social-consent">
        By continuing with one of the options below, you agree to the
 ...
 *    Social signup is allowed only for Free tier ID 4.
 *    Paid tiers must use standard PMPro checkout.
 */
add_action('pmpro_checkout_after_form', function () {
    ...
    if (!$level_id && isset($_REQUEST['level'])) {
    ...
    if ($level_id === 2 || $level_id === 3) {
        echo '<div class="shoshin-social-auth-note">';
        echo 'Account registration via social networks is only available for free memberships.';
        echo '</div>';
        return;
    }
...
/**
 * UAC FINAL FIX: Replace Woo "temporary password" notice for social users
 * with clearer social-login password guidance.
 *
 * Also marks the social user as having set a site password after password reset.
 */
add_action('woocommerce_before_account_navigation', function() {
    ...
    if (!get_user_meta($user_id, 'uac_registered_via_social_login', true)) {
        return;
    }

    /*
     * If Woo redirects here after a successful password reset,
     * mark the social account as now having a site password.
     */
    if (isset($_GET['password-reset']) && $_GET['password-reset'] === 'true') {
    update_user_meta($user_id, 'uac_social_site_password_set', 1);

    if (function_exists('wc_clear_notices') && function_exists('wc_add_notice')) {
        wc_clear_notices();
        wc_add_notice('Your site password has been set successfully.', 'success');
    }

    return;
}

    if (get_user_meta($user_id, 'uac_social_site_password_set', true)) {
        return;
    }

    if (!function_exists('wc_get_notices') || !function_exists('wc_set_notices') || !function_exists('wc_add_notice')) {
        return;
    }

    $notices = wc_get_notices();
    $removed_temp_notice = false;
    ...
    foreach ($notices as $type => $messages) {
        foreach ($messages as $key => $notice) {
            ...
            if (stripos(wp_strip_all_tags($message), 'temporary password') !== false) {
                unset($notices[$type][$key]);
                $removed_temp_notice = true;
            }
        }
    }
    ...
    if ($removed_temp_notice) {
        wc_add_notice(
            'You signed up with a social account. You can continue using social sign-in, or <a href="' . esc_url(wp_lostpassword_url()) . '">set a site password here</a>.',
            'notice'
        );
    }
...
/**
 * Update PMPro Terms checkbox label to include Privacy Policy.
 */
add_filter('pmpro_tos_field_label', function($tos_label, $tospage) {

    return 'I agree to the <a href="/terms-of-use/" target="_blank" rel="noopener">Terms of Use</a> and acknowledge the <a href="/privacy-policy/" target="_blank" rel="noopener">Privacy Policy</a>.';

}, 10, 2);

/**
 * UAC FIX: Ensure social registrations ALWAYS get Free membership (prevents orphan users)
 */
add_action('nsl_register_new_user', function($user_id, $provider) {

    // Normalize provider safely (prevents fatal error)
    ...
    update_user_meta($user_id, 'uac_registered_via_social_login', 1);
    update_user_meta($user_id, 'uac_social_provider', $provider_id);

    // Assign FREE membership (ID = 4), unless a higher legacy entitlement is pending/processed.
if (function_exists('pmpro_changeMembershipLevel') && !uac_user_has_or_pending_legacy_entitlement((int) $user_id)) {
...
/**
 * UAC: Mark social users as having a site password once they complete password reset.
 */
add_action('after_password_reset', function($user, $new_pass) {
    ...
    update_user_meta((int) $user->ID, 'uac_social_site_password_set', 1);

}, 10, 2);

/**
 * UAC: Add body class when social user has not yet set a site password.
 */
add_filter('body_class', function($classes) {
    ...
    if (
    get_user_meta($user_id, 'uac_registered_via_social_login', true) &&
    !get_user_meta($user_id, 'uac_social_site_password_set', true) &&
    !(isset($_GET['password-reset']) && $_GET['password-reset'] === 'true')
) {
    $classes[] = 'uac-social-no-site-password';
}
...
/**
 * UAC: Redirect legacy/default PMPro membership levels page to custom membership page.
 */
add_action('template_redirect', function() {
    ...
    $request_uri = isset($_SERVER['REQUEST_URI']) ? strtok((string) $_SERVER['REQUEST_URI'], '?') : '';

    if (untrailingslashit($request_uri) === '/membership-levels') {
        wp_safe_redirect(home_url('/membership/'), 301);
        exit;
    }
...
/**
 * GTM / Meta / GA4
 * Push successful account registration event once, immediately after first checkout/registration.
 */
add_action('wp_footer', function () {
    ...
    if (!get_user_meta((int) $user_id, '_uac_fire_registration_complete_once', true)) {
        return;
    }

    delete_user_meta((int) $user_id, '_uac_fire_registration_complete_once');

    ?>
    <script>
    window.dataLayer = window.dataLayer || [];
    window.dataLayer.push({
        event: 'account_registration_complete'
    });
    </script>
    <?php
...
add_action('template_redirect', function () {
    ...
    if ($path !== '/membership-account/') {
        return;
    }

    wp_safe_redirect(home_url('/membership-account/membership-billing/'), 301);
    exit;

}, 1);

/**
 * UAC: Control wp-admin access.
 *
 * - Allow admin-ajax.php.
 * - Allow site admins into wp-admin.
 * - Redirect non-admin profile/account screens to Woo My Account.
 * - Redirect all other non-admin wp-admin access to Resource Center.
 */
add_action('admin_init', function () {
    ...
    if (
        strpos($script, '/wp-admin/profile.php') !== false ||
        strpos($script, '/wp-admin/user-edit.php') !== false
    ) {
        wp_safe_redirect(home_url('/my-account/'), 302);
        exit;
    }

    wp_safe_redirect(home_url('/resource-center/'), 302);
    exit;
