...
function replaceFilterEverythingMarkup(doc) {
  ...
  function replaceWholeNodes(selector) {
    var currentNodes = document.querySelectorAll(selector);
    var newNodes = doc.querySelectorAll(selector);

    if (!currentNodes.length || !newNodes.length) {
      return false;
    }

    var count = Math.min(currentNodes.length, newNodes.length);

    for (var i = 0; i < count; i++) {
      currentNodes[i].replaceWith(newNodes[i].cloneNode(true));
    }

    return true;
  }

  if (replaceWholeNodes('.wpc-filters-main-wrap')) return true;
  if (replaceWholeNodes('.wpc-filters-widget-wrapper')) return true;
  if (replaceWholeNodes('.widget_wpc_filters_widget')) return true;
...
function applyPriceFilterAjax(url) {
  ...
  xhr.onload = function () {
    ...
    runPostRenderUiSync();
    runPostSwapFilterEverythingSettle();
  };
...
var SHOSHIN_FE_TAXONOMY_TO_VAR = {
  ...
  'product_item_format': 'item_format',
  'product_estimated_delivery': 'estimated_delivery',
  'product_material': 'material',
  'product_color': 'color',
  'product_scale': 'scale',
  'product_fulfillment': 'fulfillment',
  'product_line': 'product_line',
  'product_type': 'product_type',
  'product_storyteller': 'storyteller',
  'product_brand': 'brand',
  'product_artist': 'artist',
  'product_hobby_use': 'hobby_use'
};
  ...
  sections.forEach(function (section) {
    ...
    items.forEach(function (item) {
      ...
      if (info && info.selected) {
        shouldShow = true;
      } else if (info && info.facetKey && available.has(info.facetKey)) {
        shouldShow = true;
      }
...
function runPostSwapFilterEverythingSettle() {
...
function hydrateDrawerFromCurrentUrl(attempt) {
  attempt = attempt || 0;

  var widget = document.querySelector('.widget_wpc_filters_widget');
  if (!widget) {
    if (attempt < 10) {
      setTimeout(function () {
        hydrateDrawerFromCurrentUrl(attempt + 1);
      }, 100);
    }
    return;
  }
...
document.addEventListener('touchend', function (e) {
  ...
  document.addEventListener('keydown', function (e) {
    ...
    var popupOpenBtn = e.target.closest('#shoshin-filter-drawer-open');
    if (popupOpenBtn) {
      return;
    }
