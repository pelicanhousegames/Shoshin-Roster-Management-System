document.addEventListener('DOMContentLoaded', function() {
  ...
  const CLASS_IMAGES = {
    ...
    'Samurai': '/wp-content/uploads/2026/02/samurai2_colorized.jpg',
  ...
  const CLASS_RULES = {
    'Daimyo': {
      ...
      ranged: [NO_RANGED_WEAPON_LABEL, 'Shuriken', 'Fukiya', 'Hankyu', 'Daikyu', 'Tanegashima', 'Ozutsu', 'Houroku-Hiya'],
    ...
    'Ninja': {
      ...
      ranged: [NO_RANGED_WEAPON_LABEL,'Shuriken','Fukiya','Hankyu','Daikyu','Tanegashima','Kunai', 'Ozutsu', 'Houroku-Hiya'],
      armor: [NO_ARMOR_LABEL, 'Do-maru'],
  ...
  const TRAINING_DEPENDENCIES = {
    ...
    'Kayakujutsu':     { melee: [], ranged: ['Tanegashima','Ozutsu','Houroku-Hiya'], support: [] },
...
{ name: 'Extended Range', when: s =>
  // Suppress only when mounted without Bajutsu; allow when (not mounted) OR (mounted + Bajutsu)
  (
    (!s.support.includes('Uma')) ||
    (s.support.includes('Uma') && s.training.includes('Bajutsu'))
  ) && (
...
// 2) Uma weapon mismatch: Uma cannot be selected if ranged is Houroku-Hiya or Ozutsu
const umaRangedMismatch = (ranged === 'Houroku-Hiya' || ranged === 'Ozutsu');

if (hasUma) {
  ['Houroku-Hiya', 'Ozutsu'].forEach(w => {
...
if (!cls) {
  rows.forEach(row => {
    const nameCell = row.querySelector('td:nth-child(2)') || row.querySelector('td:nth-child(1)');
...
rows.forEach(row => {
  ...
  function updateCharacterStats() {
    ...
    if (state.abilities.includes('Ki Endurance')) body += 1;
...
/* BEGIN MRBPA: Lucky Hits adjustments (2026-01-11)
  1) Remove [B] Lucky Hits on 6 only
  2) Remove weapon-selected gating for [M]/[R] Lucky Hits
*/
if (meleeIneptActive) {
...
/* END MRBPA: Lucky Hits adjustments (2026-01-11) */


/* BEGIN MRBPA: Water Inept ship restriction (2026-01-11)
  3) Add [P] May not operate ship (if Water Inept)
*/
if (waterIneptActive) {
  mods.push('[B] May not attack in water');
  mods.push('[P] May not operate ship');
}
/* END MRBPA: Water Inept ship restriction (2026-01-11) */
  ...
  if (meleeName === 'Kanabo' && meleeProficient) {
    ...
    if (!meleeIneptActive) {
      if ((meleeName === 'Katana' && hasKenjutsu) || (meleeName === 'Nodachi' && hasIaijutsu)) {
        mods.push('[M] >1 vs Restrained: Decapitate');
      }
    }
...
if (!rangedIneptActive) {
  // Suppress Extended Range MRBPA tags only when mounted without Bajutsu
  const allowExtendedRangeWhileMounted = (!hasUma || hasBajutsu);

  if (allowExtendedRangeWhileMounted) {
  ...
  function updateProficienciesSummary() {
    ...
    const STAT_ONLY = ['Agile','Ki Endurance'];
    ...
    if (cls === 'Sohei' && checkedAbilities.includes('Ki Endurance')) abilitiesForProfs.push('Ki Endurance');
    ...
    (function addHighDefense() {
      ...
      if (state.abilities.includes('Concealment')) defense += 1;
      if (defense >= 6) items.push('Resilience');
    })();
