document.addEventListener('DOMContentLoaded', function () {
  ...
  /* BEGIN RMS SOFT LOCK — LOCK IMAGE RESOLVER (My Rosters) */
  // Provide lock image via either:
  // 1) window.ShoshinRmsLockImgUrl (global), OR
  // 2) data-shoshin-lock-img attribute on the listEl
  // Falls back to /wp-content/uploads/2026/02/padlock.jpg if not provided.
  var RMS_LOCK_ICON_URL = (function () {
    try {
      var g = (window && window.ShoshinRmsLockImgUrl) ? String(window.ShoshinRmsLockImgUrl) : '';
      if (g) return g;
    } catch (e) {}
    try {
      var a = listEl.getAttribute('data-shoshin-lock-img') || '';
      if (a) return String(a);
    } catch (e2) {}
    return '/wp-content/uploads/2026/02/padlock.jpg';
  })();
  /* END RMS SOFT LOCK — LOCK IMAGE RESOLVER (My Rosters) */

    /* BEGIN RMS SOFT LOCK — SHARED HELPERS (keep identical across my-assets + my-rosters) */
  function rmsIsLocked(o) {
    if (!o) return false;

    var v = null;
    if (o.locked != null) v = o.locked;
    else if (o.isLocked != null) v = o.isLocked;
    else if (o.is_locked != null) v = o.is_locked;
    else if (o.rmsLocked != null) v = o.rmsLocked;
    else if (o.rms_locked != null) v = o.rms_locked;
    else if (o.rmsIsLocked != null) v = o.rmsIsLocked;
    else if (o.rms_is_locked != null) v = o.rms_is_locked;
    else if (o.softLocked != null) v = o.softLocked;
    else if (o.soft_locked != null) v = o.soft_locked;
  ...
  function rmsLockImg() {
    try {
      if (typeof RMS_LOCK_ICON_URL !== 'undefined' && RMS_LOCK_ICON_URL) {
        return String(RMS_LOCK_ICON_URL);
      }
    } catch (_) {}
    return '/wp-content/uploads/2026/02/padlock.jpg';
  }

  function rmsPickImg(normalUrl, locked) {
    return locked ? rmsLockImg() : String(normalUrl || '');
  }

  function rmsDisableToggle(cbEl, wrapEl) {
    try { if (cbEl) cbEl.disabled = true; } catch (_) {}
    try { if (cbEl && cbEl.style) cbEl.style.cursor = 'not-allowed'; } catch (_) {}
    try {
      if (wrapEl) {
        wrapEl.classList.add('is-disabled', 'is-rms-locked');
        if (wrapEl.style) wrapEl.style.cursor = 'not-allowed';
      }
    } catch (_) {}
  }
  /* END RMS SOFT LOCK — SHARED HELPERS (keep identical across my-assets + my-rosters) */

    /* BEGIN RMS SOFT LOCK — AUTO-UNLOCK + RERENDER PROMOTED ROSTER (My Rosters) */
  function rmsClearAllLockFlags(o) {
  ...
  function rmsPromoteNextNewestLockedRoster() {
    ...
    if (best && bestId > 0) {
      rmsClearAllLockFlags(best);
      return best;
    }
    return null;
  }

  function rmsRerenderRosterCardByEntryId(entryId) {
    entryId = asInt(entryId, 0);
    if (!entryId || !Array.isArray(rosters)) return;

    // Find updated roster object + its index (renderRosterCard needs idx)
    var idx = -1;
    var rObj = null;
    for (var i = 0; i < rosters.length; i++) {
      var rid = asInt(rosters[i].entryId || rosters[i].id || rosters[i].entry_id, 0);
      if (rid === entryId) {
        idx = i;
        rObj = rosters[i];
        break;
    ...
    if (!rObj || idx < 0) return;

    // Replace existing DOM card with a freshly-rendered one
    var oldCard = getRosterCardElByEntryId(entryId);
    if (!oldCard || !oldCard.parentNode) return;

    var newCard = renderRosterCard(rObj, idx);
    oldCard.parentNode.replaceChild(newCard, oldCard);
  }
  /* END RMS SOFT LOCK — AUTO-UNLOCK + RERENDER PROMOTED ROSTER (My Rosters) */

    /* BEGIN RMS SOFT LOCK — AUTO-UNLOCK NEXT NEWEST AFTER DELETE (My Rosters) */
  function rmsClearAllLockFlags(o) {
    if (!o || typeof o !== 'object') return;
    o.locked = 0;
    o.isLocked = 0;
  ...
  function rmsPromoteNextUnlockedRosterInMemory() {
  ...
  /* END RMS SOFT LOCK — AUTO-UNLOCK NEXT NEWEST AFTER DELETE (My Rosters) */
...
function withInchesIfNumeric(v) {
  ...
  // Normalize any weird quote encodings first (handles u2033/u2032 cases)
  try {
    if (typeof assignUnitsNormalizeQuotes === 'function') {
      s = assignUnitsNormalizeQuotes(s);
    }
  } catch (_) {}
...
// Daimyo, Samurai, Ashigaru, Sohei, Ninja, Onmyoji, Taiho, Wasen
var ORDER = {
  'daimyo': 0,
  'samurai': 1,
  'ashigaru': 2,
  ...
  function makeUnitKey(u) {
    ...
    // Normalize support classes so unitKey matches assigned_units_json.
    // - Ships/subtypes => cls "Sailing Ships"
    // - Taiho => cls "Artillery"
    if (kind === 'support') {
      var clsLow = String(cls || '').toLowerCase().replace(/\s+/g, ' ').trim();
      if (
        clsLow === 'wasen' ||
        clsLow === 'mokuzo hansen' ||
        clsLow === 'makuzo hansen' ||
        clsLow === 'kobaya' ||
        clsLow === 'sekibune' ||
        clsLow === 'atakebune'
      ) {
  ...
  function postAjax(action, payload) {
    ...
    return fetch(AJAX_URL, { method: 'POST', body: fd, credentials: 'same-origin' })
        ...
        return res.text().then(function (txt) {
          ...
          try {
            ...
            json = null;
                    ...
                    /* BEGIN RMS SOFT LOCK — INTERCEPT LOCKED RESPONSE (postAjax) */
          if (maybeShowLockedModalFromJson(json)) {
            // Silent block: show modal, then reject to stop caller flows
            throw new Error('__RMS_LOCKED__');
          }
          /* END RMS SOFT LOCK — INTERCEPT LOCKED RESPONSE (postAjax) */
      ...
      .catch(function (err) {
        /* BEGIN RMS SOFT LOCK — SUPPRESS NORMAL ERROR FLOW FOR LOCKED */
        if (err && err.message === '__RMS_LOCKED__') {
          // Modal already shown; keep sentinel so callers can suppress their inline errors.
          throw err;
        }
        /* END RMS SOFT LOCK — SUPPRESS NORMAL ERROR FLOW FOR LOCKED */
...
function supportBucketFromUnit(u) {
  // Mirror your semantics:
  // - Taiho => Artillery
  // - Sailing Ships (Wasen + future ship subtypes) => Ships
  ...
  // Artillery
  if (name === 'taiho' || cls === 'artillery' || st === 'taiho' || st === 'artillery') return 'Artillery';
  ...
  function computeRosterTotalsFromAssigned(assignedArr) {
    ...
    for (var i = 0; i < assignedArr.length; i++) {
      ...
      // Initiative: prefer top-level, but if it resolves to 0 and stats.ini contains a positive number,
      // use stats.ini (legacy snapshots sometimes store ini only in stats).
      var iniTop = (u && u.ini != null) ? u.ini : null;
      var iniStats = (u && u.stats && u.stats.ini != null) ? u.stats.ini : null;

      var ini = asInt((iniTop != null ? iniTop : iniStats), 0);

      if (ini === 0 && iniStats != null && isNumericLike(iniStats) && asInt(iniStats, 0) > 0) {
        ini = asInt(iniStats, 0);
      }
...
/* BEGIN RMS SOFT LOCK — GENERIC LOCKED MODAL (My Rosters) */
var rmsLockedModal = null;
var rmsLockedBackdrop = null;
var rmsLockedCtx = null;

function setRmsLockedCtxFromRosterEntryId(entryId) {
  try {
    var r = getRosterObjByEntryId(entryId);
    if (!r) { rmsLockedCtx = null; return; }

    var refId = String(r.refId || r.ref_id || '').trim();
    var name  = String(r.name || r.roster_name || 'Untitled Roster').trim();

    rmsLockedCtx = {
      kind: 'roster',
      name: name,
      refId: refId,
      entryId: asInt(r.entryId || r.id || r.entry_id, 0)
    };
  } catch (e) {
    rmsLockedCtx = null;
  }
...
/* BEGIN RMS SOFT LOCK — CONTEXT SETTER (asset) */
function setRmsLockedCtxFromOwnedAsset(owned) {
  try {
    if (!owned) { rmsLockedCtx = null; return; }

    var refId = String(owned.refId || owned.ref_id || '').trim();

    // Prefer visible "Type" label like the UI uses
    // For Sailing Ships: show subtype (Kobaya/Sekibune/Atakebune), not base Wasen.
    var name = '';

    // Characters
    name = String(owned.cls || owned.class || '').trim();

    if (!name) {
      var supportType = String(owned.supportType || owned.type || owned.name || '').trim();

      // Sailing Ships (Wasen + legacy) -> derive subtype from hull size category (match /my-assets logic)
      if (
        supportType === 'Wasen' ||
        supportType === 'Mokuzo Hansen' ||
        supportType === 'Kobaya' ||
        supportType === 'Sekibune' ||
        supportType === 'Atakebune'
      ) {
        var hullCatRaw =
          String(
            owned.asset_size ||
        ...
        var hullCat = hullCatRaw.match(/^(Medium|Large|Huge|Gargantuan|Colossal)\b/i);
        var hull = hullCat ? (hullCat[1].charAt(0).toUpperCase() + hullCat[1].slice(1).toLowerCase()) : '';

        var subtype = '';
        if (hull === 'Medium' || hull === 'Large') subtype = 'Kobaya';
        else if (hull === 'Huge' || hull === 'Gargantuan') subtype = 'Sekibune';
        else if (hull === 'Colossal') subtype = 'Atakebune';

        // If already stored as subtype, keep it
        if (!subtype && (supportType === 'Kobaya' || supportType === 'Sekibune' || supportType === 'Atakebune')) {
          subtype = supportType;
        }

        name = String(subtype || supportType || 'Locked Asset').trim();
      } else {
        // Non-ship supports
        name = String(supportType || owned.name || 'Locked Asset').trim();
      }
    }

    if (!name) name = 'Locked Asset';

    rmsLockedCtx = {
      kind: 'asset',
      name: name,
      refId: refId,
      unitKey: String(owned.unitKey || owned.unit_key || '').trim(),
      entryId: asInt(owned.entryId || owned.id || owned.entry_id, 0)
    };
  } catch (e) {
    rmsLockedCtx = null;
  }
}
/* END RMS SOFT LOCK — CONTEXT SETTER (asset) */

function ensureRmsLockedModal() {
  if (rmsLockedModal && rmsLockedBackdrop) return;

  rmsLockedBackdrop = document.createElement('div');
  rmsLockedBackdrop.className = 'shoshin-modal-backdrop';
  rmsLockedBackdrop.setAttribute('aria-hidden', 'true');

  rmsLockedModal = document.createElement('div');
  rmsLockedModal.className = 'shoshin-modal shoshin-rms-locked-modal';
  rmsLockedModal.setAttribute('role', 'dialog');
  rmsLockedModal.setAttribute('aria-modal', 'true');

  rmsLockedModal.innerHTML = `
    ...
    <div class="shoshin-modal-body shoshin-preview-body-wrap">
      <h2 id="shoshin-rms-locked-title" style="margin-top:0;">Locked</h2>

      <div class="shoshin-preview-asset-row"
           style="margin-top:12px; display:flex; align-items:center; gap:16px; text-align:left;">

        <img class="shoshin-preview-asset-img shoshin-rms-locked-img"
             src="${esc(RMS_LOCK_ICON_URL)}"
             alt="Locked"
             style="
               width:96px;
               height:96px;
        ...
        <div style="display:flex; flex-direction:column; justify-content:center; text-align:left;">
          <div class="shoshin-preview-asset-name shoshin-rms-locked-name"
               style="font-weight:700; font-size:1.05rem; text-align:left;">
            Locked Item
          </div>
          <div class="shoshin-preview-asset-ref shoshin-rms-locked-ref"
               style="opacity:0.85; margin-top:4px; text-align:left;">
          </div>
        </div>

      </div>

      <div class="shoshin-modal-msg" style="margin:14px 0 0; line-height:1.4;"></div>
  ...
  document.body.appendChild(rmsLockedBackdrop);
  document.body.appendChild(rmsLockedModal);

  function close() { setModalVisible(rmsLockedModal, rmsLockedBackdrop, false); }

  rmsLockedBackdrop.addEventListener('click', close);

  var xBtn = rmsLockedModal.querySelector('.shoshin-modal-x');
  if (xBtn) xBtn.addEventListener('click', close);

  var closeBtn = rmsLockedModal.querySelector('.shoshin-modal-btn-cancel');
  if (closeBtn) closeBtn.addEventListener('click', close);

  document.addEventListener('keydown', function (e) {
    if (!rmsLockedModal) return;
    if (!rmsLockedModal.classList.contains('is-open')) return;
    if (e.key === 'Escape') close();
  });
}

function openRmsLockedModal(msg) {
  ensureRmsLockedModal();

  var ctx = rmsLockedCtx || null;

  var titleEl = rmsLockedModal ? rmsLockedModal.querySelector('#shoshin-rms-locked-title') : null;
  var nameEl  = rmsLockedModal ? rmsLockedModal.querySelector('.shoshin-rms-locked-name') : null;
  var refEl   = rmsLockedModal ? rmsLockedModal.querySelector('.shoshin-rms-locked-ref') : null;
  var imgEl   = rmsLockedModal ? rmsLockedModal.querySelector('.shoshin-rms-locked-img') : null;
  var msgEl   = rmsLockedModal ? rmsLockedModal.querySelector('.shoshin-modal-msg') : null;

  if (imgEl) imgEl.setAttribute('src', String(RMS_LOCK_ICON_URL || ''));

  if (ctx && ctx.kind === 'roster') {
    if (titleEl) titleEl.textContent = 'Locked Roster';
    if (nameEl)  nameEl.textContent  = String(ctx.name || 'Locked Roster');
    if (refEl)   refEl.textContent   = String(ctx.refId || '');
  } else if (ctx && ctx.kind === 'asset') {
    if (titleEl) titleEl.textContent = 'Locked Asset';
    if (nameEl)  nameEl.textContent  = String(ctx.name || 'Locked Asset');
    if (refEl)   refEl.textContent   = String(ctx.refId || '');
  } else {
    if (titleEl) titleEl.textContent = 'Locked';
    if (nameEl)  nameEl.textContent  = 'Locked Item';
  ...
  if (msgEl) {
    msgEl.innerHTML = String(
      msg ||
      'This item is locked because your Paid Membership term has ended and you are over the asset limit for the Free Membership Tier.<br><br>Only the newest item of the same type may be used.<br>(A page refresh may be required to update after asset deletion.)'
    );
  }

  setModalVisible(rmsLockedModal, rmsLockedBackdrop, true);
}

/* BEGIN RMS SOFT LOCK — LOCKED RESPONSE NORMALIZER (My Rosters) */
function maybeShowLockedModalFromJson(json) {
  try {
    if (!json) return false;

    // Support BOTH response shapes:
    // A) { ok:false, locked:true, msg:"..." }
    // B) WP envelope: { success:false, data:{ locked:true, msg:"..." } }
    var locked = false;
    var msg = '';

    if (json.locked === true || (json.ok === false && json.locked === true)) {
      locked = true;
      msg = json.msg || '';
    } else if (json.success === false && json.data && json.data.locked === true) {
      locked = true;
      msg = (json.data.msg || json.data.message || '');
    }

    if (locked) {
      openRmsLockedModal(msg || 'This item is locked because your Paid Membership term has ended and you are over the asset limit for the Free Membership Tier.<br><br>Only the newest item of the same type may be used.<br>(A page refresh may be required to update after asset deletion.)');
      return true;
    }
  } catch (e) {}
...
/* END RMS SOFT LOCK — LOCKED RESPONSE NORMALIZER (My Rosters) */
/* END RMS SOFT LOCK — GENERIC LOCKED MODAL (My Rosters) */
  ...
  host.innerHTML =
    '<div class="shoshin-assign-asset">' +
      ...
      '<div class="shoshin-assign-asset-meta">' +
        ...
        '<div class="shoshin-assign-asset-ref">' + esc(ref || '—') + '</div>' +
      ...
      updateBtn.addEventListener('click', function () {
        ...
        postAjax('shoshin_update_roster_banner', {
          ...
          .catch(function (err) {
            ...
            /* BEGIN RMS SOFT LOCK — SUPPRESS INLINE ERROR (banner) */
            if (err && err.message === '__RMS_LOCKED__') {
              // Modal already shown; do not show inline error text.
              return;
            }
            /* END RMS SOFT LOCK — SUPPRESS INLINE ERROR (banner) */
...
var ASSIGN_UNITS_CLASS_IMAGES = {
    ...
    Ninja: '/wp-content/uploads/2026/02/ninja_colorized.jpg',
    Onmyoji: '/wp-content/uploads/2026/02/onmyoji_colorized.jpg',

  Artillery: '/wp-content/uploads/2026/02/ozutsu_colorized.jpg',
  Taiho: '/wp-content/uploads/2026/02/ozutsu_colorized.jpg',
  'Sailing Ships': '/wp-content/uploads/2026/02/kobaya_colorized.jpg'
};

function assignUnitsResolveCanonicalImg(uOrAsset) {
  if (!uOrAsset) return ASSIGN_UNITS_FALLBACK_IMAGE;

  // Derive deterministically from class/type for Bulk Assign.
  // Do not trust persisted img/imgUrl here because stale paths break the modal
  // and can destabilize unitKey matching.
  var cls = String(
    uOrAsset.cls ||
    uOrAsset.class ||
    uOrAsset.className ||
  ...
  // If this is a ship subtype (Wasen/Kobaya/Sekibune/Atakebune), force the class image to "Sailing Ships"
  // because ownedAssets sometimes provides subtype as "supportType" instead of cls="Sailing Ships".
  ...
  if (
    ...
    low === 'sekibune' ||
    low === 'atakebune' ||
    low.indexOf('mokuzo') !== -1
  ) {
    cls = 'Sailing Ships';
  }

  // Taiho may arrive as supportType/type instead of cls="Artillery"
  if (low === 'taiho') {
    cls = 'Artillery';
  }
...
// RMS: determine whether an owned asset is locked (older duplicates, over free tier, etc.)
function isRmsLockedAsset(a) {
  try { return rmsIsLocked(a); } catch (_) {}
  return false;
}
...
function assignUnitsGetDisplayType(asset) {
  // Use the unified resolver that already infers Ships subtype from stats.
  // This ensures Wasen renders as "Sailing Ships" in Bulk Assign UI.
  return getAssignUnitsDisplayType(asset);
}
...
function assignUnitsRenderRestriction(rosterObj) {
  ...
  var hasAssigned = rosterHasDaimyoAssigned(rosterObj);
  ...
  // Always show as blocked (red)
  el.style.display = 'block';
  el.classList.add('is-visible');
  el.classList.add('is-blocked');

  // Messaging states (inverse use case vs /my-assets):
  // - hasAssigned: roster already has a Daimyo (so all Daimyo rows disabled)
  // - hasStaged: user selected a Daimyo in this modal (so other Daimyo rows disabled)
  // - else: general rule reminder
  if (hasAssigned) {
    el.innerHTML =
      '<b>Restriction:</b> This clan already has an assigned Daimyo. Remove it before assigning another.';
  } else if (hasStaged) {
    el.innerHTML =
      '<b>Restriction:</b> Only one Daimyo may be assigned to a clan. A Daimyo is currently selected.';
  } else {
    el.innerHTML =
      '<b>Restriction:</b> Only one Daimyo may be assigned to a clan.';
  }
}
...
function assignUnitsUpdateRowFromUi(rowEl) {
  ...
  var isOn = !!(toggle && toggle.checked);
    ...
    // ------------------------------------------------------------
  // UX: Dim only when UNASSIGNED and toggle is OFF.
  // Assigned rows (curQty >= 1) should never be dim.
  // Also: if user toggles ON, remove dim immediately.
  // ------------------------------------------------------------
  try {
    var isDisabledRow = rowEl.classList.contains('is-disabled');
// UX: Dim rows ONLY when unassigned AND toggle is OFF.
// If current QTY >= 1 OR toggle is ON, row must NOT be dim.
var shouldDimNow = (curQty === 0 && !isOn);

    rowEl.classList.toggle('shoshin-assign-units-row-dim', shouldDimNow);

    if (shouldDimNow) rowEl.setAttribute('data-dim', '1');
    else rowEl.removeAttribute('data-dim');

    // Preserve other inline styles (e.g., cursor); only change dim-related props.
    rowEl.style.opacity = shouldDimNow ? '0.55' : '';
    rowEl.style.filter  = shouldDimNow ? 'grayscale(1)' : '';
  } catch (_) {}
...
if (isOn && addQty > 0) {
  ...
  // UX: Dim rows ONLY when unassigned AND toggle is OFF.
// If current QTY >= 1 OR toggle is ON, row must NOT be dim (even if disabled).
var shouldDim = (curQty === 0 && !isOn);

if (shouldDim) {
  rowEl.setAttribute('data-dim', '1');
  rowEl.classList.add('shoshin-assign-units-row-dim');

  // Match initial render dim style
  rowEl.style.opacity = '0.55';
  rowEl.style.filter = 'grayscale(1)';
} else {
  rowEl.removeAttribute('data-dim');
  rowEl.classList.remove('shoshin-assign-units-row-dim');

  // Clear dim style (row can still be disabled, but should not look dim if assigned/on)
  rowEl.style.opacity = '';
  rowEl.style.filter = '';
}
...
function assignUnitsBindListEventsOnce() {
  ...
  // Hover lift for dim rows (but NEVER for disabled rows)
  list.addEventListener('mouseover', function (e) {
    var t = e && e.target ? e.target : null;
    if (!t) return;

    var row = t.closest ? t.closest('.shoshin-assign-units-row[data-unit-key]') : null;
    if (!row) return;

    if (row.classList.contains('is-disabled')) return;
    if (String(row.getAttribute('data-dim') || '') !== '1') return;
...
function assignUnitsNormalizeQuotes(s) {
  ...
  s = s
    ...
    .replace(/\u2032/g, "'")
    .replace(/\u2033/g, '"')
    // ALSO normalize literal escaped sequences that may come through from JSON strings
    // e.g. "18\u2033" showing as "18u2033" in the UI
    .replace(/\\u2032/gi, "'")
    .replace(/\\u2033/gi, '"')
    .replace(/\u2013|\u2014/g, '-') // dashes
    .replace(/\u00A0/g, ' ');       // nbsp
...
function assignUnitsBuildCanonicalAssignedUnitFromOwnedAsset(asset, unitKey) {
  ...
  var cls  = String(asset.cls || asset.class || asset.className || asset.type || asset.supportType || '').trim() || '—';
  var refId = String(asset.refId || asset.ref_id || '').trim() || '—';

  // Normalize support cls so saved assigned_units_json stays canonical
  ...
  var out = {
    ...
    stats: stats,
    unitKey: safeUnitKey,
    isLocked: isRmsLockedAsset(asset)
  };
...
function assignUnitsSubmit() {
  ...
  fetch(AJAX_URL, {
    ...
    .then(function (json) {
      ...
      /* BEGIN RMS SOFT LOCK — INTERCEPT LOCKED RESPONSE (bulk assign) */
      if (maybeShowLockedModalFromJson(json)) {
        throw new Error('__RMS_LOCKED__');
      }
      /* END RMS SOFT LOCK — INTERCEPT LOCKED RESPONSE (bulk assign) */
    ...
    .catch(function (err) {
      ...
      /* BEGIN RMS SOFT LOCK — SUPPRESS INLINE ERROR (bulk assign) */
      if (err && err.message === '__RMS_LOCKED__') {
        // Modal already shown; do not show inline error text.
        return;
      }
      /* END RMS SOFT LOCK — SUPPRESS INLINE ERROR (bulk assign) */
...
function getAssetPoints(u) {
  ...
  // WPForms-style hidden field naming (support assets)
  if (p == null && u && u.asset_total_cost != null) p = u.asset_total_cost;
...
function getAssetInitiative(u) {
  ...
  // WPForms-style hidden field naming
  if (v == null && u && u.asset_ini != null) v = u.asset_ini;

  // --- FIX FOR WASEN / SUPPORT ASSET INIT ---
  // assigned_units_json stores ini: 0 as a placeholder
  // If that happens AND stats.ini exists and is numeric, use stats.ini instead
  var topIni = asInt(v, 0);
  ...
  if (
    ...
    u.stats &&
    u.stats.ini != null &&
    /^[0-9]+$/.test(String(u.stats.ini).trim())
  ) {
    return asInt(u.stats.ini, 0);
  }
  // --- END FIX ---
...
function getAssetLeadership(u) {
  ...
  // WPForms-style hidden field naming
  if (v == null && u && u.asset_ldr != null) v = u.asset_ldr;
  ...
  function getAssignUnitsDisplayType(u) {
    ...
    // Support assets show name/title, BUT Sailing Ships should display subtype inferred from MOV (MVP: no subtype field stored)
    if (kind === 'support') {

      var clsRaw = String(u.cls || u.class || u.className || u.supportType || '').trim();
      var clsLow = clsRaw.toLowerCase();

// Sailing Ships subtype inference:
// Medium/Large => Kobaya (6″), Huge/Gargantuan => Sekibune (7″), Colossal => Atakebune (8″)
//
// NOTE: Owned-assets payloads may not include cls/className; fall back to name/title checks.
var nameLow = String(u.name || u.title || u.supportType || '').trim().toLowerCase();
var isShip =
  (clsLow.indexOf('sailing') !== -1 || clsLow.indexOf('ship') !== -1) ||
  (nameLow === 'wasen' || nameLow === 'mokuzo hansen');

if (isShip) {
var mvRaw = (u.stats && u.stats.mov) || u.mov || u.asset_mov || u.assetMov || '';

  var mvNum = parseInt(String(mvRaw).replace(/[^\d]/g, ''), 10);

  if (mvNum === 6) return 'Kobaya';
  if (mvNum === 7) return 'Sekibune';
  if (mvNum === 8) return 'Atakebune';
}
...
function getRosterAssignedQtyForAsset(assetOrUnit, currentQtyMap) {
  ...
  var ref = String(
    ...
    assetOrUnit.ref_id ||
    assetOrUnit.ref ||
    assetOrUnit.referenceId ||
    assetOrUnit.reference_id ||
    ''
  ).trim();
...
function buildPreviewAssignedList(rosterObj) {
  ...
  for (var i = 0; i < baseGrouped.length; i++) {
    ...
    // Default key from roster snapshot
    var k = String(u.unitKey || '').trim();
    if (!k) k = makeUnitKey(u);

    // Canonicalize to ownedAssets unitKey when possible (prevents Character key mismatch)
    if (Array.isArray(ownedAssets) && ownedAssets.length) {
      var uKind = normalizeKind(u.kind);
      var uRef = String(
        u.refId ||
        u.ref_id ||
        u.ref ||
      ...
      // Try to find matching owned asset; if found, use its unitKey
      for (var j = 0; j < ownedAssets.length; j++) {
        ...
        // 1) Exact key match
        if (ak && k && ak === k) { k = ak; break; }

        // 2) Identity match: kind + ref + cls
        var aKind = normalizeKind(a.kind);
        var aRef = String(
          a.refId ||
          a.ref_id ||
          a.ref ||
        ...
        if (uRef && aRef && uRef === aRef && uKind === aKind) {
          // If we have class/type info, require it to match; otherwise accept kind+ref match.
          if (!uCls || !aCls || uCls === aCls) { k = ak; break; }
        }
      }
    }
...
function buildAssignUnitsRowHtml(u, rosterObj, currentQtyMap, rosterHasDaimyo, selectedDaimyoKey) {
  ...
  var img = assignUnitsResolveCanonicalImg(u);
if (rmsIsLocked(u)) img = rmsLockImg();
...
// BEGIN RMS A3 — disable locked assets + keep existing Daimyo restrictions
var isLockedAsset = rmsIsLocked(u);

// Keep your existing Daimyo restriction logic, but ALSO force-disable if locked.
var isDisabled = false;

var thisKey = String(unitKey || '').trim();

// Daimyo rule: if a Daimyo is selected in this modal, disable all OTHER Daimyo rows.
// If the roster already has a Daimyo and none is selected, disable all Daimyo rows.
if (typeof isDaimyoRow !== 'undefined' && isDaimyoRow) {
  if (selectedDaimyoKey) {
    if (String(selectedDaimyoKey) !== thisKey) isDisabled = true;
  } else if (rosterHasDaimyo) {
    isDisabled = true;
  }
}

// RMS: locked assets always disabled
if (isLockedAsset) isDisabled = true;

// Checked state should reflect BOTH:
// 1) currently assigned qty (currentQtyMap)
// 2) staged qty in the modal (getAssignUnitsSelectedAddQty)
var curQtyForChecked = 0;
try { curQtyForChecked = asInt(currentQtyMap && (currentQtyMap[thisKey] || 0), 0); } catch (_) {}
var stagedQtyForChecked = 0;
try { stagedQtyForChecked = asInt(getAssignUnitsSelectedAddQty(thisKey), 0); } catch (_) {}

var checked = (!isDisabled && (stagedQtyForChecked > 0));
// END RMS A3
 ...
 // ============================================================
// NEW: Dim rules
// - Dim if unassigned AND toggle is OFF (dash rows)
// - Dim if DISABLED while unassigned (blocked rows)
// - Dim if currently assigned (curQty>0) EXCEPT Daimyo (keep active-looking)
// ============================================================
var isCurrentlyAssigned = (asInt(curQty, 0) > 0);

// If a row is disabled but is currently assigned, we generally DO NOT dim it
// Toggle ON always wins: never dim when checked (visual = “active”).
// UX: Dim rows ONLY when unassigned AND toggle is OFF.
// If current QTY >= 1 OR toggle is ON, row must NOT be dim.
var shouldDim = (curQty === 0 && !checked);

  var disabledTitle = '';
  if (isDisabled && isDaimyoRow) {
    if (rosterHasDaimyo) {
      disabledTitle = 'Disabled: This clan already has an assigned Daimyo.';
    } else if (selectedDaimyoKey && selectedDaimyoKey !== unitKey) {
      disabledTitle = 'Disabled: Only one Daimyo may be selected.';
    } else {
      disabledTitle = 'Disabled.';
    }
  }

  var titleAttr = disabledTitle ? (' title="' + esc(disabledTitle) + '"') : '';
  var dimAttr = shouldDim ? ' data-dim="1" style="opacity:0.55;filter:grayscale(1);"' : '';

  // Cursor behavior:
  // - Daimyo disabled rows: not-allowed
  // - other disabled rows (if any): default handled elsewhere, but keep pointer off
  var cursorStyle = '';
  if (isDisabled && isDaimyoRow) {
    cursorStyle = 'cursor:not-allowed;';
  }
  if (cursorStyle) {
    // If already dimming via style="", append cursor in same attribute.
    if (shouldDim) {
      dimAttr = ' data-dim="1" style="opacity:0.55;filter:grayscale(1);' + cursorStyle + '"';
    } else {
      dimAttr = ' style="' + cursorStyle + '"';
    }
  }

  return (
    '<div class="shoshin-assign-units-row' +
      (isDashState ? ' shoshin-assign-units-row-muted' : '') +
      (shouldDim ? ' shoshin-assign-units-row-dim' : '') +
...
var clsKey = String(asset.cls || asset.class || asset.className || '').toLowerCase().trim();
  ...
  var items = ownedAssets.slice().filter(function (u) {
    if (isAssignedFilter) {
      // Show only CURRENTLY assigned units for this roster.
      // IMPORTANT: Do NOT rely on direct unitKey map hits — Characters often mismatch unitKey shape.
      return (getRosterAssignedQtyForAsset(u, currentQtyMap) > 0);
    }
...
if (!items.length) {
  list.innerHTML =
    '<div class="shoshin-assets-empty shoshin-asset-empty-state">' +
      ...
      '<img ' +
        ...
        'class="shoshin-asset-empty-image"' +
      '/>' +
    '</div>';

  // IMPORTANT: Load More wrapper is outside list.innerHTML, so it can remain visible from a prior render.
  var loadMoreWrap = assignUnitsModal ? assignUnitsModal.querySelector('.shoshin-assign-units-load-more-wrap') : null;
  if (loadMoreWrap) loadMoreWrap.style.display = 'none';

  // Reset paging so the next non-empty filter starts correctly.
  if (assignUnitsCtx) assignUnitsCtx.visibleLimit = ASSIGN_UNITS_PAGE_SIZE;
...
function recomputeAssignUnitsPercentages() {
  ...
  for (var i = 0; i < afterAssigned.length; i++) {
    ...
    itemsUnits.push({ unitKey: k, rowTotal: qty });
        ...
        // Prefer stats.ini when it's numeric (characters store ini=0 placeholder at top-level)
    var iniStat = (u.stats && u.stats.ini != null) ? String(u.stats.ini).trim() : '';
    var ini = 0;
    if (iniStat && /^[0-9]+$/.test(iniStat)) {
      ini = asInt(iniStat, 0);
    } else {
      ini = asInt((u.ini != null ? u.ini : 0), 0);
    }
  ...
  function applyLargestRemainder(items, denom, selector) {
    ...
    for (var p = 0; p < items.length; p++) {
      ...
      var rem = exact - fl;
            ...
            floors.push({ unitKey: items[p].unitKey, floor: fl, rem: rem, has: (items[p].rowTotal > 0) });
    ...
    // Distribute remainder ONLY among rows that actually contribute (>0) to this metric.
    // This prevents 0-value rows (e.g., Wasen INI=0) from receiving 1% due to tie remainders.
    var eligible = [];
    for (var e = 0; e < floors.length; e++) {
      if (floors[e].has) eligible.push(floors[e]);
    }

    // denom !== 0 implies at least one contributor, but guard anyway.
    if (!eligible.length) eligible = floors;

    for (var a = 0; a < remaining; a++) {
      eligible[a % eligible.length].floor += 1;
    }
...
function ensurePrintModal() {
  ...
  var THUMB_LANDSCAPE = '/wp-content/uploads/2026/01/thumbland.jpg';
  var THUMB_PORTRAIT  = '/wp-content/uploads/2026/01/thumbport.jpg';
...
'<div class="shoshin-print-thumb-frame" style="background:transparent; padding:0;">' +
  '<img class="shoshin-print-thumb-img" src="' + THUMB_LANDSCAPE + '" alt="Layout #1" style="display:block; max-width:100%; height:auto; box-shadow: 0 8px 28px rgba(0,0,0,0.18);">' +
'</div>' +
...
'<div class="shoshin-print-thumb-frame" style="background:transparent; padding:0;">' +
  '<img class="shoshin-print-thumb-img" src="' + THUMB_PORTRAIT + '" alt="Layout #2" style="display:block; max-width:100%; height:auto; box-shadow: 0 8px 28px rgba(0,0,0,0.18);">' +
'</div>' +
...
syncModeToggles('init');
  ...
  // NOTE:
  // Consolidated/Comprehensive exclusivity is enforced ONLY by syncModeToggles()
  // via the direct checkbox listeners above.
  // Do not duplicate mode logic here (prevents both toggles being ON).
  // (Other toggles are independent; no delegated handling required.)
...
if (host) {
  ...
  var assigned = [];
  try {
    assigned = parseAssignedUnits(rosterObj, (typeof ownedIndex !== 'undefined' ? ownedIndex : null));
  } catch (_) {
    assigned = [];
  }
  ...
  // Prefer the same resolver used elsewhere (banner/icon fields vary by source)
var icon = '';
if (rosterObj) {
  icon = String(getRosterIcon(rosterObj) || '').trim();
  if (!icon) icon = String(getRosterBannerUrlFromRosterObj(rosterObj) || '').trim();
  if (!icon) icon = String(rosterObj.icon || rosterObj.icon_url || rosterObj.iconUrl || '').trim();
}
...
function parseAssignedUnits(rosterObj, ownedIndex) {
  try {
    const raw = rosterObj && rosterObj.assigned_units_json ? rosterObj.assigned_units_json : '';
    if (!raw) return [];

    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];

    // If no ownedIndex provided, return parsed as-is (legacy behavior)
    if (!ownedIndex) return parsed;

    // Rehydrate + prune:
    // - If an assigned row has entryId, and we have a current owned asset for it,
    //   override display fields (refId/name/img/icon/etc) from ownedIndex.
    // - If an assigned row has entryId but no longer exists in ownedIndex, drop it.
    const out = [];
    for (const row of parsed) {
      if (!row || typeof row !== 'object') continue;

      const eid = row.entryId ? parseInt(row.entryId, 10) : 0;

      // If it doesn't have an entryId, we can't validate it — keep it.
      if (!eid) {
        out.push(row);
        continue;
      }

      const live = ownedIndex[eid];

      // If the asset no longer exists (deleted in WPForms backend), prune it.
      if (!live) continue;

      // Keep operational identity from the roster row (unitKey, qty, kind, entryId)
      // but refresh display-ish fields from the live asset.
      const merged = { ...row };

      // Common display fields to refresh (only if present on live)
      if (live.refId != null) merged.refId = live.refId;
      if (live.name != null) merged.name = live.name;

      // Image fields (different parts of the app use different keys)
      if (live.img != null) merged.img = live.img;
      if (live.icon != null) merged.icon = live.icon;
      if (live.portrait != null) merged.portrait = live.portrait;

      // Stat-ish fields (safe to refresh; qty still comes from roster assignment)
      if (live.points != null) merged.points = live.points;
      if (live.ini != null) merged.ini = live.ini;
      if (live.initiative != null) merged.initiative = live.initiative;
      if (live.honor != null) merged.honor = live.honor;
...
var units = parseAssignedUnits(rosterObj, ownedIndex);
...
function buildCombatModifiersBlockHTML(unitEntryId, refId, qty, cost, sample) {
  ...
  var displayTitle =
    ...
    '';
      ...
      /* BEGIN SHOSHIN — PRINT L1: CHARACTER TITLE USE TYPE (CLS) */
  // Characters store name=refId, so displayTitle must use cls to avoid REFID/REFID duplication.
  try {
    if (sample && String(sample.kind || '').toLowerCase().trim() === 'character' && sample.cls) {
      displayTitle = String(sample.cls);
    }
  } catch (e) {}
  /* END SHOSHIN — PRINT L1: CHARACTER TITLE USE TYPE (CLS) */


      /* BEGIN SHOSHIN — PRINT L1: DISPLAY SHIP SUBTYPE (MVP) */
  // For support assets (especially Sailing Ships), use the unified display-type resolver.
  // This infers Kobaya/Sekibune/Atakebune from MOV when the stored name is just "Wasen".
  try {
    if (sample && String(sample.kind || '').toLowerCase().trim() === 'support') {
      var _t = getAssignUnitsDisplayType(sample);
      if (_t) displayTitle = _t;
    }
  } catch (e) {}
  /* END SHOSHIN — PRINT L1: DISPLAY SHIP SUBTYPE (MVP) */
...
var turnSeqHtml =
  '<div class="qr-tight">' +
    '<div><b>Initiative Phase:</b> Recalculate <b>total clan Initiative</b>. Player order = highest → lowest. Highest may choose to go last.</div>' +
    '<div><b>Movement Phase:</b> Alternate moving one unit at a time (by player order). One movement per activation.</div>' +
    '<div><b>Action Phase:</b> Alternate activating one unit to attack/use abilities/interact (by player order). One action per activation.</div>' +
    '<div><b>Resolve Conditions:</b> Resolve all active conditions.</div>' +
    '<div><b>End Round:</b> Proceed to the next round.</div>' +
  '</div>';

var moveEngageHtml =
  '<table class="qr-mini qr-move-types">' +
    '<thead>' +
      '<tr>' +
        '<th>Type</th>' +
        '<th>Effect</th>' +
    ...
    '<tbody>' +
      '<tr>' +
        '<td><b>March</b></td>' +
        '<td>Move up to base Movement.</td>' +
      '</tr>' +
      '<tr>' +
        '<td><b>Sprint</b></td>' +
        '<td>Move up to 2× base Movement; ends turn.</td>' +
      '</tr>' +
      '<tr>' +
        '<td><b>Charge</b></td>' +
        '<td>Move up to 1.5× base Movement to melee range; immediately make 1 melee attack; ends turn.</td>' +
      '</tr>' +
      '<tr>' +
        '<td><b>Climb</b></td>' +
        '<td>Movement must end on a horizontal surface.</td>' +
      '</tr>' +
      '<tr>' +
        '<td><b>Overwatch</b></td>' +
        '<td>Arquebus/Artillery may forgo movement to fire during enemy movement (interupts movement upon hit); no extended range.</td>' +
      '</tr>' +
    '</tbody>' +
  '</table>';

var moveModsTable =
  '<table class="qr-mini qr-move-mods">' +
    ...
    '<tbody>' +
      ...
      '<tr><td><b>Water<br>(Swim)</b></td><td>Small/Medium unarmored: −3&quot • Armored: requires Suieijutsu to move • Cavalry: max 4&quot • Large: halved • Huge+: unaffected • Enter terrain: non-Suieijutsu must end move at edge first.</td></tr>' +
    '</tbody>' +
  '</table>';
...
var combatModsTable =
  '<table class="qr-mini">' +
    ...
    '<tbody>' +
      ...
      '<tr><td>Cavalry Piercer (Yari/Naginata + Training)</td><td>+1 To-Hit vs cavalry</td></tr>' +
      '<tr><td>Flank (first friendly striking; engaged)</td><td>+1 To-Hit</td></tr>' +
...
var keyRulesHtml =
  '<div class="qr-tight">' +
    '<div><b>Game Setup:</b> WYSIWYG and pre-measuring optional; implement if consensus.</div>' +
    '<div><b>Movement:</b> Units may not pass through others; bases must go around.</div>' +
    '<div><b>Nighttime:</b> Game begins in daytime; night occurs every two rounds.</div>' +
    '<div><b>Disputes:</b> Roll d6; highest result decides.</div>' +
    '<div><b>Tiebreakers:</b> Higher Unit Initiative wins; if tied, roll d6; highest wins.</div>' +
    '<div><b>Halving:</b> Always round down; minimum 1.</div>' +
    '<div><b>Engagement:</b> Bases touching.</div>' +
    '<ul class="qr-ul">' +
      '<li>Units cannot make ranged attacks while engaged.</li>' +
      '<li>Units cannot be targeted by ranged attacks if engaged by 2+ enemy units.</li>' +
      '<li>Disengaging may provoke a melee <b>Opportunity Attack</b> (if applicable).</li>' +
    '</ul>' +
    '<div><b>Immunity:</b> Always wins out.</div>' +
    '<div><b>Contested:</b> Objects/assets can only be used if no enemy is engaged with them.</div>' +
...
var moraleHtml =
  '<div class="qr-tight">' +
    ...
    '<ul class="qr-ul">' +
      '<li>Daimyō removed from play = entire clan.</li>' +
      '<li>50% of clan units (round down) removed in a single round = entire clan.</li>' +
      '<li>Losing an objective = scenario-based.</li>' +
      '<li>Forced by abilities/auras = affected unit.</li>' +
    '</ul>' +
    '<div><b>Routed:</b> Unit must sprint toward nearest table edge; no actions. Leaving the table grants XP to routing player. Test Morale each round until passed.</div>' +
  '</div>';

var honorHtml =
  '<div class="qr-tight">' +
      '<div><b> Honor (XP) =</b> defeated enemy unit’s <b>Leadership</b>.</div>' +
    '<div><b>Honor (XP) gained when:</b></div>' +
    '<ul class="qr-ul">' +
      '<li>You reduce an enemy unit’s Body to 0 (removed from game).</li>' +
      '<li>A routed enemy flees off the table.</li>' +
      '<li>You capture an enemy (Restrained at end of game = <b>double XP</b>).</li>' +
    '</ul>' +
    '<div><b>No XP</b> is granted for voluntary fleeing.</div>' +

  '</div>';

var losTargetHtml =
  '<div class="qr-tight">' +
    '<div><b>Line of Sight (LOS):</b> Draw base edge → base edge.</div>' +
    '<div><b>Full Cover (≥50% obscured):</b> Target cannot be selected.</div>' +
    '<div><b>Area of Effect (AOE):</b> Partial overlap = affected; measure from base edge.</div>' +
    '<div><b>AOE Targeting Restrictions:</b> No effect if LOS is fully blocked by stone/metal or if the target is fully enclosed.</div>' +
  '</div>';
...
var col1 =
  '<div class="qr-col">' +
    ...
    outlineBlock('Morale System', moraleHtml) +
    outlineBlock('Gaining Honor (XP)', honorHtml) +
    outlineBlock('Line of Sight & Area of Effect', losTargetHtml) +
    tableBlock('Movement Types', moveEngageHtml) +
  ...
  var combatResHtml =
    '<ol class="qr-ol">' +
      '<li><b>To-Hit:</b> Roll d6 vs target <b>Defense</b> / <b>Resistance</b>; hit on <b>≥ TN</b> after modifiers.</li>' +
      '<li><b>Criticals:</b> Natural 6 = Critical Hit; natural 1 = Critical Miss. Modified 7+ = Auto Critical Hit; modified 0− = Auto Critical Miss.</li>' +
      '<li><b>Multi-Attack:</b> Roll attacks consecutively. First hit ends remaining attempts. A Critical Miss ends all attempts.</li>' +
      '<li><b>Lucky Hit:</b> If only a 6 can hit, a natural 6 hits with no critical effects</b>.</li>' +
      '<li><b>Resilience:</b> When critically hit, defender saves vs Defense (roll ≤ Defense). Success negates critical effect; treat as normal hit. Defense 6 = Resilience.</li>' +
      ...
      '<li><b>Water Combat:</b> Small/Medium units without Suieijutsu cannot attack in water.</li>' +
    '</ol>';

      var actionTypesTable =
    '<table class="qr-mini qr-actions">' +
      '<thead><tr><th>Category</th><th>Includes</th></tr></thead>' +
      '<tbody>' +
        '<tr><td><b>General</b></td><td>Capture or drop items; douse flames, manipulate or force objects; interact with scenario assets (doors, objectives).</td></tr>' +
        '<tr><td><b>Combat</b></td><td>Melee/ranged/opportunity attacks, Charge, Overwatch, Reload.</td></tr>' +
        '<tr><td><b>Abilities</b></td><td>Use abilities, Alert, Conceal, Arrest, Rescue, Decapitate.</td></tr>' +
      '</tbody>' +
    '</table>';
  ...
  var condTable =
    '<table class="qr-mini qr-conds">' +
      ...
      '<tbody>' +
        ...
        '<tr><td><b>Routed</b></td><td>≤ Leadership</td><td>Must sprint to nearest table edge; remove unit if it exits.</td></tr>' +
        '<tr><td><b>Stunned</b></td><td>—</td><td>Lose remaining movement/actions this round; remove at round end</td></tr>' +
        '<tr><td><b>Concentration</b></td><td>≤ Initiative</td><td>Loss of ability effect; Save only when damage is taken</td></tr>' +
        '<tr><td><b>Frightened</b></td><td>≤ Leadership</td><td>Suffer -1 To-Hit penalty</td></tr>' +
        '<tr><td><b>Poisoned</b></td><td>≤ Body<br>(current)</td><td>Suffer 1 damage</td></tr>' +
        '<tr><td><b>Bleeding</b></td><td>≤ Body<br>(current)</td><td>Suffer 1 damage; do not remove condition if save is successful</td></tr>' +
        '<tr><td><b>Burning</b></td><td>≤ Defense<br>or<br>General Action</td><td>Suffer 1 damage; lasts up to two consecutive rounds; water automatically removes condition.</td></tr>' +
        '<tr><td><b>Concealed (Alert)</b></td><td>≥ Initiative<br>(target unit)</td><td>Must be within 2&quot; of hidden unit</td></tr>' +
        '<tr><td><b>Restrained<br>(Arrest / Rescue)</b></td><td>≥ Initiative<br>(target unit)</td><td>Hojojutsu required; must be engaged; may rescue self.</td></tr>' +
        '<tr><td><b>Restrained<br>(Decapitate)</b></td><td>&gt; 1 To-Hit<br>(unmodified)</td><td>Katana or Nodachi required; must cause enough damage to kill Restrained unit; all enemy units within 3&quot; check Morale</td></tr>' +
  ...
  '<td>—</td>' +
  '<td>Artillery, Sailing, Flight, Unitization & Collective Movement → see the rulebook for these rules.</td>' +
'</tr>' +
...
var col2 =
  '<div class="qr-col">' +
    ...
    tableBlock('Action Types', actionTypesTable) +
...
.print-page[data-print-layout$="qr"] .qr-body{
  padding: 0.24in 0.24in 0.20in !important;
  position: relative;
}
...
/* TWO COLUMNS (AoC portrait by default). Collapse-height blocks. */
.print-page[data-print-layout$="qr"] .qr-cols{
  display: flex;
  gap: 24px;
  align-items: stretch;

  /* fill remaining height under header WITHOUT forcing overflow */
  flex: 1 1 auto;
  min-height: 0;

  /* Portrait QR: widen column span to align with header/logo edges */
  margin-left: -12px;
  margin-right: -12px;

  /* IMPORTANT: do NOT use negative bottom margin here (causes qr-page scroll overflow in print) */
  margin-bottom: 0;
}


/* Landscape QR: fit pass (DESIGN22-11) */
.print-page[data-print-layout="layout1-qr"] .qr-cols{
  gap: 16px;                 /* keep your current column breathing room */
  margin-left: -8px;        /* reclaim width (matches earlier "push to edge" intent) */
  margin-right: -8px;
}

/* === LAYOUT1-QR (LANDSCAPE SHEET) === */
/* Header tune */
.print-page[data-print-layout="layout1-qr"] .qr-title{
  font-size: 15px;           /* slightly smaller to reclaim height */
}


/* Section headings + body text tune */
.print-page[data-print-layout="layout1-qr"] .qr-sec-h{
  font-size: 11px;
  margin: 0 0 3px 0;         /* was 4px */
}

/* Outline line rhythm (landscape) */
.print-page[data-print-layout="layout1-qr"] .qr-sec-outline .qr-tight{
  font-size: 9.5px;
  line-height: 1.12;
}
.print-page[data-print-layout="layout1-qr"] .qr-sec-outline .qr-tight > div{
  margin: 0 0 1px 0;         /* was 2px */
}

/* Bullets: tighten just a touch (no style change) */
.print-page[data-print-layout="layout1-qr"] .qr-ul{
  margin: 2px 0 0 0;         /* was larger due to duplicated rules */
}
.print-page[data-print-layout="layout1-qr"] .qr-ul li{
  margin: 0 0 1px 0;
}

/* Table sections: reclaim padding + cell padding */
.print-page[data-print-layout="layout1-qr"] .qr-sec-table{
  padding: 5px;              /* was 6px */
}
...
/* Tight helpers (QR) — tight outline spacing + remove numbering (NO TABLE CHANGES) */
.print-page[data-print-layout$="qr"] .qr-sp6{ height: 6px; }

/* OUTLINE BLOCKS: tight, consistent rhythm */
.print-page[data-print-layout$="qr"] .qr-sec-outline .qr-sec-b{
  line-height: 1.15;
}

/* Any outline block using qr-tight: unify spacing between lines (tight) */
.print-page[data-print-layout$="qr"] .qr-sec-outline .qr-tight{
  font-size: 10px;
  line-height: 1.15;
}
.print-page[data-print-layout$="qr"] .qr-sec-outline .qr-tight > div{
  margin: 0 0 2px 0;
}
.print-page[data-print-layout$="qr"] .qr-sec-outline .qr-tight > div:last-child{
  margin-bottom: 0;
}

/* === QR COLUMN VERTICAL DISTRIBUTION (restore top/bottom alignment) === */
.print-page[data-print-layout$="qr"] .qr-cols{
  flex: 1 1 auto;   /* fill remaining space under header */
  min-height: 0;    /* allow flex children to size without forcing overflow */
}

.print-page[data-print-layout$="qr"] .qr-col{
  display: flex;
  flex-direction: column;
  justify-content: space-between; /* distribute any remaining vertical slack */
  flex: 1 1 0;
  min-height: 0;
}

/* ensure blocks themselves don't add extra "stack" margins */
.print-page[data-print-layout$="qr"] .qr-col > *{
  margin-top: 0;
  margin-bottom: 0;
}

/* === QR OUTLINE INTERNAL SPACING NORMALIZATION (fix Morale/LOS gaps) === */
.print-page[data-print-layout$="qr"] .qr-sec-outline ul{
  margin: 0;                 /* kills browser default margins */
}

.print-page[data-print-layout$="qr"] .qr-sec-outline .qr-ul{
  margin: 2px 0 0 0;         /* tiny, consistent lead-in before bullets */
}

/* If qr-sp6 exists inside outline blocks, keep it tight */
.print-page[data-print-layout$="qr"] .qr-sec-outline .qr-sp6{
  height: 4px;               /* was 6px earlier; reduces "random" looking gaps */
}


/* Ordered lists used as "outline": remove numbering + tight spacing */
.print-page[data-print-layout$="qr"] .qr-ol{
  margin: 0;
  padding-left: 0;
  list-style: none;          /* removes 1.,2.,3. */
}
.print-page[data-print-layout$="qr"] .qr-ol li{
  margin: 0 0 2px 0;
}
.print-page[data-print-layout$="qr"] .qr-ol li:last-child{
  margin-bottom: 0;
}

/* Bullet lists: keep bullets (per spec), but keep tight rhythm */
.print-page[data-print-layout$="qr"] .qr-ul{
  margin: 3px 0 0 0;
  padding-left: 16px;        /* keeps bullet indent */
}
.print-page[data-print-layout$="qr"] .qr-ul li{
  margin: 0 0 2px 0;
}
.print-page[data-print-layout$="qr"] .qr-ul li:last-child{
  margin-bottom: 0;
}


/* Ordered lists used as "outline": remove numbering + match spacing */
.print-page[data-print-layout$="qr"] .qr-ol{
  margin: 0;
  padding-left: 0;
  list-style: none;          /* removes 1.,2.,3. */
...
/* Bullet lists: keep bullets (per spec), but match rhythm */
.print-page[data-print-layout$="qr"] .qr-ul{
  margin: 4px 0 0 0;
  padding-left: 16px;        /* keeps bullet indent */
}
...
/* ===== BEGIN QR: widen first column for Movement tables ===== */
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-types th:nth-child(1),
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-types td:nth-child(1){
  width: 18.5%;
  white-space: nowrap;
}
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-types th:nth-child(2),
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-types td:nth-child(2){
  width: 70%;
}

.print-page[data-print-layout$="qr"] .qr-mini.qr-move-mods th:nth-child(1),
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-mods td:nth-child(1){
  width: 20%;
  white-space: nowrap;
}
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-mods th:nth-child(2),
.print-page[data-print-layout$="qr"] .qr-mini.qr-move-mods td:nth-child(2){
  width: 76%;
}
/* ===== END QR: widen first column for Movement tables ===== */
...
/* BEGIN SHOSHIN PRINT — QR1 LANDSCAPE FINAL FIT OVERRIDES (DESIGN22-12) */

/* ====================== BEGIN QR1 LOGO PLACEMENT FIX (DESIGN22-14) ====================== */
.print-page[data-print-layout="layout1-qr"] .qr-logo{
  position: absolute;
  left: 0;
  top: 6px;                 /* whitespace above logo */
  transform: none;          /* remove vertical centering */
  height: 40px;             /* match other logos */
  width: auto;
}
/* ====================== END QR1 LOGO PLACEMENT FIX (DESIGN22-14) ====================== */



.print-page[data-print-layout="layout1-qr"] .qr-head{
  padding-top: 6px;
}



/* tighten the overall usable height */
.print-page[data-print-layout="layout1-qr"] .qr-body{
  padding: 0.22in 0.26in 0.18in !important;
}
...
.print-page[data-print-layout="layout1-qr"] .qr-sec-b{
  font-size: 10px;
  line-height: 1.14;
}
...
.print-page[data-print-layout="layout1-qr"] .qr-sec-outline .qr-tight > div:last-child{
  margin-bottom: 0;
}

/* IMPORTANT: force tight list spacing for LANDSCAPE (overrides duplicate rules above) */
.print-page[data-print-layout="layout1-qr"] .qr-ol{
  margin: 0;
  padding-left: 0;
  list-style: none;
}
.print-page[data-print-layout="layout1-qr"] .qr-ol li{
  margin: 0 0 1px 0;
}
.print-page[data-print-layout="layout1-qr"] .qr-ol li:last-child{
  margin-bottom: 0;
}
...
.print-page[data-print-layout="layout1-qr"] .qr-ul li:last-child{
  margin-bottom: 0;
}
...
.print-page[data-print-layout="layout1-qr"] .qr-mini{
  font-size: 8.8px;
}

/* Landscape QR — Action Types column rebalance */
.print-page[data-print-layout="layout1-qr"] 
table.qr-mini.qr-actions th:first-child,
.print-page[data-print-layout="layout1-qr"] 
table.qr-mini.qr-actions td:first-child{
  width: 18.5%;
}

.print-page[data-print-layout="layout1-qr"] 
table.qr-mini.qr-actions th:last-child,
.print-page[data-print-layout="layout1-qr"] 
table.qr-mini.qr-actions td:last-child{
  width: 62%;
}

/* Landscape QR — Conditions & Saving Throws column rebalance */
.print-page[data-print-layout="layout1-qr"]
table.qr-mini.qr-conds th:nth-child(1),
.print-page[data-print-layout="layout1-qr"]
table.qr-mini.qr-conds td:nth-child(1){
  width: 22%;
}

.print-page[data-print-layout="layout1-qr"]
table.qr-mini.qr-conds th:nth-child(2),
.print-page[data-print-layout="layout1-qr"]
table.qr-mini.qr-conds td:nth-child(2){
  width: 22%;
}

.print-page[data-print-layout="layout1-qr"]
table.qr-mini.qr-conds th:nth-child(3),
.print-page[data-print-layout="layout1-qr"]
table.qr-mini.qr-conds td:nth-child(3){
  width: 56%;
}



.print-page[data-print-layout="layout1-qr"] .qr-mini th,
.print-page[data-print-layout="layout1-qr"] .qr-mini td{
  padding: 2px 4px;
}

/* LANDSCAPE: use reclaimed inter-block space to give col2 tables more vertical breathing room */
.print-page[data-print-layout="layout1-qr"] .qr-col:nth-child(2) .qr-mini th,
.print-page[data-print-layout="layout1-qr"] .qr-col:nth-child(2) .qr-mini td{
  padding: 2px 4px;
}


/* tallest block: conditions */
.print-page[data-print-layout="layout1-qr"] .qr-conds{
  font-size: 8.4px;
  line-height: 1.02;
}

/* MINIMUM vertical breathing room between blocks (prevents "touching tables" when tight) */
.print-page[data-print-layout="layout1-qr"] .qr-col{
  gap: 1px;
}


/* END SHOSHIN PRINT — QR1 LANDSCAPE FINAL FIT OVERRIDES (DESIGN22-12) */
    ...
    confirmBtn.addEventListener('click', function () {
      ...
      postAjax('shoshin_set_unit_qty', {
        ...
        .catch(function (err) {
          ...
          /* BEGIN RMS SOFT LOCK — SUPPRESS INLINE ERROR (unassign) */
          if (err && err.message === '__RMS_LOCKED__') {
            // Modal already shown; do not show inline error text.
            return;
          }
          /* END RMS SOFT LOCK — SUPPRESS INLINE ERROR (unassign) */
...
if (curQ === 1) {
  ...
  if (descEl) {
    descEl.textContent =
      'Removing ' + label + (refId ? ' with REF ID ' + refId : '') +
  ...
  if (descEl) {
    descEl.textContent =
      'Select the quantity of ' + label + (refId ? ' with REF ID ' + refId : '') +
  ...
  if (refEl)   refEl.textContent   = refId ? ('' + refId) : '';
...
if (qtySel) {
  ...
  function applyRosterFilterAndPaging() {
    ...
    if (msgEl) {
      if (matching.length === 0) {
        msgEl.innerHTML =
          '<h2><em>You currently do not have any clans with these points totals.</em></h2>' +
    ...
    if (moreEl) {
      ...
      if (loadMoreBtn) {
        ...
        loadMoreBtn.textContent = remaining > 0 ? ('Load more (' + remaining + ')') : 'Load more';
      }
    }
  ...
  function getRosterIcon(r) {
    ...
    // Fallback roster icon (existing behavior)
    if (!iconUrl) iconUrl = '/wp-content/uploads/2025/12/Helmet-grey.jpg';

    /* BEGIN RMS SOFT LOCK — USE RMS_LOCK_ICON_URL (NO RMS_LOCK_ICON_URL) */
    // If caller sets r.locked, override iconUrl here (single place)
    // RMS_LOCK_ICON_URL is defined at top-scope by the resolver near the fallback paths.
    if (rmsIsLocked(r)) iconUrl = rmsLockImg();
    /* END RMS SOFT LOCK — USE RMS_LOCK_ICON_URL (NO RMS_LOCK_ICON_URL) */
  ...
  function renderAssignedStripRow(u, rosterEntryId, clanPointsForPercent, percentMap) {
    ...
    // BEGIN RMS C1 — determine roster lock state
var rosterObjC1 = getRosterObjByEntryId(rosterEntryId);
var isLockedRoster = rmsIsLocked(rosterObjC1);
// END RMS C1


   // BEGIN RMS C1 — padlock images if roster locked OR asset locked
var img = String(u.img || u.image || u.imgUrl || '').trim();

// Determine whether THIS assigned unit should be treated as locked.
// 1) roster lock always locks everything
// 2) unit snapshot may carry lock flags (locked/isLocked/etc.)
// 3) if snapshot does NOT carry lock flags, fall back to ownedAssets by unitKey
var isLockedUnit = false;

try { isLockedUnit = rmsIsLocked(u); } catch (_) {}
if (!isLockedUnit) {
  try { isLockedUnit = rmsIsLocked(u.unit); } catch (_) {}
}

if (!isLockedUnit) {
  try {
    var unitKeyForLock = String(u.unitKey || '').trim();
    if (!unitKeyForLock) unitKeyForLock = makeUnitKey(u);

    if (unitKeyForLock) {
      var ownedA = assignUnitsGetOwnedAssetByUnitKey(unitKeyForLock);
      if (ownedA && isRmsLockedAsset(ownedA)) isLockedUnit = true;
    }
  } catch (_) {}
}

if (isLockedRoster || isLockedUnit) img = rmsLockImg();
// END RMS C1

/* BEGIN RMS SOFT LOCK — REMOVE RMS_LOCK_ICON_URL (USE RMS_LOCK_ICON_URL ABOVE) */
// (No-op) This block intentionally removed.
// Padlock override is already handled via RMS_LOCK_ICON_URL earlier in this function.
/* END RMS SOFT LOCK — REMOVE RMS_LOCK_ICON_URL (USE RMS_LOCK_ICON_URL ABOVE) */
    if (!img) img = '/wp-content/uploads/2025/12/Helmet-grey.jpg';

    var kind = String(u.kind || '').toLowerCase().trim();

// Characters: show class (Daimyo/Samurai/etc.)
// Support: show the asset name (Ozutsu/Wasen/etc.)
var unitType;
if (kind === 'support') {
  unitType = String(u.name || u.title || u.supportType || u.cls || u.class || '').trim() || '—';

  // Sailing Ships / Wasen: display subtype by inferring from MOV (MVP: no schema changes)
  var clsKey = String(u.cls || u.class || u.className || u.supportType || u.name || u.title || '')
    .toLowerCase()
    .replace(/\s+/g, ' ')
    .trim();

  var isShip =
    (clsKey.indexOf('sailing') !== -1 || clsKey.indexOf('ship') !== -1) ||
    (clsKey === 'wasen' || clsKey === 'mokuzo hansen');

  if (isShip) {
    var movRaw = pick('mov', 'movement');
    var movNum = null;
    if (movRaw != null) {
      var s2 = String(movRaw).replace(/[^\d.]/g, '').trim();
      if (s2) movNum = parseFloat(s2);
    }

    if (movNum === 6) unitType = 'Kobaya';
    else if (movNum === 7) unitType = 'Sekibune';
    else if (movNum === 8) unitType = 'Atakebune';
  }
} else {
...
mov  = assignUnitsNormalizeQuotes(mov);
    ...
    // Row3 display rule (characters):
    // If ini resolves to 0 but stats.ini contains a positive number, use stats.ini.
    if (kind !== 'support') {
      var iniNum = asInt(ini, 0);
      if (iniNum === 0) {
        var statsIni2 = (u && u.stats && u.stats.ini != null) ? String(u.stats.ini).trim() : '';
        if (statsIni2 && isNumericLike(statsIni2) && asInt(statsIni2, 0) > 0) {
          ini = asInt(statsIni2, 0);
        }
      }
    }
...
'</td>' +
        '<td class="shoshin-assigned-actions-td" style="text-align:center;white-space:nowrap;">' +
          '<button type="button" class="shoshin-btn shoshin-btn-unassign" data-tooltip="Unassign Units" aria-label="Unassign Units" data-entry-id="' + esc(rosterEntryId) + '"' +
            ((isLockedRoster || isLockedUnit) ? ' data-rms-locked="1"' : '') +
          '>' +
...
// If totals are missing or incomplete, compute from assigned list
// (initiative was previously allowed to remain 0 even when units exist)
if (
  (clanPoints <= 0 || !asInt(r.unitCount, 0) || !asInt(r.initiative, 0)) &&
  Array.isArray(grouped) && grouped.length
) {
...
var masterClassAvail = Math.floor(clanPoints / 125);
    ...
    var iconUrl = getRosterIcon(r);
        ...
        /* BEGIN RMS SOFT LOCK — PADLOCK IMAGE OVERRIDE (Roster) */
    if (rmsIsLocked(r)) {
      iconUrl = rmsLockImg();
    }
    /* END RMS SOFT LOCK — PADLOCK IMAGE OVERRIDE (Roster) */
  ...
  listEl.addEventListener('click', function (evt) {
    ...
    /* BEGIN RMS SOFT LOCK — CAPTURE ROSTER CONTEXT FOR LOCKED MODAL */
try {
  var ctxCard = btn.closest('.shoshin-roster-card');
  if (ctxCard) {
    var ctxRid = asInt(ctxCard.getAttribute('data-roster-entry-id'), 0);
    if (ctxRid) setRmsLockedCtxFromRosterEntryId(ctxRid);
  }
} catch (e) {}
/* END RMS SOFT LOCK — CAPTURE ROSTER CONTEXT FOR LOCKED MODAL */
    ...
    if (btn.classList.contains('shoshin-btn-picture-roster')) {
      ...
      /* BEGIN RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (picture) */
try {
  setRmsLockedCtxFromRosterEntryId(ridPic);
  var rPic = getRosterObjByEntryId(ridPic);
  if (rPic && rPic.locked) {
    openRmsLockedModal(null);
    return;
  }
} catch (e) {}
/* END RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (picture) */
    ...
    if (btn.classList.contains('shoshin-btn-assign-roster')) {
      ...
      /* BEGIN RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (assign/bulk) */
try {
  setRmsLockedCtxFromRosterEntryId(ridAssign);
  var rAssign = getRosterObjByEntryId(ridAssign);
  if (rAssign && rAssign.locked) {
    openRmsLockedModal(null);
    return;
  }
} catch (e) {}
/* END RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (assign/bulk) */
...
if (btn.classList.contains('shoshin-btn-print')) {
  ...
  /* BEGIN RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (print) */
try {
  setRmsLockedCtxFromRosterEntryId(ridPrint);
  var rPrint = getRosterObjByEntryId(ridPrint);
  if (rPrint && rPrint.locked) {
    openRmsLockedModal(null);
    return;
  }
} catch (e) {}
/* END RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (print) */
...
// END SHOSHIN PRINT P1-A
    ...
    if (btn.classList.contains('shoshin-btn-delete')) {
      ...
      var descText = 'Deleting this clan roster is permanent and is not recoverable!';
      if (rName || rRef) {
        descText =
          'Delete "' + (rName || 'this clan roster') + '" with REF ID "' + (rRef || '—') + '"? ' +
          'Deleting this clan roster is permanent and is not recoverable!';
      }
      ...
      openDeleteModal(titleText, descText, function () {
        ...
        return postAjax('shoshin_delete_wpforms_entry', {
          ...
          // Was the deleted roster unlocked at time of delete?
          var wasLocked = false;
          try { wasLocked = rmsIsLocked(rosterObj); } catch (_) {}
          ...
          // If we deleted an UNLOCKED roster, promote the newest locked one to unlocked
          // AND re-render the promoted roster card so ROW1/ROW3 images update immediately.
          try {
            if (!wasLocked) {
              var promoted = rmsPromoteNextNewestLockedRoster();
              if (promoted) {
                var pid = asInt(promoted.entryId || promoted.id || promoted.entry_id, 0);
                if (pid) rmsRerenderRosterCardByEntryId(pid);
              }
            }
          } catch (_) {}
          ...
          // Re-apply filter/paging (will re-render with new lock states)
          if (typeof applyRosterFilterAndPaging === 'function') {
            applyRosterFilterAndPaging();
          }
        });
    ...
    if (btn.classList.contains('shoshin-btn-remove')) {
      ...
      var descText2 =
        'This will remove all quantities and the action cannot be undone.';

      if (unitLabel || unitRef) {
        var display2 = unitLabel || 'this unit';
        descText2 =
          'Remove ' + display2 + (unitRef ? ' with REF ID ' + unitRef : '') + ' from this clan? ' +
          'This will remove all quantities and the action cannot be undone.';
      }
      ...
      try {
        ...
        for (var kA = 0; kA < groupedA.length; kA++) {
          ...
          if (String(rowA.unitKey || '').trim() === unitKey) {
            ...
            // Image (padlock if locked)
var rowALocked = rmsIsLocked(rowA);
if (rowALocked) {
  imgA = String(rmsLockImg() || '').trim();
} else {
...
/* BEGIN RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (unassign) */
try {
  setRmsLockedCtxFromRosterEntryId(rosterEntryIdU);
  if (rosterObjU && rmsIsLocked(rosterObjU)) {
    openRmsLockedModal(null);
    return;
  }
} catch (e) {}
/* END RMS SOFT LOCK — PRE-BLOCK LOCKED ROSTER (unassign) */

/* BEGIN RMS SOFT LOCK — PRE-BLOCK LOCKED ASSET (unassign) */
try {
  var ownedU = assignUnitsGetOwnedAssetByUnitKey(unitKeyU);
  if (ownedU && rmsIsLocked(ownedU)) {

    // Override locked modal context to show LOCKED ASSET details (not roster)
    try { setRmsLockedCtxFromOwnedAsset(ownedU); } catch (_) {}

    openRmsLockedModal(null);
    return;
  }
} catch (e2) {}
/* END RMS SOFT LOCK — PRE-BLOCK LOCKED ASSET (unassign) */
...
var isDaimyoU = false;
  ...
  for (var jU = 0; jU < assignedGroupedU.length; jU++) {
    ...
    if (String(rowU && rowU.unitKey) === unitKeyU) {
      ...
      // For display:
      // - Characters: show class/type (NOT name, which may mirror refId after auto-heal)
      // - Supports: show name/title (Ozutsu, Mokuzo Hansen, etc.)
      var kindU = String(rowU.kind || '').trim();
      if (kindU === 'character') {
        unitLabelU = String(rowU.cls || rowU.class || '').trim();
      } else {
        // Support assets: Sailing Ships should display subtype inferred from MOV (MVP)
        var clsTmp = String(rowU.cls || rowU.class || rowU.className || rowU.supportType || '').trim();
        var clsLowTmp = clsTmp.toLowerCase();

        if (clsLowTmp.indexOf('sailing') !== -1 || clsLowTmp.indexOf('ship') !== -1) {
          var mvRaw2 = (rowU && rowU.stats && rowU.stats.mov != null) ? String(rowU.stats.mov) : String(rowU.mov || '');
          var mvNum2 = parseInt(String(mvRaw2).replace(/[^\d]/g, ''), 10);

          if (mvNum2 === 6) unitLabelU = 'Kobaya';
          else if (mvNum2 === 7) unitLabelU = 'Sekibune';
          else if (mvNum2 === 8) unitLabelU = 'Atakebune';
          else unitLabelU = String(rowU.name || rowU.title || rowU.supportType || rowU.cls || rowU.class || '').trim();
        } else {
          unitLabelU = String(rowU.name || rowU.title || rowU.supportType || rowU.cls || rowU.class || '').trim();
        }
      }
...
style.textContent =
  ...
  /* BEGIN SHOSHIN — Bulk Assign Empty State (scoped + force override) */
  '.shoshin-assets-empty.shoshin-asset-empty-state{' +
  ...
  '.shoshin-assets-empty.shoshin-asset-empty-state .shoshin-asset-empty-image{' +
    'display:block !important;' +
    ...
    'opacity:.9;' +
  '}' +
  /* END SHOSHIN — Bulk Assign Empty State */
...
document.head.appendChild(style);
    ...
    // BEGIN SHOSHIN — /my-rosters Bulk Assign — Muted Rows + Hover Lift
    '.shoshin-assign-units-row.shoshin-assign-units-row-muted{' +
      'opacity:.45 !important;' +
      'filter:grayscale(100%) !important;' +
      'transition:opacity .12s ease, filter .12s ease;' +
      'cursor:pointer;' +
    '}' +
    '.shoshin-assign-units-row.shoshin-assign-units-row-muted:hover{' +
      'opacity:.90 !important;' +
      'filter:none !important;' +
    '}' +
    // END SHOSHIN — /my-rosters Bulk Assign — Muted Rows + Hover Lift
