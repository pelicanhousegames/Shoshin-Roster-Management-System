...
// Hidden output fields (WPForms #2501)
const FIELD_ID_H_GAME_SYSTEM      = 20;
const FIELD_ID_H_M_DMG            = 21;
const FIELD_ID_H_M_CRT            = 22;
const FIELD_ID_H_M_DIS            = 23;
const FIELD_ID_H_R_DMG            = 24;
const FIELD_ID_H_R_CRT            = 25;
const FIELD_ID_H_R_DIS            = 26;
const FIELD_ID_H_ATK              = 27;
const FIELD_ID_H_DEF              = 28;
const FIELD_ID_H_MOV              = 29;
const FIELD_ID_H_BOD              = 30;
const FIELD_ID_H_LDR              = 31;
const FIELD_ID_H_INI              = 32;
const FIELD_ID_H_TOTAL_COST       = 33;
const FIELD_ID_H_MRBPA            = 34;
const FIELD_ID_H_RULES_MODS       = 35;
const FIELD_ID_H_SIZE             = 36;
const FIELD_ID_H_DIMENSION        = 37;
const FIELD_ID_H_RYU              = 38;
...
const SUPPORT_ASSET_IMAGES = {
  ...
  // Legacy alias (do not remove yet; prevents regressions if any markup/entries still use it)
...
const SUPPORT_ASSET_STATS = {
  ...
  // ==================================================================
    ...
    lastMunitionCount: 0 // tracks how many munitions are checked so we can detect "new check"
...
function getSingleSelectText(fieldId) {
  const el = $(supportFieldSelector(fieldId));
  if (!el) return '';

  // WPForms hidden/text inputs
...
function getSingleSelectValue(fieldId) {
  ...
  function computeHullStats(width, length) {
    ...
    const movementByCategory = {
      Medium: 6, Large: 6, Huge: 7, Gargantuan: 7, Colossal: 8
    };
...
// Vessel identity: new canonical (Wasen) + legacy alias (Mokuzo Hansen)
const isWasen   = assetName === 'Wasen';
const isMokuzo  = assetName === 'Mokuzo Hansen';
const isVessel  = isWasen || isMokuzo;

// Derive subtype from Hull Size category (Field #13)
const hullCat = String(state.hullSizeCategory || '').trim();
let vesselSubtype = '';
if (isVessel) {
  if (hullCat === 'Medium' || hullCat === 'Large') vesselSubtype = 'Kobaya';
  else if (hullCat === 'Huge' || hullCat === 'Gargantuan') vesselSubtype = 'Sekibune';
  else if (hullCat === 'Colossal') vesselSubtype = 'Atakebune';
}

// Display name (summary): show subtype if available; otherwise show the chosen asset name
const displayName = (isVessel && vesselSubtype) ? vesselSubtype : assetName;

// Meta key (rules/training tables may still reference Wasen/Mokuzo; subtype shares same rules)
const vesselTableKey = isWasen ? 'Wasen' : (isMokuzo ? 'Mokuzo Hansen' : '');
const tableKey = isVessel ? 'Wasen' : assetName; // preferred forward-facing key


    // Field visibility
const fields = {
  vesselSize: { visible: isVessel, disabled: !isVessel, locked: false, reasons: !isVessel ? ['asset_mismatch'] : [] },
  vesselDim:  { visible: isVessel, disabled: !isVessel, locked: false, reasons: !isVessel ? ['asset_mismatch'] : [] },

  detachment: { visible: isTaiho, disabled: !isTaiho, locked: false, reasons: !isTaiho ? ['asset_mismatch'] : [] },
  munitions:  { visible: isTaiho, disabled: !isTaiho, locked: false, reasons: !isTaiho ? ['asset_mismatch'] : [] },
  crewHtml:   { visible: isTaiho }
    };
    ...
    // Vessel Dim options:
const hullSizeChosen = isVessel && !!state.hullSizeCategory;
    ...
    dimBase.forEach(o => {
      ...
      if (isPlaceholder) {
        dropdowns.vesselDim[val] = {
          value: val,
          baseText: txt,
          visible: fields.vesselDim.visible || isTaiho,
      ...
      // Taiho special allowed dimension: 1×2
      const isTaihoDim = isTaiho && /1\s*[×x]\s*2/i.test(txt);
  ...
  reasons = ['asset_mismatch'];
      ...
      dropdowns.vesselDim[val] = {
        value: val,
        baseText: txt,
        visible: fields.vesselDim.visible ? visible : isTaihoDim,
...
if (isVessel && state.dimLabel) {
    ...
    const selectedMunitions = isTaiho ? state.selectedMunitions : [];

    // Detachment display-only
    const detachmentApplied = isTaiho && state.detachmentUserSet && !!state.detachmentMode;
...
if (isVessel) {
    ...
    // Summary
// For vessels, meta/images/stats are keyed off Wasen (or legacy Mokuzo), while summary name can be subtype
const metaKey = isVessel ? (SUPPORT_ASSET_META['Wasen'] ? 'Wasen' : vesselTableKey) : assetName;
const meta = SUPPORT_ASSET_META[metaKey] || null;

let summary = {
  name: assetName ? displayName : 'Select a Support Asset',
      ...
      sizeText: '--',
      imageSrc: SUPPORT_ASSET_IMAGES[metaKey] || SUPPORT_ASSET_IMAGES.default || ''
    };
    ...
    const trainingReq = !assetName ? ['None'] : (isTaiho ? ['Kayakujutsu'] : (isVessel ? ['Suieijutsu'] : ['None']));
    const equipItems  = !assetName ? ['None'] : (isTaiho ? (selectedMunitions.length ? selectedMunitions : ['None']) : ['None']);

    // Table filters
const tables = {
  // For vessels, rules/training are shared; keep the key as Wasen (forward) while allowing legacy rows in render
  training: { enabled: !!assetName, assetName: isVessel ? 'Wasen' : assetName },
  rules:    { assetName: isVessel ? 'Wasen' : assetName },
  munitions:{ showSection: isTaiho, selected: selectedMunitions }
};


    // Hidden fields
    const hidden = {
      assetEquipItems: isTaiho && selectedMunitions.length ? selectedMunitions.join(', ') : ''
    };
    ...
    // Taiho: Reset Detachment/Crew (field #19) anytime the user CHECKS an additional munition.
    // We detect this by watching the selected munition count increase.
    const munCount = (state.selectedMunitions && state.selectedMunitions.length) ? state.selectedMunitions.length : 0;

    if (isTaiho) {
      const prevCount = Number(runtime.lastMunitionCount || 0);

      // Count increased => user checked a new munition
      if (munCount > prevCount) {
        patch.ensureDetachmentPlaceholder = true;
      }

      // Update tracker every pass
      runtime.lastMunitionCount = munCount;
    } else {
      // Not Taiho => keep tracker clean
      runtime.lastMunitionCount = 0;
    }
    ...
    // Mokuzo defaults
if ((ctx.isInit || assetChanged) && isVessel) {
...
isVessel,
  ...
  const key = model.tables.rules.assetName;
  // Vessel compatibility: allow legacy Mokuzo rules rows while we transition markup
  const show = (asset === key) || (key === 'Wasen' && asset === 'Mokuzo Hansen');
...
const key = model.tables.training.assetName;
const show = !allowedAssets.length
  || allowedAssets.includes(key)
  // Vessel compatibility: allow legacy Mokuzo training rows while we transition markup
  || (key === 'Wasen' && allowedAssets.includes('Mokuzo Hansen'));
  ...
  function renderMunitionsSectionAndTable(model) {
    ...
    // Surgical: only toggle the message element itself (do NOT hide field #12 container)
...
// Always compute "selected" from the live DOM (Field #8 is the sole source of truth)
const selected = $all(supportFieldSelector(FIELD_ID_MUNITIONS) + ' input[type="checkbox"]:checked')
  .map(cb => String(cb.value || '').trim())
  .filter(Boolean);
    ...
    // LAST WRITE: toggle ONLY the message div
const shouldShowEmpty = !!model.tables.munitions.showSection && selected.length === 0;
if (emptyMsg) emptyMsg.style.setProperty('display', shouldShowEmpty ? 'block' : 'none', 'important');
  }

function renderHiddenFields(model) {
  // Helper: set hidden field safely
  function setHidden(fieldId, val) {
    const el = $(supportFieldSelector(fieldId));
    if (!el) return;
    el.value = (val === null || val === undefined) ? '' : String(val);
  }

  // Helper: format dimension label into inches with quotes: 4 × 11 => 4" × 11"
  function formatDimInches(label) {
    const m = String(label || '').match(/(\d+)\s*[×x]\s*(\d+)/i);
    if (!m) return String(label || '').trim();
    return m[1] + '" × ' + m[2] + '"';
  }

  const assetName = model.assetName || '';
  const isTaiho  = !!model.isTaiho;
  const isVessel  = !!model.isVessel;

  // Source selections (display text)
  const hullSizeText = getSingleSelectText(FIELD_ID_VESSEL_SIZE);
  const dimText      = getSingleSelectText(FIELD_ID_VESSEL_DIM);

  // Weapon stats (Taiho only)
  const wpn = model.weaponStats || { damage: '--', critical: '--', distance: '--' };

  // Ryu (training requirement)
  const ryu = !assetName ? '' : (isTaiho ? 'Kayakujutsu' : (isVessel ? 'Suieijutsu' : ''));

  // MRBPA modifiers: newline-separated (matches your backend view)
  const mods = Array.isArray(model.modifiers) ? model.modifiers : [];
  const mrbpaText = mods.length ? mods.join('\n') : '';

  // Core stats
  // Note: model.panelStats already reflects detachment effects for Taiho.
  const ps = model.panelStats || {};
  const mov = ps.movement || '--';
  const bod = ps.toughness || '--';

  // Cost
  const totalCost = (typeof model.totalCost === 'number' && model.totalCost > 0) ? String(model.totalCost) : '';

  // Write hidden fields
  setHidden(FIELD_ID_H_GAME_SYSTEM, getSingleSelectText(FIELD_ID_GAME_SYSTEM) || 'Path of Ascension');

  // Melee is always '--' for current support assets
  setHidden(FIELD_ID_H_M_DMG, '--');
  setHidden(FIELD_ID_H_M_CRT, '--');
  setHidden(FIELD_ID_H_M_DIS, '--');

  // Ranged weapon stats (Taiho only; otherwise '--')
  setHidden(FIELD_ID_H_R_DMG, isTaiho ? (wpn.damage || '--') : '--');
  setHidden(FIELD_ID_H_R_CRT, isTaiho ? (wpn.critical || '--') : '--');
  setHidden(FIELD_ID_H_R_DIS, isTaiho ? (wpn.distance || '--') : '--');

  // Core statline
  setHidden(FIELD_ID_H_ATK, ps.attack || '--');
  setHidden(FIELD_ID_H_DEF, ps.resistance || '--');
  setHidden(FIELD_ID_H_MOV, mov || '--');
  setHidden(FIELD_ID_H_BOD, bod || '--');
  setHidden(FIELD_ID_H_LDR, ps.leadership || '--');
  setHidden(FIELD_ID_H_INI, ps.initiative || '--');

  // Totals / text blocks
  setHidden(FIELD_ID_H_TOTAL_COST, totalCost);
  setHidden(FIELD_ID_H_MRBPA, mrbpaText);

  // Rules mods: leave existing WPForms-driven value alone unless you explicitly want JS to own it.
  // (We do NOT overwrite it here to avoid regressions.)
  // setHidden(FIELD_ID_H_RULES_MODS, ...);

// Size + dimension:
// - Vessels: persist selected hull size + dimension
// - Taiho: default size to Large and dimension to 1" × 2"
if (isVessel) {
  setHidden(FIELD_ID_H_SIZE, hullSizeText || '');
  setHidden(FIELD_ID_H_DIMENSION, dimText ? formatDimInches(dimText) : '');
} else if (isTaiho) {
  setHidden(FIELD_ID_H_SIZE, 'Large');
  setHidden(FIELD_ID_H_DIMENSION, '1" × 2"');
} else {
  setHidden(FIELD_ID_H_SIZE, '');
  setHidden(FIELD_ID_H_DIMENSION, '');
}


  // Ryu + equipment items
  setHidden(FIELD_ID_H_RYU, ryu);

  // Equip items: Taiho munitions list (already in your model.hidden)
  const equipText = (model.hidden && model.hidden.assetEquipItems) ? model.hidden.assetEquipItems : '';
  setHidden(FIELD_ID_ASSET_EQUIP_ITEMS, equipText);
}
  ...
  function applyStatePatch(model) {
    ...
    if (patch.ensureDetachmentPlaceholder && model.isTaiho) {
      setSelectPlaceholder(FIELD_ID_DETACHMENT);
      runtime.detachmentUserSet = false;
      mutated = true;
    }
...
if (patch.setHullSizeToMedium && model.isVessel) {
...
if (patch.autoPickDim1x3 && model.isVessel) {
...
// ==================================================================
// MUNITIONS EMPTY MESSAGE — POST-RENDER REASSERT (WPForms redraw safe)
// ==================================================================
let _munitionsReassertTimer = null;

function reassertMunitionsEmptyMessageSoon() {
  if (_munitionsReassertTimer) {
    clearTimeout(_munitionsReassertTimer);
    _munitionsReassertTimer = null;
  }

  _munitionsReassertTimer = setTimeout(function () {
    _munitionsReassertTimer = null;

    const emptyMsg = document.getElementById('shoshin-munitions-empty');
    if (!emptyMsg) return;

    // Only relevant when Taiho is selected
    const assetName = canonicalLabel(getSingleSelectText(FIELD_ID_ASSET_TYPE));
    const isTaiho = assetName === 'Taiho';

    // Field #8 is the sole source of truth
    const selected = $all(supportFieldSelector(FIELD_ID_MUNITIONS) + ' input[type="checkbox"]:checked')
      .map(cb => String(cb.value || '').trim())
      .filter(Boolean);

    const shouldShow = isTaiho && selected.length === 0;

    // Force-write (important) because WPForms may rewrite styles during redraw
    emptyMsg.style.setProperty('display', shouldShow ? 'block' : 'none', 'important');
  }, 0);
}
  ...
  function initShoshinSupportAssetTool() {
    ...
    if (typeSelect) {
        ...
        runtime.detachmentUserSet = false;
        runtime.lastMunitionCount = 0; // reset munition count tracker on asset change
    ...
    // Munitions change (delegated to the FORM so it survives WPForms rerenders)
formEl.addEventListener('change', function (e) {
  const t = e && e.target;
  if (!t || !t.matches) return;

  // Only react to checkbox changes inside Field #8
  if (t.matches('input[type="checkbox"]') && t.closest(supportFieldSelector(FIELD_ID_MUNITIONS))) {

    // RULE: Any time a munition is SELECTED, reset Field #19 to default.
    if (t.checked) {
      setSelectPlaceholder(FIELD_ID_DETACHMENT);
      runtime.detachmentUserSet = false;
    }
    ...
    // WPForms can redraw after our render; reassert the empty message one tick later.
    reassertMunitionsEmptyMessageSoon();
  }
