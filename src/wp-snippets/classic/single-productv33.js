(function () {
  function initShoshinCappedGallery() {
    ...
    var thumbItems = thumbs ? Array.prototype.slice.call(thumbs.querySelectorAll('li')) : [];

    /*
      IMPORTANT:
      Multi-image galleries can initialize in phases. Do not classify as
      single-image just because thumbs are not ready yet.
    */
    ...
    /* Multi-image gallery: always clear stale single-image state */
    if (gallery.classList) {
      gallery.classList.remove('shoshin-single-image-gallery');
    }

    /*
      If thumbs are not ready yet, retry shortly instead of bailing into
      the single-image path.
    */
    if (!thumbs || !thumbItems.length) {
      window.setTimeout(initShoshinCappedGallery, 120);
      window.setTimeout(initShoshinCappedGallery, 350);
      window.setTimeout(initShoshinCappedGallery, 800);
      return;
    }

    // Defer duplicate guard until we confirm full readiness
    var visibleCount = 5;
    var hiddenCount = Math.max(0, thumbItems.length - visibleCount);

    // If nothing to cap, we still mark as enhanced and exit
    if (hiddenCount <= 0) {
      thumbs.dataset.shoshinEnhanced = '1';
      return;
    }

    // Avoid duplicate init ONLY after we know we need to enhance
  ...
  function initShoshinVariableTopPrice() {
    ...
    var resolvedVariationState = null;
    ...
    function applyPurchaseUiState() {
      ...
      if (window.shoshinExpressDividerController && typeof window.shoshinExpressDividerController.refresh === 'function') {
        ...
        function syncPurchaseUiFromResolvedVariation() {
      if (!resolvedVariationState) {
        syncPurchaseUiFromDom();
        return;
      }

      if (!resolvedVariationState.isPurchasable) {
        purchaseUiState = 'hidden';
      } else if (resolvedVariationState.isZeroPrice || currentDisplayedPriceIsZero()) {
    ...
    function syncPurchaseUiFromDom() {
      if (resolvedVariationState && resolvedVariationState.variationId) {
        syncPurchaseUiFromResolvedVariation();
        return;
      }
    ...
    if (window.jQuery) {
      ...
      $form.on('found_variation show_variation', function (evt, variation) {
        ...
        resolvedVariationState = {
          variationId: variation && variation.variation_id ? String(variation.variation_id) : '',
          isPurchasable: !!(variation && variation.is_purchasable && variation.is_in_stock !== false),
          isZeroPrice: !!(
            variation &&
            typeof variation.display_price !== 'undefined' &&
            parseFloat(variation.display_price) <= 0
          )
        };

        setTimeout(function () {
          syncPurchaseUiFromResolvedVariation();
          scheduleVariableUiSync(220);
      ...
      $form.on('reset_data hide_variation woocommerce_variation_has_changed', function () {
        setTimeout(function () {
          ...
          if (!variationId || variationId === '0') {
            resolvedVariationState = null;
            restoreDefaultPriceBlock();
            restoreDefaultStockBlock();
          }

          scheduleVariableUiSync(120);
          queueVariableUiSyncPasses([250, 700]);
        }, 120);
      });
    }
...
function runInit() {
  ...
  window.setTimeout(initShoshinCappedGallery, 150);
  window.setTimeout(initShoshinCappedGallery, 400);
  window.setTimeout(initShoshinCappedGallery, 900);
  ...
  window.addEventListener('load', function () {
    window.setTimeout(initShoshinCappedGallery, 100);
    window.setTimeout(initShoshinCappedGallery, 350);
  });
