...
function isCustomMetaValue(el) {
  ...
  return (
    ...
    txt.indexOf('Available for Purchase') === 0 ||
    txt.indexOf('Instant Download') === 0 ||
    txt.indexOf('Out of Stock') === 0 ||
    txt.indexOf('Currently Unavailable') === 0
  );
}
...
function getLineType(line, index) {
  ...
  if (index === 0) {
    ...
    if (value === 'Available for Purchase') {
      return 'availability purchase';
    }
  ...
  if (
    value.indexOf('Produced in a batch run') !== -1 ||
  ...
  if (
    value.indexOf('📦 Fulfilled by Pelican House Games') === 0 ||
    value.indexOf('📦 Made to order — fulfilled by our partner') === 0
  ) {
    return 'fulfillment';
  }

  if (
    value.indexOf('Instant digital delivery') !== -1 ||
    value.indexOf('📥 Instant digital delivery') === 0
  ) {
    return 'digital';
  }
  ...
  function requestMiniCartQuantityUpdate(control, nextQty) {
    ...
    rows.forEach(function (row) {
      ...
      if (!priceWrap || !removeLink || !qtyText) return;

      var qtyInput = row.querySelector('.elementor-menu-cart__product-price input.qty');
      var isReadOnlyQty =
        !qtyInput ||
        qtyInput.type === 'hidden' ||
        qtyInput.disabled ||
        qtyInput.readOnly ||
        qtyInput.closest('.hidden') !== null;
  ...
  function initMiniCartQtyHydration() {
    ...
    document.addEventListener('wc_fragments_refreshed', syncCartUI, true);
...
function syncLimitedDownloadButtonsFromCartState() {
  var lockedDownloadProductUrls = {};
  var lockedDownloadProductIds = {};

  document.querySelectorAll('.elementor-menu-cart__product').forEach(function (row) {
    var nameLink = row.querySelector('.elementor-menu-cart__product-name a[href]');
    var removeLink = row.querySelector('.elementor-menu-cart__product-remove a[data-product_id]');
    var qtyInput = row.querySelector('.elementor-menu-cart__product-price input.qty');
    var isLockedSingleCopy =
      !qtyInput ||
      qtyInput.type === 'hidden' ||
      qtyInput.disabled ||
      qtyInput.readOnly ||
    ...
    if (!isLockedSingleCopy) return;

    if (nameLink) {
      var href = nameLink.getAttribute('href') || '';
      if (href) {
        lockedDownloadProductUrls[href] = true;
      }
    }

    if (removeLink) {
      var pid = removeLink.getAttribute('data-product_id') || '';
      if (pid) {
        lockedDownloadProductIds[String(pid)] = true;
      }
    }
  });
  ...
  document.querySelectorAll(
    ...
    var btn = card.querySelector('a.add_to_cart_button[data-product_id], a.button.product_type_simple[data-product_id]');
    if (!btn) return;

    var productId = btn.getAttribute('data-product_id') || '';
    var productLink =
      card.querySelector('a.woocommerce-LoopProduct-link[href], h2 a[href], .woocommerce-loop-product__title a[href]') ||
      card.querySelector('a[href]');
    var href = productLink ? (productLink.getAttribute('href') || '') : '';

    var isDownloadCard =
      card.classList.contains('product_cat-downloads') ||
      !!card.querySelector('.posted_in a[href*="/product-category/downloads"]') ||
      !!card.querySelector('.product_cat-downloads');

    if (!isDownloadCard) {
      btn.classList.remove('disabled', 'shoshin-disabled-add-to-cart', 'is-locked', 'added');
      btn.classList.add('ajax_add_to_cart', 'add_to_cart_button');
      btn.removeAttribute('aria-disabled');
      btn.removeAttribute('tabindex');
      btn.removeAttribute('aria-label');
      btn.onclick = null;

      if ((btn.textContent || '').trim() === 'In Cart' || (btn.textContent || '').indexOf('Add to Cart') !== -1) {
    ...
    var inCart =
      (!!productId && !!lockedDownloadProductIds[String(productId)]) ||
      (!!href && !!lockedDownloadProductUrls[href]);

    if (inCart) {
      btn.classList.remove('ajax_add_to_cart', 'added');
      btn.classList.add('disabled', 'shoshin-disabled-add-to-cart', 'is-locked');
      ...
      btn.style.setProperty('background', '#e2e2e2', 'important');
      btn.style.setProperty('color', '#777', 'important');
    ...
    btn.removeAttribute('aria-label');
    btn.onclick = null;
    btn.setAttribute('href', '?add-to-cart=' + productId);
...
function unlockLiveSingleQuantityCtas(productId) {
  if (!productId) return;

  var form = document.querySelector('.single-product form.cart');
  if (!form) return;

  var hiddenAdd = form.querySelector('[name="add-to-cart"]');
  var currentProductId = hiddenAdd ? (hiddenAdd.value || '') : '';
  if (!currentProductId || String(currentProductId) !== String(productId)) return;
  ...
  document.querySelectorAll(selector).forEach(function (btn) {
    ...
    btn.classList.remove('disabled', 'shoshin-disabled-add-to-cart', 'is-locked', 'added', 'shoshin-added-to-cart', 'loading');
    btn.removeAttribute('aria-disabled');
    btn.removeAttribute('tabindex');
  ...
  if (qty) {
    qty.readOnly = false;
    qty.removeAttribute('aria-readonly');
  }

  form.classList.remove('shoshin-download-in-cart');
  form.classList.add('shoshin-non-download-cart-state');
}

function isSingleProductDownloadable() {
  return (
    !!document.querySelector('.single-product .product_cat-downloads') ||
    !!document.querySelector('.single-product .posted_in a[href*="/product-category/downloads"]') ||
    !!document.querySelector('.single-product .product-type-downloadable') ||
    !!document.querySelector('.single-product .product.downloadable') ||
    !!document.querySelector('.single-product .product.downloadable-product')
  );
}

function singleProductCartContainsCurrentDownload() {
  var form = document.querySelector('.single-product form.cart');
  ...
  document.querySelectorAll('.elementor-menu-cart__product').forEach(function (row) {
    ...
    var rowProductId = removeLink ? (removeLink.getAttribute('data-product_id') || '') : '';
    var rowHref = nameLink ? (nameLink.getAttribute('href') || '') : '';
    var rowPath = rowHref ? rowHref.replace(/^https?:\/\/[^/]+/i, '').replace(/\/+$/, '') : '';

    if (
      (rowProductId && String(rowProductId) === String(currentProductId)) ||
      (rowPath && rowPath === currentUrl)
    ) {
      inCart = true;
    }
  });
...
function syncSingleProductPurchaseStateFromCartTruth() {
  ...
  var currentProductId = hiddenAdd ? (hiddenAdd.value || '') : '';
  if (!currentProductId) {
    refreshSingleProductExpressState();
    return;
  }

  if (!isSingleProductDownloadable()) {
    unlockLiveSingleQuantityCtas(currentProductId);
    refreshSingleProductExpressState();
    return;
  }

  if (singleProductCartContainsCurrentDownload()) {
    disableLiveSingleQuantityCtas(currentProductId);
  } else {
    unlockLiveSingleQuantityCtas(currentProductId);
  }
...
function disableLiveSingleQuantityCtas(productId) {
  ...
  var productTypeHint = document.querySelector('.single-product .product');
  var isDownloadProduct =
    !!document.querySelector('.single-product .product_cat-downloads') ||
    !!document.querySelector('.single-product .posted_in a[href*="/product-category/downloads"]') ||
    !!document.querySelector('.single-product .product-type-downloadable') ||
    !!document.querySelector('.single-product .product.downloadable');

  if (!isDownloadProduct && productTypeHint && !productTypeHint.classList.contains('downloadable')) {
    return;
  }
  ...
  document.querySelectorAll(selector).forEach(function (btn) {
    ...
    btn.classList.remove('ajax_add_to_cart', 'added', 'shoshin-added-to-cart', 'loading', 'disabled');
    btn.classList.add('shoshin-disabled-add-to-cart', 'is-locked');
    btn.removeAttribute('aria-disabled');
    btn.removeAttribute('tabindex');
    btn.removeAttribute('aria-busy');
    btn.removeAttribute('disabled');
  ...
  form.classList.add('shoshin-download-in-cart');
  form.classList.remove('shoshin-non-download-cart-state');
}

function refreshSingleProductExpressState() {
  var form = document.querySelector('.single-product form.cart');
  if (!form) return;

  var payPalNodes = document.querySelectorAll(
    ...
    '.single-product .paypal-button-number-0, ' +
    '.single-product .paypal-button-number-1, ' +
    '.single-product iframe.component-frame'
  );

  var stripeWraps = document.querySelectorAll(
    '.single-product #wc-stripe-express-checkout-element, ' +
    '.single-product .wcpay-express-checkout-wrapper'
  );

  payPalNodes.forEach(function (el) {
    if (!el) return;
    el.style.opacity = '';
    el.style.pointerEvents = '';
    el.style.filter = '';
    el.style.visibility = '';
    el.classList.remove('disabled', 'is-disabled', 'ppcp-disabled');
    el.removeAttribute('aria-disabled');
    el.removeAttribute('disabled');
  });

  stripeWraps.forEach(function (el) {
    if (!el) return;
    el.style.opacity = '';
    el.style.pointerEvents = '';
    el.style.filter = '';
...
function syncCartUI() {
  ...
  syncLimitedDownloadButtonsFromCartState();
  syncSingleProductPurchaseStateFromCartTruth();
  refreshSingleProductExpressState();
}
  ...
  function bindLiveSingleQuantityCtaSync() {
    ...
    if (window.jQuery) {
      ...
      document.body.addEventListener('added_to_cart', runSync);
      document.body.addEventListener('removed_from_cart', runSync);
      document.body.addEventListener('wc_fragments_refreshed', runSync);
      document.body.addEventListener('wc_fragments_loaded', runSync);
    }
  ...
  function getClassicOrderReview() {
    ...
    function getVisibleCheckoutRequiredRows(form) {
    if (!form) return [];
    ...
    return Array.from(
      ...
      // Ignore hidden containers and hidden fields
      if (row.matches('.woocommerce-hidden, .hidden')) return false;
  ...
  function getCheckoutFieldInput(row) {
    if (!row) return null;
  ...
  function getCheckoutFieldLabel(row, input) {
    var label = row ? row.querySelector('label') : null;
    var text = label ? (label.textContent || '').replace(/\*/g, '').trim() : '';

    if (!text && input) {
      text = input.getAttribute('placeholder') || input.getAttribute('aria-label') || '';
  ...
  function showCheckoutInlineError(row, input, message) {
    if (!row || !input) return;

    clearCheckoutInlineError(row, input);
    ...
    input.classList.add('input-text', 'shoshin-checkout-invalid');

    var err = document.createElement('div');
    err.className = 'shoshin-checkout-inline-error';
  ...
  function clearCheckoutInlineError(row, input) {
    if (!row) return;

    row.classList.remove('woocommerce-invalid', 'woocommerce-invalid-required-field');

    var existing = row.querySelector('.shoshin-checkout-inline-error');
    if (existing) {
      existing.remove();
    }
    ...
    if (input) {
      ...
      input.classList.remove('shoshin-checkout-invalid');
    }
  }

  function validateCheckoutRequiredRow(row) {
    var input = getCheckoutFieldInput(row);
    ...
    showCheckoutInlineError(
      row,
      input,
      getCheckoutFieldLabel(row, input) + ' is required.'
    );

    return false;
  }

  function validateClassicCheckoutRequiredFields() {
    if (!isClassicCheckoutPage()) return true;

    var form = getClassicCheckoutForm();
    if (!form) return true;

    var rows = getVisibleCheckoutRequiredRows(form);
    var firstInvalidInput = null;
    var allValid = true;

    rows.forEach(function(row) {
      var input = getCheckoutFieldInput(row);
      var valid = validateCheckoutRequiredRow(row);
  ...
  function bindClassicCheckoutInlineValidation() {
    if (!isClassicCheckoutPage()) return;

    var form = getClassicCheckoutForm();
    if (!form) return;

    if (form.dataset.shoshinInlineValidationBound !== '1') {
      form.dataset.shoshinInlineValidationBound = '1';

      form.addEventListener('submit', function(e) {
        if (!validateClassicCheckoutRequiredFields()) {
          e.preventDefault();
      ...
      form.addEventListener('input', function(e) {
        var row = e.target.closest('.validate-required');
        if (!row) return;
        validateCheckoutRequiredRow(row);
      }, true);
    ...
    if (placeOrder && placeOrder.dataset.shoshinInlineValidationBound !== '1') {
      placeOrder.dataset.shoshinInlineValidationBound = '1';
  ...
  function buildClassicCheckoutShell() {
    ...
    var legacyContact = document.querySelector('#contact_details');
    var accountFields = document.querySelector('.woocommerce-account-fields');

    /*
      Current checkout no longer always outputs #contact_details.
      In that case, synthesize a Contact card from the billing email field only,
      and leave the remaining billing fields in the Billing card.
    */
    var billing =
      (col1 && col1.querySelector('.woocommerce-billing-fields')) ||
      document.querySelector('.col-1 .woocommerce-billing-fields');
...
/*
  CRITICAL CHECKOUT CONTRACT NOTE

  WooCommerce no longer guarantees the presence of #contact_details.
  This shell must:
  - Prefer #contact_details when present (legacy behavior)
  - Fallback to billing_email field when absent
  - NEVER treat full billing block as contact

  This preserves correct card order:
  Express → Contact → Shipping → Billing → Notes → Payment

  Do not simplify this without verifying Woo DOM structure.
*/


    /* CONTACT + ACCOUNT CREATION */
    if (legacyContact) {
      var contactCard = wrapCard(legacyContact, 'card-contact');
      if (contactCard) {
      ...
      var billingEmailField =
        billing ? billing.querySelector('#billing_email_field, .form-row[id*="billing_email"]') : null;

      if (billingEmailField && isSafeToMove(billingEmailField)) {
        var contactCard = createCard('card-contact', 'Contact Information');
        moveNode(billingEmailField, contactCard);
...
function normalizeClassicCheckoutShippingNotices() {
  if (!isClassicCheckoutPage()) return;
  ...
  document.querySelectorAll(
    ...
    if (/^Either Name or Company is required$/i.test(txt)) {
      notice.textContent = 'Name Missing: Please enter a recipient name.';
      return;
    }

    if (/^We couldn\'t verify the shipping address you entered\./i.test(txt)) {
      notice.textContent = 'Street Address Missing: Please enter a valid shipping address.';
    }
  });
}

function toggleBatchAuthorizeRowForGateway() {
  if (!isClassicCheckoutPage()) return;

  var authorizeRow = document.querySelector('.shoshin-classic-batch-authorize-row');
  if (!authorizeRow) return;

  var checkedGateway = document.querySelector('input[name="payment_method"]:checked');
  var gateway = checkedGateway ? (checkedGateway.value || '') : '';
  ...
  authorizeRow.style.display = isStripe ? '' : 'none';

  if (!isStripe) {
    var checkbox = authorizeRow.querySelector('input[type="checkbox"]');
...
function reconcileClassicCheckoutShell() {
  ...
  toggleBatchAuthorizeRowForGateway();
  normalizeClassicCheckoutShippingNotices();
  toggleClassicCheckoutExpressCardForZeroTotal();
  bindClassicCheckoutInlineValidation();
}
