...
add_action('wp', function () {
  ...
  // Never interrupt WooCommerce order confirmation / thank-you flow.
...
function shoshin_product_excludes_pmpro_member_discount( $product ) {
	if ( ! $product || ! is_a( $product, 'WC_Product' ) ) {
		return false;
	}

	if ( $product->get_meta( '_shoshin_exclude_member_discount' ) === 'yes' ) {
		return true;
	}
	...
	if ( $product->is_type( 'variation' ) ) {
		...
		if ( $parent && is_a( $parent, 'WC_Product' ) ) {
			return $parent->get_meta( '_shoshin_exclude_member_discount' ) === 'yes';
		}
	}
...
/* =========================
   SHOSHIN — EXCLUSION GUARD
   ========================= */
if ( shoshin_product_excludes_pmpro_member_discount( $product ) ) {
...
function shoshin_resolve_variation_pricing_for_current_user($variation) {
    ...
    $public_current = $has_sale ? $sale : $regular;

/* =========================
   SHOSHIN — EXCLUSION GUARD
   ========================= */
if ( shoshin_product_excludes_pmpro_member_discount( $variation ) ) {
	return array(
		'regular'               => (float) $regular,
		'public_current'        => (float) $public_current,
		'final_current'         => (float) $public_current,
...
function shoshin_get_simple_product_final_price_for_current_user( $product ) {
    if ( ! $product || ! is_a( $product, 'WC_Product' ) ) {
        return '';
    }
    ...
    /*
     * Variations use the dedicated variation resolver.
     */
    if ( $product->is_type( 'variation' ) ) {
        $resolved = shoshin_resolve_variation_pricing_for_current_user( $product );
        ...
        if ( array_key_exists( 'final_current', $resolved ) ) {
            return (float) wc_format_decimal( $resolved['final_current'] );
        }
    ...
    /*
     * Parent variable products should not be priced directly.
     */
    if ( $product->is_type( 'variable' ) ) {
        return '';
    }
    ...
    /*
     * Base public price precedence:
     * sale price if valid, otherwise regular, otherwise current edit price.
     * We intentionally use raw/edit prices here so cart math is authoritative
     * and does not depend on gateway-specific checkout paths.
     */
    $regular_raw = $product->get_regular_price( 'edit' );
    $sale_raw    = $product->get_sale_price( 'edit' );
    $current_raw = $product->get_price( 'edit' );

    $regular = ( $regular_raw !== '' && $regular_raw !== null ) ? (float) wc_format_decimal( $regular_raw ) : 0.0;
    $sale    = ( $sale_raw !== '' && $sale_raw !== null ) ? (float) wc_format_decimal( $sale_raw ) : 0.0;
    $current = ( $current_raw !== '' && $current_raw !== null ) ? (float) wc_format_decimal( $current_raw ) : 0.0;
    ...
    if ( $base_price <= 0 ) {
    	...
    	if ( shoshin_product_excludes_pmpro_member_discount( $product ) ) {
		return max( 0.0, (float) wc_format_decimal( $base_price ) );
	}

    /*
     * First, honor any PMPro level price overrides.
     * Lowest applicable paid-member price wins.
     */
    $discount_level_ids = shoshin_get_pmpro_discount_level_ids_for_product( $product );

    $lowest_applicable = $base_price;
    $has_level_price   = false;
    $is_included       = false;
    ...
    foreach ( $discount_level_ids as $level_id ) {
        ...
        $level_price = (float) wc_format_decimal( $level_price );

        if ( $level_price <= 0 ) {
            $lowest_applicable = 0.0;
            $has_level_price   = true;
            $is_included       = true;
            break;
        }

        if ( ! $has_level_price || $level_price < $lowest_applicable ) {
            $lowest_applicable = $level_price;
            $has_level_price   = true;
        }
    }
    ...
    /*
     * Then apply the current member discount percentage on top of the
     * effective base/level price. This intentionally allows sale stacking.
     */
    $context = shoshin_get_current_user_pmpro_context();

    if ( ! empty( $context['is_member'] ) && empty( $context['is_free_level'] ) && ! empty( $context['discount_decimal'] ) ) {
        $discount_decimal = (float) $context['discount_decimal'];

        if ( $discount_decimal > 0 && $discount_decimal < 1 ) {
            $lowest_applicable = $lowest_applicable * ( 1 - $discount_decimal );
        }
    }

    return max( 0.0, (float) wc_format_decimal( $lowest_applicable ) );
}

add_action( 'woocommerce_before_calculate_totals', 'shoshin_apply_pmpro_member_cart_prices', 20 );

function shoshin_apply_pmpro_member_cart_prices( $cart ) {
    if ( is_admin() && ! wp_doing_ajax() ) {
        return;
    }
    ...
    foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
        ...
        /*
         * Preserve the original Woo price once so repeated recalculations
         * do not compound member discounts.
         */
        if ( ! isset( $cart_item['shoshin_original_unit_price'] ) ) {
            $cart->cart_contents[ $cart_item_key ]['shoshin_original_unit_price'] = (float) wc_format_decimal( $product->get_price( 'edit' ) );
        }

        /*
         * External/affiliate products are never member-discounted.
         */
        if ( $product->is_type( 'external' ) ) {
            $original = isset( $cart_item['shoshin_original_unit_price'] )
                ? (float) $cart_item['shoshin_original_unit_price']
                : (float) wc_format_decimal( $product->get_price( 'edit' ) );
        ...
        /*
         * Variations use the existing authoritative variation resolver.
         */
        if ( $product->is_type( 'variation' ) ) {
            $resolved = shoshin_resolve_variation_pricing_for_current_user( $product );
            ...
            if ( array_key_exists( 'final_current', $resolved ) ) {
                $product->set_price( (float) wc_format_decimal( $resolved['final_current'] ) );
            }
        ...
        $final_price = shoshin_get_simple_product_final_price_for_current_user( $product );

        if ( $final_price === '' || $final_price === null ) {
            $original = isset( $cart_item['shoshin_original_unit_price'] )
                ? (float) $cart_item['shoshin_original_unit_price']
                : (float) wc_format_decimal( $product->get_price( 'edit' ) );
        ...
        $product->set_price( (float) wc_format_decimal( $final_price ) );
    }
...
function shoshin_member_discount_exclusion_notice_html( $product ) {
    if (
        function_exists( 'shoshin_product_excludes_pmpro_member_discount' )
        && shoshin_product_excludes_pmpro_member_discount( $product )
    ) {
        return '<div class="shoshin-price-savings shoshin-price-member-teaser">This item is not eligible for membership discounts</div>';
    }
...
' . ( shoshin_member_discount_exclusion_notice_html( $product ) ?: '<div class="shoshin-price-savings shoshin-price-member-teaser">' . $teaser_text . '</div>' ) . '
        </div>';
    }
    ...
    if (!$sale_active && !$member_discount_active) {
        ...
        if (!shoshin_has_annual_membership_discount()) {
            $teaser_html = '<div class="shoshin-price-savings shoshin-price-member-teaser">' . ($membership_free_for_others ? 'This item is included with a <a href="/membership">Supporting Membership</a>' : shoshin_format_member_teaser_text($guest_member_amount, $guest_member_percent, false, false)) . '</div>';
                ...
                $exclusion_notice_html = shoshin_member_discount_exclusion_notice_html( $product );

        if ( $exclusion_notice_html !== '' ) {
            $teaser_html = $exclusion_notice_html;
        }
 ...
 */
/**
 * Resolve the effective product for storefront messaging.
 * Variations inherit shipping class / delivery taxonomy from parent when needed.
 */
function shoshin_storefront_context_product( $product ) {
    if ( ! $product || ! is_a( $product, 'WC_Product' ) ) {
        return null;
    }
...
/**
 * Resolve shipping class slug for storefront messaging.
 *
 * Fallback rules:
 * - preserve any real assigned shipping class
 * - external products with no class resolve to external-affiliate
 * - downloadable / virtual products with no class resolve to download
 * - otherwise fall back to unassigned
 */
function shoshin_storefront_get_shipping_class_slug( $product ) {
    $context_product = shoshin_storefront_context_product( $product );

    if ( ! $context_product || ! is_a( $context_product, 'WC_Product' ) ) {
        return 'unassigned';
    }

    $slug = sanitize_title( (string) $context_product->get_shipping_class() );

    if ( $slug !== '' ) {
        return $slug;
    }

    if ( $context_product->is_type( 'external' ) ) {
        return 'external-affiliate';
    }

    if ( $context_product->is_downloadable() || $context_product->is_virtual() ) {
        return 'download';
    }
...
/**
 * Resolve first assigned Estimated Delivery term name from custom taxonomy.
 */
function shoshin_storefront_get_estimated_delivery_term_name( $product ) {
    $context_product = shoshin_storefront_context_product( $product );
    ...
    if ( ! taxonomy_exists( 'product_estimated_delivery' ) ) {
        return '';
    }

    $terms = wc_get_product_terms(
        $context_product->get_id(),
        'product_estimated_delivery',
        array( 'fields' => 'names' )
    );

    if ( empty( $terms ) || is_wp_error( $terms ) || empty( $terms[0] ) ) {
        return '';
    }
...
/**
 * Determine whether a product should be treated as unavailable for storefront messaging.
 */
function shoshin_storefront_is_unavailable( $product ) {
    if ( ! $product || ! is_a( $product, 'WC_Product' ) ) {
        return false;
    }

    $shipping_class = shoshin_storefront_get_shipping_class_slug( $product );

    /*
     * External affiliate is off-platform and should always remain in stock.
     */
    if ( $shipping_class === 'external-affiliate' ) {
        return false;
    }
...
/**
 * Determine whether a product should be blocked from purchase entirely.
 * Pre-launch rule:
 * - unassigned = not purchasable
 * - external-affiliate remains off-platform and untouched
 */
function shoshin_storefront_is_purchase_blocked( $product ) {
    if ( ! $product || ! is_a( $product, 'WC_Product' ) ) {
        return false;
    }
    ...
    return ( $shipping_class === 'unassigned' );
}

/**
 * Map shipping class + actual availability -> storefront status label.
 */
function shoshin_storefront_get_status_label( $product ) {
    $shipping_class = shoshin_storefront_get_shipping_class_slug( $product );
    $is_unavailable = shoshin_storefront_is_unavailable( $product );

    if ( $shipping_class === 'external-affiliate' ) {
        return 'In Stock';
    }

    if ( $shipping_class === 'unassigned' ) {
        return 'Currently Unavailable';
    }

    if ( $is_unavailable ) {
        if ( $shipping_class === 'immediate' ) {
            return 'Out of Stock';
        }

        if (
            $shipping_class === 'pod-shop3d' ||
            $shipping_class === 'delayed-batch' ||
            $shipping_class === 'download' ||
            $shipping_class === 'unassigned'
        ) {
            return 'Currently Unavailable';
        }
    ...
    if ( $shipping_class === 'pod-shop3d' || $shipping_class === 'delayed-batch' ) {
...
/**
 * Delivery fallback map by shipping class.
 */
function shoshin_storefront_get_delivery_value( $product ) {
    $term_name = shoshin_storefront_get_estimated_delivery_term_name( $product );
    if ( $term_name !== '' ) {
        return $term_name;
    }
...
/*
 * Server-side purchase guard for unassigned products.
 * This is the authoritative lockout.
 */
add_filter( 'woocommerce_is_purchasable', function( $purchasable, $product ) {
    if ( shoshin_storefront_is_purchase_blocked( $product ) ) {
        return false;
    }
...
add_filter( 'woocommerce_variation_is_purchasable', function( $purchasable, $variation ) {
    if ( shoshin_storefront_is_purchase_blocked( $variation ) ) {
        return false;
    }
...
function shoshin_attribute_driven_stock_html($html, $product) {
    ...
    /*
     * For variations:
     * - actual stock state should still come from the selected variation
     * - shipping class / estimated delivery inherit from the parent via
     *   shoshin_storefront_context_product()
     */
    $status_label = shoshin_storefront_get_status_label( $product );
    $delivery     = shoshin_storefront_get_delivery_value( $product );
    ...
    $output .= '<div class="shoshin-stock-status__subtext">';
    $output .= '<span class="shoshin-stock-status__truck" aria-hidden="true">🚚</span>';
    $output .= '<span class="shoshin-stock-status__subtext-label">Estimated Delivery:</span> ';
    $output .= '<span class="shoshin-stock-status__subtext-value">' . esc_html($delivery) . '</span>';
    $output .= '</div>';
    $output .= '</div>';
...
/*
 * Hard block add-to-cart for unassigned products.
 * This is the final server-side fallback.
 */
add_filter('woocommerce_add_to_cart_validation', function ($passed, $product_id, $quantity, $variation_id = 0, $variations = array()) {
    ...
    if ($product && shoshin_storefront_is_purchase_blocked($product)) {
        return false;
    }
...
add_action('woocommerce_before_add_to_cart_button', function () {
    ...
    $blocked = shoshin_storefront_is_purchase_blocked($product);
    $in_cart = shoshin_product_is_limited_and_in_cart($product);

    if (!$blocked && !$in_cart) {
        return;
    }

    $button_text = $blocked ? 'Currently Unavailable' : 'In Cart';
    ?>
    <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      if (button) {
        ...
        button.classList.add('shoshin-disabled-add-to-cart', 'is-locked');
        ...
        button.innerHTML = <?php echo wp_json_encode($button_text); ?>;
      }

      var qty = form.querySelector('input.qty');
      if (qty) {
        qty.readOnly = true;
        qty.setAttribute('aria-readonly', 'true');
      }
    });
    </script>
    <?php
    ...
    wc_enqueue_js("
        jQuery(function ($) {
            ...
            $(document).on('submit', '.single-product form.cart', function (e) {
                ...
                var isLockedDownload =
                    \$button.hasClass('shoshin-disabled-add-to-cart') &&
                    $.trim(\$button.text()) === 'In Cart';

                if (\$button.is(':disabled') || isLockedDownload) {
                    return;
                }
...
function shoshin_cart_meta_get_availability_label( $product, $cart_item ) {
    ...
    if ( $shipping_class === 'download' ) {
        ...
        $is_digital = true;
    } elseif ( taxonomy_exists( 'product_item_format' ) ) {
        $item_format_terms = wc_get_product_terms( $product->get_id(), 'product_item_format', array( 'fields' => 'names' ) );
        if ( ! is_wp_error( $item_format_terms ) && ! empty( $item_format_terms ) ) {
            foreach ( $item_format_terms as $term_name ) {
                if ( strtolower( trim( (string) $term_name ) ) === 'digital' ) {
                    $is_digital = true;
                    break;
                }
            }
    ...
    return shoshin_storefront_get_status_label( $product );
}
}
...
function shoshin_cart_meta_build_lines( $product, $cart_item = array() ) {
    ...
    $delivery_text = shoshin_storefront_get_delivery_value( $product );
    ...
    if ( $shipping_class === 'download' ) {
        $is_digital = true;
    } elseif ( $product instanceof WC_Product && $product->is_downloadable() ) {
        $is_digital = true;
    } elseif ( taxonomy_exists( 'product_item_format' ) ) {
        $item_format_terms = wc_get_product_terms( $product->get_id(), 'product_item_format', array( 'fields' => 'names' ) );
        if ( ! is_wp_error( $item_format_terms ) && ! empty( $item_format_terms ) ) {
    ...
    if ( $is_digital ) {
        $delivery_text = 'Immediate';
    }

    if ( $delivery_text !== '' ) {
        $lines[] = array(
            'class' => 'shoshin-cart-meta__line--delivery',
            'text'  => '🚚 Estimated Delivery: ' . $delivery_text,
        );
    }

    // 3) Special message
    if ( $shipping_class === 'delayed-batch' ) {
        $lines[] = array(
            'class' => 'shoshin-cart-meta__line--warning',
            'text'  => '📦 Produced in a batch run — cannot be canceled after purchase',
        );
    } elseif ( $is_digital ) {
        $lines[] = array(
            'class' => 'shoshin-cart-meta__line--digital',
            'text'  => '📥 Instant digital delivery — access from your account after purchase',
        );
    } elseif ( $shipping_class === 'immediate' || $shipping_class === 'external-affiliate' ) {
        $lines[] = array(
            'class' => 'shoshin-cart-meta__line--fulfillment',
            'text'  => '📦 Fulfilled by Pelican House Games',
        );
    } elseif ( $shipping_class === 'pod-shop3d' ) {
        $lines[] = array(
            'class' => 'shoshin-cart-meta__line--fulfillment',
            'text'  => '📦 Made to order — fulfilled by our partner',
        );
    }
...
if ( ! function_exists( 'shoshin_cart_item_is_batch_production' ) ) {
	function shoshin_cart_item_is_batch_production( $product, $cart_item = array() ) {
		...
		if ( ! class_exists( 'SSB_Settings' ) || ! method_exists( 'SSB_Settings', 'get_fulfillment_lanes_runtime' ) ) {
			return false;
		}
		...
		$batch = ! empty( $lanes['batch'] ) && is_array( $lanes['batch'] ) ? $lanes['batch'] : array();
		$assigned_ids = ! empty( $batch['shipping_classes'] ) && is_array( $batch['shipping_classes'] )
			? array_map( 'absint', $batch['shipping_classes'] )
			: array();

		return in_array( $shipping_class_id, $assigned_ids, true );
	}
}
...
if ( ! function_exists( 'shoshin_get_batch_shipping_authorize_label' ) ) {
	function shoshin_get_batch_shipping_authorize_label() {
		return 'I authorize Pelican House Games to charge shipping for Batch Production items at a later date when they are ready to ship. This charge will be applied to my original payment method.';
	}
}
...
add_action( 'woocommerce_review_order_before_submit', function() {
	...
	$ack_checked        = ! empty( $_POST['shoshin_batch_production_ack_classic'] ) ? 'checked="checked"' : '';
	$authorize_checked  = ! empty( $_POST['shoshin_batch_shipping_authorize_classic'] ) ? 'checked="checked"' : '';
	...
	echo '<input type="checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox" name="shoshin_batch_production_ack_classic" id="shoshin_batch_production_ack_classic" value="1" ' . $ack_checked . ' />';
	echo '<span class="shoshin-classic-batch-ack-text">' . esc_html( shoshin_get_batch_ack_label() ) . '</span>';
	echo '</label>';
	echo '</p>';

	echo '<p class="form-row form-row-wide shoshin-classic-batch-authorize-row validate-required" style="margin-top:10px;">';
	echo '<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox" for="shoshin_batch_shipping_authorize_classic">';
	echo '<input type="checkbox" class="woocommerce-form__input woocommerce-form__input-checkbox input-checkbox" name="shoshin_batch_shipping_authorize_classic" id="shoshin_batch_shipping_authorize_classic" value="1" ' . $authorize_checked . ' />';
	echo '<span class="shoshin-classic-batch-authorize-text">' . esc_html( shoshin_get_batch_shipping_authorize_label() ) . '</span>';
	echo '</label>';
	echo '</p>';
}, 20 );
...
add_action( 'woocommerce_checkout_process', function() {
	...
	$chosen_payment = isset( $_POST['payment_method'] ) ? wc_clean( wp_unslash( $_POST['payment_method'] ) ) : '';
	$is_stripe      = in_array( $chosen_payment, array( 'stripe', 'woocommerce_payments', 'wcpay' ), true );
	$authorize_later = ! empty( $_POST['shoshin_batch_shipping_authorize_classic'] );

	if ( $is_stripe && ! $authorize_later ) {
		wc_add_notice(
			'Please authorize Pelican House Games to charge shipping later for Batch Production items before placing your order.',
			'error'
		);
	}
}, 20 );
...
add_action( 'woocommerce_checkout_create_order', function( $order ) {
	...
	if ( $authorize_later ) {
		$order->update_meta_data( '_shoshin_batch_shipping_authorize', 'yes' );
		$order->update_meta_data( '_shoshin_batch_shipping_authorize_text', shoshin_get_batch_shipping_authorize_label() );
		$order->update_meta_data( '_shoshin_batch_shipping_authorize_utc', gmdate( 'Y-m-d H:i:s' ) );
	}
}, 20, 1 );
...
if ( ! function_exists( 'shoshin_render_batch_estimate_bridge' ) ) {
	function shoshin_render_batch_estimate_bridge() {
		if ( is_admin() || ( ! is_cart() && ! is_checkout() ) || is_order_received_page() ) {
			return;
		}
		?>
		<script>
		(function() {
			...
			function findDeferredShippingRows() {
				var rows = document.querySelectorAll('tr.shipping, tr.woocommerce-shipping-totals.shipping, .woocommerce-shipping-totals.shipping');
				return Array.prototype.filter.call(rows, function(row) {
					var text = (row.textContent || '').toLowerCase();
					return text.indexOf('shipping calculated at a later date') !== -1 || text.indexOf('shipping can be estimated at checkout') !== -1;
				});
			}

function findAddressNode(row) {
  return row.querySelector('.woocommerce-shipping-destination, .shipping-destination');
}

function getShippingValueCell(row) {
  return row.querySelector('td[data-title="Shipment"], td');
}

function ensureEstimateContainer(row) {
  var existing = row.querySelector('.shoshin-batch-estimate-shell');
  if (existing) return existing;

  var shell = document.createElement('div');
  shell.className = 'shoshin-batch-estimate-shell';
  shell.style.marginBottom = '8px';

  var target = getShippingValueCell(row);
...
function renderBatchEstimateShell(row, state) {
  var shell = ensureEstimateContainer(row);
  var addressNode = findAddressNode(row);
  var addressKnown = !!(state && state.address_known);
  var hasEstimate = !!(state && state.has_estimate && state.estimate);

  var linkHtml = '';
  var estimateHtml = '';
  var helperHtml = '';

  if (isCartPage() && !addressKnown) {
    helperHtml = '<div class="shoshin-batch-estimate-helper">Shipping can be estimated at checkout.</div>';
  }

  if ((isCartPage() && addressKnown && !hasEstimate) || (isCheckoutPage() && addressKnown && !hasEstimate)) {
    linkHtml = '<div class="shoshin-batch-estimate-link-wrap"><a href="#" class="shoshin-batch-estimate-link">Estimate Shipping</a></div>';
  }

  if (hasEstimate) {
    var estimateLabel = 'Estimate: ' + state.estimate.formatted_amount;

    if (state.estimate.service) {
  estimateLabel = 'Estimate: ' + state.estimate.service + ': <span class="woocommerce-Price-amount amount">' + state.estimate.formatted_amount + '</span>';
}

    estimateHtml = '<div class="shoshin-batch-estimate-value">' + estimateLabel + '</div>';
  }

  shell.innerHTML = linkHtml + estimateHtml + helperHtml;

  var cell = getShippingValueCell(row);
  if (!cell) {
    cell = row;
  }
  ...
  shell.style.marginBottom = '6px';

  function getCheckoutAddressHtmlFromFields() {
    ...
    var address1 = value('#shipping_address_1') || value('#billing_address_1');
    var address2 = value('#shipping_address_2') || value('#billing_address_2');
    var city = value('#shipping_city') || value('#billing_city');
    var stateText = selectedText('#shipping_state') || selectedText('#billing_state');
    var postcode = value('#shipping_postcode') || value('#billing_postcode');
    var countryText = selectedText('#shipping_country') || selectedText('#billing_country');
    ...
    if (countryText && countryText !== 'United States (US)') {
      lines.push(countryText);
    }
  ...
  if (isCheckoutPage()) {
    var methodList = cell.querySelector('ul.woocommerce-shipping-methods, #shipping_method');
    if (methodList) {
      methodList.style.display = 'none';
    }

    var existingWrap = row.querySelector('.shoshin-batch-checkout-wrap');
    if (!existingWrap) {
      existingWrap = document.createElement('div');
      existingWrap.className = 'shoshin-batch-checkout-wrap';
      existingWrap.style.display = 'flex';
      existingWrap.style.flexDirection = 'column';
      existingWrap.style.alignItems = 'flex-end';
      existingWrap.style.textAlign = 'right';
      existingWrap.style.gap = '4px';
      existingWrap.style.width = '100%';
      cell.appendChild(existingWrap);
    }

    if (!shell.parentNode || shell.parentNode !== existingWrap) {
      existingWrap.appendChild(shell);
    }

    var addressText = '';
    if (state && state.address_html) {
      addressText = String(state.address_html).trim();
    } else if (state && state.address_text) {
      addressText = String(state.address_text).trim();
    } else if (addressNode) {
      addressText = (addressNode.innerHTML || '').trim();
    }

    if (!addressText) {
      addressText = getCheckoutAddressHtmlFromFields();
    }
    ...
    var addressDisplay = row.querySelector('.shoshin-batch-known-address');
    if (addressText) {
      if (!addressDisplay) {
        addressDisplay = document.createElement('div');
        addressDisplay.className = 'shoshin-batch-known-address';
        addressDisplay.style.textAlign = 'right';
        addressDisplay.style.margin = '0';
        existingWrap.appendChild(addressDisplay);
      } else if (addressDisplay.parentNode !== existingWrap) {
        existingWrap.appendChild(addressDisplay);
      }
    ...
    var deferredText = row.querySelector('.shoshin-batch-deferred-line');
    if (!deferredText) {
      deferredText = document.createElement('div');
      deferredText.className = 'shoshin-batch-deferred-line';
      deferredText.textContent = 'Shipping will be charged at a later date.';
      deferredText.style.textAlign = 'right';
      deferredText.style.marginTop = '0';
      existingWrap.appendChild(deferredText);
    } else {
      deferredText.style.display = '';
      deferredText.style.textAlign = 'right';
      deferredText.style.marginTop = '0';
      if (deferredText.parentNode !== existingWrap) {
        existingWrap.appendChild(deferredText);
      }
    }
...
if (isCartPage()) {
  var cartMethodList = cell.querySelector('ul.woocommerce-shipping-methods, #shipping_method');

  // Unknown address:
  // keep Woo deferred message visible, hide our custom shell/address helper.
  if (!addressKnown && !hasEstimate) {
    if (cartMethodList) {
      cartMethodList.style.display = '';
    }
  ...
  // Known address or estimate exists:
  // hide Woo deferred message and use our custom shell.
  if (cartMethodList) {
    cartMethodList.style.display = 'none';
  }
  ...
  if (addressNode) {
    addressNode.style.display = '';
    addressNode.style.textAlign = 'right';
  }

  if (addressNode && addressNode.parentNode) {
    shell.parentNode.insertBefore(shell, addressNode);
  } else if (shell.parentNode !== cell) {
    cell.insertBefore(shell, cell.firstChild);
  }
...
function getAllShippingRows() {
  return Array.prototype.filter.call(
    document.querySelectorAll('tr.shipping, tr.woocommerce-shipping-totals.shipping'),
    function(row) {
...
function detectShipmentLane(row) {
  if (!row) return '';

  var text = (row.textContent || '').toLowerCase();

  if (
    row.querySelector('.shoshin-batch-estimate-shell') ||
    row.querySelector('.shoshin-batch-deferred-line') ||
    text.indexOf('estimate shipping') !== -1 ||
    text.indexOf('shipping will be charged at a later date') !== -1
  ) {
    return 'batch';
  }

  if (text.indexOf('flat rate') !== -1) {
    return 'dropship';
  }

  if (
    row.querySelector('input[type="radio"]') ||
    row.querySelector('.woocommerce-shipping-calculator') ||
    row.querySelector('.shipping-calculator-button') ||
    row.querySelector('.woocommerce-no-shipping-available-html') ||
    row.querySelector('.woocommerce-cart-no-shipping-available-html')
  ) {
    return 'immediate';
  }
...
function ensureShipmentSubtitle(row, lane) {
  if (!row || !lane) return;

  var labelCell = row.querySelector('th');
  if (!labelCell) return;

  var subtitle = labelCell.querySelector('.shoshin-shipment-subtitle');
  var map = {
    immediate: 'Immediate',
    dropship: 'Made to Order',
    batch: 'Batch Production'
  };

  if (!subtitle) {
    subtitle = document.createElement('div');
    subtitle.className = 'shoshin-shipment-subtitle';
    labelCell.appendChild(subtitle);
  }

  subtitle.textContent = '(' + (map[lane] || '') + ')';
}

function suppressImmediateAddressControls(row) {
  if (!row || !isCartPage()) return;
  if (detectShipmentLane(row) !== 'immediate') return;

  row.querySelectorAll(
    '.shipping-calculator-button,' +
    '.woocommerce-shipping-calculator,' +
    '.shipping-calculator-form,' +
    'a[href*="calc_shipping"],' +
...
function normalizeImmediateUnknownAddressState(row, state) {
  if (!row) return;
  if (detectShipmentLane(row) !== 'immediate') return;
...
function normalizeBatchUnknownAddressState(row, state) {
  if (!row) return;
  if (detectShipmentLane(row) !== 'batch') return;

  var addressKnown = !!(state && state.address_known);
  var hasEstimate = !!(state && state.has_estimate && state.estimate);

  var linkWrap = row.querySelector('.shoshin-batch-estimate-link-wrap');
  var value = row.querySelector('.shoshin-batch-estimate-value');
...
function normalizeBatchLinkTypography(row) {
  if (!row) return;
  if (detectShipmentLane(row) !== 'batch') return;

  var linkWrap = row.querySelector('.shoshin-batch-estimate-link-wrap');
  var link = row.querySelector('.shoshin-batch-estimate-link');
...
function normalizeKnownAddressDisplay(rows, state) {
  if (!rows || !rows.length) return;

  var addressKnown = !!(state && state.address_known);
  if (!addressKnown) return;

  var lastVisibleRow = rows[rows.length - 1];

  rows.forEach(function(row) {
    var nativeAddress = row.querySelector('.woocommerce-shipping-destination, .shipping-destination');
    var customAddress = row.querySelector('.shoshin-batch-known-address');
    var isLast = row === lastVisibleRow;

    if (nativeAddress) {
      nativeAddress.style.display = isLast ? '' : 'none';
      if (isLast) {
        nativeAddress.style.textAlign = 'right';
      }
    }

    if (customAddress) {
      customAddress.style.display = isLast ? '' : 'none';
      if (isLast) {
        customAddress.style.textAlign = 'right';
      }
    }
  });
}

function applyShipmentPresentationUi(state) {
  var rows = getAllShippingRows();
  if (!rows.length) return;

  rows.forEach(function(row) {
    var lane = detectShipmentLane(row);

    ensureShipmentSubtitle(row, lane);
    suppressImmediateAddressControls(row);
    normalizeImmediateUnknownAddressState(row, state);
    normalizeBatchUnknownAddressState(row, state);
    normalizeBatchLinkTypography(row);
  });

  normalizeKnownAddressDisplay(rows, state);
}

			function fetchBatchEstimateState(callback) {
				var formData = new FormData();
				formData.append('action', 'ssb_get_batch_estimate_state');
				...
				fetch(ajaxUrl, {
					...
					callback(json && json.success && json.data ? json.data : null);
				}).catch(function() {
					callback(null);
				});
			}

			function requestBatchEstimate(linkEl) {
				var formData = new FormData();
				formData.append('action', 'ssb_get_batch_estimate');

				linkEl.textContent = 'Estimating...';
				...
				fetch(ajaxUrl, {
					...
					if (!json || !json.success || !json.data || !json.data.estimate) {
						throw new Error((json && json.data && json.data.message) ? json.data.message : 'Unable to estimate shipping.');
					}

					applyBatchEstimateUi();
				}).catch(function(error) {
					linkEl.textContent = 'Estimate Shipping';
					linkEl.style.pointerEvents = '';
					window.alert(error && error.message ? error.message : 'Unable to estimate shipping.');
				});
			}

function applyBatchEstimateUi() {
  fetchBatchEstimateState(function(state) {
    if (state && state.has_batch_package) {
      findDeferredShippingRows().forEach(function(row) {
        renderBatchEstimateShell(row, state);
      });
    }

    applyShipmentPresentationUi(state || null);
  });
}

			document.addEventListener('click', function(event) {
				var link = event.target.closest('.shoshin-batch-estimate-link');
				if (!link) return;

				event.preventDefault();
				requestBatchEstimate(link);
			});

			document.addEventListener('DOMContentLoaded', applyBatchEstimateUi);
			...
			if (window.jQuery && window.jQuery(document.body).length) {
				window.jQuery(document.body).on('updated_checkout updated_cart_totals', function() {
					window.setTimeout(applyBatchEstimateUi, 50);
				});
			}
		})();
		</script>
...
add_action( 'wp_footer', 'shoshin_render_batch_estimate_bridge', 101 );
...
add_action('woocommerce_check_cart_items', 'shoshin_minimum_order_amount');

function shoshin_minimum_order_amount() {
    ...
    if ($cart_total < $minimum) {
        wc_add_notice(
            sprintf(
                'The minimum order total is $%s. Please add another item to continue.',
...
/**
 * Shoshin — Exclude selected WooCommerce products from PMPro member discounts.
 */

add_action( 'woocommerce_product_options_pricing', function () {
	woocommerce_wp_checkbox( array(
		'id'          => '_shoshin_exclude_member_discount',
		'label'       => 'Exclude PMPro member discount',
		'description' => 'Prevents PMPro WooCommerce member discounts from applying to this product.',
		'desc_tip'    => true,
	) );
} );
...
add_action( 'woocommerce_admin_process_product_object', function ( $product ) {
	$product->update_meta_data(
		'_shoshin_exclude_member_discount',
		isset( $_POST['_shoshin_exclude_member_discount'] ) ? 'yes' : 'no'
	);
} );
