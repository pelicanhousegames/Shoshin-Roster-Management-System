...
if ($has_unassigned_items) {
    ...
    protected static function render_v1_workspace_inner($order) {
        ...
        error_log('SSB STATE DEBUG → ' . SSB_State::get_order_state($order));
        error_log('SSB SHIPMENT ID DEBUG → ' . (string) get_post_meta($order->get_id(), '_shoshin_shippo_shipment_id', true));
