...
class SSB_Webhooks {
    ...
    // Shippo sends a header like: HTTP_SHIPPO_SIGNATURE
    $signature = '';

    if (!empty($headers['shippo-signature'][0])) {
        $signature = $headers['shippo-signature'][0];
    }

    // Optional: allow empty for now if you haven't configured secret yet
    $expected = get_option('ssb_shippo_webhook_secret', '');

    if (!$expected) {
        return true; // fallback (v1 safe mode)
    }

    if ($signature !== $expected) {
        return new WP_Error('forbidden', 'Invalid webhook signature', ['status' => 403]);
    }
