...
protected static function get_cheapest_shippo_rate_for_batch_package(array $package) {
    if (!function_exists('WC') || !WC()->shipping) {
        error_log('SSB Batch Estimate: WooCommerce shipping unavailable.');
        return array(
            'success' => false,
    ...
    $debug_destination = isset($estimate_package['destination']) && is_array($estimate_package['destination'])
        ? $estimate_package['destination']
        : array();

    $debug_contents = array();
    ...
    error_log('SSB Batch Estimate: package meta = ' . wp_json_encode(array(
        'package_id'          => isset($estimate_package['package_id']) ? absint($estimate_package['package_id']) : 0,
        'lane_key'            => isset($estimate_package['ssb_lane_key']) ? (string) $estimate_package['ssb_lane_key'] : '',
        'group_key'           => isset($estimate_package['ssb_group_key']) ? (string) $estimate_package['ssb_group_key'] : '',
        'group_behavior'      => isset($estimate_package['ssb_group_behavior']) ? (string) $estimate_package['ssb_group_behavior'] : '',
        'destination'         => $debug_destination,
        'contents_cost'       => isset($estimate_package['contents_cost']) ? (float) $estimate_package['contents_cost'] : 0,
        'contents_signature'  => self::get_package_contents_signature($estimate_package),
        'destination_hash'    => self::get_package_destination_hash($estimate_package),
    ...
    if (empty($rates)) {
        error_log('SSB Batch Estimate: no rates returned from calculate_shipping_for_package(). package=' . wp_json_encode($calculated_package));
        return array(
            'success' => false,
    ...
    foreach ($rates as $rate_id => $rate) {
        ...
        $debug_rates[] = array(
            ...
            'label'     => $label,
            'cost'      => $cost,
            'is_shippo' => $is_shippo ? 'yes' : 'no',
        );
    }

    error_log('SSB Batch Estimate: raw rates = ' . wp_json_encode($debug_rates));
    ...
    if ($cheapest === null) {
        error_log('SSB Batch Estimate: rates returned, but none matched rate_is_shippo().');
        return array(
            'success' => false,
            'message' => 'No shipping estimate was returned for this address.',
        );
    ...
    error_log('SSB Batch Estimate: cheapest shippo estimate = ' . wp_json_encode($cheapest));
