...
public static function inject_pmpro_fields( $content, $args ) {
	...
	if ( false === strpos( $content, 'name="redirect_to"' ) ) {
		$redirect_markup = sprintf(
			'<input type="hidden" name="redirect_to" value="%s" />',
			esc_attr( self::default_redirect_url() )
		);
	}

	return $content . $redirect_markup . self::shared_field_markup( 'pmpro' );
}
