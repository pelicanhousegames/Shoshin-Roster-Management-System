...
function redirectPmproToWpLogin(form) {
	const target = getRedirect(form) || window.location.href;
	const url = new URL('/wp-login.php', window.location.origin);
	url.searchParams.set('redirect_to', target);

	const username = qs(form, ['input[name="log"]', 'input[name="username"]']);
	if (username && username.value) {
		url.searchParams.set('log', username.value);
	}
	...
	window.location.assign(url.toString());
		...
		const isWooCheckoutForm = !!(
			form &&
			form.matches('form.woocommerce-form-login') &&
			SiteInlineWF2FA.isWooCheckout
		);

		if (!isWooCheckoutForm || form.dataset.inlineWfCheckoutRedirectBound === '1') {
			return;
		}
	...
	function bindForm(form) {
		...
		form.addEventListener('submit', async function (e) {
			...
			try {
				...
				// Live Wordfence reCAPTCHA on PMPro can return HTML instead of JSON.
				// Hand off to native wp-login in that case.
				if (result && result._non_json) {
					if (getFormContext(form) === 'pmpro') {
						setMessage(form, 'Additional verification was triggered. Redirecting to secure login…', false);
						redirectPmproToWpLogin(form);
						return;
					}
